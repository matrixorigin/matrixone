...
- `WhereBinder.BindColRef` 只在 depth=0、未限定名称、所有可见 FROM 作用域均无同名列时尝试本层别名；歧义 FROM 列保持原错误。
...
变量只影响当前连接，没有新目录格式、磁盘状态或协议字段。老版本不识别 SET，应用只应向已升级节点发送；关闭开关是回退方法。重连恢复默认 0，已有会话迁移使用既有系统变量重放路径（需验证是否涵盖动态 session 变量）。不增加权限或租户访问能力。
