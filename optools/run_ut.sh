...
function resolve_embedded_cluster_test_scope(){
    # Keep the complete-cluster classification in one place. Every package in
    # this scope starts LogService/TN/CN services and must not overlap another
    # go test process on the same CI runner.
...
function run_tests(){
    ...
    local embedded_cluster_test_scope
    if ! embedded_cluster_test_scope=$(resolve_embedded_cluster_test_scope); then
    ...
    if [[ $SKIP_TESTS == 'race' ]]; then
        ...
        local embedded_cluster_status=0

        regular_test_scope=$(remove_packages_from_scope \
            "${test_scope}" ${embedded_cluster_test_scope})
        if [[ -n "${regular_test_scope}" ]]; then
            logger "INF" "Run regular UT packages with parallelism ${UT_PARALLEL}"
            LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" CGO_CFLAGS="${CGO_CFLAGS}" CGO_LDFLAGS="${CGO_LDFLAGS}" go test ${GO_MODULE_MODE} -short -v -json -tags "${TAGS}" -p ${UT_PARALLEL} -timeout "${UT_TIMEOUT}m" $regular_test_scope > $UT_REPORT
            regular_status=$?
        else
            : > "${UT_REPORT}"
        fi
        ...
        LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" CGO_CFLAGS="${CGO_CFLAGS}" CGO_LDFLAGS="${CGO_LDFLAGS}" go test ${GO_MODULE_MODE} -short -v -json -tags "${TAGS}" -p 1 -timeout "${UT_TIMEOUT}m" $embedded_cluster_test_scope >> $UT_REPORT
        embedded_cluster_status=$?

        if (( regular_status != 0 || embedded_cluster_status != 0 )); then
            UT_TEST_STATUS=1
        fi
    else
        ...
        # These packages need exclusive runner access because NewTestService
        # callers bind fixed ports.
        ...
        # A package in this group starts one or more complete LogService/TN/CN
        # clusters. Package-level parallelism would multiply those control
        # planes on the same runner and can starve Dragonboat ticks and HAKeeper
        # heartbeats even though each package passes in isolation.
        logger "INF" "Run embedded-cluster race-test packages serially"
        LD_LIBRARY_PATH="${LD_LIBRARY_PATH}" CGO_CFLAGS="${CGO_CFLAGS}" CGO_LDFLAGS="${CGO_LDFLAGS}" go test ${GO_MODULE_MODE} -short -v -json -tags "${TAGS}" -p 1 -timeout "${UT_TIMEOUT}m" -race $embedded_cluster_test_scope >> $UT_REPORT
        embedded_cluster_status=$?
        ...
        if (( light_status != 0 || serial_status != 0 || embedded_cluster_status != 0 || heavy_status != 0 || plan_status != 0 )); then
            UT_TEST_STATUS=1
        fi
    fi
