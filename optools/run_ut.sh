...
function run_tests(){
    ...
    # AVX-512-only SIMD kernels can't execute on a runner without AVX-512 (their
    # tests t.Skip), so they'd score ~0% and drag coverage. Exclude them only when
    # this runner lacks AVX-512; on an AVX-512 runner they run (~96%) and stay
    # counted. Linux-only: Darwin is arm64 and never compiles these files, so the
    # /proc/cpuinfo probe (which would fail-open there) must be gated on the OS.
    if [[ "$(uname -s)" == "Linux" ]] && ! grep -qm1 avx512f /proc/cpuinfo 2>/dev/null; then
        leave_out="$leave_out distance_func_narrow_amd64.go distance_func_narrow_f16_amd64.go distance_func_narrow_int8_amd64.go"
    fi
