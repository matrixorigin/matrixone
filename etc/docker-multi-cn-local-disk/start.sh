...
# Detect configs generated by an older generator that predate the HAKeeper
# bootstrap fix. Those log.toml files lack a [logservice] section, so the log
# store panics with "shard not bootstrapped" on first start. They are gitignored
# (generated) and survive `make dev-clean`, so force a regeneration here instead
# of forcing users to delete them by hand.
STALE_CONFIGS=false
if [ "$MISSING_CONFIGS" = false ] && ! grep -q '^\[logservice\]' log.toml; then
    STALE_CONFIGS=true
fi
...
# Generate configuration files if missing, stale, or if config.env exists
if [ "$MISSING_CONFIGS" = true ]; then
    echo "Configuration files not found. Generating with defaults (metrics enabled)..."
    ./generate-config.sh
    echo ""
elif [ "$STALE_CONFIGS" = true ]; then
    echo "Existing log.toml predates the HAKeeper bootstrap fix (no [logservice] section)."
    echo "Regenerating configuration so the cluster can bootstrap..."
    ./generate-config.sh
    echo ""
