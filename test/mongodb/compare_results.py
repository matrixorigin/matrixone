...
"""Versioned NESR differential oracle for sorted JSON-lines results."""
import argparse
import hashlib
import json
import math
...
def digest(row):
    ...
    return hashlib.sha256(b"nesr-mongo-v1\0" + encoded).hexdigest()
...
def main():
    ...
    parser.add_argument("--keys", default="pump,crew,datetime")
    parser.add_argument(
        "--tolerance-columns",
        default=("engine_rpm,pump_rate,disch_pressure,engine_oil_pressure,"
                 "engine_coolant_temp,lube_oil_pressure,engine_hours,pumping_hours"),
    )
    ...
    try:
        ...
        json.dump({"contract": "nesr-mongo-v1", "failures": [{"class": "invalid-input", "error": str(error)}]}, sys.stdout, indent=2)
        print()
        return 2
    json.dump({"contract": "nesr-mongo-v1", "summary": counts,
