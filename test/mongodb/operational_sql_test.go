...
func TestOperationalSQLTemplatesParse(t *testing.T) {
	...
	for _, name := range []string{"nesr_incremental.sql", "nesr_backfill.sql", "nesr_archive_reset_gate.sql"} {
...
func TestIncrementalTemplateAlignsReplayBoundsToResultGrain(t *testing.T) {
	_, source, _, ok := runtime.Caller(0)
	require.True(t, ok)
	data, err := os.ReadFile(filepath.Join(filepath.Dir(source), "sql", "nesr_incremental.sql"))
	require.NoError(t, err)
	...
	require.Contains(t, template, "engine_rpm float")
	require.Contains(t, template, "''nesr-mongo-v1-exact|''")
