...
func run(ctx context.Context, db *sql.DB, host string, r *report) error {
	...
	statements := []string{
		...
		"create mongodb connection if not exists mongodb_ci with ('hosts'='" + host + "','replica_set'='rs0','auth_source'='nesr_source','auth_mechanism'='SCRAM-SHA-256','credential_secret_ref'='secret://env/MO_MONGODB_E2E_CREDENTIAL','tls_mode'='disabled','read_preference'='primary','read_concern'='majority','options_json'='{\"direct\":true}')",
		"create external table mongodb_ci.raw(mongo_id char(24) mongodb_path '_id', pump varchar(20), crew varchar(10), ts datetime(3) mongodb_convert 'try_null', value double mongodb_convert 'try_null', batch_name varchar(50) mongodb_path 'batch') engine=mongodb with ('connection'='mongodb_ci','database'='nesr_source','collection'='raw','schema_mode'='explicit','conversion_mode'='strict','max_parallelism'='1')",
	}
	...
	if err := expectScalar(ctx, db, "select count(*) from mongodb_ci.raw", "5"); err != nil {
		return err
	}
	if err := expectRows(ctx, db,
		"select mongo_id,pump,crew,cast(ts as char),coalesce(cast(value as char),'NULL'),coalesce(batch_name,'NULL') from mongodb_ci.raw order by mongo_id",
		manifest.Rows); err != nil {
		return err
	}
	if err := expectScalar(ctx, db, "select count(*) from mongodb_ci.raw where value >= 14", "3"); err != nil {
		return err
	}
	if err := expectScalar(ctx, db, "select count(*) from mongodb_ci.raw where batch_name is null", "3"); err != nil {
		return err
	}
	...
	cancel()
	if err := db.QueryRowContext(cancelCtx, "select count(*) from mongodb_ci.raw").Scan(new(string)); err == nil {
	...
	windowSQL := "select count(*) from (select _wstart, pump, crew, avg(value) from mongodb_ci.raw where pump='p1' group by pump,crew interval(ts,1,minute) gapfill(partition) fill(null)) x"
	...
	for _, statement := range []string{
		"create table mongodb_ci.target(pump varchar(20), crew varchar(10), minute datetime, value double, primary key(pump,crew,minute))",
		"create table mongodb_ci.watermark(id int primary key, high datetime)",
		"insert into mongodb_ci.watermark values(1,'2026-07-27 10:00:00')",
	} {
		if _, err := db.ExecContext(ctx, statement); err != nil {
	...
	if _, err = tx.ExecContext(ctx, "replace into mongodb_ci.target select pump,crew,_wstart,avg(value) from mongodb_ci.raw where ts >= '2026-07-27 10:00:00' and ts < '2026-07-27 10:03:00' group by pump,crew interval(ts,1,minute) gapfill(partition) fill(null)"); err == nil {
		_, err = tx.ExecContext(ctx, "update mongodb_ci.watermark set high='2026-07-27 10:03:00' where id=1")
	}
	if err != nil {
		_ = tx.Rollback()
	...
	if err := expectScalar(ctx, db, "select cast(high as char) from mongodb_ci.watermark where id=1", "2026-07-27 10:03:00"); err != nil {
		return err
	}
	r.Cases = append(r.Cases, "atomic-target-watermark")

	// The fixture contains one malformed numeric value. A strict mapping must
	// abort the statement and leave both target and watermark unchanged even if
	// earlier source rows were already converted.
	if _, err := db.ExecContext(ctx, "create external table mongodb_ci.raw_strict(mongo_id char(24) mongodb_path '_id', pump varchar(20), crew varchar(10), ts datetime(3), value double) engine=mongodb with ('connection'='mongodb_ci','database'='nesr_source','collection'='raw','schema_mode'='explicit','conversion_mode'='strict','max_parallelism'='1')"); err != nil {
		return fmt.Errorf("create strict mapping: %w", err)
	}
	var targetBefore, watermarkBefore string
	if err := db.QueryRowContext(ctx, "select count(*) from mongodb_ci.target").Scan(&targetBefore); err != nil {
		return err
	}
	if err := db.QueryRowContext(ctx, "select cast(high as char) from mongodb_ci.watermark where id=1").Scan(&watermarkBefore); err != nil {
		return err
	}
	...
	_, scanErr := failedTx.ExecContext(ctx, "replace into mongodb_ci.target select pump,crew,_wstart,avg(value) from mongodb_ci.raw_strict where ts >= '2026-07-27 10:00:00' and ts < '2026-07-27 10:04:00' group by pump,crew interval(ts,1,minute) gapfill(partition) fill(null)")
	if scanErr == nil {
		_ = failedTx.Rollback()
		return fmt.Errorf("strict MongoDB conversion unexpectedly succeeded")
	}
	_ = failedTx.Rollback()
	if err := expectScalar(ctx, db, "select count(*) from mongodb_ci.target", targetBefore); err != nil {
		return fmt.Errorf("target rollback: %w", err)
	}
	if err := expectScalar(ctx, db, "select cast(high as char) from mongodb_ci.watermark where id=1", watermarkBefore); err != nil {
	...
	// result key and does not create additional target rows.
	idempotentTx, err := db.BeginTx(ctx, nil)
	if err != nil {
		return err
	}
	if _, err = idempotentTx.ExecContext(ctx, "replace into mongodb_ci.target select pump,crew,_wstart,avg(value) from mongodb_ci.raw where ts >= '2026-07-27 10:00:00' and ts < '2026-07-27 10:03:00' group by pump,crew interval(ts,1,minute) gapfill(partition) fill(null)"); err == nil {
		_, err = idempotentTx.ExecContext(ctx, "update mongodb_ci.watermark set high='2026-07-27 10:03:00' where id=1")
	}
	if err != nil {
	...
	if err := expectQueryFailure(ctx, db, "select count(*) from mongodb_ci.raw", "disabled"); err != nil {
		return err
	}
...
func verifyShowCreate(ctx context.Context, db *sql.DB) error {
	...
	if err := db.QueryRowContext(ctx, "show create table mongodb_ci.raw").Scan(&tableName, &ddl); err != nil {
		return fmt.Errorf("SHOW CREATE MongoDB table: %w", err)
	}
	lower := strings.ToLower(ddl)
	if tableName != "raw" || !strings.Contains(lower, "engine = mongodb") ||
