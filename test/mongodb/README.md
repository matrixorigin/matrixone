# MongoDB connector validation and NESR operations
...
1. Run `sql/source_conflicts.js` against live and every archive segment for the whole old-key period. Record gaps and the earliest recoverable timestamp; the old MO target is not evidence that no crew was overwritten.
2. Create the `(pump, crew, datetime)` target. Install `sql/nesr_incremental.sql` for live ingestion and use `sql/nesr_backfill.sql` for bounded crew/day history slices; each stored-procedure call commits its target rows and checkpoint together. Pause/retry only from a committed checkpoint. The SQL task body contains one `CALL`, because a multi-statement task body is not transactional while a top-level stored procedure is.
...
4. Measure Mongo CPU/lag/connections, scanned rows/bytes, CN memory/CPU and end-to-end p50/p95. Do not cut over until a 3-million-row bounded run completes inside the agreed five-minute budget without exceeding source limits.
5. Enable the account feature gate for shadow reads, then move only the scheduler. Old and new writers must use separate targets and watermarks. Stop Java/Python only after the required connector gate, history repair, differential, failure recovery and owner sign-offs pass.
