...
SELECT BIT_AND(SIN(10)), BIT_OR(SIN(10)), BIT_XOR(SIN(10));
...
SELECT BIT_XOR(score) FROM t RIGHT JOIN t1 ON t.id = t1.id;
...
SELECT BIT_AND(CAST(str1 AS INT)) FROM t;
...
SELECT BIT_OR(CAST(t.str2 AS DOUBLE)), BIT_XOR(CAST(t1.str1 AS DOUBLE)) FROM t LEFT JOIN t1 ON t.id = t1.id;
