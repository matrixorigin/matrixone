...
insert into readings values
  (1, 20260514, 10.0), (1, 20260515, 11.0),
  (2, 20260514, 20.0), (2, 20260515, 21.0),
  (3, 20260514, 30.0), (3, 20260515, 31.0);
...
select site_id,
       case
         when date_id between
              (select max(date_id) from readings) - interval 7 day
              and (select max(date_id) from readings)
         then 'recent' else 'older'
       end as bucket
from readings
where site_id <= 3
order by site_id, date_id;
