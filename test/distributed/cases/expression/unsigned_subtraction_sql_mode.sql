...
create table t_widths (
    ...
    u32 int unsigned,
    u64 bigint unsigned,
    -- BIT arithmetic uses the exact DECIMAL128 domain; it is not subject to
    -- the integer unsigned-subtraction result boundary below.
    b bit(8)
);
...
select (cast(0 as unsigned) + abs(0)) - 1 as result;
select (cast('18446744073709551615' as unsigned) + 1) - cast('18446744073709551615' as unsigned) as result;
-- The integer columns switch to the signed BIGINT result domain here; BIT
-- remains DECIMAL128 because its arithmetic contract is independent of this
-- integer-only SQL mode.
...
-- Bare peers must acquire their integer result bound at EXECUTE, not lose it.
set @bare_one = 1;
set @bare_two = 2;
prepare unsigned_bare_add from 'select (cast(? as unsigned) + ?) - cast(? as unsigned) as result';
execute unsigned_bare_add using @max_uint64, @bare_one, @max_uint64;
execute unsigned_bare_add using @max_uint64, @zero_decimal, @max_uint64;
execute unsigned_bare_add using @max_uint64, @bare_one, @max_uint64;
deallocate prepare unsigned_bare_add;
prepare unsigned_bare_mul from 'select (cast(? as unsigned) * ?) - cast(? as unsigned) as result';
execute unsigned_bare_mul using @max_uint64, @bare_two, @max_uint64;
execute unsigned_bare_mul using @max_uint64, @bare_one, @max_uint64;
deallocate prepare unsigned_bare_mul;
set session sql_mode = 'NO_UNSIGNED_SUBTRACTION';
prepare unsigned_bare_add from 'select (cast(? as unsigned) + ?) - cast(? as unsigned) as result';
execute unsigned_bare_add using @max_uint64, @bare_one, @max_uint64;
deallocate prepare unsigned_bare_add;
