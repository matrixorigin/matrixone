...
-- The two 10k builds are independent consumers. Launch both before the single
-- barrier and retain both endpoint probes for each type.
...
-- No metadata row exists for an empty HNSW model. Start both bulk CDC
-- loads immediately, then wait until each serialized 10k model is materialized.
