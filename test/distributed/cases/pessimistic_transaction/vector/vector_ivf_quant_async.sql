...
-- settle for all indexes at once. Two well-separated clusters [1..]/[50..]; each
-- side is queried after each settle.
SET probe_limit=10;
...
select a from qu8 order by l2_distance(v,'[1,1,1,1]'), a limit 3;
select a from qu8 order by l2_distance(v,'[54,54,54,54]'), a limit 3;
...
select a from qu8 order by l2_distance(v,'[2,2,2,2]'), a limit 3;
select a from qu8 order by l2_distance(v,'[53,53,53,53]'), a limit 3;
