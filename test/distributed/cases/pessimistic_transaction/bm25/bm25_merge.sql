...
select id from t where bm25(txt) against('green') order by id;
select id from t where bm25(txt) against('orange') order by id;
select id from t where bm25(txt) against('yellow') order by id;
