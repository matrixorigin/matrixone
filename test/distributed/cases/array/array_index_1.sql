...
select cluster_centers(b kmeans '2,vector_cosine_ops') from t1;
SELECT value FROM  (SELECT cluster_centers(b kmeans '2,vector_cosine_ops,kmeansplusplus,false') AS centers FROM t1) AS subquery CROSS JOIN  UNNEST(subquery.centers) AS u;

-- 1.b spherical kmeans on vecf32 column
SELECT value FROM  (SELECT cluster_centers(b kmeans '2,vector_cosine_ops,kmeansplusplus,true') AS centers FROM t1) AS subquery CROSS JOIN  UNNEST(subquery.centers) AS u;
...
SELECT value FROM  (SELECT cluster_centers(c kmeans '2,vector_cosine_ops,kmeansplusplus,true') AS centers FROM t1) AS subquery CROSS JOIN  UNNEST(subquery.centers) AS u;
...
create index idx1 using IVFFLAT on tbl(embedding) lists = 2 op_type 'vector_cosine_ops';
show index from tbl;
...
create index idx2 using IVFFLAT on tbl(b) lists = 2 op_type 'vector_cosine_ops';
show index from tbl;
show create table tbl;
...
create index idx3 using IVFFLAT on tbl(a) lists = 2 op_type 'vector_cosine_ops';
create index idx4 using IVFFLAT on tbl(b,c) lists = 2 op_type 'vector_cosine_ops';
...
insert into tbl values(8, "[130,40,90]");
create index idx8 using IVFFLAT on tbl(b) lists = 2 op_type 'vector_cosine_ops';
show index from tbl;
show create table tbl;
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
create index idx12 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
show index from tbl;
show create table tbl;
...
create index idx13 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
show index from tbl;
show create table tbl;
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
create index idx14 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
show index from tbl;
show create table tbl;
insert into tbl values(15, "[1,3,5]"); -- inserted to centroid 1 of version 0
insert into tbl values(18, "[130,40,90]"); -- inserted to centroid 2 of version 0
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
create index idx15 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
create index idx16 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
delete from tbl;
...
insert into tbl values("7", "70", "[120,50,70]");
insert into tbl values("8", "80", "[130,40,90]");
create index idx17 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
...
drop table if exists tbl;
create table tbl(id int primary key, embedding vecf32(3), key idx18 using ivfflat (embedding) lists=2 op_type "vector_cosine_ops");
insert into tbl values(1, "[1,2,3]");
insert into tbl values(2, "[1,2,4]");
insert into tbl values(3, "[1,2.4,4]");
insert into tbl values(4, "[1,2,5]");
...
create index idx19 using ivfflat on tbl1(data) lists=2 op_type "vector_cosine_ops";
insert into tbl1 values(9, "[130,40,90]");

drop table if exists tbl2;
create table tbl2(id int primary key, data vecf32(3), key idx20 using ivfflat (data) lists=2 op_type "vector_cosine_ops");
...
insert into tbl values(4, "[1,2,5]");
insert into tbl values(5, "[1,3,5]");
create index idx20 using ivfflat on tbl(data) lists=2 op_type "vector_cosine_ops";
...
insert into tbl values(1, "[1,2,3]");
insert into tbl values(2, "[1,2,4]");
create index idx21 using ivfflat on tbl(data) lists=3 op_type "vector_cosine_ops";
...
drop table if exists tbl;
create table tbl(id int primary key, data vecf32(3));
create index idx22 using ivfflat on tbl(data) lists=3 op_type "vector_cosine_ops";
insert into tbl values(1, "[1,2,3]");
insert into tbl values(2, "[1,2,4]");
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
create index idx23 using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
...
--Project
--  ->  Sort
--        Sort Key: cosine_distance(tbl.embedding, cast('[120,51,70]' AS VECF32)) ASC
--        Limit: 3
--        ->  Join
...
insert into tbl values(7, "[120,50,70]");
insert into tbl values(8, "[130,40,90]");
SELECT id, embedding FROM tbl ORDER BY cosine_distance(embedding,'[120,51,70]') ASC LIMIT 3;
create index idx using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
SELECT id, embedding FROM tbl ORDER BY cosine_distance(embedding,'[120,51,70]') ASC LIMIT 3;
...
SELECT id,id2, embedding FROM tbl ORDER BY cosine_distance(embedding,'[120,51,70]') ASC LIMIT 3;
create index idx using ivfflat on tbl(embedding) lists=2 op_type "vector_cosine_ops";
SELECT id,id2, embedding FROM tbl ORDER BY cosine_distance(embedding,'[120,51,70]') ASC LIMIT 3;
SET @test_limit = 3;
...
create index idx01 using ivfflat on vector_index_05(c) lists=4 op_type "vector_cosine_ops";
...
create index idx01 using ivfflat on vector_index_04(c) lists=5 op_type "vector_cosine_ops";
...
create index idx01 using ivfflat on vector_index_08(b) lists=3 op_type "vector_cosine_ops";
...
create index idx01 using ivfflat on vector_index_09(b) lists=3 op_type "vector_cosine_ops";
select * from vector_index_09 order by cosine_distance(b,"[1, 0, 1, 6, 6, 17, 47]");
...
-- 43. Invalid vector inside cosine_distance Query.
drop table if exists vector_index_07;
create table vector_index_07(a int primary key, b vecf32(128),c int,key c_k(c));
create index idx01 using ivfflat on vector_index_07(b) lists=5 op_type "vector_cosine_ops";
...
select * from vector_index_07 order by  cosine_distance(b, "abc") ASC LIMIT 2;
...
create index idx01 using ivfflat on vector_index_08(b)  lists = 2 op_type 'vector_cosine_ops';
alter table vector_index_08 add column d vecf32(3) not null after c;
select * from vector_index_08;
