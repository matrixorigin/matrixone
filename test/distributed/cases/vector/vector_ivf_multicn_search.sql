...
-- A post-filter query must rank the complete IVF candidate budget before the
-- base-table filter. The keep rows are deliberately spread beyond the first
-- few table rows so an arrival-order FUNCTION_SCAN limit cannot define the
-- result. probe_limit=5 also exercises the customer's small-probe setting.
set probe_limit = 5;
