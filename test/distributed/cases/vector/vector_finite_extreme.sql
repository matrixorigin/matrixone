-- Finite-extreme boundaries of the vector distance functions. A value that the float32 distance
-- domain can represent must be returned whichever kernel runs (the batch kernel is selected when
-- exactly one operand is constant and the column has no nulls); a value it cannot represent is
...
-- l2_distance: the square overflows float32 while the distance is representable. Constant-vs-column
-- (batch kernel) and column-vs-column (per-row) must agree.
...
select l2_distance(v, '[0,0]') = l2_distance('[0,0]', v) as symmetric from t32 order by id;

-- Same shape with a NULL in the column disables batching; the value must not change.
