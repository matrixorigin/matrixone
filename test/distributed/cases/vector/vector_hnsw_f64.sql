...
create table vector_index_00(a bigint primary key, b vecf64(128),c int,key c_k(c), KEY idx using hnsw(b) op_type 'vector_l2_ops');

SET experimental_hnsw_index = 1;

-- create vector index: create->create index->insert
CREATE TABLE vector_empty(a bigint primary key, b vecf64(32), KEY `idxname` USING HNSW(b) M = 48 EF_CONSTRUCTION = 64 EF_SEARCH = 64 OP_TYPE "vector_l2_ops");
DROP TABLE vector_empty;

create table vector_index_01(a bigint primary key, b vecf64(128),c int,key c_k(c));

-- empty data
create index idx01 using hnsw on vector_index_01(b) op_type "vector_l2_ops" M 48 EF_CONSTRUCTION 64 EF_SEARCH 64;

-- data inserted not included in index
...
create table vector_index_02(a bigint primary key, b vecf64(128),c int,key c_k(c));
...
-- two rows inserted into index
create index idx01 using hnsw on vector_index_02(b) op_type "vector_l2_ops";
show create table vector_index_02;
desc vector_index_02;

-- result found in index
...
-- duplicate index part
create index idx01 using hnsw on vector_index_02(b) op_type "vector_l2_ops";
-- table not created
create index idx01 using hnsw on vector_index_03(b) op_type "vector_l2_ops";
create table vector_index_03(a bigint primary key, b vecf64(3));
insert into vector_index_03 values(1,"[56,23,6]"),(2,"[77,45,3]"),(3,"[8,56,3]");
-- add is keyword and cannot be name of the index
create index add using hnsw on vector_index_03(b) op_type "vector_l2_ops";
create index `add` using hnsw on vector_index_03(b) op_type "vector_l2_ops";
show create table vector_index_03;
-- wrong vector type
create index idx02 using hnsw on vector_index_03(a) op_type "vector_l2_ops";
create index idx02 using hnsw on vector_index_03(c) op_type "vector_l2_ops";
create index idx02 using hnsw on vector_index_03(b) op_type "vector_l2_ops";
-- no op_type
create index idx02 using hnsw on vector_index_03(b) ;
-- duplicate index part
create index idx03 using hnsw on vector_index_03(b) op_type "vector_l2";

-- create vector index: one table make more vector index
create table vector_index_04(a bigint primary key, b vecf64(3),c vecf64(4));
...
create index idx01 using hnsw on vector_index_04(b) op_type "vector_l2_ops";
create index idx02 using hnsw on vector_index_04(c) op_type "vector_l2_ops";
...
alter table vector_index_04 alter reindex idx01 hnsw;
alter table vector_index_04 alter reindex idx02 hnsw;
show create table vector_index_04;
desc vector_index_04;
...
create table vector_index_05(a bigint primary key, b vecf64(3),c vecf64(4));
create index idx01 using hnsw on vector_index_05(c) op_type "vector_l2_ops";
show create table vector_index_05;
alter table vector_index_05 alter reindex idx01 hnsw;
show create table vector_index_05;

-- alter reindex: syntax check
create table vector_index_06(a bigint primary key, b vecf64(3),c vecf64(4));
...
-- index not created. cannot reindex
alter table vector_index_06 alter reindex idx01 hnsw;
create index idx01 using hnsw on vector_index_06(c) op_type "vector_l2_ops";
-- index not created
alter table vector_index_06 alter reindex idx hnsw;
-- table not found
alter table vector_index_07 alter reindex idx01 hnsw;

-- L2_DISTANCE(）
create table vector_index_07(a bigint primary key, b vecf64(128),c int,key c_k(c));
...
create table vector_index_08(a bigint auto_increment primary key, b vecf64(128),c int,key c_k(c));
...
create index idx01 using hnsw on vector_index_08(b) op_type "vector_l2_ops";
...
insert into vector_index_08 values(9774 ,"[1, 0, 1, 6, 6, 17, 47, 39, 2, 0, 1, 25, 27, 10, 56, 130, 18, 5, 2, 6, 15, 2, 19, 130, 42, 28, 1, 1, 2, 1, 0, 5, 0, 2, 4, 4, 31, 34, 44, 35, 9, 3, 8, 11, 33, 12, 61, 130, 130, 17, 0, 1, 6, 2, 9, 130, 111, 36, 0, 0, 11, 9, 1, 12, 2, 100, 130, 28, 7, 2, 6, 7, 9, 27, 130, 83, 5, 0, 1, 18, 130, 130, 84, 9, 0, 0, 2, 24, 111, 24, 0, 1, 37, 24, 2, 10, 12, 62, 33, 3, 0, 0, 0, 1, 3, 16, 106, 28, 0, 0, 0, 0, 17, 46, 85, 10, 0, 0, 1, 4, 11, 4, 2, 2, 9, 14, 8, 8]",3),(9775,"[0, 1, 1, 3, 0, 3, 46, 20, 1, 4, 17, 9, 1, 17, 108, 15, 0, 3, 37, 17, 6, 15, 116, 16, 6, 1, 4, 7, 7, 7, 9, 6, 0, 8, 10, 4, 26, 129, 27, 9, 0, 0, 5, 2, 11, 129, 129, 12, 103, 4, 0, 0, 2, 31, 129, 129, 94, 4, 0, 0, 0, 3, 13, 42, 0, 15, 38, 2, 70, 129, 1, 0, 5, 10, 40, 12, 74, 129, 6, 1, 129, 39, 6, 1, 2, 22, 9, 33, 122, 13, 0, 0, 0, 0, 5, 23, 4, 11, 9, 12, 45, 38, 1, 0, 0, 4, 36, 38, 57, 32, 0, 0, 82, 22, 9, 5, 13, 11, 3, 94, 35, 3, 0, 0, 0, 1, 16, 97]",5),(9776,"[10, 3, 8, 5, 48, 26, 5, 16, 17, 0, 0, 2, 132, 53, 1, 16, 112, 6, 0, 0, 7, 2, 1, 48, 48, 15, 18, 31, 3, 0, 0, 9, 6, 10, 19, 27, 50, 46, 17, 9, 18, 1, 4, 48, 132, 23, 3, 5, 132, 9, 4, 3, 11, 0, 2, 46, 84, 12, 10, 10, 1, 0, 12, 76, 26, 22, 16, 26, 35, 15, 3, 16, 15, 1, 51, 132, 125, 8, 1, 2, 132, 51, 67, 91, 8, 0, 0, 30, 126, 39, 32, 38, 4, 0, 1, 12, 24, 2, 2, 2, 4, 7, 2, 19, 93, 19, 70, 92, 2, 3, 1, 21, 36, 58, 132, 94, 0, 0, 0, 0, 21, 25, 57, 48, 1, 0, 0, 1]",3);
alter table vector_index_08 add column d vecf64(3) not null after c;
create index idx02 using hnsw on vector_index_08(d) op_type "vector_l2_ops";
...
alter table vector_index_08 alter reindex idx02 hnsw;
select * from vector_index_08 order by  L2_DISTANCE(d,"[2.36,5.021,9.222]") desc limit 1;
...
select * from vector_index_08 order by  L2_DISTANCE(e,"[2.36,5.021,9.222]") desc limit 1;
--explain select * from vector_index_08 where a>1 order by  L2_DISTANCE(c,"[1.02,0.2,3.2]") desc;
...
alter table vector_index_09 alter reindex idx01 hnsw;
select * from vector_index_09 order by  L2_DISTANCE(b,"[1, 0, 1, 6, 6, 17, 47]");
alter table vector_index_09 drop index idx01;
show create table vector_index_09;

-- vector index transaction
create table vector_index_10(a bigint primary key, b vecf64(128),c int,key c_k(c));
set autocommit=0;
create index idx01 using hnsw on vector_index_10(b) op_type "vector_l2_ops";
...
alter table vector_index_10 alter reindex idx01 hnsw;
show create table vector_index_10;
...
create index idx01 using hnsw on t3(b) op_type "vector_l2_ops";
...
create table vector_ip_01(a bigint primary key, b vecf64(128), c int, key c_k(c));
...
drop table if exists vector_cos_01;
create table vector_cos_01(a bigint primary key, b vecf64(128),c int,key c_k(c));
...
insert into vector_cos_01 values(9777, "[16, 15, 0, 0, 5, 46, 5, 5, 4, 0, 0, 0, 28, 118, 12, 5, 75, 44, 5, 0, 6, 32, 6, 49, 41, 74, 9, 1, 0, 0, 0, 9, 1, 9, 16, 41, 71, 80, 3, 0, 0, 4, 3, 5, 51, 106, 11, 3, 112, 28, 13, 1, 4, 8, 3, 104, 118, 14, 1, 1, 0, 0, 0, 88, 3, 27, 46, 118, 108, 49, 2, 0, 1, 46, 118, 118, 27, 12, 0, 0, 33, 118, 118, 8, 0, 0, 0, 4, 118, 95, 40, 0, 0, 0, 1, 11, 27, 38, 12, 12, 18, 29, 3, 2, 13, 30, 94, 78, 30, 19, 9, 3, 31, 45, 70, 42, 15, 1, 3, 12, 14, 22, 16, 2, 3, 17, 24, 13]",4),(9778,"[41, 0, 0, 7, 1, 1, 20, 67, 9, 0, 0, 0, 0, 31, 120, 61, 25, 0, 0, 0, 0, 10, 120, 90, 32, 0, 0, 1, 13, 11, 22, 50, 4, 0, 2, 93, 40, 15, 37, 18, 12, 2, 2, 19, 8, 44, 120, 25, 120, 5, 0, 0, 0, 2, 48, 97, 102, 14, 3, 3, 11, 9, 34, 41, 0, 0, 4, 120, 56, 3, 4, 5, 6, 15, 37, 116, 28, 0, 0, 3, 120, 120, 24, 6, 2, 0, 1, 28, 53, 90, 51, 11, 11, 2, 12, 14, 8, 6, 4, 30, 9, 1, 4, 22, 25, 79, 120, 66, 5, 0, 0, 6, 42, 120, 91, 43, 15, 2, 4, 39, 12, 9, 9, 12, 15, 5, 24, 36]",4);
create index idx01 using hnsw on vector_cos_01(b) op_type "vector_cosine_ops";
