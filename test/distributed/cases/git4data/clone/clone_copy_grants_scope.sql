...
drop snapshot if exists sp_cg_scope_src;
...
--   cg_scope_r_after_snapshot is granted only after the snapshot is created
create role cg_scope_r_dt, cg_scope_r_t, cg_scope_r_parent, cg_scope_r_child, cg_scope_r_after_snapshot;
...
grant connect on account * to cg_scope_r_after_snapshot;
...
create user cg_scope_u_after_snapshot identified by '111' default role cg_scope_r_after_snapshot;
...
create snapshot sp_cg_scope_src for table cg_scope_src src;
grant select on table cg_scope_src.src to cg_scope_r_after_snapshot;
...
create table cg_scope_dst.snapshot_copy clone cg_scope_src.src {snapshot = "sp_cg_scope_src"} copy grants;

-- Direct table-object grants: source and COPY GRANTS targets should have the
-- currently visible explicit table grants. The snapshot clone should not copy
-- cg_scope_r_after_snapshot because that grant was created after the snapshot.
...
where t.reldatabase in ('cg_scope_src', 'cg_scope_dst')
  and t.relname in ('src', 'same_plain', 'same_copy', 'cross_copy', 'snapshot_copy')
...
-- snapshot_copy proves COPY GRANTS reads source grants at the snapshot TS instead
-- of the current source table grants.
...
join mo_catalog.mo_role_privs rp
 ...
 and rp.obj_id = t.rel_logical_id
where t.reldatabase in ('cg_scope_src', 'cg_scope_dst')
  and t.relname in ('src', 'same_copy', 'cross_copy', 'snapshot_copy')
  and rp.role_name like 'cg_scope_r_%'
...
select * from cg_scope_src.same_copy order by a;
select * from cg_scope_dst.cross_copy order by a;
select * from cg_scope_dst.snapshot_copy order by a;
-- @session

-- Grants created after the snapshot should be copied to live clone targets, but
-- not to the snapshot clone.
-- @session:id=9&user=acc_clone_copy_grants_scope:cg_scope_u_after_snapshot:cg_scope_r_after_snapshot&password=111
select * from cg_scope_src.src order by a;
select * from cg_scope_src.same_copy order by a;
select * from cg_scope_dst.cross_copy order by a;
select * from cg_scope_dst.snapshot_copy order by a;
...
join mo_catalog.mo_role_privs rp
 ...
 and rp.obj_id = t.rel_logical_id
where t.reldatabase in ('cg_scope_src', 'cg_scope_dst')
  and t.relname in ('same_copy', 'cross_copy', 'snapshot_copy')
  and rp.role_name in ('cg_scope_r_parent', 'cg_scope_r_child')
order by t.reldatabase, t.relname, rp.role_name, rp.privilege_name;

drop snapshot if exists sp_cg_scope_src;
drop user cg_scope_u_dt, cg_scope_u_t, cg_scope_u_child, cg_scope_u_after_snapshot;
drop user cg_scope_u_db_star, cg_scope_u_global_star, cg_scope_u_db_object, cg_scope_u_account_object;
drop role cg_scope_r_dt, cg_scope_r_t, cg_scope_r_parent, cg_scope_r_child, cg_scope_r_after_snapshot;
