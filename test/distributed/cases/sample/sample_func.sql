...
-- test `sample(n rows) using rows` syntax.
-- it's same as sample(n rows) but will avoid centroids skewed.
select count(*) from (select sample(c1, 2 rows) using rows from s_t3);
select c1, sample(c2, 1 rows) using rows from s_t3 group by c1 order by c1;