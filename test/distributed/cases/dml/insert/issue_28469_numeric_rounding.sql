...
CREATE TABLE mixed_float_dst(i INT, d DOUBLE);
INSERT INTO mixed_float_dst
SELECT 1,(1000000000000000000/1)*1000000000000000000*1000000000000000000;
UPDATE mixed_float_dst
SET i=2,d=(1000000000000000000/1)*1000000000000000000*1000000000000000000
WHERE i=1;
SELECT * FROM mixed_float_dst;
DROP TABLE mixed_float_dst;
CREATE TABLE predicate_domain_dst(i INT);
INSERT INTO predicate_domain_dst SELECT 1
WHERE (1000000000000000000/1)*1000000000000000000*1000000000000000000>0;
UPDATE predicate_domain_dst SET i=2
WHERE (1000000000000000000/1)*1000000000000000000*1000000000000000000>0;
INSERT INTO predicate_domain_dst SELECT 3
HAVING (1000000000000000000/1)*1000000000000000000*1000000000000000000>0;
INSERT INTO predicate_domain_dst SELECT (SELECT 7/2
WHERE (1000000000000000000/1)*1000000000000000000*1000000000000000000>0);
CREATE TABLE predicate_aggregate_src(x BIGINT);
INSERT INTO predicate_aggregate_src VALUES (5);
INSERT INTO predicate_domain_dst SELECT SUM(x/2) FROM predicate_aggregate_src HAVING SUM(x/2)>0;
SELECT * FROM predicate_domain_dst ORDER BY i;
DROP TABLE predicate_aggregate_src;
DROP TABLE predicate_domain_dst;
CREATE TABLE control_domain_src(x BIGINT);
INSERT INTO control_domain_src VALUES (1000000000000000000);
CREATE TABLE control_domain_dst(i INT);
INSERT INTO control_domain_dst SELECT 1 FROM control_domain_src
HAVING SUM((x/1)*x*x)>0;
INSERT INTO control_domain_dst SELECT 1 FROM control_domain_src
GROUP BY (x/1)*x*x;
INSERT INTO control_domain_dst SELECT ROW_NUMBER() OVER(ORDER BY (x/1)*x*x) FROM control_domain_src;
INSERT INTO control_domain_dst SELECT CASE WHEN (x/1)*x*x>0 THEN 1 ELSE 0 END FROM control_domain_src;
...
CREATE TABLE ondup_float_dst(id INT PRIMARY KEY, x BIGINT, z DOUBLE, r DOUBLE, i BIGINT);
INSERT INTO ondup_float_dst VALUES
(1,1000000000000000000,0,1E54,0),
(2,1000000000000000000,0,1E54,0),
(3,1000000000000000000,0,1E54,0);
UPDATE ondup_float_dst SET i=((x/1)*x*x+z)/r WHERE id=1;
INSERT INTO ondup_float_dst VALUES (2,0,0,1,0) ON DUPLICATE KEY UPDATE
i=((x/1)*x*x+z)/r;
INSERT INTO ondup_float_dst VALUES (3,0,0,1E54,0) ON DUPLICATE KEY UPDATE
i=((x/1)*x*x+VALUES(z))/VALUES(r);
SELECT id,i FROM ondup_float_dst ORDER BY id;
DROP TABLE ondup_float_dst;
...
CREATE TABLE shared_rel_src(x BIGINT);
INSERT INTO shared_rel_src VALUES (5);
CREATE TABLE shared_window_a(d DOUBLE,i BIGINT);
INSERT INTO shared_window_a SELECT SUM(x/2) OVER(),SUM(x/2) OVER() FROM shared_rel_src;
CREATE TABLE shared_window_b(i BIGINT,d DOUBLE);
INSERT INTO shared_window_b SELECT SUM(x/2) OVER(),SUM(x/2) OVER() FROM shared_rel_src;
SELECT i,d FROM shared_window_a;
SELECT i,d FROM shared_window_b;
INSERT INTO shared_rel_src VALUES (7);
CREATE TABLE shared_lag_dst(d DOUBLE,i BIGINT);
INSERT INTO shared_lag_dst SELECT
LAG(x/2,1,x/2) OVER(ORDER BY x),LAG(x/2,1,x/2) OVER(ORDER BY x) FROM shared_rel_src;
SELECT MIN(i) FROM shared_lag_dst;
CREATE TABLE shared_union_dst(d DOUBLE,i BIGINT);
INSERT INTO shared_union_dst SELECT q,q FROM
(SELECT x/2 q FROM shared_rel_src UNION ALL SELECT x/2 FROM shared_rel_src) u;
SELECT MIN(i) FROM shared_union_dst;
CREATE TABLE signed_boundary_dst(i BIGINT);
INSERT INTO signed_boundary_dst SELECT
CAST(((1000000000000000000/1)*1000000000000000000*1000000000000000000+0E0)/1E54 AS SIGNED);
SELECT * FROM signed_boundary_dst;
CREATE TABLE sample_dst(i BIGINT);
INSERT INTO sample_dst SELECT SAMPLE(x/2,1 ROWS) FROM shared_rel_src WHERE x=5;
SELECT * FROM sample_dst;
DROP TABLE sample_dst;
DROP TABLE signed_boundary_dst;
DROP TABLE shared_union_dst;
DROP TABLE shared_lag_dst;
DROP TABLE shared_window_a;
DROP TABLE shared_window_b;
DROP TABLE shared_rel_src;
