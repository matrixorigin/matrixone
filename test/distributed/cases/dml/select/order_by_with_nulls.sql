...
SELECT * FROM t1 ORDER BY name IS NULL, name ASC;
SELECT * FROM t1 ORDER BY name IS NOT NULL, name ASC;
...
SELECT * FROM t1 WHERE id > 1 ORDER BY id DESC, salary;
SELECT * FROM t1 WHERE id BETWEEN 1 AND 4 ORDER BY id DESC, dept IS NULL, dept, salary IS NOT NULL, salary DESC;
SELECT id,name FROM t1 ORDER BY salary IS NOT NULL, salary DESC, dept DESC, name ASC;

-- JOIN -> explicit NULL ordering with MySQL-compatible boolean expressions
DROP TABLE IF EXISTS t1;
DROP TABLE IF EXISTS t2;
CREATE TABLE t1(
    id INT,
...
SELECT * FROM t1 JOIN t2 ON t1.id = t2.id ORDER BY name IS NOT NULL, name DESC;
SELECT * FROM t1 LEFT JOIN t2 ON t1.id = t2.id ORDER BY name IS NULL, name ASC;
SELECT * FROM t1 RIGHT JOIN t2 ON t1.id = t2.id ORDER BY name ASC, nation DESC;
SELECT * FROM t1 INNER JOIN t2 ON t1.id = t2.id ORDER BY name DESC, nation ASC;
...
SELECT * FROM t1 LEFT JOIN t2 ON t1.id = t2.id ORDER BY sex IS NULL, sex ASC;
SELECT * FROM t1 RIGHT JOIN t2 ON t1.id = t2.id ORDER BY home ASC;
SELECT * FROM t1 INNER JOIN t2 ON t1.id = t2.id ORDER BY home ASC, sex IS NOT NULL, sex DESC;
