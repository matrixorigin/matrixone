...
-- restore to non-sys account: db level
-- @session:id=1&user=acc01:test_account&password=111
...
select * from pri01;
drop table if exists aff01;
create table aff01 (col1 int auto_increment);
insert into aff01 values(1),(2),(3);
-- @session
...
restore account acc01 database db11 table aff01 from snapshot sp18;

-- @session:id=1&user=acc01:test_account&password=111
select * from aff01;
select count(*) from aff01;
insert into aff01 values(4);
select * from aff01;
drop database db11;
-- @session
drop snapshot sp17;
