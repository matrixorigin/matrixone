...
select (select count(*) from v_direct) = 2 as direct_rows_ok, (select count(*) from v_where) = 2 as where_rows_ok, (select count(*) from v_named) = 2 as named_rows_ok, (select count(*) from v_table_star) = 2 as table_star_rows_ok, (select count(*) from v_join) = 1 as join_rows_ok, (select count(*) from v_subquery) = 2 as subquery_rows_ok, (select count(*) from v_nested) = 2 as nested_rows_ok, (select cnt from v_count) = 2 as count_value_ok;
