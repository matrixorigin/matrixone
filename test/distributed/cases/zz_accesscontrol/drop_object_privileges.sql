...
-- Rolling upgrades can retain historical hidden-index grants. Both direct
-- DROP INDEX and base-table DROP must remove those child-object rows.
drop database if exists legacy_index_grant_db;
drop role if exists legacy_index_grant_role;
create database legacy_index_grant_db;
create table legacy_index_grant_db.drop_index_t(a int, index idx_a(a));
create table legacy_index_grant_db.drop_table_t(a int, index idx_a(a));
create role legacy_index_grant_role;
grant select on table legacy_index_grant_db.drop_index_t to legacy_index_grant_role;
grant select on table legacy_index_grant_db.drop_table_t to legacy_index_grant_role;
set @drop_index_base_id = (select rel_logical_id from mo_catalog.mo_tables
                            where reldatabase = 'legacy_index_grant_db' and relname = 'drop_index_t');
set @drop_table_base_id = (select rel_logical_id from mo_catalog.mo_tables
                            where reldatabase = 'legacy_index_grant_db' and relname = 'drop_table_t');
set @drop_index_base_physical_id = (select rel_id from mo_catalog.mo_tables
                                     where reldatabase = 'legacy_index_grant_db' and relname = 'drop_index_t');
set @drop_table_base_physical_id = (select rel_id from mo_catalog.mo_tables
                                     where reldatabase = 'legacy_index_grant_db' and relname = 'drop_table_t');
set @drop_index_hidden_id = (select rel_logical_id from mo_catalog.mo_tables
                              where reldatabase = 'legacy_index_grant_db'
                                and relname = (select distinct index_table_name from mo_catalog.mo_indexes
                                               where table_id = @drop_index_base_physical_id));
set @drop_table_hidden_id = (select rel_logical_id from mo_catalog.mo_tables
                              where reldatabase = 'legacy_index_grant_db'
                                and relname = (select distinct index_table_name from mo_catalog.mo_indexes
                                               where table_id = @drop_table_base_physical_id));
insert into mo_catalog.mo_role_privs
select role_id, role_name, obj_type, @drop_index_hidden_id, privilege_id, privilege_name,
       privilege_level, operation_user_id, granted_time, with_grant_option
from mo_catalog.mo_role_privs
where role_name = 'legacy_index_grant_role' and obj_id = @drop_index_base_id
  and privilege_name = 'select';
insert into mo_catalog.mo_role_privs
select role_id, role_name, obj_type, @drop_table_hidden_id, privilege_id, privilege_name,
       privilege_level, operation_user_id, granted_time, with_grant_option
from mo_catalog.mo_role_privs
where role_name = 'legacy_index_grant_role' and obj_id = @drop_table_base_id
  and privilege_name = 'select';
select count(*) as legacy_hidden_grants_before_drop
from mo_catalog.mo_role_privs
where role_name = 'legacy_index_grant_role'
  and obj_id in (@drop_index_hidden_id, @drop_table_hidden_id);
drop index idx_a on legacy_index_grant_db.drop_index_t;
drop table legacy_index_grant_db.drop_table_t;
select count(*) as legacy_hidden_grants_after_drop
from mo_catalog.mo_role_privs
where role_name = 'legacy_index_grant_role'
  and obj_id in (@drop_index_hidden_id, @drop_table_hidden_id);
drop database legacy_index_grant_db;
drop role legacy_index_grant_role;
