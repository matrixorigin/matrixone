...
select part_id, count(*) as rows_per_part
from hive_fanout_small group by part_id order by part_id;
select part_id, count(*) as files_per_part
from (
    select part_id, __mo_filepath from hive_fanout_small group by part_id, __mo_filepath
) fp group by part_id order by part_id;
