-- =====================================================================
-- vector_cagra_wrapped_dist.sql — GPU regression for issue #26961 on CAGRA.
--
...
-- pruned; CAGRA keeps the base table scan so it never orphaned. This locks in
-- that the SHARED SELECT-side distance rewrite (replaceDistFnExprsWithScoreCol /
-- sameQueryVector) keeps CAGRA correct when the SELECT-side distance is WRAPPED
-- by a scalar (CAST/ROUND/arithmetic) or bound to an alias while the unwrapped
-- distance is the ORDER BY key.
--
-- CAGRA is approximate: only the top-1 exact-match neighbor is deterministic.
...
-- graph_degree=8 / intermediate_graph_degree=16 / itopk_size=32 fully connect
-- and fully visit this toy set so the exact match is stable (see vector_cagra.sql).
-- =====================================================================

SET experimental_cagra_index = 1;
SET cagra_threads_build = 7;

drop database if exists cagra_wrapped;
create database cagra_wrapped;
use cagra_wrapped;
...
create index idx using cagra on t(v) op_type 'vector_l2_ops' graph_degree=8 intermediate_graph_degree=16 itopk_size=32;
