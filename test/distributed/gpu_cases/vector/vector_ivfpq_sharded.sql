-- =====================================================================
-- vector_ivfpq_sharded.sql — IVF-PQ index in SHARDED distribution mode
--
-- GPU REQUIRED. Exercises the SHARDED dispatch path for IVF-PQ (the dataset
-- is split into N shards, each trains its own centroids + PQ codebook; search
-- fans out to all shards and merges the top-k). On a single-GPU host we
-- present N logical GPUs (all mapped to physical device 0) via the test-only
-- session variable gpu_multi_simulation — see pkg/vectorindex.SimulateDevices.
--
-- Data: 128 well-separated rows. The shard splitter rounds each shard down to
-- a multiple of 32 rows, so 128 rows gives 64/64 for a 2-way split (each shard
-- has enough rows to train lists=10 coarse centroids). Recall-robust params
-- (m=8, kmeans_train_percent=100, probe_limit) keep the per-shard PQ residual
-- tiny so each probe's zero-distance row is the deterministic top-1 in
...
drop database if exists ivfpq_sharded;
create database ivfpq_sharded;
use ivfpq_sharded;
