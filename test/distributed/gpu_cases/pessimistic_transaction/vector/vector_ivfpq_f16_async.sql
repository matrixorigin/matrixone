-- =====================================================================
-- vector_ivfpq_f16_async.sql — IVF-PQ vecf16 base + ISCP CDC INSERT/DELETE/UPDATE
--
-- GPU REQUIRED. The vecf16 twin of vector_ivfpq_async.sql. It proves the
...
-- IvfpqSync.AppendRecords steps the stream by that same width; the search-side
-- replayEventChunks[cuvs.Float16] then reinterprets the bytes back to half and
-- feeds the f16 brute-force overflow.
--
-- ASYNC CREATE INDEX: ivfpq_create is deferred to the first CDC iteration
...
-- overflow. Like vector_ivfpq_async / vector_hnsw_async, ALL ops run first,
...
-- exercised (IvfpqSearch over base B == cuvs.Float16).
--
-- Determinism notes (IVF-PQ is a quantized + clustered approximate index;
-- values are exact in f16):
...
--     neighbor by a wide margin, stable regardless of PQ approximation.
...
drop database if exists ivfpq_f16_cdc;
create database ivfpq_f16_cdc;
use ivfpq_f16_cdc;
...
set @stbl = (select index_table_name from mo_catalog.mo_indexes
    where table_id=(select rel_id from mo_catalog.mo_tables
                    where relname='t' and reldatabase='ivfpq_f16_cdc')
