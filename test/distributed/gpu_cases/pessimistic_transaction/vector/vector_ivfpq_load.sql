-- =====================================================================
-- vector_ivfpq_load.sql — IVF-PQ on a real 128-dim SIFT dataset
--
-- GPU REQUIRED. Loads the real sift128_base_10k dataset (10k rows) and
-- builds an IVF-PQ index over it, then loads a second 10k-row file so the
-- new data flows through ISCP CDC into the tag=1 brute-force overflow.
-- Mirrors the ivf3 block of pessimistic_transaction/vector/vector_ivf_async.
--
-- The two-phase load exercises a real-scale build (tag=0 centroids +
-- codebook from the first 10k) plus the CDC overflow path (tag=1 from the
-- second 10k). m=8 splits the 128-dim vector into 8 PQ subquantizers.
--
-- Determinism: each query vector is itself a member of the dataset, so the
-- exact-match neighbor (zero distance) is the guaranteed top-1 even though
-- IVF-PQ is approximate. Queries 1-2 hit rows in the first file (main
-- index); queries 3-4 hit rows in the second file (CDC overflow).
-- =====================================================================
...
drop database if exists ivfpq_load;
create database ivfpq_load;
use ivfpq_load;

create table t(a bigint primary key, b vecf32(128));

-- Phase 1: 10k real SIFT rows, then build the IVF-PQ model over them.
...
create index idx using ivfpq on t(b) op_type 'vector_l2_ops' lists=100 m=8 bits_per_code=8 ASYNC;
...
set @stbl = (
    ...
    where name = 'idx' and algo = 'ivfpq' and algo_table_type = 'ivfpq_index'
      and table_id in (
...
-- tag=1 overflow on top of the InitSQL-built tag=0 model.
...
-- Queries 3-4: members of the second file → CDC overflow (tag=1) exact hits.
select a from t order by l2_distance(b, "[59, 0, 0, 1, 1, 1, 5, 100, 41, 0, 0, 4, 57, 34, 31, 115, 4, 0, 0, 12, 30, 33, 43, 85, 21, 0, 0, 14, 25, 9, 10, 60, 99, 11, 0, 0, 0, 0, 10, 55, 68, 1, 0, 3, 115, 65, 42, 115, 32, 3, 0, 4, 13, 21, 104, 115, 81, 15, 15, 23, 9, 2, 21, 75, 43, 20, 1, 0, 10, 2, 2, 20, 52, 35, 32, 61, 79, 8, 7, 41, 50, 106, 96, 20, 8, 2, 11, 39, 115, 48, 53, 11, 3, 0, 2, 43, 35, 11, 0, 1, 13, 7, 0, 1, 115, 58, 54, 29, 1, 2, 0, 3, 32, 115, 99, 34, 1, 0, 0, 0, 35, 15, 52, 44, 9, 0, 0, 18]") ASC LIMIT 1;

select a from t order by l2_distance(b, "[0, 0, 0, 0, 0, 101, 82, 4, 2, 0, 0, 0, 3, 133, 133, 8, 46, 1, 2, 13, 15, 29, 87, 50, 22, 1, 0, 16, 25, 6, 18, 49, 5, 2, 0, 2, 3, 59, 70, 19, 18, 2, 0, 11, 42, 37, 30, 13, 133, 13, 4, 53, 28, 3, 8, 42, 77, 6, 11, 103, 36, 0, 0, 32, 7, 15, 59, 27, 2, 0, 2, 5, 14, 5, 55, 52, 51, 3, 2, 5, 133, 21, 10, 38, 26, 1, 0, 64, 71, 3, 10, 118, 53, 5, 6, 28, 33, 26, 73, 15, 0, 0, 0, 22, 13, 15, 133, 133, 4, 0, 0, 15, 107, 62, 46, 91, 9, 1, 7, 16, 28, 4, 0, 27, 33, 4, 15, 25]") ASC LIMIT 1;

drop database ivfpq_load;
