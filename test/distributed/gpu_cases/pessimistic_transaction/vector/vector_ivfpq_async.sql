-- =====================================================================
-- vector_ivfpq_async.sql — IVF-PQ async build + ISCP CDC INSERT/DELETE/UPDATE
--
-- GPU REQUIRED. Covers the ASYNC CREATE INDEX path: ivfpq_create is
-- deferred to the first CDC iteration (stashed as ConsumerInfo.InitSQL),
-- and a batch of DML flows through the ISCP CDC pipeline into the IVF-PQ
...
--
-- Determinism notes (IVF-PQ is a quantized + clustered approximate index):
...
SET kmeans_train_percent = 37;
SET kmeans_max_iteration = 12;

drop database if exists ivfpq_cdc;
create database ivfpq_cdc;
use ivfpq_cdc;
...
-- Async build: ivfpq_create deferred to first CDC iteration via InitSQL.
create index ix using ivfpq on t (v)
    op_type 'vector_l2_ops' lists=2 m=2 bits_per_code=8 ASYNC;
show create table t;
...
set @stbl = (select index_table_name from mo_catalog.mo_indexes
    where table_id=(select rel_id from mo_catalog.mo_tables
                    where relname='t' and reldatabase='ivfpq_cdc')
