...
-- UPDATE ops run first. The durable CDC-tail readiness query below uses a bounded @wait_expect;
-- it reads only ivfpq_meta CDC-frame metadata, then the original searches run
-- as final assertions without polling the vector index.
--
...
-- Capture the cumulative committed CDC-frame record count after CREATE and
-- before the post-CREATE DML. The initial ASYNC build writes storage tag=0
-- and an ordinary base metadata row, so it cannot pollute the cdc_tail: baseline.
...
set @capture_tail_sql = concat(
    'select coalesce(sum(nrow), 0) into @tail_baseline from `',
    database(), '`.`', @mtbl,
    '` where prefix_eq(index_id, ''cdc_tail:'')'
);
...
-- Wait for all eleven CDC event records generated by the nine post-CREATE
-- DML statements. Save writes each frame's nrow with its tail row and
-- watermark in one transaction, so this metadata-only poll spans
-- chunks/iterations without executing a vector search. It assumes the
-- provenance-enabled metadata table exposes nrow; a rolling narrow table is
-- not compatible with this gate.
set @wait_tail_sql = concat(
    'select coalesce(sum(nrow), 0) - @tail_baseline >= 11 as cdc_tail_ready from `',
    database(), '`.`', @mtbl,
    '` where prefix_eq(index_id, ''cdc_tail:'')'
);
