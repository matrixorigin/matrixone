-- =====================================================================
-- vector_ivfpq_copy_alter.sql — a COPY ALTER must leave the IVF-PQ index with a
-- BASE (tag=0) sub-index, not just the CDC tail.
--
-- GPU REQUIRED.
--
-- IVF-PQ is identity-always-async, so cloneUnaffectedIndexes SKIPS the whole index
-- (SkipWholeIndex) and the replacement table's hidden index tables start EMPTY. IvfpqSync
-- is stateless across flushes: it only APPENDS tag=1 event chunks and never writes a tag=0
-- sub-index, which is written only by ivfpq_create. With AlterCopyInitSQL returning
