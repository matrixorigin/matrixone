...
| `vector_cagra_copy_alter.sql` | CAGRA | `gpu_cases/pessimistic_transaction/vector/` | **COPY ALTER keeps a base index (#29011)**: `ALTER TABLE ... ADD COLUMN` skips the whole index clone (SkipWholeIndex), so the replacement hidden tables start empty; CagraSync only appends tag=1 chunks, so without the REINDEX seed from `AlterCopyInitSQL` the replacement index would hold no tag=0 sub-index at all. Asserts a tag=0 base before AND after the ALTER, plus correct search, row count and the new column |
| `vector_ivfpq_copy_alter.sql` | IVF-PQ | `gpu_cases/pessimistic_transaction/vector/` | same COPY ALTER base-index coverage as `vector_cagra_copy_alter.sql` |
