...
|---|---|---|---|
| batch marshal/unmarshal | `pkg/container/batch/batch.go` | MORPC v31 trailer保留来源；v30 numeric-prefix peer对source-only batch降级丢弃，旧 prepared metadata仍按旧gate拒绝 | 五类来源 buffered round-trip；v30 negative/v31 positive；invalid/reuse UT |
| CN dispatch / remote result | dispatch sender、remote-run receiver | 双向使用 MORPC v31 source gate | dispatch/remoterun version-matrix UT |
| grouping batch codec | batch grouping codec | grouping bitmap与五类来源同时 round-trip | 五类来源 grouping streaming UT |
| aggregate/group state encode/decode/merge | `aggexec.SaveIntermediateResult*`、`group.saveAggregateChunkForProtocol` | MORPC v31携带来源；旧协议显式省略；merge使用贡献规则；未知值拒绝且decoder可复用 | 五类 full/chunk round-trip、group protocol gate、same/mixed merge、invalid/reuse UT |
| Process parameters | Process/frontend codecs | SQLPrepare/COMStmt/UserVariable保留至 Param executor；v31远端携带 | process/frontend UT |
...
- 当前独立 StringSource wire capability：`MORPCVersion31`；`MORPCVersion30`归属main的prepared numeric-prefix common-type cast。
