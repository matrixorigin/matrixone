...
| GROUP BY | `pkg/sql/colexec/group` | key按SQL值分组；current/standby destination在preflight成功后显式retain，覆盖finalLength==Length的existing-row更新；selected append与existing update完成后由publication统一normalize；hash commit后不得分配 | all-existing Literal→COMStmt逆序、current-full(8192)+standby-new、borrowed ownership、Group/MergeGroup spill/retry UT |
| aggregate | `pkg/sql/colexec/aggexec` | outer preflight预留的sidecar在完整BatchFill/BatchMerge期间retain，same-source中间更新不得提前normalize；所有成功/错误出口统一finalize；fixed MIN/MAX覆盖每个非NULL loser/tie/transient winner | ANY Literal→COMStmt逆序、fixed-min loser+transient、bytes-min、MAX_BY fill+merge reject-next-allocation；Group/MergeGroup逆序spill/retry UT |
...
|---|---|---|---|
| batch marshal/unmarshal | `pkg/container/batch/batch.go` | MORPC v37 trailer保留来源；v36 JSON-comparison/exact-param-type peer对source-only batch降级丢弃，旧 prepared metadata仍按旧gate拒绝 | 五类来源 buffered round-trip；v36 negative/v37 positive；invalid/reuse UT |
| CN dispatch / remote result | dispatch sender、remote-run receiver | 双向使用 MORPC v37 source gate | dispatch/remoterun version-matrix UT |
| grouping batch codec | batch grouping codec | grouping bitmap与五类来源同时 round-trip | 五类来源 grouping streaming UT |
| aggregate/group state encode/decode/merge | `aggexec.SaveIntermediateResult*`、`group.saveAggregateChunkForProtocol` | MORPC v37携带来源；旧协议显式省略；merge使用贡献规则；未知值拒绝且decoder可复用 | 五类 full/chunk round-trip、group protocol gate、same/mixed merge、invalid/reuse UT |
| Process parameters | Process/frontend codecs | SQLPrepare/COMStmt/UserVariable保留至 Param executor；v37远端携带 | process/frontend UT |
...
- 当前独立 StringSource wire capability：`MORPCVersion37`；`MORPCVersion36`归属main的prepared JSON comparison execution and exact parameter types。
