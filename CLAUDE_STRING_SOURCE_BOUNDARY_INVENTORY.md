...
| GROUP BY | `pkg/sql/colexec/group` | key按SQL值分组；preview内按group ID把最终source保存在UnitLimit有界的operator-owned scratch，selected append以override发布且绝不修改borrowed input；sidecar从preflight跨new-row append与existing-row update保持，全部完成后统一normalize；同源保持、异源→Expression | hash-group正反序、borrowed sidecar identity/MPool ownership、共享ANY_VALUE参数、existing+new两批publication、same-preview/post-commit allocation rejection、跨batch、spill/reload、reset UT |
