...
func (c *tableCache) insertAutoValues(
	...
	for _, col := range c.cols {
		...
		if txnOp != nil && pkMap != nil {
			if _, ok := pkMap[col.ColName]; ok {
				// Checks if the value generated by incrservice has been manually inserted by the user before, if not sure, it needs to be regenerated.
				_, _, rel, err := eng.GetRelationById(ctx, txnOp, tableID)
				if err != nil {
					return 0, err
				}
				fromTs := types.TimestampToTS(cc.lastAllocateAt)
				toTs := types.TimestampToTS(currentTs)
