...
type AutoIncrementService interface {
	...
	InsertValues(ctx context.Context, tableID uint64, bat *batch.Batch, estimate int64, eng engine.Engine, txn client.TxnOperator, pkSet map[string]struct{}) (uint64, error)
...
type incrTableCache interface {
	...
	commit()
	columns() []AutoColumn
	insertAutoValues(ctx context.Context, tableID uint64, bat *batch.Batch, estimate int64, eng engine.Engine, txn client.TxnOperator, pkSet map[string]struct{}) (uint64, error)
	currentValue(ctx context.Context, tableID uint64, col string) (uint64, error)
