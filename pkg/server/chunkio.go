...
// chunkIO is a helper to read and write data in chunk format.
type chunkIO struct {
...
func newChunkIO(bufReader *bufferedConnReader) *chunkIO {
	p := &chunkIO{sequence: 0}
	p.setBufReader(bufReader)
	return p
}

func (p *chunkIO) setBufReader(bufReader *bufferedConnReader) {
	p.bufReader = bufReader
	p.bufWriter = bufio.NewWriterSize(bufReader, defaultWriterSize)
}

func (p *chunkIO) setReadTimeout(timeout time.Duration) {
	p.readTimeout = timeout
}

func (p *chunkIO) readOne() ([]byte, error) {
	var header [4]byte
	if p.readTimeout > 0 {
...
func (p *chunkIO) readFull() ([]byte, error) {
	data, err := p.readOne()
	if err != nil {
		return nil, err
	}
...
func (p *chunkIO) write(data []byte) error {
	length := len(data) - 4
