...
// Package idxcron is the bm25 index's idxcron hook. A bm25 index participates
// in scheduled compaction (SyncDescriptor.IdxcronAction = "bm25_reindex"); the
// cron executor runs `ALTER … REINDEX … BM25 [MERGE] FORCE_SYNC` when Updatable
// returns true — MERGE folds the tag=1 CdcTail into the tag=0 base, or a full
// REBUILD from source once the dead-doc fraction is high (ReindexOption).
package idxcron
...
// dev/test deploy can lower it via env MO_IDXCRON_BM25_TAIL_THRESHOLD (e.g. 1) to
...
var TailChunkThreshold = func() int64 {
	if v := os.Getenv("MO_IDXCRON_BM25_TAIL_THRESHOLD"); v != "" {
...
func (Hooks) ReindexOption(in idxcronplugin.UpdatableInput) (string, error) {
	...
	for _, idx := range in.TableDef.Indexes {
		if idx.IndexName == in.IndexName &&
			idx.IndexAlgoTableType == catalog.Bm25Index_TblType_Metadata {
