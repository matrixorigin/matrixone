...
func listBaseSubsByRecency(sqlproc *sqlexec.SqlProcess, cfg TableConfig) ([]baseSubMeta, error) {
	sql := fmt.Sprintf("SELECT %s, %s, %s, %s FROM %s ORDER BY %s ASC, %s ASC",
		catalog.Bm25Index_TblCol_Metadata_Index_Id, catalog.Bm25Index_TblCol_Metadata_Recency,
		catalog.Bm25Index_TblCol_Metadata_Filesize, catalog.Bm25Index_TblCol_Metadata_Nrow,
		sqlquote.QualifiedIdent(cfg.DbName, cfg.MetadataTable),
		catalog.Bm25Index_TblCol_Metadata_Recency, catalog.Bm25Index_TblCol_Metadata_Index_Id)
