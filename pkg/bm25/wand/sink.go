...
func NextTailChunkIdSql(cfg TableConfig) string {
	tailMax := fmt.Sprintf("COALESCE((SELECT MAX(%s) FROM %s WHERE %s = %s AND %s = %d), 0)",
		catalog.Bm25Index_TblCol_Storage_Chunk_Id, sqlquote.QualifiedIdent(cfg.DbName, cfg.IndexTable),
	...
	baseMax := fmt.Sprintf("COALESCE((SELECT MAX(%s) FROM %s), 0)",
		catalog.Bm25Index_TblCol_Metadata_Recency, sqlquote.QualifiedIdent(cfg.DbName, cfg.MetadataTable))
...
func FileChunkInsertSqls(cfg TableConfig, id string, startChunkId int64, path string, dataLen int, tag int) []string {
	prefix := fmt.Sprintf("INSERT INTO %s (%s, %s, %s, %s) VALUES ",
		...
		catalog.Bm25Index_TblCol_Storage_Index_Id, catalog.Bm25Index_TblCol_Storage_Chunk_Id,
		catalog.Bm25Index_TblCol_Storage_Data, catalog.Bm25Index_TblCol_Storage_Tag)
