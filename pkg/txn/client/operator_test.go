...
func TestCommitUnknownResolutionAfterClientCloseDoesNotReactivateWaiter(t *testing.T) {
	...
	RunTxnTests(func(c TxnClient, sender rpc.TxnSender) {
		...
		go func() {
			_, err := c.New(ctx, newTestTimestamp(0), WithUserTxn())
			newErrC <- err
		}()
		require.Eventually(t, func() bool {
			_, _, waiting := txnAdmissionCounts(c)
		...
		case <-time.After(time.Second):
			t.Fatal("client Close did not release the queued transaction")
		}

		// Resolution can outlive txn-client shutdown. It still releases the
		// retained slot exactly once, but must not publish a closed waiter.
		resolver.completeResolution()
		users, active, waiting := txnAdmissionCounts(c)
