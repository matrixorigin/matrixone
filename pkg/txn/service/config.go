...
type Config struct {
	...
	// for each DN node. Default is 400.
	MaxConnectionsPerDN int `toml:"max-conns-per-dn"`
}

func (cfg *Config) validate() {
	cfg.Logger = logutil.Adjust(cfg.Logger).With(zap.Uint64("DN", cfg.Metadata.ShardID))
	if cfg.Storage == nil {
		cfg.Logger.Fatal("Missing TxnStorage")
	}
	if cfg.Metadata.ShardID == 0 || cfg.Metadata.Address == "" {
		cfg.Logger.Fatal("Invalid DN Metadata", zap.String("metadata", cfg.Metadata.DebugString()))
	}

	if cfg.PayloadCopyBufferBytes == 0 {
		cfg.PayloadCopyBufferBytes = 16 * 1024
	}
	if cfg.MaxConnectionsPerDN == 0 {
		cfg.MaxConnectionsPerDN = 400
	}
}
