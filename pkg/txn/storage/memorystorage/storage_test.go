...
func testRead[
	Resp types.ProtoUnmarshaler,
	Req types.ProtoMarshaler,
](
	ctx context.Context,
	t *testing.T,
	s *Storage,
	...
	buf := make([]byte, req.ProtoSize())
	n, err := req.MarshalToSizedBuffer(buf)
	if err != nil {
		return err
	}

	res, err := s.Read(ctx, txnMeta, op, buf[:n])
	if err != nil {
		return err
	}
...
func testWrite[
	...
	data, err := s.Write(ctx, txnMeta, op, buf[:n])
	if err != nil {
		return err
	}
