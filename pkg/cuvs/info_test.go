		...
		t.Run(name, func(t *testing.T) {
			...
			case "float32":
				...
				case "CAGRA":
					...
					bp.GraphDegree = 128
					index, err = NewGpuCagra[float32, float32](dataset, n_vectors, dimension, L2Expanded, bp, devices, 1, distMode, nil)
				case "IVF-Flat":
				...
				case "IVF-PQ":
					...
					bp.NLists = 1000
					bp.M = 16
					index, err = NewGpuIvfPq[float32, float32](dataset, n_vectors, dimension, L2Expanded, bp, devices, 1, distMode, nil)
				}
			case "Float16":
				...
				case "CAGRA":
					...
					bp.IntermediateGraphDegree = 256
					bp.GraphDegree = 128
					index, err = NewGpuCagra[Float16, Float16](dataset, n_vectors, dimension, L2Expanded, bp, devices, 1, distMode, nil)
				case "IVF-Flat":
					bp := DefaultIvfFlatBuildParams()
					bp.NLists = 1000
				...
				case "IVF-PQ":
					...
					bp.NLists = 1000
					bp.M = 16
					index, err = NewGpuIvfPq[Float16, Float16](dataset, n_vectors, dimension, L2Expanded, bp, devices, 1, distMode, nil)
				}
			case "int8":
