...
func checkSubscriptionValidInRestore(
	...
	if !execResultArrayHasData(erArray) {
		return false, moerr.NewInternalError(newCtx, "there is no publication account %s", accName)
	}
	...
	if !execResultArrayHasData(erArray) {
		return false, moerr.NewInternalError(newCtx, "there is no publication %s", pubName)
	}
