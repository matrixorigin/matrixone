...
func newPreparedExecuteEnvForSQLWithCompilerContext(
	...
	prepareStmt := &PrepareStmt{
		...
		numericPrefixConsumer: preparedPlanHasNumericPrefixConsumer(executionPlan, paramCount),
		...
		hasPaginationParams:           plan2.PreparedPlanHasPaginationParams(executionPlan),
		hasLagLeadParams:              len(plan2.PreparedLagLeadParamPositions(executionPlan)) > 0,
	}
...
func TestInitExecuteStmtParamRebindsOnlyDomainSensitiveComparison(t *testing.T) {
	ses, prepareStmt, cw, execCtx := newPreparedExecuteEnvForSQL(t, 216, "select ? = ?")
	defer func() {
		cw.proc.SetPrepareParams(nil)
		prepareStmt.Close()
	}()
	require.Equal(t, []int32{0, 1}, prepareStmt.runtimeTextComparisonParamPositions)
	...
	install := func(left, right string, leftType, rightType defines.MysqlType) {
		t.Helper()
		cw.proc.SetPrepareParams(nil)
		...
		prepareStmt.params = vector.NewVec(types.T_text.ToType())
		require.NoError(t, vector.AppendBytes(prepareStmt.params, []byte(left), false, cw.proc.Mp()))
		require.NoError(t, vector.AppendBytes(prepareStmt.params, []byte(right), false, cw.proc.Mp()))
		prepareStmt.ParamTypes = []byte{byte(leftType), 0, byte(rightType), 0}
	}

	// Stable numeric domains must not materialize values, copy the plan, or
	// bypass the prepare-time compile.
	install("1", "1", defines.MYSQL_TYPE_LONGLONG, defines.MYSQL_TYPE_LONGLONG)
	retComp, stablePlan, stmt, _, owned, err := initExecuteStmtParam(
		execCtx, ses, cw, nil, prepareStmt.Name)
	require.NoError(t, err)
	if owned && stmt != nil {
		stmt.Free()
	...
	install("1", "1.00", defines.MYSQL_TYPE_LONGLONG, defines.MYSQL_TYPE_VAR_STRING)
	...
	require.True(t, cw.runtimeTextComparisonSpecialization)
	require.Nil(t, cw.runtimeCacheTarget,
		"value-sensitive comparison shapes must not enter the category cache")

	// A later mixed-domain value gets a fresh binding rather than inheriting a
	// plan whose exact numeric conversion was selected for the first value.
	install("2", "2.00", defines.MYSQL_TYPE_LONGLONG, defines.MYSQL_TYPE_VAR_STRING)
	retComp, reboundPlan, stmt, _, owned, err := initExecuteStmtParam(
		execCtx, ses, cw, nil, prepareStmt.Name)
	require.NoError(t, err)
	if owned && stmt != nil {
		stmt.Free()
	...
	require.Nil(t, retComp)
	require.NotSame(t, runtimePlan, reboundPlan)

	root := reboundPlan.GetQuery().Nodes[reboundPlan.GetQuery().Steps[len(reboundPlan.GetQuery().Steps)-1]]
...
func BenchmarkPreparedRuntimeTextComparisonAdmission(b *testing.B) {
	_, prepareStmt, cw, _ := newPreparedExecuteEnvForSQL(b, 217, "select ? = ?")
	defer func() {
		cw.proc.SetPrepareParams(nil)
		prepareStmt.Close()
	}()
