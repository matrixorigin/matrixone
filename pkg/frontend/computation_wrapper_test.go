...
func TestCOMStmtUnsignedArithmeticRetainsTypedIntermediateBound(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			name:              "top_level_explicit_addition_keeps_native_error",
			query:             "select cast(? as unsigned) + ?",
			peer:              1,
			peerSigned:        true,
			nativeErrorDomain: "BIGINT UNSIGNED",
		},
		{
			name:                 "bare_addition",
		...
		{
			name:                 "bare_modulo_function_signed_peer",
			query:                "select mod(cast(? as unsigned), ?) is not null",
			peer:                 2,
			peerSigned:           true,
			control:              true,
			runtimeBoundOperator: "mod",
		},
		{
		...
		for _, sqlMode := range []string{"", mysql.SQLModeNoUnsignedSubtraction} {
			t.Run(tc.name+"/"+sqlMode, func(t *testing.T) {
				...
				if tc.control {
					...
					if tc.nativeErrorDomain != "" {
						require.Contains(t, err.Error(), tc.nativeErrorDomain,
							"a top-level prepared arithmetic error must retain the native integer domain")
					}
				}
				after, err := prepareStmt.PreparePlan.GetDcl().GetPrepare().Plan.Marshal()
				require.NoError(t, err)
