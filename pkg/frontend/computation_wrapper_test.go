...
func TestInitExecuteStmtParamKeepsOrdinaryBinaryQueryOnCachedFastPath(t *testing.T) {
	ses, prepareStmt, cw, execCtx := newPreparedExecuteEnvForSQL(t, 105,
		"select 1 order by ?, ?, ?, ?")
	defer func() {
		cw.proc.SetPrepareParams(nil)
		prepareStmt.Close()
...
func TestPreparedDirectResultRejectsInvalidDecimalDescriptor(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "invalid syntax", value: "not-a-decimal"},
		{name: "precision overflow", value: strings.Repeat("9", 77)},
	} {
		t.Run(test.name, func(t *testing.T) {
			ses, prepareStmt, cw, execCtx := newPreparedExecuteEnvForSQL(t, 130, "select ? as result")
			defer func() {
				cw.proc.SetPrepareParams(nil)
				prepareStmt.Close()
			}()
			...
			_, _, _, _, _, err := initExecuteStmtParam(execCtx, ses, cw, nil, prepareStmt.Name)
			require.Error(t, err)
			require.ErrorContains(t, err, "binary DECIMAL parameter")
		})
	}
}
...
func TestInitExecuteStmtParamKeepsCachedCompileForExplicitCast(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "decimal", sql: "select cast(? as decimal(38,9))"},
		{name: "integer", sql: "select cast(? as int)"},
		{name: "double", sql: "select cast(? as double)"},
	} {
		t.Run(test.name, func(t *testing.T) {
			ses, prepareStmt, cw, execCtx := newPreparedExecuteEnvForSQL(t, 132, test.sql)
			defer func() {
				cw.proc.SetPrepareParams(nil)
				prepareStmt.Close()
			}()
			require.Empty(t, prepareStmt.directResultParamPositions,
				"an explicit cast fixes the result type")
			prepareStmt.params = vector.NewVec(types.T_text.ToType())
			require.NoError(t, vector.AppendBytes(
			...
			require.Nil(t, cw.paramVals,
				"explicit casts must not enter direct-result runtime specialization")
		})
	}
}

func TestPreparedDirectResultTypesIgnoreNumericParamsOutsideResults(t *testing.T) {
	...
	cw.proc.SetPrepareParamsWithReusableMeta(
		prepareStmt.params, nil,
		[]vector.PrepareParamKind{vector.PrepareParamNone, vector.PrepareParamInteger}, nil)
	values, specializedPositions, err := preparedParamValuesWithRuntimeTypes(
		cw.proc, prepareStmt.ParamTypes, prepareStmt.directResultParamPositions)
	require.NoError(t, err)
	require.Empty(t, specializedPositions,
		"the nested numeric descriptor must not enter direct-result specialization")
	require.Len(t, values, 2)
	require.False(t, values[0].(plan2.ParamValue).HasRuntimeType)
	require.False(t, values[1].(plan2.ParamValue).HasRuntimeType)
}

func TestPreparedDirectResultTypesEnrichOnlyNumericResults(t *testing.T) {
	_, prepareStmt, cw, _ := newPreparedExecuteEnvForSQL(
	...
	cw.proc.SetPrepareParamsWithReusableMeta(
		...
		[]vector.PrepareParamKind{
			vector.PrepareParamNone, vector.PrepareParamInteger, vector.PrepareParamFloat,
		}, nil)
	values, specializedPositions, err := preparedParamValuesWithRuntimeTypes(
		cw.proc, prepareStmt.ParamTypes, prepareStmt.directResultParamPositions)
	require.NoError(t, err)
	require.Equal(t, []int32{1}, specializedPositions)
	require.Len(t, values, 3)
	directTextParam := values[0].(plan2.ParamValue)
	directParam := values[1].(plan2.ParamValue)
	nestedParam := values[2].(plan2.ParamValue)
	require.False(t, directTextParam.HasRuntimeType,
		"a nonnumeric direct result must remain outside direct-result specialization")
	...
	require.False(t, nestedParam.HasRuntimeType,
		"a numeric descriptor outside direct-result positions must stay unenriched")
	...
	runtimePlan, specialized, err := plan2.FillValuesOfParamsInPlanWithSpecializationAtPositions(
		context.Background(), originalPlan, values, specializedPositions)
	require.NoError(t, err)
...
func TestInitExecuteStmtParamSpecializesDirectDecimalResult(t *testing.T) {
	ses, prepareStmt, cw, execCtx := newPreparedExecuteEnvForSQL(t, 134, "select ? as result")
	defer func() {
		cw.proc.SetPrepareParams(nil)
		prepareStmt.Close()
	}()
	prepareStmt.params = vector.NewVec(types.T_text.ToType())
	require.NoError(t, vector.AppendBytes(
		prepareStmt.params, []byte("0.00"), false, cw.proc.Mp()))
	prepareStmt.ParamTypes = []byte{byte(defines.MYSQL_TYPE_NEWDECIMAL), 0}
	...
	require.Equal(t, types.T_decimal64, types.T(root.ProjectList[0].Typ.Id))
	require.Equal(t, int32(2), root.ProjectList[0].Typ.Width)
	require.Equal(t, int32(2), root.ProjectList[0].Typ.Scale)
}
