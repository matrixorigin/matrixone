...
func TestInitExecuteStmtParamFreesParamsOnResolveError(t *testing.T) {
	...
	params, _, _, _, err := buildExecuteUserParams(ses, cw.proc, execPlan.Args)
...
func TestBuildExecuteUserParamsHonorsStoredProcedureScope(t *testing.T) {
	...
	params, paramVals, paramIsBin, paramKinds, err := buildExecuteUserParams(ses, cw.proc, args)
	require.NoError(t, err)
	defer params.Free(cw.proc.Mp())
...
func TestBuildExecuteUserParamsNormalizesBoolValueAndRuntimeTypeTogether(t *testing.T) {
	...
	params, values, _, kinds, err := buildExecuteUserParams(ses, cw.proc, []*plan.Expr{{
