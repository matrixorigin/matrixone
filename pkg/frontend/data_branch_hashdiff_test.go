...
func TestCompareTupleValueWithVectorNormalizesValues(t *testing.T) {
	...
	t.Run("decimal256 compares without fallback", func(t *testing.T) {
		vec := vector.NewVec(types.New(types.T_decimal256, 39, 4))
		defer vec.Free(mp)
		val, err := types.ParseDecimal256("12345678901234567890123456789012345.6789", 39, 4)
		require.NoError(t, err)
		require.NoError(t, vector.AppendFixed(vec, val, false, mp))

		cmp, err := compareTupleValueWithVector(val, vec, 0)
		require.NoError(t, err)
		require.Zero(t, cmp)
	})
