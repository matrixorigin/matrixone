...
// ---------------------------------------------------------------------------
// UT-U8 — doDropSnapshot rejects branch-managed rows by sname prefix.
// ---------------------------------------------------------------------------

// TestDropSnapshotRejectBranch pins the sentinel error produced when a user
// tries to drop a branch-managed snapshot. On 3.0-dev `mo_snapshots` has
// no `kind` column, so the reject is driven entirely by the
// `__mo_branch_<tid>` sname prefix — no catalog lookup required.
func TestDropSnapshotRejectBranch(t *testing.T) {
	ctx := context.Background()

	require.True(t, isBranchManagedSname("__mo_branch_42"),
		"sname prefix helper must recognise the canonical branch name")
	require.False(t, isBranchManagedSname("__mo_branch"),
		"prefix helper must require the trailing underscore")
	require.False(t, isBranchManagedSname("user_sp"),
		"unrelated names must not be flagged")
...
// ---------------------------------------------------------------------------
// UT-U9 — `SHOW SNAPSHOTS` plan excludes branch-managed snames.
// ---------------------------------------------------------------------------
...
// `sname NOT LIKE '__mo_branch_%'` predicate (3.0-dev has no `kind`
// column, so the sname prefix is the only discriminator available). We
// check the source text directly because the builder is unexported in the
// plan package; a source-level check is stable across refactors that keep
// the visible behaviour intact.
func TestShowSnapshotsExcludesBranch(t *testing.T) {
	...
	// The predicate must either name the branch sname prefix literally or
	// reference the databranchutils constant (via a %s-substituted
	// fmt.Sprintf form).
	require.Regexp(t,
		regexp.MustCompile(`sname\s+NOT\s+LIKE\s+'(?:__mo_branch_%|%s)'`),
		string(body),
	)
}
