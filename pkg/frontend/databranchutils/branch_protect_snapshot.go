...
// rows. The suffix is the decimal child table id. On 3.0-dev `mo_snapshots`
// has no `kind` column, so this prefix is the sole discriminator between
// user-managed and branch-managed rows. Keep this in sync with the design
// doc §4.3.
const BranchSnapshotSnamePrefix = "__mo_branch_"

// BranchSnapshotSnameLikePattern is the SQL LIKE pattern callers append to
// WHERE clauses to either include or exclude branch-managed rows. Using a
// shared constant keeps the magic string in one place.
const BranchSnapshotSnameLikePattern = BranchSnapshotSnamePrefix + "%"
...
// IsBranchManagedSname returns true if the given sname identifies a
// branch-managed snapshot row. Used at every user-facing entry point that
// must hide branch-kind rows (SHOW SNAPSHOTS, DROP SNAPSHOT, RESTORE,
// {snapshot='...'} hint, etc.).
func IsBranchManagedSname(sname string) bool {
	return strings.HasPrefix(sname, BranchSnapshotSnamePrefix)
}
...
//
// On 3.0-dev `mo_snapshots` has no `kind` column, so branch-managed rows
// are identified entirely by their synthesised `__mo_branch_<child_tid>`
// sname. Adding a defensive `sname LIKE '__mo_branch_%'` predicate (even
// though `sname IN (...)` already restricts to those exact names) makes
// the DELETE scope visible at a glance and prevents future refactors
// from accidentally widening the match.
func BuildBranchSnapshotDeleteSQL(snames []string) string {
	...
	b.WriteString("delete from mo_catalog.mo_snapshots where sname like '")
	b.WriteString(BranchSnapshotSnameLikePattern)
