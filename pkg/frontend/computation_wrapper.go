...
type TxnComputationWrapper struct {
	...
	preparedWorkloadPolicy       schedule.WorkloadPolicySet
}
...
// rebuildPreparedCompileCache replaces one cached compile generation after any
// of its materialized dependencies changed (plan/schema, SQL mode, or workload
// policy). The replacement is fully compiled before publication. If rebuilding
// fails, the old generation is invalidated: retaining a topology whose plan
// dependency already changed would let a later execution reuse stale state.
func rebuildPreparedCompileCache(
	execCtx *ExecCtx,
	ses *Session,
	prepareStmt *PrepareStmt,
	...
	var replacement *compile.Compile
	if _, ok := preparedPlan.Plan.(*plan.Plan_Query); ok &&
		shouldCachePrepareCompile(preparedPlan) &&
		!executionIntent.Explicit {
		var err error
		// Prepare-time compiles are cached and must not retain a
		// statement-owned trace. The execution path attaches the current
		// wrapper trace after cache retrieval.
		replacement, err = createCompile(
			execCtx,
			ses,
			ses.proc,
			originSQL,
		...
		if err != nil {
			if !moerr.IsMoErrCode(err, moerr.ErrCantCompileForPrepare) {
				previous := prepareStmt.compile
				prepareStmt.compile = nil
				if previous != nil {
					previous.FreeOperator()
					previous.SetIsPrepare(false)
					previous.Release()
				}
				return err
			}
		...
		// AP pipelines are execution-time state and are not retained by the
		// prepared-statement cache.
		if replacement != nil && !replacement.IsTpQuery() {
			replacement.SetIsPrepare(false)
			replacement.Release()
			replacement = nil
		}
	}
	...
	previous := prepareStmt.compile
	prepareStmt.compile = replacement
	if previous != nil {
		previous.FreeOperator()
		previous.SetIsPrepare(false)
		previous.Release()
...
func createCompile(
	...
	var policySet schedule.WorkloadPolicySet
