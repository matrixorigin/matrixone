...
func initExecuteStmtParamWithResolverInSession(
	...
	if execPlan == nil && execCtx.input != nil && execCtx.input.isBinaryProtExecute &&
		...
		!runtimePlanApplied {
		var directSpecialized bool
		cwft.paramVals, directSpecialized, err = preparedParamValuesWithRuntimeTypes(
			cwft.proc, prepareStmt.ParamTypes, prepareStmt.directResultParamPositions)
		if err != nil {
			return nil, nil, nil, originSQL, false, err
		}
		if directSpecialized {
			var fillErr error
...
func preparedParamValues(proc *process.Process, paramTypes []byte) ([]any, error) {
	...
	for i := range values {
		...
		if i*2+1 < len(paramTypes) {
			...
			if mysqlType == defines.MYSQL_TYPE_TINY &&
...
func preparedParamValuesWithRuntimeTypes(
	...
	// Start from the protocol-neutral values. This helper is intentionally
	// scoped to direct result positions; inferring runtime types for unrelated
	// function arguments here would turn a direct-result metadata refresh into
	// the broad per-execute specialization that this path avoids.
	values, err := preparedParamValues(proc, nil)
	if err != nil {
	...
	for i := 0; i < params.Length(); i++ {
		...
		// Keep the protocol provenance and specialization flags produced by
		// preparedParamValues. Only the execution type is enriched for a direct
		// result marker; rebuilding the value from scratch would silently drop
		// RetainParamRef/EnableNumericPrefix and change later plan rebinding.
		paramValue.RuntimeType = runtimeType
		paramValue.HasRuntimeType = true
		values[i] = paramValue
		specialized = true
