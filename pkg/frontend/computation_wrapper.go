...
func initExecuteStmtParamWithResolverInSession(
	...
	if prepareStmt.params != nil && prepareStmt.params.Length() > 0 { // use binary protocol
		...
		if needsRuntimeParamVals {
			...
			if runtimeDirectResultCandidate && !runtimeNumericPrefixCandidate && !runtimeNumericOverloadCandidate {
...
// specializePreparedExecutionPlan consumes generation-cached static plan
// capability plus execute-specific numeric-overload and direct-result admission.
// It must not derive the static capability itself because this function is on
...
func buildPlanForCompileRetry(
	...
	forceNumericOverload := len(plan2.PreparedPlanNumericFallbackParamPositions(retryPlan)) > 0
	runtimePlan, _, applied, err := specializePreparedExecutionPlan(
		...
		forceNumericOverload, preparedRetry.directResultSpecialization, needsRuntimeSpecialization)
	if err != nil {
		return nil, err
	}
	if applied {
