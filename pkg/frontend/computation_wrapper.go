...
func (cwft *TxnComputationWrapper) Compile(any any, fill func(*batch.Batch) error) (interface{}, error) {
	...
	if _, ok := cwft.stmt.(*tree.Execute); ok {
		...
		retComp, plan, stmt, sql, err := replaceExecutePlan(execCtx.reqCtx, cwft.ses.(*Session), cwft, executePlan)
		if err != nil {
			return nil, err
		}
...
// replaceExecutePlan replaces the plan of the EXECUTE by the plan generated by
// the PREPARE and setups the params for the plan.
func replaceExecutePlan(reqCtx context.Context, ses *Session, cwft *TxnComputationWrapper, execPlan *plan.Execute) (*compile.Compile, *plan.Plan, tree.Statement, string, error) {
	...
	for _, obj := range preparePlan.GetSchemas() {
		change, err := ses.txnCompileCtx.ifTableDefChange(obj.SchemaName, obj.ObjName, uint64(obj.Obj), uint32(obj.Server))
		if err != nil {
