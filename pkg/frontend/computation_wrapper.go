...
func initExecuteStmtParamWithResolverInSession(
	...
	if prepareStmt.params != nil && prepareStmt.params.Length() > 0 { // use binary protocol
		...
		if kinds == nil && binaryString == nil {
			cwft.proc.SetPrepareParams(prepareStmt.params)
		} else {
			cwft.proc.SetPrepareParamsWithMeta(prepareStmt.params, nil, kinds, binaryString)
		}
...
func mysqlPrepareParamIsBinaryString(mysqlType defines.MysqlType) bool {
	switch mysqlType {
