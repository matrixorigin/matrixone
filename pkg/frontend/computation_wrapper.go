...
func (cwft *TxnComputationWrapper) Compile(any any, fill func(*batch.Batch) error) (interface{}, error) {
	...
	if _, ok := cwft.stmt.(*tree.Execute); ok {
		...
		retComp, plan, stmt, sql, err := resolveExecutePlan(execCtx.reqCtx, cwft.ses.(*Session), cwft, executePlan)
		if err != nil {
			return nil, err
		}
...
// resolveExecutePlan replaces the plan of the EXECUTE by the plan generated by
// the PREPARE and setups the params for the plan.
func resolveExecutePlan(reqCtx context.Context, ses *Session, cwft *TxnComputationWrapper, execPlan *plan.Execute) (*compile.Compile, *plan.Plan, tree.Statement, string, error) {
