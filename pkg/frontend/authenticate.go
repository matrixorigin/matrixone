...
func doAlterAccount(ctx context.Context, ses *Session, aa *tree.AlterAccount) (err error) {
	...
	alterAccountFunc := func() (rtnErr error) {
		...
		if execResultArrayHasData(erArray) {
			...
			if !aa.IfExists {
				rtnErr = moerr.NewInternalError(ctx, "there is no account %s", aa.Name)
				return
			}
		}
...
func doCreateStage(ctx context.Context, ses *Session, cs *tree.CreateStage) (err error) {
	...
	if stageExist {
		if !cs.IfNotExists {
			err = moerr.NewInternalError(ctx, "the stage %s exists", cs.Name)
			return
		} else {
			// do nothing
			return err
...
func doCheckFilePath(ctx context.Context, ses *Session, ep *tree.ExportParam) (err error) {
	...
	if !strings.Contains(filePath, ":") {
		...
		if execResultArrayHasData(erArray) {
			err = moerr.NewInternalError(ctx, "stage exists, please try to check and use a stage instead")
			return err
		} else {
			// use the filepath
			return err
...
func doDropStage(ctx context.Context, ses *Session, ds *tree.DropStage) (err error) {
	...
	if !stageExist {
		if !ds.IfNotExists {
			err = moerr.NewInternalError(ctx, "the stage %s not exists", ds.Name)
			return err
		} else {
			// do nothing
			return err
...
func doAlterPublication(ctx context.Context, ses *Session, ap *tree.AlterPublication) (err error) {
	...
	if !tenantInfo.IsAdminRole() {
		err = moerr.NewInternalError(ctx, "only admin can alter publication")
		return err
	}
	...
	createNewAccount := func() (rtnErr error) {
		...
		if exists {
			if !ca.IfNotExists { //do nothing
				rtnErr = moerr.NewInternalError(ctx, "the tenant %s exists", ca.Name)
				return
			}
			return rtnErr
		} else {
		...
		if cf.Import {
			...
			if cf.Language == string(tree.PYTHON) {
				if !strings.HasSuffix(cf.Body, ".py") &&
					...
					err = moerr.NewInvalidInput(ctx, "file '"+cf.Body+"', only support '*.py', '*.whl'")
					return err
				}
				if strings.HasSuffix(cf.Body, ".whl") {
					dotIdx := strings.LastIndex(cf.Handler, ".")
					if dotIdx < 1 {
						err = moerr.NewInvalidInput(ctx, "handler '"+cf.Handler+"', when you import a *.whl, the handler should be in the format of '<file or module name>.<function name>'")
						return err
					}
				}
			}
