...
func protocolVersionForTenantInitializationWithContext(
	...
	if version < defines.MORPCVersion88 {
		...
		// contract. Only the new VIEWS function needs the v87 predecessor
	...
	// v88; the latter says that the local catalog fence has completed and new
	// v88 metadata may be published. Keep account creation on the predecessor
	...
	if floorValue, present := rt.GetGlobalVariables(
		...
		if !valid || floor < defines.MORPCVersion88 {
	...
	supported, err := compile.AllCNsSupportProtocolWithContext(ctx, proc, defines.MORPCVersion88)
	if err != nil {
