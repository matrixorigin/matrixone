...
func protocolVersionForTenantInitializationWithContext(
	...
	if version < defines.MORPCVersion89 {
		...
		// contract. Only the new VIEWS function needs the v88 predecessor
	...
	// v89; the latter says that the local catalog fence has completed and new
	// v89 metadata may be published. Keep account creation on the predecessor
	...
	if floorValue, present := rt.GetGlobalVariables(
		...
		if !valid || floor < defines.MORPCVersion89 {
	...
	supported, err := compile.AllCNsSupportProtocolWithContext(ctx, proc, defines.MORPCVersion89)
	if err != nil {
