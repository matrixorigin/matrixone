...
func outputColumnsFromRewriteSelectStatement(stmt tree.SelectStatement) ([]rewriteRuleOutputColumn, bool) {
	...
	case *tree.UnionClause:
		// Only allow merging union distinct operations (those generated by our merge logic)
		// Other set operations (UNION ALL, INTERSECT, EXCEPT, MINUS) have different semantics
		// and merging them could change the original semantics
		...
		// Other set operations (UNION ALL, INTERSECT, EXCEPT, MINUS) are not mergeable
		return nil, false
	case *tree.ParenSelect:
		if s.Select == nil {
			return nil, false
