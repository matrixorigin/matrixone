...
func rewriteRuleSingleTableExpr(expr tree.TableExpr) (*tree.AliasedTableExpr, bool) {
	...
	if parenExpr, ok := expr.(*tree.ParenTableExpr); ok {
		expr = parenExpr.Expr
	}
	...
	if parenExpr, ok := tableExpr.Expr.(*tree.ParenTableExpr); ok {
		tableExprCopy := *tableExpr
		tableExprCopy.Expr = parenExpr.Expr
		tableExpr = &tableExprCopy
	}
