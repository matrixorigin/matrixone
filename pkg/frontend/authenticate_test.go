...
func TestCreateTablesInformationSchemaPreservesKnownProtocolFallback(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Without a process there is no all-CN proof, so even a v84 local
		// runtime must use the safe predecessor VIEWS definition.
		{name: "protocol 84 without process", protocol: defines.MORPCVersion84, wantCurrentColumns: true, wantCurrentRoles: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			// Protocols below v84 also use the predecessor VIEWS definition; the
...
func TestProtocolVersionForTenantInitializationUsesLegacyForUnknownRuntime(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, defines.MORPCVersion84, version)
...
func TestCreateTablesInformationSchemaUsesCommonProtocolGate(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{
			name: "all CNs at v85 use current DDL",
			responses: map[string]int64{
				"cn-v85-query": defines.MORPCVersion85,
				"cn-v84-query": defines.MORPCVersion85,
			},
			wantLatest: true,
		},
		{
...
func TestCreateTablesInformationSchemaUsesCallerContext(t *testing.T) {
	...
	queryClient := newMockQueryClient()
	queryClient.cnResponses["cn-v85-query"] = &querypb.Response{
		GetProtocolVersion: &querypb.GetProtocolVersionResponse{Version: defines.MORPCVersion85},
	}
	proc.Base.QueryClient = queryClient
	rt.SetGlobalVariables(moruntime.ClusterService, cluster)
	rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion85)
