...
func TestCreateTablesInformationSchemaPreservesKnownProtocolFallback(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Without a process there is no all-CN proof, so even a v87 local
		// runtime must use the safe predecessor VIEWS definition.
		{name: "protocol 87 without process", protocol: defines.MORPCVersion87, wantCurrentColumns: true, wantCurrentRoles: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			// Protocols below v88 also use the predecessor VIEWS definition; the
...
func TestProtocolVersionForTenantInitializationUsesLegacyForUnknownRuntime(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, defines.MORPCVersion87, version)
...
func TestCreateTablesInformationSchemaUsesCommonProtocolGate(t *testing.T) {
	...
	cluster := &tenantInitializationProtocolCluster{cns: []metadata.CNService{
		{ServiceID: "cn-v88", QueryAddress: "cn-v88-query", PipelineServiceAddress: "cn-v88-pipeline"},
		{ServiceID: "cn-v87", QueryAddress: "cn-v87-query", PipelineServiceAddress: "cn-v87-pipeline"},
	}}
	t.Cleanup(func() {
		if hadProtocol {
			rt.SetGlobalVariables(moruntime.MOProtocolVersion, oldProtocol)
		} else {
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion88)
		}
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			responses: map[string]int64{
				"cn-v88-query": defines.MORPCVersion88,
				"cn-v87-query": defines.MORPCVersion87,
			},
			authoringFloor: defines.MORPCVersion88,
			wantLegacy:     true,
		},
		{
			name: "all CNs at v88 use current DDL",
			responses: map[string]int64{
				"cn-v88-query": defines.MORPCVersion88,
				"cn-v87-query": defines.MORPCVersion88,
			},
			authoringFloor: defines.MORPCVersion88,
			wantLatest:     true,
		},
...
func TestCreateTablesInformationSchemaUsesCallerContext(t *testing.T) {
	...
	queryClient := newMockQueryClient()
	queryClient.cnResponses["cn-v88-query"] = &querypb.Response{
		GetProtocolVersion: &querypb.GetProtocolVersionResponse{Version: defines.MORPCVersion88},
	}
	proc.Base.QueryClient = queryClient
	rt.SetGlobalVariables(moruntime.ClusterService, cluster)
	rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion88)
