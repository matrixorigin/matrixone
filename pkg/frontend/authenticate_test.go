...
func TestCreateTablesInformationSchemaPreservesKnownProtocolFallback(t *testing.T) {
	...
	for _, test := range []struct {
		...
		// Without a process there is no all-CN proof, so even a v88 local
		// runtime must use the safe predecessor VIEWS definition.
		{name: "protocol 88 without process", protocol: defines.MORPCVersion88, wantCurrentColumns: true, wantCurrentRoles: true},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			// Protocols below v89 also use the predecessor VIEWS definition; the
...
func TestProtocolVersionForTenantInitializationUsesLegacyForUnknownRuntime(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, defines.MORPCVersion88, version)
...
func TestCreateTablesInformationSchemaUsesCommonProtocolGate(t *testing.T) {
	...
	cluster := &tenantInitializationProtocolCluster{cns: []metadata.CNService{
		{ServiceID: "cn-v89", QueryAddress: "cn-v89-query", PipelineServiceAddress: "cn-v89-pipeline"},
	...
	t.Cleanup(func() {
		if hadProtocol {
			...
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion89)
		}
	...
	for _, test := range []struct {
	...
	}{
		{
			...
			responses: map[string]int64{
				"cn-v89-query": defines.MORPCVersion89,
				"cn-v88-query": defines.MORPCVersion88,
			},
			authoringFloor: defines.MORPCVersion89,
			wantLegacy:     true,
		},
		{
			name: "all CNs at v89 use current DDL",
			responses: map[string]int64{
				"cn-v89-query": defines.MORPCVersion89,
				"cn-v88-query": defines.MORPCVersion89,
			},
			authoringFloor: defines.MORPCVersion89,
			wantLatest:     true,
		},
...
func TestCreateTablesInformationSchemaUsesCallerContext(t *testing.T) {
	...
	queryClient := newMockQueryClient()
	queryClient.cnResponses["cn-v89-query"] = &querypb.Response{
		GetProtocolVersion: &querypb.GetProtocolVersionResponse{Version: defines.MORPCVersion89},
	}
	proc.Base.QueryClient = queryClient
	rt.SetGlobalVariables(moruntime.ClusterService, cluster)
	rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion89)
