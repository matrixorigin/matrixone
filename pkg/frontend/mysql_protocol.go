...
func (mp *MysqlProtocolImpl) handleHandshake(payload []byte) (bool, error) {
	...
	if err := mp.authenticateUser(authResponse); err != nil {
		...
		_ = mp.sendErrPacket(uint16(fail.ErrorCode), string(fail.SqlStates[0]), "Access denied for user")
		return false, err
	}
...
func (mp *MysqlProtocolImpl) SendResultSetTextBatchRowSpeedup(mrs *MysqlResultSet, cnt uint64) error {
	...
	for i := uint64(0); i < cnt; i++ {
		...
		if err != nil {
			...
			err1 := mp.sendErrPacket(uint16(moerr.ER_UNKNOWN_ERROR), DefaultMySQLState, err.Error())
			if err1 != nil {
				return err1
			}
			return err
