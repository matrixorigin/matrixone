...
func GetExplainColumn(ctx context.Context, explainColName string) ([]*plan.ColDef, []interface{}, error) {
	cols := []*plan.ColDef{
		{
...
func executeResultRowStmt(ses *Session, execCtx *ExecCtx) (err error) {
	...
	case *tree.Select:
		...
		ses.rs = &plan.ResultColDef{ResultCols: planner.GetResultColumnsFromPlan(execCtx.cw.Plan())}
	...
	case *tree.ExplainAnalyze, *tree.ExplainPhyPlan:
		...
		if _, ok := statement.(*tree.ExplainAnalyze); ok {
			explainColName = planner.GetPlanTitle(query, txnHaveDDL)
		} else {
			explainColName = planner.GetPhyPlanTitle(query, txnHaveDDL)
		}
...
func (resper *MysqlResp) respStreamResultRow(ses *Session,
	...
	case *tree.ExplainAnalyze:
		...
		explainColName := planner.GetPlanTitle(queryPlan.GetQuery(), txnHaveDDL)
		//if it is the plan from the EXECUTE,
		// replace the plan by the plan generated by the PREPARE
		if len(execCtx.cw.ParamVals()) != 0 {
			queryPlan, err = planner.FillValuesOfParamsInPlan(execCtx.reqCtx, queryPlan, execCtx.cw.ParamVals())
			if err != nil {
				return
			}
		}
