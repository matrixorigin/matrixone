	...
	case tree.RESTORELEVELACCOUNT:
		...
		if len(stmt.AccountName) == 0 {
			...
			if pitrRecord.level == tree.PITRLEVELACCOUNT.String() && pitrRecord.accountName != string(stmt.AccountName) {
				return moerr.NewInternalErrorNoCtxf("pitr %s is created for %s not allowed to restore account %s", pitrRecord.pitrName, pitrRecord.accountName, string(stmt.SrcAccountName))
			}
