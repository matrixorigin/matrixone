...
// isBranchManagedSname reports whether the given mo_snapshots.sname
// identifies a branch-managed row. On 3.0-dev `mo_snapshots` has no `kind`
// column, so this prefix is the only available discriminator.
func isBranchManagedSname(sname string) bool {
	return databranchutils.IsBranchManagedSname(sname)
}
...
func createBranchProtectSnapshot(
	...
	// Branch protect snapshots are identified entirely by their
	// `__mo_branch_<child_tid>` sname — 3.0-dev's mo_snapshots has no
	// `kind` column to mark them with, so the sname prefix is the single
	// discriminator used by every user-facing entry point that must hide
	// branch rows (§7 of design doc, adapted for 3.0-dev).
	//
	// The 8-column insert matches the schema exactly. Values are
	// interpolated via fmt.Sprintf because every user-controllable string
	// here (account name, db/table name) has already passed through the MO
	// parser/catalog path, so it is a legal MySQL identifier and never
	// carries a quote that could break the literal.
	insertSQL := fmt.Sprintf(
		insertIntoMoSnapshots,
		newUUID.String(),
		sname,
		receipt.snapshotTS,
