		...
		genBatch := func() *batch.Batch {
			return allocTestBatch(
				...
				[]types.Type{
					...
					types.Type{Oid: types.T_int16},
					types.Type{Oid: types.T_uint16},
					types.Type{Oid: types.T_int32},
					types.Type{Oid: types.T_uint32},
					types.Type{Oid: types.T_int64},
					types.Type{Oid: types.T_uint64},
					types.Type{Oid: types.T_float32},
					types.Type{Oid: types.T_float64},
					types.Type{Oid: types.T_char},
					types.Type{Oid: types.T_varchar},
					types.Type{Oid: types.T_date},
					types.Type{Oid: types.T_datetime},
				},
				3)
		}
...
func Test_handleSelectVariables(t *testing.T) {
	convey.Convey("handleSelectVariables succ", t, func() {
		...
		v := "tx_isolation"
		proto := NewMysqlClientProtocol(0, ioses, 1024, pu.SV)
		ses := &Session{Mrs: &MysqlResultSet{}, protocol: proto}
		mce := &MysqlCmdExecutor{}
		mce.PrepareSessionBeforeExecRequest(ses)

		convey.So(mce.handleSelectVariables(v), convey.ShouldBeNil)

		v = ""
		convey.So(mce.handleSelectVariables(v), convey.ShouldNotBeNil)
...
func Test_handleShowVariables(t *testing.T) {
	convey.Convey("handleShowVariables succ", t, func() {
		...
		convey.So(mce.handleShowVariables(nil), convey.ShouldBeNil)
	})
}
