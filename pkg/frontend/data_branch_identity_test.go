...
func TestPrepareLogicalRestoreDatabaseCapability(t *testing.T) {
	...
	prepared, err = prepareLogicalRestoreDatabase(ctx, "branch_db", marked, defines.MORPCVersion75)
	require.NoError(t, err)
...
func TestPreflightLogicalRestoreDatabases(t *testing.T) {
	...
	loaded = nil
	require.NoError(t, preflightLogicalRestoreDatabases(ctx, dbNames, defines.MORPCVersion95, load))
