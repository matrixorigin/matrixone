...
func TestViewMetadataStatusFailsClosedBeforeActivationCompletes(t *testing.T) {
	require.True(t, viewMetadataStatusIsCurrent(
		false, "", "", catalog.ViewRefreshStatusLegacyScan, false))
	require.False(t, viewMetadataStatusIsCurrent(true, catalog.ViewRefreshStatusCurrent,
	...
	for _, status := range []string{
		...
		require.False(t, viewMetadataStatusIsCurrent(
			true, catalog.ViewRefreshStatusCurrent, "", status, true))
		require.False(t, viewMetadataStatusIsCurrent(
			true, catalog.ViewRefreshStatusCurrent, "", status, false))
	}
}
