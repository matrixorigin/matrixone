...
func (m *GlobalSysVarsMgr) Get(accountId uint32, ses *Session, ctx context.Context, bh BackgroundExec) (*SystemVariables, error) {
	...
	if startedVars != nil {
		mutationGeneration = startedVars.getMutationGeneration()
	...
	currentVars.applyCatalogSnapshot(sysVarsMp, mutationGeneration, startCommitTS)
	return currentVars, nil
}
...
func (sv *SystemVariables) applyCatalogSnapshot(
	...
	startMutationGeneration uint64,
	startCommitTS pbtimestamp.Timestamp,
) {
	sv.mu.Lock()
	defer sv.mu.Unlock()

	// A successful SET GLOBAL that raced this catalog read is newer than the
	// snapshot. Preserve the published cache rather than rolling any variable
	// back to the older catalog image.
	if sv.mutationGeneration != startMutationGeneration {
		return
	}
