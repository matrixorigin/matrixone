...
func TestNewObjectWriter(t *testing.T) {
	...
	indexes, err := objectReader.ReadIndex(blocks[0].GetExtent(), idxs, BloomFilterType, nil)
	assert.Nil(t, err)
	assert.Equal(t, 3, len(indexes))
	assert.Equal(t, "test index 0", string(indexes[0].(*BloomFilter).buf))
	indexes, err = objectReader.ReadIndex(blocks[0].GetExtent(), idxs, ZoneMapType, nil)
	assert.Nil(t, err)
	assert.Equal(t, 3, len(indexes))
