...
func Sort(col containers.Vector, idx []uint32) (ret containers.Vector) {
	...
	for i := 0; i < n; i++ {
		dataWithIdx[i] = sortElem{data: col.Get(i).(types.Decimal64), idx: uint32(i)}
	}
...
func Merge(col []containers.Vector, src *[]uint32, fromLayout, toLayout []uint32) (ret []containers.Vector, mapping []uint32) {
	...
	for i := 0; i < nBlk; i++ {
		heap[i] = heapElem{data: col[i].Get(0).(types.Decimal64), src: uint32(i), next: 1}
	}
	heapInit(heap)
	...
	for i := 0; i < len(toLayout); i++ {
		for j := 0; j < int(toLayout[i]); j++ {
			...
			if int(top.next) < int(fromLayout[top.src]) {
				heapPush(&heap, heapElem{data: col[top.src].Get(int(top.next)).(types.Decimal64), src: top.src, next: top.next + 1})
			}
		}
	}
	return
