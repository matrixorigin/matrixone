...
func TestAggregator(t *testing.T) {
	...
	targetBytes := []byte(`[4,5,10.000,15,20,25,2,0,34.9262]`)
	for idx := 0; idx < 4; idx++ {
	...
	targetBytes = []byte(`[4,5,2.000,15,20,25,2,0,34.9262]`) // re-calculate memory usage in FillRow
	for idx := 0; idx < 4; idx++ {
	...
	require.Equal(t, []byte(`[4,5,10.000,15,20,25,0,0,34.9262]`), results[0].(*StatementInfo).GetStatsArrayBytes())
	results[0].(*StatementInfo).FillRow(ctx, row) // re-calculate memory usage in FillRow
	require.Equal(t, []byte(`[4,5,2.000,15,20,25,0,0,34.9262]`), results[0].(*StatementInfo).GetStatsArrayBytes())
...
func TestAggregator_DisableAgg(t *testing.T) {
	type fields struct {
		elems int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < tt.fields.elems; i++ {
				...
				require.Equal(t, err, ErrFilteredOut)
			}
