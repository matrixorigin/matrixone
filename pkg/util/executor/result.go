...
// rows for each call.
//
// Deprecated: Use ReadRowsWithRowCount instead
...
// ReadRowsWithRowCount read all rows, apply is used to read cols data in a row. If apply return
...
func (res Result) ReadRowsWithRowCount(apply func(rows int, cols []*vector.Vector) bool) {
	for _, rows := range res.Batches {
