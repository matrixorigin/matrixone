...
//
// If you want to run this code, you need some source data, that could copy from folder `mo-data-old-checkpoint/etl/sys/logs/...`, which generated by mo-service startup with config `etc/launch-tae-logservice`
package main
...
func main() {
	...
	fs, err := fileservice.NewLocalETLFS(defines.ETLFileServiceName, "mo-data-old-checkpoint/etl")
	if err != nil {
	...
	if len(files) == 0 {
		logutil.Infof("skipping, no mo-data-old-checkpoint/etl folder")
		return
	}
