...
var dummyViewCreateSql = "CREATE VIEW IF NOT EXISTS `db_dummy`.`view` as select `str`, mo_log_date(`__mo_filepath`) as `__mo_log_date`, `__mo_filepath` from `db_dummy`.`tbl_dummy` where `str` = \"NIL\""
