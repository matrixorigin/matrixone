...
var (
	...
	InformationSchemaColumnsDDL = fmt.Sprintf("CREATE VIEW information_schema.COLUMNS AS select "+
		...
		"and mc.att_is_hidden = 0 and mc.att_relname!='%s' and mc.att_relname not like '%s' and mc.attname != '%s' and mc.att_relname not like '%s' and mc.att_relname != '%s' and not regexp_like(lower(mt.relname), '%s') and %s",
	...
	InformationSchemaTableConstraintsDDL = fmt.Sprintf("CREATE VIEW information_schema.TABLE_CONSTRAINTS AS SELECT "+
		...
		"FROM mo_catalog.mo_indexes idx "+
		"join mo_catalog.mo_tables tbl on idx.table_id = tbl.rel_id "+
		"where not regexp_like(lower(tbl.relname), '%s') and %s UNION ALL "+
		...
		"FROM mo_check_constraints() cc", catalog.LifecycleRestoreTableSQLRegexpPattern, catalog.NonTemporaryTableSQLPredicate("tbl"))
