...
var (
	...
	InformationSchemaColumnsDDL = fmt.Sprintf("CREATE VIEW information_schema.COLUMNS AS select "+
		...
		"case when mc.att_is_auto_increment = 1 then 'auto_increment' when mc.attr_has_generated = 1 then mo_show_visible_bin(mc.attr_generated, 6) else '' end as EXTRA,"+
		"'select,insert,update,references' as `PRIVILEGES`,"+
		"mc.att_comment as COLUMN_COMMENT,"+
		"case when mc.attr_has_generated = 1 then cast(mo_show_visible_bin(mc.attr_generated, 5) as varchar(500)) else cast('' as varchar(500)) end as GENERATION_EXPRESSION,"+
