...
var (
	InformationSchemaKeyColumnUsage = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.KEY_COLUMN_USAGE("+
	...
	InformationSchemaColumns = fmt.Sprintf("CREATE VIEW %s.COLUMNS AS select "+
		...
		"where account_id = current_account_id() and att_relname!='%s' and att_relname not like '%s' and attname != '%s'",
		InformationDBConst, catalog.MOAutoIncrTable, catalog.PrefixPriColName+"%", catalog.Row_ID)

	InformationSchemaProfiling = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.PROFILING ("+
	...
	InformationSchemaUserPrivileges = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.USER_PRIVILEGES ("+
	...
	InformationSchemaSchemata = fmt.Sprintf("CREATE VIEW %s.SCHEMATA AS SELECT "+
	...
	InformationSchemaCharacterSets = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.CHARACTER_SETS("+
	...
	InformationSchemaTriggers = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.TRIGGERS("+
	...
	InformationSchemaTables = fmt.Sprintf("CREATE VIEW IF NOT EXISTS %s.TABLES AS "+
		...
		"WHERE tbl.account_id = current_account_id() and tbl.relname not like '%s' and tbl.relkind != '%s';",
		InformationDBConst, catalog.IndexTableNamePrefix+"%", catalog.SystemPartitionRel)

	InformationSchemaPartitions = fmt.Sprintf("CREATE VIEW IF NOT EXISTS %s.`PARTITIONS` AS "+
		"SELECT "+
		"'def' AS `TABLE_CATALOG`,"+
		...
		"WHERE `tbl`.`account_id` = current_account_id() and `tbl`.`partitioned` = 1", InformationDBConst)

	InformationSchemaViews = fmt.Sprintf("CREATE VIEW IF NOT EXISTS %s.VIEWS AS "+
		"SELECT 'def' AS `TABLE_CATALOG`,"+
		"tbl.reldatabase AS `TABLE_SCHEMA`,"+
		"tbl.relname AS `TABLE_NAME`,"+
	...
	InformationSchemaStatistics = fmt.Sprintf("CREATE VIEW IF NOT EXISTS %s.`STATISTICS` AS "+
		"select 'def' AS `TABLE_CATALOG`,"+
		"`tbl`.`reldatabase` AS `TABLE_SCHEMA`,"+
		"`tbl`.`relname` AS `TABLE_NAME`,"+
	...
	InformationSchemaReferentialConstraints = fmt.Sprintf("CREATE VIEW %s.REFERENTIAL_CONSTRAINTS AS "+
		"SELECT DISTINCT "+
	...
	InformationSchemaEngines = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.ENGINES ("+
		"ENGINE varchar(64),"+
		"SUPPORT varchar(8),"+
		"COMMENT varchar(160),"+
	...
	InformationSchemaRoutines = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.ROUTINES ("+
	...
	InformationSchemaParameters = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.PARAMETERS("+
	...
	InformationSchemaKeywords = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.KEYWORDS ("+
		"WORD varchar(64),"+
		"RESERVED int unsigned"+
		")", InformationDBConst)

	InformationSchemaSchemaPrivileges = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.`SCHEMA_PRIVILEGES` ("+
		"`GRANTEE` varchar(292) NOT NULL DEFAULT '',"+
		"`TABLE_CATALOG` varchar(512) NOT NULL DEFAULT '',"+
	...
	InformationSchemaTablePrivileges = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.`TABLE_PRIVILEGES` ("+
		"`GRANTEE` varchar(292) NOT NULL DEFAULT '',"+
		"`TABLE_CATALOG` varchar(512) NOT NULL DEFAULT '',"+
		"`TABLE_SCHEMA` varchar(64) NOT NULL DEFAULT '',"+
	...
	InformationSchemaColumnPrivileges = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.`COLUMN_PRIVILEGES` ("+
		"`GRANTEE` varchar(292) NOT NULL DEFAULT '',"+
		"`TABLE_CATALOG` varchar(512) NOT NULL DEFAULT '',"+
		"`TABLE_SCHEMA` varchar(64) NOT NULL DEFAULT '',"+
		"`TABLE_NAME` varchar(64) NOT NULL DEFAULT '',"+
	...
	InformationSchemaCollations = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.COLLATIONS ("+
	...
	InformationSchemaTableConstraints = fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s.TABLE_CONSTRAINTS ("+
		"CONSTRAINT_CATALOG varchar(64),"+
		"CONSTRAINT_SCHEMA varchar(64),"+
		"CONSTRAINT_NAME varchar(64),"+
		"TABLE_SCHEMA varchar(64),"+
	...
	InformationSchemaTableEvents = fmt.Sprintf("CREATE TABLE IF NOT EXISTS EVENTS ("+
