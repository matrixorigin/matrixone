...
func TestInformationSchemaViewsMetadata(t *testing.T) {
	...
	assert.Contains(t, InformationSchemaViewsDDL, "nullif(json_extract_string(tbl.viewdef, '$.definition'), '') is not null")
	assert.Contains(t, InformationSchemaViewsDDL, "cast(json_extract_string(tbl.viewdef, '$.definition') as text)")
	...
	if false { // retired regexp extractor fixtures; parser-derived metadata is covered by plan and BVT tests.
		prefix := regexp.MustCompile("^$")
		executableCommentPrefix := regexp.MustCompile("^$")
		tests := []struct {
			name       string
			createSQL  string
			definition string
