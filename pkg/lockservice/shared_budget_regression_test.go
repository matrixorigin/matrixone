...
func TestRemoteMixedModeBudgetKeepsOwnerExactWithCompactOriginRoute(t *testing.T) {
	runLockServiceTestsWithAdjustConfig(
		t,
		[]string{"s1", "s2"},
		time.Second*10,
		func(_ *lockTableAllocator, services []*service) {
			...
			// The v20 owner is authoritative for both physical ownership and wait-for
			...
			runLockServiceTestsWithAdjustConfig(
				...
				func(_ *lockTableAllocator, services []*service) {
					...
					for _, bookkeepingService := range []*service{owner, origin} {
						...
						txn.RUnlock()
						expected := 3
						if bookkeepingService == origin && origin != owner {
							expected = 1
						}
						require.Equal(t, expected, locks.len())
						locks.unref()
					}
		...
		t.Run(tt.name, func(t *testing.T) {
			runLockServiceTestsWithAdjustConfig(
				...
				func(_ *lockTableAllocator, services []*service) {
					...
					// remain in the authoritative owner ledger. A v20 origin needs only
			...
			runLockServiceTestsWithAdjustConfig(
				...
				func(_ *lockTableAllocator, services []*service) {
					...
					for _, bookkeepingService := range bookkeepingServices {
						...
						require.Equal(t, expected, keys.len())
						keys.unref()
					}
