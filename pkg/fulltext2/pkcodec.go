...
// AppendColumnBuffer decodes a streamed ColumnBuffer batch straight into vec, appending
// each element with the TYPED vector.AppendFixed/AppendBytes — no per-element boxing on the
// consumer side either (the mirror of the producer's box-free encode). The Data layout
// is the docmap encoding: fixed-width types are N contiguous width-byte little-endian
