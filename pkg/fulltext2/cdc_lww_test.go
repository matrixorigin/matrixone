...
// TestCdcUpsertThenDeletePhantom documents (does NOT reorder) the upsert->delete case:
// the delete frame sorts below the insert (delete-first framing, which same-batch
// UPDATE relies on), so the engine still returns the upserted term. This phantom is
// masked upstream by the source-table join (the pk row is gone), so it is not
// user-visible — reordering the frames would break UPDATE, so it stays. The base term
// is correctly gone.
func TestCdcUpsertThenDeletePhantom(t *testing.T) {
	...
	require.Empty(t, queryIDs(t, idx, "alpha"), "base term superseded")
	// Engine-level phantom (source-join masks it at query time); asserted to pin the
	// documented behavior, NOT to endorse it as user-visible-correct.
	require.Equal(t, []any{int64(1)}, queryIDs(t, idx, "beta"))
}
