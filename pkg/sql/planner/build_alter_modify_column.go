...
func modifyColPosition(
	ctx context.Context,
	tableDef *plan.TableDef,
	oCol, nCol *plan.ColDef,
	pos *tree.ColumnPosition,
) error {
	if pos != nil && pos.Typ != tree.ColumnPositionNone {
		// detete old column
		tableDef.Cols = RemoveIf[*plan.ColDef](tableDef.Cols, func(col *plan.ColDef) bool {
		...
		tableDef.Cols = append(
			...
			append([]*plan.ColDef{nCol}, tableDef.Cols[targetPos:]...)...,
		)
	} else {
...
func checkColumnForeignkeyConstraint(ctx CompilerContext, tbInfo *plan.TableDef, originalCol, newCol *plan.ColDef) error {
	...
	for _, referredTblId := range tbInfo.RefChildTbls {
		...
		var referredFK *plan.ForeignKeyDef
...
func checkPriKeyConstraint(ctx context.Context, col *plan.ColDef, hasDefaultValue, hasNullFlag bool, priKeyDef *plan.PrimaryKeyDef) error {
	hasPriKeyFlag := false
	if col.Primary {
		hasPriKeyFlag = true
