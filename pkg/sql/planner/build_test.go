...
func TestVisitRule(t *testing.T) {
	...
	mock := NewMockOptimizer(false)
	ctx := context.TODO()
	pl, err := runOneStmt(mock, t, sql)
	if err != nil {
		t.Fatalf("should not error, sql=%s", sql)
	}
	getParamRule := NewGetParamRule()
	vp := NewVisitPlan(pl, []VisitPlanRule{getParamRule})
	...
	vp = NewVisitPlan(pl, []VisitPlanRule{resetParamOrderRule})
	err = vp.Visit(ctx)
	if err != nil {
		t.Fatalf("should not error, sql=%s", sql)
	}
	...
	vp = NewVisitPlan(pl, []VisitPlanRule{resetParamRule})
	err = vp.Visit(ctx)
	if err != nil {
		t.Fatalf("should not error, sql=%s", sql)
	}
...
func testDeepCopy(logicPlan *plan.Plan) {
	switch logicPlan.Plan.(type) {
	case *plan.Plan_Query:
		_ = plan.DeepCopyPlan(logicPlan)
	case *plan.Plan_Ddl:
		_ = plan.DeepCopyPlan(logicPlan)
	case *plan.Plan_Dcl:
	}
