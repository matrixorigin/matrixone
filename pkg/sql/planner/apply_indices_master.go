...
func (builder *QueryBuilder) applyIndicesForFiltersUsingMasterIndex(nodeID int32, scanNode *plan.Node, indexDef *plan.IndexDef) int32 {
	...
	for i, filterExp := range scanNode.FilterList {
		...
		if i != 0 {
			wherePrevPkEqCurrPk, _ := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{
				currIndexPkCol,
				prevIndexPkCol,
			})
			lastNodeId = builder.appendNode(&plan.Node{
				...
				OnList:   []*plan.Expr{wherePrevPkEqCurrPk},
			}, builder.ctxByNode[nodeID])
		}

		prevIndexPkCol = plan.DeepCopyExpr(currIndexPkCol)
		prevLastNodeId = lastNodeId
	}
	lastNodeFromIndexTbl := builder.qry.Nodes[lastNodeId]
	lastNodeFromIndexTbl.Limit = plan.DeepCopyExpr(scanNode.Limit)
	lastNodeFromIndexTbl.Offset = plan.DeepCopyExpr(scanNode.Offset)
	...
	lastNodeId = builder.appendNode(&plan.Node{
		...
		Limit:    plan.DeepCopyExpr(lastNodeFromIndexTbl.Limit),
		Offset:   plan.DeepCopyExpr(lastNodeFromIndexTbl.Offset),
	}, builder.ctxByNode[nodeID])
...
func makeIndexTblScan(builder *QueryBuilder, bindCtx *BindContext, filterExp *plan.Expr,
	idxTableDef *plan.TableDef, idxObjRef *plan.ObjectRef, scanSnapshot *plan.Snapshot, colDefs []*plan.ColDef) (int32, int32) {
	...
	case "=":
		...
		filterList, _ = BindFuncExprImplByPlanExpr(builder.GetContext(), "prefix_eq", []*plan.Expr{
	...
	case "between":
		...
		filterList, _ = bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "prefix_between", []*plan.Expr{
			indexKeyCol, // __mo_index_idx_col
	...
	case "in_range":
		...
		filterList, _ = bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "prefix_in_range", []*plan.Expr{
			indexKeyCol, // __mo_index_idx_col
			serialExpr1, // serial_full("0","value1")
			serialExpr2, // serial_full("0","value2")
		})
	...
	case "in":
		...
		filterList, _ = bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "prefix_in", []*plan.Expr{
			indexKeyCol,           // __mo_index_idx_col
