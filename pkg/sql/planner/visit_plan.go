...
type VisitPlanRule interface {
	MatchNode(*plan.Node) bool
	IsApplyExpr() bool
	ApplyNode(*plan.Node) error
	ApplyExpr(*plan.Expr) (*plan.Expr, error)
}
...
func NewVisitPlan(pl *plan.Plan, rules []VisitPlanRule) *VisitPlan {
	return &VisitPlan{
		plan:         pl,
		isUpdatePlan: false,
		rules:        rules,
...
func (vq *VisitPlan) visitNode(ctx context.Context, qry *plan.Query, node *plan.Node, idx int32) error {
...
func (vq *VisitPlan) exploreNode(ctx context.Context, rule VisitPlanRule, node *plan.Node, _ int32) error {
	var err error
	if node.Limit != nil {
	...
	applyAndResetType := func(e *plan.Expr) (*plan.Expr, error) {
		oldType := e.Typ
