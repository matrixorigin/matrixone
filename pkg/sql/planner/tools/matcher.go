...
func (matcher *ColumnRef) GetAssignedVar(node *plan.Node, aliases UnorderedMap[string, string]) (*VarRef, error) {
...
type GetFunc func(*planner.QueryBuilder, *plan.Node) []VarRef
...
func (matcher *OutputMatcher) SimpleMatch(node *plan.Node) bool {
	return node.NodeType == plan.Node_PROJECT
}
...
func (matcher *ExprMatcher) GetAssignedVar(node *plan.Node, aliases UnorderedMap[string, string]) (*VarRef, error) {
	var res *VarRef
	checkedExprs := make([]*plan.Expr, 0)
...
func NewJoinMatcher(joinType plan.Node_JoinType, conds []string, filters []string) *JoinMatcher {
	ret := &JoinMatcher{
		JoinTyp:    joinType,
		OnCondsStr: conds,
		FiltersStr: filters,
...
func (matcher *JoinMatcher) SimpleMatch(node *plan.Node) bool {
	return node.NodeType == plan.Node_JOIN &&
		node.JoinType == matcher.JoinTyp
}

func (matcher *JoinMatcher) DeepMatch(ctx context.Context, node *plan.Node, aliases UnorderedMap[string, string]) (*MatchResult, error) {
	if !matcher.SimpleMatch(node) {
		return FailMatched(), nil
	}
...
func (matcher *AggrFuncMatcher) GetAssignedVar(node *plan.Node, aliases UnorderedMap[string, string]) (*VarRef, error) {
