...
func TTableScanWithoutColRef(tableName string) *MatchPattern {
	return TNode(plan.Node_TABLE_SCAN).With(&TableScanMatcher{
		TableName: tableName,
	})
}
...
func TOutputWithoutOutputs(child *MatchPattern) *MatchPattern {
	return TNode(plan.Node_PROJECT, child)
}
...
func TJoin(joinTyp plan.Node_JoinType,
	onConds []string,
	filters []string,
	left, right *MatchPattern) *MatchPattern {
	return TNode(plan.Node_JOIN, left, right).With(
...
func TAggr(aggrs UnorderedMap[string, *AggrFuncMatcher], children ...*MatchPattern) *MatchPattern {
	ret := TNode(plan.Node_AGG, children...)
...
func (pattern *MatchPattern) WithExactAssignedOutputs(expectedAliases []RValueMatcher) *MatchPattern {
	fun := func(builder *planner.QueryBuilder, node *plan.Node) []VarRef {
		ret := make([]VarRef, 0)
...
func (pattern *MatchPattern) WithExactOutputs(outputs ...string) *MatchPattern {
	pattern.Matchers = append(pattern.Matchers,
		&SymbolsMatcher{
			GetFunc: func(builder *planner.QueryBuilder, node *plan.Node) []VarRef {
				AssertFunc(node.NodeType == plan.Node_PROJECT, "must be project node")
				ret := make([]VarRef, 0)
				for _, expr := range node.ProjectList {
					col := expr.GetCol()
					ret = append(ret, VarRef{
...
func SimpleMatch(pattern *MatchPattern, node *plan.Node) []*MatchingState {
...
func SimpleMatchMatchers(pattern *MatchPattern, node *plan.Node) bool {
...
func MatchSteps(ctx context.Context, query *plan.Query, pattern *MatchPattern) (*MatchResult, error) {
	//fmt.Println(pattern)
...
func Match(ctx context.Context, nodes []*plan.Node, node *plan.Node, pattern *MatchPattern) (*MatchResult, error) {
