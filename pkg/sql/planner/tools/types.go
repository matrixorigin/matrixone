...
type Matcher interface {
	...
	SimpleMatch(*plan.Node) bool

	// DeepMatch check the internal structure about Node
	DeepMatch(context.Context, *plan.Node, UnorderedMap[string, string]) (*MatchResult, error)
...
type RValueMatcher interface {
	GetAssignedVar(*plan.Node, UnorderedMap[string, string]) (*VarRef, error)
	String() string
}
