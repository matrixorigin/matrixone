...
func (builder *QueryBuilder) pushdownFilters(nodeID int32, filters []*plan.Expr, separateNonEquiConds bool) (int32, []*plan.Expr) {
				...
				case *plan.Expr_F:
					if exprImpl.F.Func.ObjName == "or" {
						...
						for _, key := range keys {
							...
							if extraFilter != nil {
								extraFilters = append(extraFilters, plan.DeepCopyExpr(extraFilter))
		...
		for i, filter := range filters {
			...
			case JoinSideNone:
				...
				case plan.Node_INNER:
					leftPushdown = append(leftPushdown, plan.DeepCopyExpr(filter))
		...
		if builder.qry.Nodes[node.Children[1]].NodeType == plan.Node_FUNCTION_SCAN {
			...
			for _, filter := range filters {
				...
				if builder.checkExprCanPushdown(filter, builder.qry.Nodes[node.Children[1]]) {
					rightPushdown = append(rightPushdown, plan.DeepCopyExpr(filter))
					down = true
				}
				if !down {
					cantPushdown = append(cantPushdown, plan.DeepCopyExpr(filter))
				}
			}
		}
	...
	case plan.Node_UNION, plan.Node_UNION_ALL, plan.Node_MINUS, plan.Node_MINUS_ALL, plan.Node_INTERSECT, plan.Node_INTERSECT_ALL:
		...
		for _, filter := range filters {
			canPushdown = append(canPushdown, replaceColRefsForSet(plan.DeepCopyExpr(filter), leftChild.ProjectList))
	...
	case plan.Node_FUNCTION_SCAN:
		...
		for _, filter := range filters {
			if onlyContainsTag(filter, node.BindingTags[0]) {
				selfFilters = append(selfFilters, plan.DeepCopyExpr(filter))
			} else {
				downFilters = append(downFilters, plan.DeepCopyExpr(filter))
			}
		}
...
func (builder *QueryBuilder) pushdownTopThroughLeftJoin(nodeID int32) {
	...
	nodePushDown = plan.DeepCopyNode(node)

	if nodePushDown.Offset != nil {
		newExpr, err := bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "+", []*plan.Expr{nodePushDown.Limit, nodePushDown.Offset})
		if err != nil {
			goto END
		}
...
func (builder *QueryBuilder) pushdownVectorIndexTopToTableScan(nodeID int32) {
	...
	scanNode.IndexReaderParam = &plan.IndexReaderParam{
		...
		Limit: plan.DeepCopyExpr(node.Limit),
	}
