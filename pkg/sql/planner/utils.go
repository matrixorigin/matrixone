...
func hasParam(expr *plan.Expr) bool {
	...
	case *plan.Expr_F:
		return slices.ContainsFunc(exprImpl.F.Args, hasParam)

	case *plan.Expr_List:
		return slices.ContainsFunc(exprImpl.List.List, hasParam)
...
func hasCorrCol(expr *plan.Expr) bool {
	...
	case *plan.Expr_F:
		return slices.ContainsFunc(exprImpl.F.Args, hasCorrCol)

	case *plan.Expr_List:
		return slices.ContainsFunc(exprImpl.List.List, hasCorrCol)
...
func hasSubquery(expr *plan.Expr) bool {
	...
	case *plan.Expr_F:
		return slices.ContainsFunc(exprImpl.F.Args, hasSubquery)

	case *plan.Expr_List:
		return slices.ContainsFunc(exprImpl.List.List, hasSubquery)
...
func HasTag(expr *plan.Expr, tag int32) bool {
	...
	case *plan.Expr_F:
		return slices.ContainsFunc(exprImpl.F.Args, func(arg *plan.Expr) bool {
			return HasTag(arg, tag)
		})

	case *plan.Expr_List:
		return slices.ContainsFunc(exprImpl.List.List, func(arg *plan.Expr) bool {
			return HasTag(arg, tag)
		})
...
func replaceColRefs(expr *plan.Expr, tag int32, projects []*plan.Expr) *plan.Expr {
	...
	case *plan.Expr_Col:
		...
		if colRef.RelPos == tag {
			expr = plan.DeepCopyExpr(projects[colRef.ColPos])
		}
	case *plan.Expr_W:
...
func replaceColRefsForSet(expr *plan.Expr, projects []*plan.Expr) *plan.Expr {
	...
	case *plan.Expr_Col:
		expr = plan.DeepCopyExpr(projects[exprImpl.Col.ColPos])
	}
...
func deduceNewFilterList(filters, onList []*plan.Expr) []*plan.Expr {
	...
	for _, onPred := range onList {
		...
		for _, filter := range filters {
			...
			if col != nil {
				newExpr := plan.DeepCopyExpr(filter)
...
func extractColRefAndLiteralsInFilter(expr *plan.Expr) (col *plan.ColRef, litType types.T, literals []*plan.Literal, colFnName string, hasDynamicParam bool) {
	fn := expr.GetF()
...
// nil otherwise.
func extractColRefInFilter(expr *plan.Expr) *plan.ColRef {
	switch exprImpl := expr.Expr.(type) {
	case *plan.Expr_Col:
		return exprImpl.Col
	case *plan.Expr_F:
...
func deduceTranstivity(expr *plan.Expr, col1, col2, col3, col4 *plan.ColRef) (bool, *plan.Expr) {
	if col1.ColRefString() == col3.ColRefString() ||
		...
		retExpr := plan.DeepCopyExpr(expr)
...
func substituteMatchColumn(expr *plan.Expr, onPredCol1, onPredCol2 *plan.ColRef) bool {
	var ret bool
	switch exprImpl := expr.Expr.(type) {
	case *plan.Expr_Col:
...
func checkStrictJoinPred(onPred *plan.Expr) (bool, *plan.ColRef, *plan.ColRef) {
...
func rejectsNull(filter *plan.Expr, proc *process.Process) bool {
	...
	filter = replaceColRefWithNull(plan.DeepCopyExpr(filter))
...
func unwindTupleComparison(ctx context.Context, nonEqOp, op string, leftExprs, rightExprs []*plan.Expr, idx int) (*plan.Expr, error) {
	...
	expr, err := BindFuncExprImplByPlanExpr(ctx, nonEqOp, []*plan.Expr{
		plan.DeepCopyExpr(leftExprs[idx]),
		plan.DeepCopyExpr(rightExprs[idx]),
	})
	if err != nil {
		return nil, err
	}
...
func GetUniqueColAndIdxFromTableDef(tableDef *plan.TableDef) ([]map[string]int, map[string]bool) {
...
func GenUniqueColJoinExpr(ctx context.Context, tableDef *plan.TableDef, uniqueCols []map[string]int, leftTag int32, rightTag int32) (*plan.Expr, error) {
	var checkExpr *plan.Expr
	var err error
	...
	for i, uniqueColMap := range uniqueCols {
		...
		for _, colIdx := range uniqueColMap {
			...
			eqExpr, err := BindFuncExprImplByPlanExpr(ctx, "=", []*plan.Expr{leftExpr, rightExpr})
			if err != nil {
				return nil, err
			}
			if condIdx == 0 {
				...
				condExpr, err = BindFuncExprImplByPlanExpr(ctx, "and", []*plan.Expr{condExpr, eqExpr})
				if err != nil {
					return nil, err
				}
			}
		...
		if i == 0 {
			...
			checkExpr, err = BindFuncExprImplByPlanExpr(ctx, "or", []*plan.Expr{checkExpr, condExpr})
			if err != nil {
				return nil, err
			}
		}
...
func GenUniqueColCheckExpr(ctx context.Context, tableDef *plan.TableDef, uniqueCols []map[string]int, colCount int) ([]*plan.Expr, error) {
	checkExpr := make([]*plan.Expr, len(uniqueCols))
...
func onlyContainsTag(filter *plan.Expr, tag int32) bool {
...
func databaseIsValid(dbName string, ctx CompilerContext, snapshot *plan.Snapshot) (string, error) {
...
func detectedExprWhetherTimeRelated(expr *plan.Expr) bool {
	if ef, ok := expr.Expr.(*plan.Expr_F); !ok {
		...
		// current_timestamp() + 1
		return slices.ContainsFunc(ef.F.Args, detectedExprWhetherTimeRelated)
	}
}

func ResetPreparePlan(ctx CompilerContext, preparePlan *plan.Plan) ([]*plan.ObjectRef, []int32, error) {
...
func HasMoCtrl(expr *plan.Expr) bool {
	...
	case *plan.Expr_F:
		...
		return slices.ContainsFunc(exprImpl.F.Args, HasMoCtrl)

	case *plan.Expr_List:
		return slices.ContainsFunc(exprImpl.List.List, HasMoCtrl)
...
func HasFkSelfReferOnly(tableDef *plan.TableDef) bool {
...
func IsFalseExpr(e *plan.Expr) bool {
...
func MakeCPKEYRuntimeFilter(tag int32, upperlimit int32, expr *plan.Expr, tableDef *plan.TableDef, notOnPk bool) *plan.RuntimeFilterSpec {
...
func MakeSerialRuntimeFilter(ctx context.Context, tag int32, matchPrefix bool, upperlimit int32, expr *plan.Expr, notOnPk bool) *plan.RuntimeFilterSpec {
...
func MakeIntervalExpr(num int64, str string) *plan.Expr {
	...
	return &plan.Expr{
		...
		Expr: &plan.Expr_List{
			List: &plan.ExprList{
				List: []*plan.Expr{arg0, arg1},
			},
		},
	}
}

func GetColExpr(typ plan.Type, relpos int32, colpos int32) *plan.Expr {
	return &plan.Expr{
		Typ: typ,
		Expr: &plan.Expr_Col{
			Col: &plan.ColRef{
...
func MakeSerialExtractExpr(ctx context.Context, fromExpr *plan.Expr, origType plan.Type, serialIdx int64) (*plan.Expr, error) {
...
func MakeInExpr(ctx context.Context, left *plan.Expr, length int32, data []byte, matchPrefix bool) *plan.Expr {
	rightArg := &plan.Expr{
		Typ: left.Typ,
		Expr: &plan.Expr_Vec{
			Vec: &plan.LiteralVec{
...
func FillValuesOfParamsInPlan(ctx context.Context, preparePlan *plan.Plan, paramVals []any) (*plan.Plan, error) {
	copied := preparePlan
...
func replaceParamVals(ctx context.Context, plan0 *plan.Plan, paramVals []any) error {
	params := make([]*plan.Expr, len(paramVals))
...
func GetRowSizeFromTableDef(tableDef *plan.TableDef, ignoreHiddenKey bool) float64 {
	size := int32(0)
...
func containGrouping(expr *plan.Expr) bool {
	var ret bool
...
func checkGrouping(ctx context.Context, expr *plan.Expr) error {
...
func ReplaceFoldExpr(proc *process.Process, expr *plan.Expr, exes *[]colexec.ExpressionExecutor) (bool, error) {
	allCanFold := true
	var err error
...
func EvalFoldExpr(proc *process.Process, expr *plan.Expr, executors *[]colexec.ExpressionExecutor) (err error) {
...
func HasFoldExprForList(exprs []*plan.Expr) bool {
	for _, e := range exprs {
...
func HasFoldValExpr(expr *plan.Expr) bool {
	switch ef := expr.Expr.(type) {
	case *plan.Expr_Fold:
		return true
	case *plan.Expr_F:
...
func DbNameOfObjRef(objRef *plan.ObjectRef) string {
...
func onlyHasHiddenPrimaryKey(tableDef *plan.TableDef) bool {
	if tableDef == nil {
		return false
	}
	pk := tableDef.GetPkey()
