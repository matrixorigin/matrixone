...
func buildDefaultExpr(col *tree.ColumnTableDef, typ plan.Type, proc *process.Process) (*plan.Default, error) {
	...
	newExpr, err := ConstantFold(batch.EmptyForConstFoldBatch, plan.DeepCopyExpr(defaultExpr), proc, false, true)
	if err != nil {
		return nil, err
	}
...
func getDefaultExpr(ctx context.Context, d *plan.ColDef) (*plan.Expr, error) {
	...
	newDefExpr := plan.DeepCopyExpr(d.Default.Expr)
	err := replaceFuncId(ctx, newDefExpr)
	return newDefExpr, err
}

func replaceFuncId(ctx context.Context, expr *plan.Expr) error {
