...
type MockCompilerContext struct {
	...
	DatabaseExistsFunc    func(string, *plan.Snapshot) bool
	GetDatabaseIdFunc     func(string, *plan.Snapshot) (uint64, error)
	ResolveAccountIdsFunc func([]string) ([]uint32, error)
	ResolveFunc           func(string, string, *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef)
}
...
func (m *MockCompilerContext) GetSnapshot() *plan.Snapshot {
	return nil
}

func (m *MockCompilerContext) SetSnapshot(snapshot *plan.Snapshot) {}
...
func (m *MockCompilerContext) ResolveIndexTableByRef(ref *plan.ObjectRef, tblName string, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error) {
	return m.Resolve(DbNameOfObjRef(ref), tblName, snapshot)
}

func (m *MockCompilerContext) ResolveSubscriptionTableById(tableId uint64, pubmeta *plan.SubscriptionMeta) (*plan.ObjectRef, *plan.TableDef, error) {
	return nil, nil, nil
}
...
func NewMockCompilerContext(isDml bool) *MockCompilerContext {
	...
	*/
	...
	objects := make(map[string]*plan.ObjectRef)
	tables := make(map[string]*plan.TableDef)
	stats := make(map[string]*plan.Stats)
	...
	for db, schema := range schemas {
		...
		for tableName, table := range schema {
			...
			colDefs := make([]*plan.ColDef, 0, len(table.cols))
			...
			tableDef := &plan.TableDef{
				...
				Name:      tableName,
				Cols:      colDefs,
				Indexes:   make([]*plan.IndexDef, len(table.idxs)),
			}
			if len(table.pks) == 1 {
...
func (m *MockCompilerContext) DatabaseExists(name string, snapshot *plan.Snapshot) bool {
...
func (m *MockCompilerContext) GetDatabaseId(dbName string, snapshot *plan.Snapshot) (uint64, error) {
...
func (m *MockCompilerContext) Resolve(dbName string, tableName string, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error) {
	name := strings.ToLower(tableName)
	tableDef := plan.DeepCopyTableDef(m.tables[name], true)
...
func (m *MockCompilerContext) ResolveById(tableId uint64, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error) {
...
func (m *MockCompilerContext) Stats(obj *plan.ObjectRef, snapshot *plan.Snapshot) (*pb.StatsInfo, error) {
	return nil, nil
}
...
func (m *MockCompilerContext) GetQueryResultMeta(uuid string) ([]*plan.ColDef, string, error) {
	return nil, "", nil
}
...
func (m *MockCompilerContext) GetSubscriptionMeta(dbName string, snapshot *plan.Snapshot) (*plan.SubscriptionMeta, error) {
	return nil, nil
}
func (m *MockCompilerContext) SetQueryingSubscription(*plan.SubscriptionMeta) {

}
func (m *MockCompilerContext) GetQueryingSubscription() *plan.SubscriptionMeta {
	return nil
}
func (m *MockCompilerContext) IsPublishing(dbName string) (bool, error) {
	return false, nil
}
func (m *MockCompilerContext) BuildTableDefByMoColumns(dbName, table string) (*plan.TableDef, error) {
	return nil, nil
}

func (m *MockCompilerContext) ResolveSnapshotWithSnapshotName(snapshotName string) (*plan.Snapshot, error) {
	return nil, nil
}
