...
func MakePlan2Decimal64ExprWithType(v types.Decimal64, typ *plan.Type) *plan.Expr {
	rawA := int64(v)
	return &plan.Expr{
		Typ: *typ,
		Expr: &plan.Expr_Lit{
...
func MakePlan2Decimal128ExprWithType(v types.Decimal128, typ *plan.Type) *plan.Expr {
	rawA := v.B0_63
	rawB := v.B64_127
	return &plan.Expr{
		Typ: *typ,
...
func makePlan2CastExpr(ctx context.Context, expr *plan.Expr, targetType plan.Type) (*plan.Expr, error) {
	var err error
	if expr == nil {
...
func funcCastForEnumType(ctx context.Context, expr *plan.Expr, targetType plan.Type) (*plan.Expr, error) {
	var err error
	if targetType.Id != int32(types.T_enum) {
		return expr, nil
	}
...
func makeHiddenColTyp() plan.Type {
	return plan.Type{
		Id:    int32(types.T_varchar),
...
func MakeHiddenColDefByName(name string) *plan.ColDef {
	return &plan.ColDef{
		Name:   name,
		Hidden: true,
...
func MakeRowIdColDef() *plan.ColDef {
	return &plan.ColDef{
		Name:   catalog.Row_ID,
		Hidden: true,
		Typ: plan.Type{
...
func GetColDefFromTable(Cols []*plan.ColDef, hidenColName string) *plan.ColDef {
	for _, coldef := range Cols {
		if coldef.Name == hidenColName {
			return plan.DeepCopyColDef(coldef)
		}
	}
