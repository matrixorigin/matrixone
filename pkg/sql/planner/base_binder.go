...
func (b *baseBinder) baseBindExpr(astExpr tree.Expr, depth int32, isRoot bool) (expr *plan.Expr, err error) {
	...
	case *tree.Tuple:
		exprs := make([]*plan.Expr, 0, len(exprImpl.Exprs))
		var planItem *plan.Expr
	...
	if len(table) == 0 {
		if binding, ok := b.ctx.bindingByCol[col]; ok {
			if binding != nil {
				...
				typ = plan.DeepCopyType(binding.types[colPos])
				table = binding.table
			} else {
...
func (b *baseBinder) bindFullTextMatchExpr(astExpr *tree.FullTextMatchExpr, depth int32, isRoot bool) (*plan.Expr, error) {

	args := make([]*plan.Expr, 2+len(astExpr.KeyParts))
...
func (b *baseBinder) bindFuncExprImplByAstExpr(name string, astArgs []tree.Expr, depth int32) (*plan.Expr, error) {
	...
	if name == "bit_cast" {
		...
		args = []*plan.Expr{binExpr, typeExpr}
		...
		args = []*plan.Expr{serialExpr, idxExpr, typeExpr}
	} else {
...
func (b *baseBinder) bindPythonUdf(udf *function.Udf, astArgs []tree.Expr, depth int32) (*plan.Expr, error) {
	args := make([]*plan.Expr, 2*len(astArgs)+2)
	...
	for i, t := range fArgTypes {
		args[len(astArgs)+i+1] = &plan.Expr{Typ: *t}
	}

	// function ret
	fRetType := udf.GetRetPlanType()
	args[2*len(astArgs)+1] = &plan.Expr{Typ: *fRetType}

	return BindFuncExprImplByPlanExpr(b.GetContext(), "python_user_defined_function", args)
}

func bindFuncExprAndConstFold(ctx context.Context, proc *process.Process, name string, args []*plan.Expr) (*plan.Expr, error) {
	...
	case "+", "-", "*", "/", "div", "%", "mod", "unary_minus", "unary_plus", "unary_tilde", "cast", "serial", "serial_full":
		if proc != nil {
			tmpexpr, _ := ConstantFold(batch.EmptyForConstFoldBatch, plan.DeepCopyExpr(retExpr), proc, false, true)
			if tmpexpr != nil {
				retExpr = tmpexpr
			}
		}
	...
	case "in_range":
		...
		if !rangeCheckVal.GetBval() {
			retExpr = plan.DeepCopyExpr(kAlwaysFalseExpr)
		} else {
			retExpr, _ = ConstantFold(batch.EmptyForConstFoldBatch, retExpr, proc, false, true)
		}
	}
...
between_fallback:
	fnArgs := retExpr.GetF().Args
	leftFn, err := BindFuncExprImplByPlanExpr(ctx, ">=", []*plan.Expr{plan.DeepCopyExpr(fnArgs[0]), fnArgs[1]})
	if err != nil {
		return nil, err
	}
...
func BindFuncExprImplByPlanExpr(ctx context.Context, name string, args []*plan.Expr) (*plan.Expr, error) {
		...
		if len(args) == 1 {
			if listExpr, ok := args[0].Expr.(*plan.Expr_List); ok {
				for i, subExpr := range listExpr.List.List {
					newSubExpr, err := BindFuncExprImplByPlanExpr(ctx, "serial", []*plan.Expr{subExpr})
					if err != nil {
						return nil, err
					}
		...
		if rightList := args[1].GetList(); rightList != nil {
			...
			if name == "in" {
				for _, expr := range orExprList {
					tmpExpr, err := BindFuncExprImplByPlanExpr(ctx, "=", []*plan.Expr{plan.DeepCopyExpr(args[0]), expr})
					if err != nil {
						return nil, err
					}
					if newExpr == nil {
						...
						newExpr, err = BindFuncExprImplByPlanExpr(ctx, "or", []*plan.Expr{newExpr, tmpExpr})
						if err != nil {
							return nil, err
						}
					}
				...
				for _, expr := range orExprList {
					...
					if newExpr == nil {
						...
						newExpr, err = BindFuncExprImplByPlanExpr(ctx, "and", []*plan.Expr{newExpr, tmpExpr})
						if err != nil {
							return nil, err
						}
					}
	...
	if err != nil {
		if name == "between" {
			leftFn, err := BindFuncExprImplByPlanExpr(ctx, ">=", []*plan.Expr{plan.DeepCopyExpr(args[0]), args[1]})
			if err != nil {
				return nil, err
			}
...
func (b *baseBinder) bindNumVal(astExpr *tree.NumVal, typ plan.Type) (*plan.Expr, error) {
	...
	returnHexNumExpr := func(val string, isBin ...bool) (*plan.Expr, error) {
		if !typ.IsEmpty() {
...
func appendCastBeforeExpr(ctx context.Context, expr *plan.Expr, toType plan.Type, isBin ...bool) (*plan.Expr, error) {
...
func resetDateFunction(ctx context.Context, dateExpr *plan.Expr, intervalExpr *plan.Expr) ([]*plan.Expr, error) {
...
func resetIntervalFunction(ctx context.Context, intervalExpr *plan.Expr) ([]*plan.Expr, error) {
	return resetIntervalFunctionArgs(ctx, intervalExpr)
}

func resetIntervalFunctionArgs(ctx context.Context, intervalExpr *plan.Expr) ([]*plan.Expr, error) {
	firstExpr := intervalExpr.GetList().List[0]
	secondExpr := intervalExpr.GetList().List[1]
