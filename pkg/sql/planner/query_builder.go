...
func NewQueryBuilder(queryType plan.Query_StatementType, ctx CompilerContext, isPrepareStatement bool, skipStats bool) *QueryBuilder {
	...
	return &QueryBuilder{
		...
		tag2Table:            make(map[int32]*plan.TableDef),
...
func (builder *QueryBuilder) copyNode(ctx *BindContext, nodeId int32) int32 {
	...
	newNode := plan.DeepCopyNode(node)
...
func (builder *QueryBuilder) remapAllColRefs(nodeID int32, step int32, colRefCnt map[[2]int32]int, colRefBool map[[2]int32]bool, sinkColRef map[[2]int32]int) (*ColRefRemapping, error) {
	...
	case plan.Node_FUNCTION_SCAN:
		...
		newTableDef := plan.DeepCopyTableDef(node.TableDef, false)
		...
		for i, col := range node.TableDef.Cols {
			...
			newTableDef.Cols = append(newTableDef.Cols, plan.DeepCopyColDef(col))
		}

		if len(newTableDef.Cols) == 0 {
			internalRemapping.addColRef([2]int32{tag, 0})
			newTableDef.Cols = append(newTableDef.Cols, plan.DeepCopyColDef(node.TableDef.Cols[0]))
		}
	...
	case plan.Node_AGG:
		...
		if child.NodeType == plan.Node_TABLE_SCAN && len(child.FilterList) == 0 && len(node.GroupBy) == 0 && child.Limit == nil && child.Offset == nil {
			child.AggList = make([]*plan.Expr, 0, len(node.AggList))
			for _, agg := range node.AggList {
				switch agg.GetF().Func.ObjName {
				case "starcount", "count", "min", "max":
					child.AggList = append(child.AggList, plan.DeepCopyExpr(agg))
				default:
					child.AggList = nil
				}
	...
	case plan.Node_DISTINCT:
		...
		node.GroupBy = make([]*plan.Expr, len(preNode.ProjectList))
		node.ProjectList = make([]*plan.Expr, len(preNode.ProjectList))
...
func (builder *QueryBuilder) bindSelect(stmt *tree.Select, ctx *BindContext, isRoot bool) (nodeID int32, err error) {
	...
	var fillVals, fillCols []*plan.Expr
	var interval, sliding, ts, wEnd *plan.Expr
	...
	var boundOffsetExpr *plan.Expr
	var boundCountExpr *plan.Expr
	...
	if len(ctx.groupConcatOrderBys) > 0 && (len(ctx.groups) > 0 || len(ctx.aggregates) > 0) {
		...
		for _, groupExpr := range ctx.groups {
			sortSpecs = append(sortSpecs, &plan.OrderBySpec{
				Expr: plan.DeepCopyExpr(groupExpr),
...
func (builder *QueryBuilder) bindTimeWindow(
	...
	fillVals, fillCols []*plan.Expr,
	interval, sliding, ts, wEnd *plan.Expr,
...
) (boundOffsetExpr, boundCountExpr *plan.Expr, rankOption *plan.RankOption, err error) {
	if astLimit != nil {
		limitBinder := NewLimitBinder(builder, ctx)
		if astLimit.Offset != nil {
...
func (builder *QueryBuilder) checkExprCanPushdown(expr *plan.Expr, node *plan.Node) bool {
	switch node.NodeType {
	case plan.Node_FUNCTION_SCAN:
...
func (builder *QueryBuilder) ResolveTsHint(tsExpr *tree.AtTimeStamp) (snapshot *plan.Snapshot, err error) {
	if tsExpr == nil {
		return
	}
	...
	var tenant *plan.SnapshotTenant
	...
	case *plan.Literal_Sval:
		if tsExpr.Type == tree.ATTIMESTAMPTIME {
			...
			if time.Now().UTC().UnixNano()-tsNano <= options.DefaultGCTTL.Nanoseconds() && 0 <= time.Now().UTC().UnixNano()-tsNano {
				snapshot = &plan.Snapshot{TS: &timestamp.Timestamp{PhysicalTime: tsNano}, Tenant: tenant}
			} else {
				var valid bool
			...
			snapshot = &plan.Snapshot{TS: &ts, Tenant: tenant}
			...
			snapshot = &plan.Snapshot{TS: tStamp, Tenant: tenant}
		} else {
	...
	case *plan.Literal_I64Val:
		if tsExpr.Type == tree.ATTIMESTAMPTIME {
			...
			snapshot = &plan.Snapshot{TS: &timestamp.Timestamp{PhysicalTime: lit.I64Val}, Tenant: tenant}
		} else if tsExpr.Type == tree.ATMOTIMESTAMP {
			if lit.I64Val <= 0 {
				err = moerr.NewInvalidArg(builder.GetContext(), "invalid timestamp value", lit.I64Val)
				return
...
func IsSnapshotValid(snapshot *plan.Snapshot) bool {
	if snapshot == nil {
		return false
	}
