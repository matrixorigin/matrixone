...
func checkTableType(ctx context.Context, tableDef *plan.TableDef) error {
...
func setTableExprToDmlTableInfo(ctx CompilerContext, tbl tree.TableExpr, tblInfo *dmlTableInfo, aliasMap map[string][2]string, withMap map[string]struct{}) error {
	...
	tblInfo.objRef = append(tblInfo.objRef, &plan.ObjectRef{
...
func initInsertStmt(builder *QueryBuilder, bindCtx *BindContext, stmt *tree.Insert, info *dmlSelectInfo) (bool, map[string]bool, map[string]bool, error) {
	...
	oldProject := append([]*plan.Expr{}, lastNode.ProjectList...)
	...
	projectList := make([]*plan.Expr, 0, len(tableDef.Cols))
	...
	info.projectList = make([]*plan.Expr, 0, len(projectList))
	info.derivedTableId = info.rootId
	...
	if len(stmt.OnDuplicateUpdate) > 0 {
		...
		rightTableDef := plan.DeepCopyTableDef(tableDef, true)
		rightObjRef := plan.DeepCopyObjectRef(tableObjRef)
		...
		if len(uniqueCols) > 0 {
			...
			for _, uniqueColMap := range uniqueCols {
				...
				for _, colIdx := range uniqueColMap {
					...
					eqExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{leftExpr, rightExpr})
					if err != nil {
						return false, nil, nil, err
					}
					if condIdx == 0 {
						...
						condExpr, err = BindFuncExprImplByPlanExpr(builder.GetContext(), "and", []*plan.Expr{condExpr, eqExpr})
						if err != nil {
							return false, nil, nil, err
						}
					}
				...
				if joinIdx == 0 {
					...
					joinConds, err = BindFuncExprImplByPlanExpr(builder.GetContext(), "or", []*plan.Expr{joinConds, condExpr})
					if err != nil {
						return false, nil, nil, err
					}
				}
...
func checkNotNull(ctx context.Context, expr *plan.Expr, tableDef *plan.TableDef, col *plan.ColDef) error {
...
func forceCastExpr2(ctx context.Context, expr *plan.Expr, t2 types.Type, targetType *plan.Expr) (*plan.Expr, error) {
	...
	return &plan.Expr{
		Expr: &plan.Expr_F{
			F: &plan.Function{
				...
				Args: []*plan.Expr{expr, targetType},
			},
		},
		Typ: targetType.Typ,
	}, nil
...
func forceCastExpr(ctx context.Context, expr *plan.Expr, targetType plan.Type) (*plan.Expr, error) {
	if targetType.Id == 0 {
		return expr, nil
	}
...
func appendForeignConstrantPlan(
	...
	if enabled && !isFkRecursionCall && len(tableDef.Fkeys) > 0 {
		...
		if lastNodeId >= 0 {
			...
			filters := make([]*plan.Expr, len(tableDef.Fkeys))
			...
			for i := range tableDef.Fkeys {
				...
				nullCheckExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "isnotnull", []*plan.Expr{colExpr})
				if err != nil {
					return err
				}
				filterExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "assert", []*plan.Expr{nullCheckExpr, errExpr})
				if err != nil {
					return err
				}
...
func appendPrimaryConstraintPlan(
	...
	if pkPos, pkTyp := getPkPos(tableDef, true); pkPos != -1 {
		...
		if needCheck && !useFuzzyFilter {
			...
			eqCheckExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{MakePlan2Int64ConstExprWithType(1), countColExpr})
			if err != nil {
				return err
			}
			varcharType := types.T_varchar.ToType()
			varcharExpr, err := makePlan2CastExpr(builder.GetContext(), &plan.Expr{
			...
			filterExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "assert", []*plan.Expr{eqCheckExpr, varcharExpr, makePlan2StringConstExprWithType(tableDef.Cols[pkPos].Name), makePlan2StringConstExprWithType(colTypes)})
			if err != nil {
				return err
			}
			filterNode := &plan.Node{
		...
		if needCheck && useFuzzyFilter {
			...
			scanTableDef := plan.DeepCopyTableDef(tableDef, false)
			scanTableDef.Cols = make([]*plan.ColDef, pkSize)
			for _, col := range tableDef.Cols {
				if i, ok := pkNameMap[col.Name]; ok {
					scanTableDef.Cols[i] = plan.DeepCopyColDef(col)
				} else if col.Name == scanTableDef.Pkey.PkeyColName {
					scanTableDef.Cols[pkSize-1] = plan.DeepCopyColDef(col)
					break
				}
			}
			...
			if len(pkFilterExprs) > 0 {
				...
				blockFilterList = make([]*plan.Expr, len(pkFilterExprs))
				for i, e := range pkFilterExprs {
					blockFilterList[i] = plan.DeepCopyExpr(e)
				}
	...
	if config.CNPrimaryCheck.Load() {
		if pkPos, pkTyp := getPkPos(tableDef, true); pkPos != -1 {
			...
			if isUpdate && updatePkCol { // update stmt && pk included in update cols
				...
				scanTableDef.Cols = []*plan.ColDef{plan.DeepCopyColDef(tableDef.Cols[pkPos]), plan.DeepCopyColDef(rowIdDef)}
				...
				scanNode := &plan.Node{
					...
					ObjRef:                 objRef,
					TableDef:               scanTableDef,
					ProjectList:            []*plan.Expr{scanPkExpr, scanRowIdExpr},
					RuntimeFilterProbeList: []*plan.RuntimeFilterSpec{MakeRuntimeFilter(rfTag, false, 0, probeExpr, false)},
				}
				...
				condExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{pkColExpr, rightExpr})
				if err != nil {
					return err
				}
				rightRowIdExpr := &plan.Expr{
					Typ: rowIdDef.Typ,
					Expr: &plan.Expr_Col{
						Col: &plan.ColRef{
							ColPos: 1,
				...
				joinNode := &plan.Node{
					...
					IsRightJoin:            true,
					OnList:                 []*plan.Expr{condExpr},
					ProjectList:            []*plan.Expr{rowIdExpr, rightRowIdExpr, pkColExpr},
				...
				// append filter node
				filterExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "not_in_rows", []*plan.Expr{
					{
						Typ: rowIdExpr.Typ,
						Expr: &plan.Expr_Col{
							Col: &plan.ColRef{ColPos: 1, Name: catalog.Row_ID},
						},
					},
					{
						Typ: rowIdExpr.Typ,
						Expr: &plan.Expr_Col{
							Col: &plan.ColRef{ColPos: 0, Name: catalog.Row_ID},
						},
					},
				})
				if err != nil {
				...
				// append assert node
				isEmptyExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "isempty", []*plan.Expr{colExpr})
				if err != nil {
					return err
				}
				...
				assertExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "assert", []*plan.Expr{isEmptyExpr, varcharExpr, makePlan2StringConstExprWithType(tableDef.Cols[pkPos].Name), makePlan2StringConstExprWithType(colTypes)})
				if err != nil {
					return err
				}
				lastNodeId = builder.appendNode(&plan.Node{
