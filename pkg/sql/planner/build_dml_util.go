...
func buildInsertPlans(
	...
	objRef *plan.ObjectRef, tableDef *plan.TableDef, lastNodeId int32, ifExistAutoPkCol bool,
	...
	var pkFilterExpr []*plan.Expr
	var newPartitionExpr *plan.Expr
	if stmt != nil {
...
func buildUpdatePlans(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, updatePlanCtx *dmlPlanCtx, addAffectedRows bool) error {
		...
		if hasAutoCol {
			...
			preNodeId = builder.appendNode(&plan.Node{
				...
				PreInsertCtx: &plan.PreInsertCtx{
					...
					TableDef:    plan.DeepCopyTableDef(updatePlanCtx.tableDef, true),
					HasAutoCol:  true,
					ColOffset:   colOffset,
					IsNewUpdate: true,
				},
	...
	newCols := make([]*plan.ColDef, 0, len(updatePlanCtx.tableDef.Cols))
	...
	projectList := make([]*plan.Expr, insertColLength)
...
func buildDeletePlans(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, delCtx *dmlPlanCtx) error {
	...
	if enabled && len(delCtx.tableDef.RefChildTbls) > 0 {
		...
		for _, tableId := range delCtx.tableDef.RefChildTbls {
			...
			var childObjRef *plan.ObjectRef
			...
			childProjectList := make([]*plan.Expr, len(childTableDef.Cols))
			...
			for idx, col := range childTableDef.Cols {
				...
				childProjectList[idx] = &plan.Expr{
					Typ: col.Typ,
					Expr: &plan.Expr_Col{
						Col: &plan.ColRef{
							ColPos: int32(idx),
				...
				childForJoinProject[idx] = &plan.Expr{
					Typ: col.Typ,
					Expr: &plan.Expr_Col{
						Col: &plan.ColRef{
							RelPos: 1,
			...
			for _, fk := range childTableDef.Fkeys {
				...
				if fk.ForeignTbl == delCtx.tableDef.TblId || fkSelfReferCond {
					...
					// build join conds
					joinConds := make([]*plan.Expr, len(fk.Cols))
					rightConds := make([]*plan.Expr, len(fk.Cols))
					leftConds := make([]*plan.Expr, len(fk.Cols))
					// use for join's projection & filter's condExpr
					var oneLeftCond *plan.Expr
					var oneLeftCondName string
					updateChildColPosMap := make(map[string]int)
					updateChildColExpr := make([]*plan.Expr, len(fk.Cols))    // use for update
						...
						for _, col := range childTableDef.Cols {
							if col.ColId == colId {
								...
								if pos, ok := delCtx.updateColPosMap[originColumnName]; ok {
									updateChildColExpr[i] = &plan.Expr{
								...
								condExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{leftExpr, rightExpr})
								if err != nil {
									return err
								}
					...
					if isUpdate {
						var filterExpr, tmpExpr *plan.Expr
						for updateName, newIdx := range updateRefColumn {
							oldIdx := nameIdxMap[updateName]
							tmpExpr, err = BindFuncExprImplByPlanExpr(builder.GetContext(), "!=", []*plan.Expr{{
							...
							if filterExpr == nil {
								...
								filterExpr, err = BindFuncExprImplByPlanExpr(builder.GetContext(), "or", []*plan.Expr{filterExpr, tmpExpr})
								if err != nil {
									return nil
								}
							}
					...
					case plan.ForeignKeyDef_NO_ACTION, plan.ForeignKeyDef_RESTRICT, plan.ForeignKeyDef_SET_DEFAULT:
						...
						*/
						copiedTableDef := plan.DeepCopyTableDef(childTableDef, true)
						...
						lastNodeId = builder.appendNode(&plan.Node{
							...
							OnList:      joinConds,
							ProjectList: []*plan.Expr{oneLeftCond},
						}, bindCtx)
						...
						assertExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "assert", []*plan.Expr{isEmptyExpr, errExpr})
						if err != nil {
							return err
						}
					...
					case plan.ForeignKeyDef_SET_NULL:
						...
						rightId := builder.appendNode(&plan.Node{
							...
							Stats:       &plan.Stats{},
							ObjRef:      childObjRef,
							TableDef:    plan.DeepCopyTableDef(childTableDef, true),
							ProjectList: childProjectList,
						}, bindCtx)
						lastNodeId = builder.appendNode(&plan.Node{
							NodeType:    plan.Node_JOIN,
					...
					case plan.ForeignKeyDef_CASCADE:
						...
						if !fkSelfReferCond {
							if isUpdate {
								...
								upPlanCtx := getDmlPlanCtx()
								upPlanCtx.objRef = childObjRef
								upPlanCtx.tableDef = plan.DeepCopyTableDef(childTableDef, true)
								upPlanCtx.updateColLength = len(rightConds)
								upPlanCtx.isMulti = false
								upPlanCtx.rowIdPos = childRowIdPos
								upPlanCtx.sourceStep = newSourceStep
...
func buildInsertPlansWithRelatedHiddenTable(
	stmt *tree.Insert, ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef,
	tableDef *plan.TableDef, updateColLength int, sourceStep int32, addAffectedRows bool, isFkRecursionCall bool,
	updatePkCol bool, pkFilterExprs []*plan.Expr, partitionExpr *plan.Expr, ifExistAutoPkCol bool,
...
func makeOneInsertPlan(
	ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
	updateColLength int, sourceStep int32, addAffectedRows bool, isFkRecursionCall bool, updatePkCol bool,
	pkFilterExprs []*plan.Expr, partitionExpr *plan.Expr, ifExistAutoPkCol bool, ifCheckPkDup bool, ifInsertFromUnique bool,
...
func appendPureInsertBranch(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef, sourceStep int32, addAffectedRows bool) {
...
func makeOneDeletePlan(
	...
	if isUK || isSK {
		...
		if isUK {
			...
			filterExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "is_not_null", []*plan.Expr{rowIdColExpr})
			if err != nil {
				return -1, err
			}
			filterNode := &plan.Node{
...
func getProjectionByLastNodeForRightJoin(builder *QueryBuilder, lastNodeId int32) []*plan.Expr {
	lastNode := builder.qry.Nodes[lastNodeId]
	projLength := len(lastNode.ProjectList)
	if projLength == 0 {
		return getProjectionByLastNode(builder, lastNode.Children[0])
	}
	projection := make([]*plan.Expr, len(lastNode.ProjectList))
...
func getProjectionByLastNode(builder *QueryBuilder, lastNodeId int32) []*plan.Expr {
	lastNode := builder.qry.Nodes[lastNodeId]
	projLength := len(lastNode.ProjectList)
	if projLength == 0 {
		return getProjectionByLastNode(builder, lastNode.Children[0])
...
func getProjectionByLastNodeWithTag(builder *QueryBuilder, lastNodeId, tag int32) []*plan.Expr {
	lastNode := builder.qry.Nodes[lastNodeId]
	projLength := len(lastNode.ProjectList)
	if projLength == 0 {
...
func haveUniqueKey(tableDef *plan.TableDef) bool {
...
func haveSecondaryKey(tableDef *plan.TableDef) bool {
	for _, indexdef := range tableDef.Indexes {
...
func isPrimaryKey(tableDef *plan.TableDef, colNames []string) bool {
...
func makeDeleteNodeInfo(ctx CompilerContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
	deleteIdx int, addAffectedRows bool, pkPos int, pkTyp plan.Type, lockTable bool) *deleteNodeInfo {
...
func appendSinkScanNodeWithTag(builder *QueryBuilder, bindCtx *BindContext, sourceStep, tag int32) int32 {
	...
	sinkScanNode := &plan.Node{
		...
		TableDef:    &plan.TableDef{Name: bindCtx.cteName},
	}
	b := bindCtx.bindings[0]
	sinkScanNode.TableDef.Cols = make([]*plan.ColDef, len(b.cols))
	for i, col := range b.cols {
		sinkScanNode.TableDef.Cols[i] = &plan.ColDef{
			Name:   col,
...
func appendRecursiveScanNode(builder *QueryBuilder, bindCtx *BindContext, sourceStep, tag int32) int32 {
	...
	b := bindCtx.bindings[0]
	recursiveScanNode.TableDef.Cols = make([]*plan.ColDef, len(b.cols))
	for i, col := range b.cols {
		recursiveScanNode.TableDef.Cols[i] = &plan.ColDef{
			Name:   col,
			Hidden: b.colIsHidden[i],
			Typ:    *b.types[i],
		}
...
func appendAggCountGroupByColExpr(builder *QueryBuilder, bindCtx *BindContext, lastNodeId int32, colExpr *plan.Expr) (int32, error) {
	aggExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "starcount", []*plan.Expr{colExpr})
	if err != nil {
		return -1, err
	}

	countType := types.T_int64.ToType()
	groupByNode := &plan.Node{
		NodeType: plan.Node_AGG,
		Children: []int32{lastNodeId},
		GroupBy:  []*plan.Expr{colExpr},
		AggList:  []*plan.Expr{aggExpr},
...
func getPkPos(tableDef *plan.TableDef, ignoreFakePK bool) (int, plan.Type) {
...
func getRowIdPos(tableDef *plan.TableDef) int {
	for i, col := range tableDef.Cols {
		if col.Name == catalog.Row_ID {
			return i
		}
...
func getHiddenColumnForPreInsert(tableDef *plan.TableDef) ([]plan.Type, []string) {
	var typs []plan.Type
	var names []string
...
func appendJoinNodeForParentFkCheck(builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef, baseNodeId int32) (int32, error) {
		...
		for _, colId := range fk.Cols {
			for fIdx, col := range tableDef.Cols {
				if col.ColId == colId {
					...
					condExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "isnotnull", []*plan.Expr{colExpr})
					if err != nil {
						return -1, err
					}
	...
	baseNodeId = builder.appendNode(&plan.Node{
		NodeType:    plan.Node_FILTER,
	...
	for _, fk := range tableDef.Fkeys {
		...
		newTableDef := plan.DeepCopyTableDef(parentTableDef, false)
		...
		scanNodeProject := make([]*plan.Expr, len(parentTableDef.Cols))
...
func appendPreInsertNode(builder *QueryBuilder, bindCtx *BindContext,
	objRef *plan.ObjectRef, tableDef *plan.TableDef,
...
func appendPreInsertSkMasterPlan(builder *QueryBuilder,
	...
	colsType := make(map[string]*plan.Type)
...
func buildSerialFullAndPKColsProjMasterIndex(builder *QueryBuilder, bindCtx *BindContext, tableDef *plan.TableDef, genLastNodeIdFn func() int32, originPkPos int, part string, colsType map[string]*plan.Type, colsPos map[string]int, originPkType plan.Type) (*plan.Node, error) {
	var err error
	...
	projectProjection := make([]*plan.Expr, 2)
...
func appendPreInsertSkVectorPlan(builder *QueryBuilder, bindCtx *BindContext, tableDef *plan.TableDef, lastNodeId int32, multiTableIndex *MultiTableIndex, isUpdate bool, idxRefs []*plan.ObjectRef, indexTableDefs []*plan.TableDef) (int32, error) {

	//1.a get vector & pk column details
	var posOriginPk, posOriginVecColumn int
	var typeOriginPk, typeOriginVecColumn plan.Type
	{
...
func recomputeMoCPKeyViaProjection(builder *QueryBuilder, bindCtx *BindContext, tableDef *plan.TableDef, lastNodeId int32, posOriginPk int) int32 {
	if tableDef.Pkey != nil && tableDef.Pkey.PkeyColName != catalog.FakePrimaryKeyColName {
		...
		projectProjection := make([]*plan.Expr, len(lastProject))
...
func appendPreInsertPlan(
	...
	********/
	...
	var preinsertUkProjection []*plan.Expr
...
func appendDeleteIndexTablePlan(
	...
	********/
	...
	scanNodeProject := make([]*plan.Expr, len(uniqueTableDef.Cols))
	...
	if partsLength == 1 {
		...
		args := make([]*plan.Expr, partsLength)
	...
	condExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{rightExpr, leftExpr})
	if err != nil {
		return -1, err
	}
	joinConds = []*plan.Expr{condExpr}
...
func appendDeleteMasterTablePlan(builder *QueryBuilder, bindCtx *BindContext,
	masterObjRef *plan.ObjectRef, masterTableDef *plan.TableDef,
	baseNodeId int32, tableDef *plan.TableDef, indexDef *plan.IndexDef,
	...
	var rightRowIdPos int32 = -1
	var rightPkPos int32 = -1
	scanNodeProject := make([]*plan.Expr, len(masterTableDef.Cols))
	...
	for idx, part := range indexDef.Parts {
		...
		currCond, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{leftExpr, rightExpr})
		if err != nil {
			return -1, err
		}
		if idx == 0 {
...
func appendDeleteIvfTablePlan(builder *QueryBuilder, bindCtx *BindContext,
	entriesObjRef *plan.ObjectRef, entriesTableDef *plan.TableDef,
	baseNodeId int32, tableDef *plan.TableDef) (int32, error) {
	...
	scanNodeProject := make([]*plan.Expr, 3)
	...
	newEntriesTableDef := plan.DeepCopyTableDef(entriesTableDef, false)
	...
	*/
	...
	condExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{origExpr, ivfExpr})
	if err != nil {
		return -1, err
	}
	joinConds = []*plan.Expr{condExpr}
...
func makePreUpdateDeletePlan(
	...
	projectProjection := make([]*plan.Expr, len(delCtx.tableDef.Cols)+delCtx.updateColLength)
	...
	if delCtx.needAggFilter {
		lastNode := builder.qry.Nodes[lastNodeId]
		groupByExprs := make([]*plan.Expr, len(delCtx.tableDef.Cols))
		aggNodeProjection := make([]*plan.Expr, len(lastNode.ProjectList))
		...
		offset := len(delCtx.tableDef.Cols)
		aggList := make([]*plan.Expr, delCtx.updateColLength)
		for i := 0; i < delCtx.updateColLength; i++ {
			pos := offset + i
			...
			aggExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "any_value", []*plan.Expr{baseExpr})
			if err != nil {
				return -1, err
			}
			aggList[i] = aggExpr
		...
		nullCheckExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "isnotnull", []*plan.Expr{rowIdExpr})
		if err != nil {
			return -1, err
		}
		filterNode := &plan.Node{
			NodeType:    plan.Node_FILTER,
			Children:    []int32{lastNodeId},
			FilterList:  []*plan.Expr{nullCheckExpr},
			ProjectList: getProjectionByLastNode(builder, lastNodeId),
		}
		lastNodeId = builder.appendNode(filterNode, bindCtx)
	}
...
func reduceSinkSinkScanNodes(qry *plan.Query) {
	if len(qry.Steps) == 1 {
		return
	}
...
func buildPreInsertMultiTableIndexes(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
	...
	for _, multiTableIndex := range multiTableIndexes {
		...
		case catalog.MoIndexIvfFlatAlgo.ToString():
			...
			lastNodeId = appendSinkScanNode(builder, bindCtx, sourceStep)
			var idxRefs = make([]*plan.ObjectRef, 3)
			var idxTableDefs = make([]*plan.TableDef, 3)
			...
			for i := range idxTableDefs {
				idxTableDefs[i].Cols = RemoveIf[*plan.ColDef](idxTableDefs[i].Cols, func(column *plan.ColDef) bool {
		...
		case catalog.MoIndexIvfFlatAlgo.ToString():
			...
			if isUpdate {
				...
				// insert ivf_sk plan
				{
					...
					projectProjection := make([]*plan.Expr, len(delCtx.tableDef.Cols))
					projectProjectionForTblJoinCentroids := make([]*plan.Expr, len(delCtx.tableDef.Cols))
					...
					insertEntriesTableDef := plan.DeepCopyTableDef(entriesTableDef, false)
					for _, col := range entriesTableDef.Cols {
						if col.Name != catalog.Row_ID {
							insertEntriesTableDef.Cols = append(insertEntriesTableDef.Cols, plan.DeepCopyColDef(col))
						}
					}
					updateColLength := 1
					addAffectedRows := false
...
func buildPreInsertRegularIndex(stmt *tree.Insert, ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
	...
	// remove row_id
	idxTableDef.Cols = RemoveIf[*plan.ColDef](idxTableDef.Cols, func(col *plan.ColDef) bool {
	...
	var insertColsNameFromStmt []string
	var pkFilterExprForHiddenTable []*plan.Expr
	...
	if indexdef.GetUnique() {
		...
		// remove row_id
		idxTableDef.Cols = RemoveIf[*plan.ColDef](idxTableDef.Cols, func(colVal *plan.ColDef) bool {
...
func buildPreInsertMasterIndex(stmt *tree.Insert, ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
	sourceStep int32, ifInsertFromUniqueColMap map[string]bool, indexdef *plan.IndexDef, idx int) error {
	idxRef, idxTableDef, err := ctx.ResolveIndexTableByRef(objRef, indexdef.IndexTableName, nil)
	if err != nil {
		return err
	...
	********/
	...
	if isUpdate {
		...
		// insert uk plan
		{
			...
			insertUniqueTableDef := plan.DeepCopyTableDef(uniqueTableDef, false)
			for _, col := range uniqueTableDef.Cols {
				if col.Name != catalog.Row_ID {
					insertUniqueTableDef.Cols = append(insertUniqueTableDef.Cols, plan.DeepCopyColDef(col))
				}
			}
...
func buildDeleteMasterIndex(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, delCtx *dmlPlanCtx,
	...
	if isUpdate {
		...
		// insert master sk plan
		{
			...
			insertEntriesTableDef := plan.DeepCopyTableDef(masterTableDef, false)
...
func buildPreInsertFullTextIndex(stmt *tree.Insert, ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef,
	tableDef *plan.TableDef, updateColLength int, sourceStep int32, ifInsertFromUniqueColMap map[string]bool, indexdef *plan.IndexDef,
	...
	ftcols := plan.DeepCopyColDefList(tokenizeColDefs)
	...
	insertEntriesTableDef := plan.DeepCopyTableDef(indexTableDef, false)
	...
	preInsertNode := &plan.Node{
		...
		PreInsertCtx: &plan.PreInsertCtx{
			Ref:           indexObjRef,
			TableDef:      plan.DeepCopyTableDef(indexTableDef, true),
			HasAutoCol:    true,
			IsOldUpdate:   false,
			CompPkeyExpr:  nil,
			ClusterByExpr: nil,
...
func buildDeleteRowsFullTextIndex(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, delCtx *dmlPlanCtx,
	indexObjRef *plan.ObjectRef, indexTableDef *plan.TableDef, indexdef *plan.IndexDef, typMap map[string]plan.Type, posMap map[string]int) (int32, int, int, plan.Type, error) {

	if delCtx.isDeleteWithoutFilters {
		// truncate and create a table scan of index table

		scanNodeProject := make([]*plan.Expr, len(indexTableDef.Cols))
		...
		newIndexTableDef := plan.DeepCopyTableDef(indexTableDef, false)
		...
		joinCond, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{leftExpr, rightExpr})
		if err != nil {
			return -1, -1, -1, plan.Type{}, err
		}
...
func buildPostDmlFullTextIndex(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, indexObjRef *plan.ObjectRef, indexTableDef *plan.TableDef, tableDef *plan.TableDef,
...
func buildPostInsertFullTextIndex(stmt *tree.Insert, ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, objRef *plan.ObjectRef, tableDef *plan.TableDef,
