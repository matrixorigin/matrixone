...
func (builder *QueryBuilder) generateRuntimeFilters(nodeID int32) {
	...
	if len(probeExprs) == 1 {
		...
		if convertToCPKey {
			leftChild.RuntimeFilterProbeList = append(leftChild.RuntimeFilterProbeList, MakeCPKEYRuntimeFilter(rfTag, 0, plan.DeepCopyExpr(probeExprs[0]), tableDef, notOnPk))
		} else {
			leftChild.RuntimeFilterProbeList = append(leftChild.RuntimeFilterProbeList, MakeRuntimeFilter(rfTag, false, 0, plan.DeepCopyExpr(probeExprs[0]), notOnPk))
		}
