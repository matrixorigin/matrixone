...
func genAsSelectCols(ctx CompilerContext, stmt *tree.Select) ([]*plan.ColDef, error) {
	var err error
	var rootId int32
...
func buildCreateSource(stmt *tree.CreateSource, ctx CompilerContext) (*plan.Plan, error) {
...
func buildCreateView(stmt *tree.CreateView, ctx CompilerContext) (*plan.Plan, error) {
	...
	snapshot := &plan.Snapshot{TS: &timestamp.Timestamp{}}
...
func buildSequenceTableDef(stmt *tree.CreateSequence, ctx CompilerContext, cs *plan.CreateSequence) error {
	...
	cs.TableDef.Pkey = &plan.PrimaryKeyDef{
...
func buildAlterSequenceTableDef(stmt *tree.AlterSequence, ctx CompilerContext, as *plan.AlterSequence) error {
	...
	as.TableDef.Pkey = &plan.PrimaryKeyDef{
		Names:       []string{Sequence_cols_name[4]},
		PkeyColName: Sequence_cols_name[4],
	}
	for i := 5; i <= 6; i++ {
...
func buildDropSequence(stmt *tree.DropSequence, ctx CompilerContext) (*plan.Plan, error) {
...
func buildAlterSequence(stmt *tree.AlterSequence, ctx CompilerContext) (*plan.Plan, error) {
...
func buildCreateSequence(stmt *tree.CreateSequence, ctx CompilerContext) (*plan.Plan, error) {
...
func buildTableDefs(stmt *tree.CreateTable, ctx CompilerContext, createTable *plan.CreateTable, asSelectCols []*plan.ColDef) error {
	...
	for _, item := range stmt.Defs {
		...
		case *tree.ColumnTableDef:
			...
			if idx := slices.IndexFunc(asSelectCols, func(c *plan.ColDef) bool { return c.Name == col.Name }); idx != -1 {
	...
	if stmt.IsAsSelect {
		...
		for i := range cols {
			...
			if !slices.ContainsFunc(asSelectCols, func(c *plan.ColDef) bool { return c.Name == cols[i].Name }) {
				if !firstCol {
...
//
// )
func buildFullTextIndexTable(createTable *plan.CreateTable, indexInfos []*tree.FullTextIndex, colMap map[string]*plan.ColDef, existedIndexes []*plan.IndexDef, pkeyName string, ctx CompilerContext) error {
...
func buildUniqueIndexTable(createTable *plan.CreateTable, indexInfos []*tree.UniqueIndex, colMap map[string]*plan.ColDef, pkeyName string, ctx CompilerContext) error {
	for _, indexInfo := range indexInfos {
		indexDef := &plan.IndexDef{}
...
func buildSecondaryIndexDef(createTable *plan.CreateTable, indexInfos []*tree.Index, colMap map[string]*plan.ColDef, existedIndexes []*plan.IndexDef, pkeyName string, ctx CompilerContext) (err error) {
	if len(pkeyName) == 0 {
...
//
// )
func buildMasterSecondaryIndexDef(ctx CompilerContext, indexInfo *tree.Index, colMap map[string]*plan.ColDef, pkeyName string) ([]*plan.IndexDef, []*plan.TableDef, error) {
	// 1. indexDef init
	indexDef := &plan.IndexDef{}
	indexDef.Unique = false
...
//
// )
func buildRegularSecondaryIndexDef(ctx CompilerContext, indexInfo *tree.Index, colMap map[string]*plan.ColDef, pkeyName string) ([]*plan.IndexDef, []*plan.TableDef, error) {
...
func buildIvfFlatSecondaryIndexDef(ctx CompilerContext, indexInfo *tree.Index, colMap map[string]*plan.ColDef, existedIndexes []*plan.IndexDef, pkeyName string) ([]*plan.IndexDef, []*plan.TableDef, error) {
...
func buildHnswSecondaryIndexDef(ctx CompilerContext, indexInfo *tree.Index, colMap map[string]*plan.ColDef, existedIndexes []*plan.IndexDef, pkeyName string) ([]*plan.IndexDef, []*plan.TableDef, error) {
...
func buildTruncateTable(stmt *tree.TruncateTable, ctx CompilerContext) (*plan.Plan, error) {
...
func buildDropView(stmt *tree.DropView, ctx CompilerContext) (*plan.Plan, error) {
	dropTable := &plan.DropTable{
		IfExists: stmt.IfExists,
	}
	if len(stmt.Names) != 1 {
...
func buildCreateIndex(stmt *tree.CreateIndex, ctx CompilerContext) (*plan.Plan, error) {
	...
	indexInfo := &plan.CreateTable{TableDef: &plan.TableDef{}}
	if uIdx != nil {
...
func buildAlterView(stmt *tree.AlterView, ctx CompilerContext) (*plan.Plan, error) {
...
func buildRenameTable(stmt *tree.RenameTable, ctx CompilerContext) (*plan.Plan, error) {
...
func buildAlterTableInplace(stmt *tree.AlterTable, ctx CompilerContext) (*plan.Plan, error) {
	tableName := string(stmt.Table.ObjectName)
	...
	for i, option := range stmt.Options {
		...
		case *tree.AlterTableModifyColumnClause:
			...
			if alterTable.CopyTableDef == nil {
				alterTable.CopyTableDef = plan.DeepCopyTableDef(tableDef, true)
			}
...
func buildLockTables(stmt *tree.LockTableStmt, ctx CompilerContext) (*plan.Plan, error) {
...
func buildUnLockTables(stmt *tree.UnLockTableStmt, ctx CompilerContext) (*plan.Plan, error) {
...
func getForeignKeyData(ctx CompilerContext, dbName string, tableDef *plan.TableDef, def *tree.ForeignKey) (*FkData, error) {
	refer := def.Refer
	fkData := FkData{
		Def: &plan.ForeignKeyDef{
	...
	name2ColDef := make(map[string]*plan.ColDef)
...
*/
func checkFkColsAreValid(ctx CompilerContext, fkData *FkData, parentTableDef *plan.TableDef) error {
...
func buildCreatePitr(stmt *tree.CreatePitr, ctx CompilerContext) (*plan.Plan, error) {
...
func buildDropPitr(stmt *tree.DropPitr, ctx CompilerContext) (*plan.Plan, error) {
...
func buildCreateCDC(stmt *tree.CreateCDC, ctx CompilerContext) (*plan.Plan, error) {
	accountId, err := ctx.GetAccountId()
	if err != nil {
		return nil, err
	}
...
func buildDropCDC(stmt *tree.DropCDC, ctx CompilerContext) (*plan.Plan, error) {
	accountId, err := ctx.GetAccountId()
	if err != nil {
		return nil, err
	}
