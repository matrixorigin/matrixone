...
func TestBuildAlterView(t *testing.T) {
	...
	ctx.EXPECT().Resolve(gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(
		func(schemaName string, tableName string, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error) {
			if schemaName == "" {
				schemaName = "db"
			}
...
func TestBuildCreatePitr(t *testing.T) {
	...
	t.Run("database level pitr, database not exist", func(t *testing.T) {
		...
		ctx.GetAccountNameFunc = func() string { return "sys" }
		ctx.GetAccountIdFunc = func() (uint32, error) { return 1, nil }
		ctx.DatabaseExistsFunc = func(string, *plan.Snapshot) bool { return false }
		stmt := baseStmt()
	...
	t.Run("database level pitr, database exists", func(t *testing.T) {
		...
		ctx.GetAccountNameFunc = func() string { return "sys" }
		ctx.GetAccountIdFunc = func() (uint32, error) { return 1, nil }
		ctx.DatabaseExistsFunc = func(string, *plan.Snapshot) bool { return true }
		ctx.GetDatabaseIdFunc = func(string, *plan.Snapshot) (uint64, error) { return 123, nil }
		stmt := baseStmt()
		stmt.Level = tree.PITRLEVELDATABASE
		stmt.DatabaseName = "db1"
		plan, err := buildCreatePitr(stmt, ctx)
	...
	t.Run("table level pitr, table not exist", func(t *testing.T) {
		...
		ctx.GetAccountIdFunc = func() (uint32, error) { return 1, nil }
		ctx.DatabaseExistsFunc = func(string, *plan.Snapshot) bool { return true }
		ctx.ResolveFunc = func(string, string, *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef) { return nil, nil }
		stmt := baseStmt()
	...
	t.Run("table level pitr, table exists", func(t *testing.T) {
		...
		ctx.GetAccountIdFunc = func() (uint32, error) { return 1, nil }
		ctx.DatabaseExistsFunc = func(string, *plan.Snapshot) bool { return true }
		ctx.ResolveFunc = func(string, string, *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef) {
			return &plan.ObjectRef{}, &plan.TableDef{TblId: 456}
		}
		stmt := baseStmt()
		stmt.Level = tree.PITRLEVELTABLE
		stmt.DatabaseName = "db1"
