...
func (builder *QueryBuilder) applyIndices(nodeID int32, colRefCnt map[[2]int32]int, idxColMap map[[2]int32]*plan.Expr) (int32, error) {
	...
	case plan.Node_JOIN:
		return builder.applyIndicesForJoins(nodeID, node), nil
...
func (builder *QueryBuilder) applyIndicesForFiltersRegularIndex(nodeID int32, node *plan.Node, colRefCnt map[[2]int32]int, idxColMap map[[2]int32]*plan.Expr) int32 {
	...
	indexes := make([]*plan.IndexDef, 0, len(node.TableDef.Indexes))
...
func (builder *QueryBuilder) applyExtraFiltersOnIndex(idxDef *plan.IndexDef, node *plan.Node, idxTableNode *plan.Node, filterIdx []int32) {
	for i := range node.FilterList {
		...
		for k := range idxDef.Parts {
			...
			newFilter := plan.DeepCopyExpr(node.FilterList[i])
...
func tryMatchMoreLeadingFilters(idxDef *plan.IndexDef, node *plan.Node, pos int32) []int32 {
...
func findLeadingFilter(idxDef *plan.IndexDef, node *plan.Node) ([]int32, bool) {
...
func (builder *QueryBuilder) replaceEqualCondition(filterList []*plan.Expr, filterPos []int32, idxTag int32, idxTableDef *plan.TableDef, numParts int) *plan.Expr {
	if numParts == 1 { //directly equal
		expr := plan.DeepCopyExpr(filterList[filterPos[0]])
		args := expr.GetF().Args
	...
	for i := range filterPos {
		...
		serialArgs[i] = plan.DeepCopyExpr(filter.GetF().Args[1])
...
func (builder *QueryBuilder) tryIndexOnlyScan(idxDef *plan.IndexDef, node *plan.Node, colRefCnt map[[2]int32]int, idxColMap map[[2]int32]*plan.Expr, scanSnapshot *plan.Snapshot) int32 {
	...
	if numParts == 1 {
		...
		for i := 0; i < numParts; i++ {
			...
			if colName == node.TableDef.Pkey.PkeyColName {
				...
				mappedExpr, _ := MakeSerialExtractExpr(builder.GetContext(), plan.DeepCopyExpr(leadingColExpr), origType, int64(i))
...
func (builder *QueryBuilder) getIndexForNonEquiCond(indexes []*plan.IndexDef, node *plan.Node) (int, []int32) {
...
func (builder *QueryBuilder) applyIndexJoin(idxDef *plan.IndexDef, node *plan.Node, filterType int, filterIdx []int32, scanSnapshot *plan.Snapshot) (int32, int32) {
	idxTag := builder.genNewBindTag()
	...
	idxTableNode := &plan.Node{
		...
		ObjRef:        idxObjRef,
		IndexScanInfo: idxScanInfo,
		ParentObjRef:  plan.DeepCopyObjectRef(node.ObjRef),
...
func (builder *QueryBuilder) getMostSelectiveIndexForPointSelect(indexes []*plan.IndexDef, node *plan.Node, ignoreStats bool) (int, []int32) {
	currentSel := 1.0
	currentIdx := -1
...
func (builder *QueryBuilder) applyIndicesForJoins(nodeID int32, node *plan.Node) int32 {
	...
	for _, idxDef := range indexes {
		...
		idxTableNodeID := builder.appendNode(&plan.Node{
			...
			ObjRef:                 idxObjRef,
			IndexScanInfo:          idxScanInfo,
			ParentObjRef:           plan.DeepCopyObjectRef(leftChild.ObjRef),
			BindingTags:            []int32{idxTag},
			ScanSnapshot:           leftChild.ScanSnapshot,
