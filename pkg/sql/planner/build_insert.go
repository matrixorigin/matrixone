...
func buildInsert(stmt *tree.Insert, ctx CompilerContext, isReplace bool, isPrepareStmt bool) (p *plan.Plan, err error) {
	start := time.Now()
	defer func() {
	...
	if len(rewriteInfo.onDuplicateIdx) > 0 {
		...
		tableDef = plan.DeepCopyTableDef(tableDef, true)
		...
		if rewriteInfo.onDuplicateNeedAgg {
			...
			aggGroupBy := make([]*plan.Expr, 0, colLen)
			aggList := make([]*plan.Expr, 0, len(dupProjection)-colLen)
			aggProject := make([]*plan.Expr, 0, len(dupProjection))
			for i := 0; i < len(dupProjection); i++ {
				if i < colLen {
					aggGroupBy = append(aggGroupBy, &plan.Expr{
					...
					aggProject = append(aggProject, &plan.Expr{
						Typ: dupProjection[i].Typ,
						Expr: &plan.Expr_Col{
							Col: &plan.ColRef{
								RelPos: -1,
					...
					aggExpr, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "any_value", []*plan.Expr{
						{
							Typ: dupProjection[i].Typ,
							Expr: &plan.Expr_Col{
								Col: &plan.ColRef{
		...
		var projectProjection []*plan.Expr
		tableDef = plan.DeepCopyTableDef(tableDef, true)
...
func getInsertColsFromStmt(ctx context.Context, stmt *tree.Insert, tableDef *plan.TableDef) ([]string, error) {
...
func canUsePkFilter(builder *QueryBuilder, ctx CompilerContext, stmt *tree.Insert, tableDef *plan.TableDef, insertColsName []string, uniqueIndexDef *plan.IndexDef) bool {
	var isCompound bool
...
func newLocationMap(tableDef *plan.TableDef, uniqueIndexDef *plan.IndexDef) *locationMap {
...
func getPkValueExpr(builder *QueryBuilder, ctx CompilerContext, tableDef *plan.TableDef, lmap *locationMap, insertColsNameFromStmt []string) (pkFilterExprs []*plan.Expr, err error) {
	...
	colExprs := make([][]*plan.Expr, len(lmap.m))
	...
	for idx, name := range insertColsNameFromStmt {
		...
		valExprs := make([]*plan.Expr, rowsCount)

		for i, data := range node.RowsetData.Cols[idx].Data {
			rowExpr := plan.DeepCopyExpr(data.Expr)
	...
	if !isCompound {
		...
		if rowsCount == 1 {
			...
			filterExpr, _ = BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{
				pkExpr,
				colExprs[0][0],
			})
		} else {
			...
			filterExpr, _ = BindFuncExprImplByPlanExpr(builder.GetContext(), "in", []*plan.Expr{
				pkExpr,
				{
					Typ: pkExpr.Typ,
					Expr: &plan.Expr_List{
	...
	return []*plan.Expr{filterExpr}, nil
}

func getRewriteToReplaceStmt(tableDef *plan.TableDef, stmt *tree.Insert, info *dmlSelectInfo, isPrepareStmt bool) *tree.Replace {
