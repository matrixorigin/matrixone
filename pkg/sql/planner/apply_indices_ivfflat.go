...
func (builder *QueryBuilder) applyIndicesForSortUsingIvfflat(nodeID int32, vecCtx *vectorSortContext, multiTableIndex *MultiTableIndex, colRefCnt map[[2]int32]int, idxColMap map[[2]int32]*plan.Expr) (int32, error) {
	...
	tableFuncNode := &plan.Node{
		...
		TableDef: &plan.TableDef{
			...
			Cols: plan.DeepCopyColDefList(kIVFSearchColDefs),
		},
		...
		TblFuncExprList: []*plan.Expr{
			...
			plan.DeepCopyExpr(ivfCtx.vecLitArg),
		},
	}
	...
	limitExpr := plan.DeepCopyExpr(limit)
	...
	if pushdownEnabled {
		...
		if oldTag != newTag {
			...
			for key, value := range idxColMap {
				if key[0] == oldTag {
					idxColMap[[2]int32{newTag, key[1]}] = plan.DeepCopyExpr(value)
				}
			}
		}
		...
		innerJoinOn, _ := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{
			{
				Typ: ivfCtx.pkType,
				Expr: &plan.Expr_Col{
					Col: &plan.ColRef{
		...
		outerOn, _ := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*plan.Expr{
			{
				Typ: ivfCtx.pkType,
				Expr: &plan.Expr_Col{
					Col: &plan.ColRef{
		...
		probeSpec2 := MakeRuntimeFilter(rfTag2, false, 0, plan.DeepCopyExpr(probeExpr2), false)
