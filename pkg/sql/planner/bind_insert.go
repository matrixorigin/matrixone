...
func (builder *QueryBuilder) bindInsert(stmt *tree.Insert, bindCtx *BindContext) (int32, error) {
	...
	if stmt.IsRestore {
		...
		builder.compCtx.SetSnapshot(&plan.Snapshot{
	...
	if len(astUpdateExprs) == 0 {
		...
		for _, col := range tableDef.Cols {
			if col.OnUpdate != nil && col.OnUpdate.Expr != nil && updateExprs[col.Name] == nil {
				newDefExpr := plan.DeepCopyExpr(col.OnUpdate.Expr)
	...
	for _, col := range tableDef.Cols {
		if col.Name == pkName && pkName != catalog.FakePrimaryKeyColName {
			lockTarget := &plan.LockTarget{
				...
				ObjRef:             plan.DeepCopyObjectRef(objRef),
...
func (builder *QueryBuilder) getInsertColsFromStmt(astCols tree.IdentifierList, tableDef *plan.TableDef) ([]string, error) {
...
func (builder *QueryBuilder) appendNodesForInsertStmt(
	...
	for i, col := range tableDef.Cols {
		if oldExpr, exists := insertColToExpr[col.Name]; exists {
			...
			//	args[k] = plan.DeepCopyExpr(insertColToExpr[part])
			//}
			//
