...
func getNormalExternalProject(stmt *tree.Load, ctx CompilerContext, tableDef *plan.TableDef, tblName string) ([]*plan.Expr, map[string]int32, map[string]int32, *plan.TableDef, error) {
	var externalProject []*plan.Expr
...
func getExternalWithColListProject(stmt *tree.Load, ctx CompilerContext, tableDef *plan.TableDef, tblName string) ([]*plan.Expr, map[string]int32, map[string]int32, *plan.TableDef, error) {
	var externalProject []*plan.Expr
	colToIndex := make(map[string]int32, 0)
	tbColToDataCol := make(map[string]int32, 0)
	var newCols []*plan.ColDef

	newTableDef := plan.DeepCopyTableDef(tableDef, true)
	colPos := 0
...
func getExternalProject(stmt *tree.Load, ctx CompilerContext, tableDef *plan.TableDef, tblName string) ([]*plan.Expr, map[string]int32, map[string]int32, *plan.TableDef, error) {
...
func buildLoad(stmt *tree.Load, ctx CompilerContext, isPrepareStmt bool) (*plan.Plan, error) {
	start := time.Now()
	defer func() {
	...
	newTableDef := plan.DeepCopyTableDef(originTableDef, true)
...
func getProjectNode(stmt *tree.Load, ctx CompilerContext, node *plan.Node, tableDef *plan.TableDef, colToIndex map[string]int32) (bool, error) {
	tblName := string(stmt.Table.ObjectName)
...
func checkNullMap(stmt *tree.Load, Cols []*plan.ColDef, ctx CompilerContext) error {
...
func makeCastExpr(stmt *tree.Load, fileName string, tableDef *plan.TableDef, node *plan.Node) []*plan.Expr {
