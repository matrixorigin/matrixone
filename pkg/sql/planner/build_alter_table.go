...
func skipPkDedup(old, new *plan.TableDef) bool {
	oldPk := old.Pkey
	newPk := new.Pkey
...
func skipUniqueIdxDedup(old, new *plan.TableDef) map[string]bool {
...
func buildAlterTableCopy(stmt *tree.AlterTable, cctx CompilerContext) (*plan.Plan, error) {
	ctx := cctx.GetContext()
...
type AlterTableContext struct {
	...
	changColDefMap map[uint64]*plan.ColDef
	UpdateSqls     []string
}
...
func initAlterTableContext(originTableDef *plan.TableDef, copyTableDef *plan.TableDef, schemaName string) *AlterTableContext {
	alterTblColMap := make(map[string]selectExpr)
	changTblColIdMap := make(map[uint64]*plan.ColDef)
...
func buildCopyTableDef(ctx context.Context, tableDef *plan.TableDef) (*plan.TableDef, error) {
	replicaTableDef := plan.DeepCopyTableDef(tableDef, true)
...
func buildAlterTable(stmt *tree.AlterTable, ctx CompilerContext) (*plan.Plan, error) {
...
func buildNotNullColumnVal(col *plan.ColDef) string {
