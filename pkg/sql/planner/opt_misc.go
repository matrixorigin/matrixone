...
func exprCanRemoveProject(expr *plan.Expr) bool {
...
func replaceColumnsForExpr(expr *plan.Expr, projMap map[[2]int32]*plan.Expr) *plan.Expr {
	...
	case *plan.Expr_Col:
		...
		if projExpr, ok := projMap[mapID]; ok {
			return plan.DeepCopyExpr(projExpr)
		}
...
func makeBetweenExprFromDateFormat(equalFunc *plan.Function, dateformatFunc *plan.Function, intervalStr string, builder *QueryBuilder) *plan.Expr {
	dateExpr := plan.DeepCopyExpr(equalFunc.Args[1])
	if intervalStr == "year" {
	...
	end, err := bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "+", []*plan.Expr{plan.DeepCopyExpr(begin), interval})
	if err != nil {
		return nil
	}
	interval = MakeIntervalExpr(1, "microsecond")
	end, err = bindFuncExprAndConstFold(builder.GetContext(), builder.compCtx.GetProcess(), "-", []*plan.Expr{plan.DeepCopyExpr(end), interval})
	if err != nil {
		return nil
	}
	args := []*plan.Expr{dateformatFunc.Args[0], begin, end}
...
func (builder *QueryBuilder) optimizeLikeExpr(nodeID int32) {
	...
	for i := range node.FilterList {
		...
		if fun != nil && fun.Func.ObjName == "like" {
			...
			if !replaceOrigin {
				newFilter = plan.DeepCopyExpr(newFilter)
			...
			if replaceOrigin {
				node.BlockFilterList = append(node.BlockFilterList, plan.DeepCopyExpr(newFilter))
			}
		}
	}
	if len(newFilters) > 0 {
		node.FilterList = append(node.FilterList, newFilters...)
		node.BlockFilterList = append(node.BlockFilterList, plan.DeepCopyExprList(newFilters)...)
	}
}
