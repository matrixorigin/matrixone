...
func getPreparePlan(ctx CompilerContext, stmt tree.Statement) (*plan.Plan, error) {
...
func buildPrepare(stmt tree.Prepare, ctx CompilerContext) (*plan.Plan, error) {
...
func buildExecute(stmt *tree.Execute, ctx CompilerContext) (*plan.Plan, error) {
	builder := NewQueryBuilder(plan.Query_SELECT, ctx, false, false)
	binder := NewWhereBinder(builder, &BindContext{})

	args := make([]*plan.Expr, len(stmt.Variables))
...
func buildDeallocate(stmt *tree.Deallocate, _ CompilerContext) (*plan.Plan, error) {
...
func buildSetVariables(stmt *tree.SetVar, ctx CompilerContext) (*plan.Plan, error) {
	var err error
...
func buildCreateAccount(stmt *tree.CreateAccount, ctx CompilerContext, isPrepareStmt bool) (*plan.Plan, error) {
	params := []tree.Expr{
		stmt.Name,
...
func buildAlterAccount(stmt *tree.AlterAccount, ctx CompilerContext, isPrepareStmt bool) (*plan.Plan, error) {
	params := []tree.Expr{
		stmt.Name,
		stmt.AuthOption.AdminName,
		stmt.AuthOption.IdentifiedType.Str,
...
func buildDropAccount(stmt *tree.DropAccount, ctx CompilerContext, isPrepareStmt bool) (*plan.Plan, error) {
	params := []tree.Expr{
		stmt.Name,
	}
	paramTypes, err := getParamTypes(params, ctx, isPrepareStmt)
