...
func filterTag(expr *plan.Expr, tag int32) *plan.Expr {
	switch exprImpl := expr.Expr.(type) {
	case *plan.Expr_F:
		for _, arg := range exprImpl.F.Args {
...
func applyAggPushdown(agg, join, leftChild *plan.Node, builder *QueryBuilder) {
	leftChildTag := leftChild.BindingTags[0]
	newAggList := plan.DeepCopyExprList(agg.AggList)
	...
	//newGroupBy := plan.DeepCopyExprList(agg.GroupBy)
	newGroupBy := []*plan.Expr{plan.DeepCopyExpr(filterTag(join.OnList[0], leftChildTag))}
...
func getJoinCondCol(cond *plan.Expr, leftTag int32, rightTag int32) (*plan.Expr_Col, *plan.Expr_Col) {
...
func addAnyValue(expr *plan.Expr, agg *plan.Node, builder *QueryBuilder) {
	...
	if idx == -1 {
		idx = len(agg.AggList)
		anyValueExpr, _ := BindFuncExprImplByPlanExpr(builder.compCtx.GetContext(), "any_value", []*plan.Expr{plan.DeepCopyExpr(expr)})
