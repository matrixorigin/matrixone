...
func AdjustNDV(info *TableStatsInfo, tableDef *plan.TableDef, s *pb.StatsInfo) {
...
func isHighNdvCols(cols []int32, tableDef *plan.TableDef, builder *QueryBuilder) bool {
	if tableDef == nil {
		return false
	}
...
func (builder *QueryBuilder) getColNDVRatio(cols []int32, tableDef *plan.TableDef) float64 {
	if tableDef == nil {
		return 0
	}
	// first to check if it is primary key.
...
func ReCalcNodeStats(nodeID int32, builder *QueryBuilder, recursive bool, leafNode bool, needResetHashMapStats bool) {
	...
	var leftStats, rightStats, childStats *plan.Stats
	...
	if node.Limit != nil && node.NodeType != plan.Node_TABLE_SCAN {
		...
		if cExpr, ok := node.Limit.Expr.(*plan.Expr_Lit); ok {
			...
			limitExpr := plan.DeepCopyExpr(node.Limit)
		...
		if node.IndexReaderParam.Limit != nil {
			...
			if cExpr, ok := node.IndexReaderParam.Limit.Expr.(*plan.Expr_Lit); ok {
				...
				// Slow path: need to fold the expression
				limitExpr := plan.DeepCopyExpr(node.IndexReaderParam.Limit)
				if _, ok := limitExpr.Expr.(*plan.Expr_F); ok {
					if !hasParam(limitExpr) {
						limitExpr, _ = ConstantFold(batch.EmptyForConstFoldBatch, limitExpr, builder.compCtx.GetProcess(), true, true)
					}
...
func computeFunctionScan(name string, exprs []*plan.Expr, nodeStat *plan.Stats) bool {
...
func getCost(start *plan.Expr, end *plan.Expr, step *plan.Expr) (float64, bool) {
	var startNum, endNum, stepNum float64
	var flag1, flag2, flag3 bool
	getInt32Val := func(e *plan.Expr) (float64, bool) {
	...
	getInt64Val := func(e *plan.Expr) (float64, bool) {
		if s, ok := e.Expr.(*plan.Expr_Lit); ok {
...
func transposeTableScanFilters(proc *process.Process, qry *plan.Query, nodeId int32) {
...
func foldTableScanFilters(proc *process.Process, qry *plan.Query, nodeId int32, foldInExpr bool) {
	node := qry.Nodes[nodeId]
	if node.NodeType == plan.Node_TABLE_SCAN && len(node.FilterList) > 0 {
		for i, e := range node.FilterList {
		...
		if builder.optimizerHints != nil {
			if builder.optimizerHints.blockFilter == 1 { //always trying to pushdown blockfilters if zonemappable
				if ExprIsZonemappable(builder.GetContext(), node.FilterList[i]) {
					copyOfExpr := plan.DeepCopyExpr(node.FilterList[i])
...
func InternalTable(tableDef *plan.TableDef) bool {
...
func compareStats(stats1, stats2 *plan.Stats) bool {
