...
func handleAddColumnPosition(ctx context.Context, tableDef *plan.TableDef, newCol *plan.ColDef, pos *tree.ColumnPosition) error {
	if pos != nil && pos.Typ != tree.ColumnPositionNone {
		targetPos, err := findPositionRelativeColumn(ctx, tableDef.Cols, pos)
		if err != nil {
			return err
		}
		tableDef.Cols = append(tableDef.Cols[:targetPos], append([]*plan.ColDef{newCol}, tableDef.Cols[targetPos:]...)...)
	} else {
...
func buildAddColumnAndConstraint(ctx CompilerContext, alterPlan *plan.AlterTable, specNewColumn *tree.ColumnTableDef, colType plan.Type) (*plan.ColDef, error) {
	newColName := specNewColumn.Name.ColName()
	...
	for _, attr := range specNewColumn.Attributes {
		...
		case *tree.AttributePrimaryKey, *tree.AttributeKey:
			...
			if alterPlan.CopyTableDef.Pkey != nil && alterPlan.CopyTableDef.Pkey.PkeyColName != catalog.FakePrimaryKeyColName {
				...
				alterPlan.CopyTableDef.Pkey = &plan.PrimaryKeyDef{
...
func checkAddColumWithUniqueKey(ctx context.Context, tableDef *plan.TableDef, uniKey *tree.UniqueIndex) (*plan.IndexDef, error) {
...
func checkVisibleColumnCnt(ctx context.Context, tblInfo *plan.TableDef, addCount, dropCount int) error {
	visibleColumCnt := 0
...
func handleDropColumnWithIndex(ctx context.Context, colName string, tbInfo *plan.TableDef) error {
...
func handleDropColumnWithPrimaryKey(ctx context.Context, colName string, tbInfo *plan.TableDef) error {
...
func checkDropColumnWithForeignKey(ctx CompilerContext, tbInfo *plan.TableDef, targetCol *plan.ColDef) error {
...
// checkModifyNewColumn Check the position information of the newly formed column and place the new column in the target location
func handleDropColumnPosition(ctx context.Context, tableDef *plan.TableDef, col *plan.ColDef) error {
	tableDef.Cols = RemoveIf[*plan.ColDef](tableDef.Cols, func(t *plan.ColDef) bool {
...
func handleDropColumnWithClusterBy(ctx context.Context, copyTableDef *plan.TableDef, originCol *plan.ColDef) error {
