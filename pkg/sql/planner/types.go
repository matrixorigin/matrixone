...
type CompilerContext interface {
	...
	DatabaseExists(name string, snapshot *plan.Snapshot) bool
	// get table definition by database/schema
	Resolve(schemaName string, tableName string, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error)
	// get index table definition by an ObjectRef, will skip unnecessary subscription check
	ResolveIndexTableByRef(ref *plan.ObjectRef, tblName string, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error)
	// get table definition by table id
	ResolveById(tableId uint64, snapshot *plan.Snapshot) (*plan.ObjectRef, *plan.TableDef, error)
	...
	ResolveUdf(name string, args []*plan.Expr) (*function.Udf, error)
	// get needed info for stats by table, NOTE: Stats May indirectly access the file service
	Stats(obj *plan.ObjectRef, snapshot *plan.Snapshot) (*pb.StatsInfo, error)
	...
	GetDatabaseId(dbName string, snapshot *plan.Snapshot) (uint64, error)

	GetProcess() *process.Process

	GetQueryResultMeta(uuid string) ([]*plan.ColDef, string, error)
	...
	GetSubscriptionMeta(dbName string, snapshot *plan.Snapshot) (*plan.SubscriptionMeta, error)
	CheckSubscriptionValid(subName, accName string, pubName string) error
	SetQueryingSubscription(meta *plan.SubscriptionMeta)
	GetQueryingSubscription() *plan.SubscriptionMeta
	IsPublishing(dbName string) (bool, error)
	BuildTableDefByMoColumns(dbName, table string) (*plan.TableDef, error)
	ResolveSubscriptionTableById(tableId uint64, pubmeta *plan.SubscriptionMeta) (*plan.ObjectRef, *plan.TableDef, error)

	ResolveSnapshotWithSnapshotName(snapshotName string) (*plan.Snapshot, error)
	...
	GetSnapshot() *plan.Snapshot
	SetSnapshot(snapshot *plan.Snapshot)
	GetViews() []string
...
type Optimizer interface {
	Optimize(stmt tree.Statement) (*plan.Query, error)
	CurrentContext() CompilerContext
}

type Rule interface {
	Match(*plan.Node) bool                           // rule match?
	Apply(*plan.Node, *plan.Query, *process.Process) // apply the rule
}
