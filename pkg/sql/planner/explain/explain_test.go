...
func TestBasicSqlExplain(t *testing.T) {
	...
	mockOptimizer := planner.NewMockOptimizer(false)
...
func TestDMLUpdate(t *testing.T) {
	...
	mockOptimizer := planner.NewMockOptimizer(true)
	runTestShouldPass(mockOptimizer, t, sqls)
}
...
func runTestShouldPass(opt planner.Optimizer, t *testing.T, sqls []string) {
...
func runOneStmt(opt planner.Optimizer, t *testing.T, sql string) error {
	t.Logf("SQL: %v\n", sql)
	...
	if stmt, ok := stmts[0].(*tree.ExplainStmt); ok {
		...
		logicPlan, err := planner.BuildPlan(ctx, stmt.Statement, false)
		if err != nil {
