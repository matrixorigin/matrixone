...
func TestGetLabelOrTitle(t *testing.T) {
	m := MarshalNodeImpl{}
	errCount := 0
	for _, v := range plan.Node_NodeType_value {
		node := &plan.Node{
			NodeType: plan.Node_NodeType(v),
			NodeId:   0,
...
func TestDMLToJson(t *testing.T) {
	...
	mock := planner.NewMockOptimizer(true)
	buildPlanMarshalTest(mock, t, sqls)
}
...
func buildPlanMarshalTest(opt planner.Optimizer, t *testing.T, sqls []string) {
	...
	for _, sql := range sqls {
		t.Logf("sql: %s \n", sql)
		pl, err := runSingleSql(opt, t, sql)
		if err != nil {
			t.Fatalf("%+v", err)
		}
		queryPlan := pl.GetQuery()
		for _, node := range queryPlan.Nodes {
			info := plan.AnalyzeInfo{
				InputRows:    12,
				OutputRows:   12,
				TimeConsumed: 5,
				InputSize:    24,
...
func runSingleSql(opt planner.Optimizer, t *testing.T, sql string) (*plan.Plan, error) {
