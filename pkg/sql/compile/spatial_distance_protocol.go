...
// on one CN while a rolling cluster still contains workers below MORPC v80.
...
func validateSpatialDistanceDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	supported, err := remoteWorkersSupportProtocol(proc,
		engine.Nodes{{Id: p.Node.Id, Addr: p.Node.Addr}}, defines.MORPCVersion80)
	if err != nil {
		return err
	}
	if !supported {
		return moerr.NewNotSupportedNoCtx(
			"remote destination does not support geodetic spatial-distance semantics (MORPC version 80)",
		)
	}
	return nil
}
