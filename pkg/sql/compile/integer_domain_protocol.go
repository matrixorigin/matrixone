...
func (c *Compile) constrainIntegerDomainWorkers(qry *plan.Query) error {
	...
	supported, err := remoteWorkersSupportProtocol(c.proc, c.cnList, defines.MORPCVersion69)
	if err != nil {
		return err
	}
	if supported {
...
func validateIntegerDomainDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	supported, err := remoteWorkersSupportProtocol(proc, engine.Nodes{{Id: p.Node.Id, Addr: p.Node.Addr}}, defines.MORPCVersion69)
	if err != nil {
		return err
	}
	if !supported {
		return moerr.NewNotSupportedNoCtx("remote destination does not support checked integer arithmetic (MORPC version 69)")
	}
	return nil
}
