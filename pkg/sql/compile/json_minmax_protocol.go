...
func (c *Compile) constrainJSONMinMaxWorkers(qry *plan.Query) error {
	...
	supported, err := remoteWorkersSupportProtocol(c.proc, c.cnList, defines.MORPCVersion75)
	if err != nil {
		return err
	}
	if supported {
...
func supportsRemoteJSONMinMax(service string) bool {
	...
	return ok && valid && protocolVersion >= defines.MORPCVersion75
}
...
func validateJSONMinMaxDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	supported, err := remoteWorkersSupportProtocol(proc,
		engine.Nodes{{Id: p.Node.Id, Addr: p.Node.Addr}}, defines.MORPCVersion75)
	if err != nil {
		return err
	}
	if !supported {
