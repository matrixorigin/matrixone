...
func (c *Compile) constrainIntegerArgumentWorkers(qry *plan.Query) error {
	...
	supported, err := remoteWorkersSupportProtocol(c.proc, c.cnList, defines.MORPCVersion76)
	if err != nil {
		return err
	}
	if supported {
...
func validateIntegerArgumentDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	if !supported {
		return moerr.NewNotSupportedNoCtx("remote destination does not support integer parameter coercion (MORPC version 76)")
	}
	return nil
}
