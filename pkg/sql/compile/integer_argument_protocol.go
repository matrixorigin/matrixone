...
func validateIntegerArgumentDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	supported, err := remoteWorkersSupportProtocol(proc, engine.Nodes{{Id: p.Node.Id, Addr: p.Node.Addr}}, defines.MORPCVersion84)
	if err != nil {
		return err
	}
	if !supported {
		return moerr.NewNotSupportedNoCtx("remote destination does not support integer parameter coercion (MORPC version 84)")
	}
	return nil
}
