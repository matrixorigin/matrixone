...
func TestIntegerArgumentProtocolConsumerIdentities(t *testing.T) {
	c, _ := expressionProtocolTestCompile(t)
	rt := moruntime.ServiceRuntime(c.proc.GetService())
	require.Equal(t, int32(202), int32(function.MAKEDATE))
	require.Equal(t, int32(262), int32(function.FORMAT))
	require.Equal(t, int32(373), int32(function.MAKETIME))
	for _, tc := range []struct {
		functionID int32
		lastLegacy int32
		lastNew    int32
	}{
		{function.MAKEDATE, 0, function.IntegerMakeDateOverload},
		{function.FORMAT, 1, function.IntegerFormat3Overload},
		{function.MAKETIME, 35, 38},
	} {
		for id := int32(0); id <= tc.lastNew; id++ {
			// No CAST is present: the consumer identity itself requires v76.
			p := remoteIPProtocolPipeline(tc.functionID, id)
			features, err := planpb.RequiredRemoteExpressionFeatures(p)
			require.NoError(t, err)
			require.Equal(t, id > tc.lastLegacy, features.IntegerParameterCoercion)
			rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion75)
			err = validateRemoteExpressionPipelineProtocol(c.proc, p)
			if id <= tc.lastLegacy {
				require.NoError(t, err)
