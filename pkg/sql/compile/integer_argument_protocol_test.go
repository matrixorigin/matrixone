...
func TestIntegerArgumentProtocolBoundaries(t *testing.T) {
	...
	for id := int32(0); id <= function.TemporalIntegerArgumentCastOverload; id++ {
		t.Run(fmt.Sprint(id), func(t *testing.T) {
			...
			require.ErrorContains(t, err, "version 81")
			require.ErrorContains(t, validateRemoteExpressionPipelineProtocol(nil, p), "version 81")
			data, err := p.Marshal()
			require.NoError(t, err)
			_, err = decodeScope(data, c.proc, true, nil)
			require.ErrorContains(t, err, "version 81")
			table := &planpb.TableDef{Cols: []*planpb.ColDef{{Default: &planpb.Default{Expr: expr}}}}
			require.ErrorContains(t, plan2.RequirePersistedExpressionProtocol(c.proc.Ctx, c.proc, table), "version 81")
			// Existing publication hooks must cover the newly shared feature too.
			require.ErrorContains(t, plan2.RequirePersistedIPFunctionProtocol(c.proc.Ctx, c.proc, table), "version 81")
			rt.SetGlobalVariables(moruntime.MOProtocolVersion, nil)
			require.ErrorContains(t, plan2.RequirePersistedExpressionProtocol(nil, c.proc, expr), "version 81")
