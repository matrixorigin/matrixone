...
func TestRemoteSpatialDistanceProtocolValidation(t *testing.T) {
	...
	t.Cleanup(func() {
		if hadPrevious {
			...
			for _, value := range []int64{defines.MORPCVersion89, defines.MORPCVersion90} {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.fenced {
				require.ErrorContains(t, err, "geodetic spatial-distance semantics require MORPC protocol version 90")
