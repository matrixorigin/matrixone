...
func (c *Compile) canRetry(err error) bool {
	if !c.disableRetry && c.isRetryErr(err) {
		return true
	} else if !c.disableRetry && c.proc.AutoPkGenByIncrService && moerr.IsMoErrCode(err, moerr.ErrDuplicateEntry) {
		logutil.Debugf("duplicate entry reported when insert auto pk no value specified(i.e. values generated by increment service)")
		logutil.Debugf("This may happen when the value of CN1's incrservice-cache interval is manually inserted on CN2 (but CN1's cache is not updated), plz refer to issue16493")
		//TODO : maybe can report a warming to user
		return true
	}
	return false
