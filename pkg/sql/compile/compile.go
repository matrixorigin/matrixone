...
func (c *Compile) compileJoin(node, left, right *plan.Node, probeScopes, buildScopes []*Scope) []*Scope {
	if node.Stats.HashmapStats.Shuffle {
		...
		if hasLocalDependency {
			return c.compileShuffleJoinAt(
				node, left, right, probeScopes, buildScopes, stageNodes, true)
		}
		if len(stageNodes) == 1 {
...
func (c *Compile) compileShuffleJoin(node, left, right *plan.Node, lefts, rights []*Scope) []*Scope {
	return c.compileShuffleJoinAt(
		node, left, right, lefts, rights, c.queryWorkerStageNodes(), false)
}

func (c *Compile) compileShuffleJoinAt(
	node, left, right *plan.Node,
	lefts, rights []*Scope,
	stageNodes engine.Nodes,
	attachRemoteSources bool,
) []*Scope {
	shuffleJoins := c.newShuffleJoinScopeListAt(
		lefts, rights, node, stageNodes, attachRemoteSources)
