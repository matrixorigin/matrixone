	...
	if n.TableDef.Partition != nil {
		...
		for _, partitionItem := range partitionDef.Partitions {
			...
			if partialResults != nil {
				if allPartialResults == nil {
					allPartialResults = partialResults
					allPartialResultTypes = partialResultTypes
				} else {
...
func (c *Compile) evalPartialResults(n *plan.Node, ranges engine.Ranges) ([]byte, []any, []types.T, error) {
	...
	return newranges, partialResults, partialResultTypes, nil
}

func mergePartialResults(n *plan.Node, allResult, result []any, allTypes []types.T) {
	for i := range n.AggList {
		agg := n.AggList[i].Expr.(*plan.Expr_F)
		name := agg.F.Func.ObjName
		switch name {
		case "starcount", "count":
			allResult[i] = allResult[i].(int64) + result[i].(int64)
		case "min":
			switch allTypes[i] {
			case types.T_bool:
				allResult[i] = allResult[i].(bool) && result[i].(bool)
			case types.T_bit:
				if allResult[i].(uint64) > result[i].(uint64) {
					allResult[i] = result[i]
				}
			case types.T_int8:
				if allResult[i].(int8) > result[i].(int8) {
					allResult[i] = result[i]
				}
			case types.T_int16:
				if allResult[i].(int16) > result[i].(int16) {
					allResult[i] = result[i]
				}
			case types.T_int32:
				if allResult[i].(int32) > result[i].(int32) {
					allResult[i] = result[i]
				}
			case types.T_int64:
				if allResult[i].(int64) > result[i].(int64) {
					allResult[i] = result[i]
				}
			case types.T_uint8:
				if allResult[i].(uint8) > result[i].(uint8) {
					allResult[i] = result[i]
				}
			case types.T_uint16:
				if allResult[i].(uint16) > result[i].(uint16) {
					allResult[i] = result[i]
				}
			case types.T_uint32:
				if allResult[i].(uint32) > result[i].(uint32) {
					allResult[i] = result[i]
				}
			case types.T_uint64:
				if allResult[i].(uint64) > result[i].(uint64) {
			...
			case types.T_float32:
				if allResult[i].(float32) > result[i].(float32) {
					allResult[i] = result[i]
				}
			case types.T_float64:
				if allResult[i].(float64) > result[i].(float64) {
					allResult[i] = result[i]
				}
			case types.T_date:
				if allResult[i].(types.Date) > result[i].(types.Date) {
					allResult[i] = result[i]
				}
			case types.T_time:
				if allResult[i].(types.Time) > result[i].(types.Time) {
					allResult[i] = result[i]
				}
			case types.T_datetime:
				if allResult[i].(types.Datetime) > result[i].(types.Datetime) {
					allResult[i] = result[i]
				}
			case types.T_timestamp:
				if allResult[i].(types.Timestamp) > result[i].(types.Timestamp) {
					allResult[i] = result[i]
				}
			case types.T_enum:
				if allResult[i].(types.Enum) > result[i].(types.Enum) {
					allResult[i] = result[i]
				}
			case types.T_decimal64:
				if allResult[i].(types.Decimal64) > result[i].(types.Decimal64) {
					allResult[i] = result[i]
				}
			case types.T_decimal128:
				if allResult[i].(types.Decimal128).Compare(result[i].(types.Decimal128)) > 0 {
					allResult[i] = result[i]
				}
			case types.T_uuid:
				if allResult[i].(types.Uuid).Gt(result[i].(types.Uuid)) {
					allResult[i] = result[i]
				}
			case types.T_TS:
				if allResult[i].(types.TS).Comp(result[i].(types.TS)) > 0 {
					allResult[i] = result[i]
				}
			case types.T_Rowid:
				if allResult[i].(types.Rowid).Great(result[i].(types.Rowid)) {
					allResult[i] = result[i]
				}
			case types.T_Blockid:
				if allResult[i].(types.Blockid).Great(result[i].(types.Blockid)) {
					allResult[i] = result[i]
				}
			}
		case "max":
			...
			case types.T_uuid:
				if allResult[i].(types.Uuid).Lt(result[i].(types.Uuid)) {
					allResult[i] = result[i]
				}
			case types.T_TS:
				if allResult[i].(types.TS).Comp(result[i].(types.TS)) < 0 {
			...
			case types.T_Rowid:
				if allResult[i].(types.Rowid).Less(result[i].(types.Rowid)) {
					allResult[i] = result[i]
				}
			case types.T_Blockid:
				if allResult[i].(types.Blockid).Less(result[i].(types.Blockid)) {
					allResult[i] = result[i]
				}
			}
		}
