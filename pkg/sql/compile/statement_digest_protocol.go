...
// older worker cannot construct the STATEMENT_DIGEST function (ID 579).
func validateStatementDigestDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	supported, err := remoteWorkersSupportProtocol(proc,
		engine.Nodes{{Id: p.Node.Id, Addr: p.Node.Addr}}, defines.MORPCVersion73)
	if err != nil {
		return err
	}
	if !supported {
		return moerr.NewNotSupportedNoCtx(
			"remote destination does not support STATEMENT_DIGEST (MORPC version 73)",
		)
	}
	return nil
}
