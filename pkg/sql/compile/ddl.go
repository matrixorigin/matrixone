...
func (s *Scope) alterTableInplace(c *Compile, cleanup *alterAutoIncrementResetCleanup) error {
	...
	if err := plan2.RequirePersistedExpressionProtocol(c.proc.Ctx, c.proc, targetTableDef); err != nil {
		return err
	}
...
func (s *Scope) createTable(c *Compile, tableCreated func()) error {
	...
	if err := plan2.RequirePersistedExpressionProtocol(c.proc.Ctx, c.proc, qry.GetTableDef()); err != nil {
		return err
	}
	for _, indexTableDef := range qry.GetIndexTables() {
		if err := plan2.RequirePersistedExpressionProtocol(c.proc.Ctx, c.proc, indexTableDef); err != nil {
			return err
		}
	}
	dbName := c.db
