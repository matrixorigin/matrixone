...
func validatePreparedPrecisionDestination(proc *process.Process, p *pipeline.Pipeline) error {
	if p == nil || p.Node == nil {
		return moerr.NewNotSupportedNoCtx("prepared scalar precision requires a versioned remote destination")
	}
	...
	if !supported {
		return moerr.NewNotSupportedNoCtx("remote destination does not support prepared scalar precision (MORPC version 95)")
	}
	return nil
}
