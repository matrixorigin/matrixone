...
func TestHexProtocolPlanAdmissionAndCachedRun(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.ErrorContains(t, c.Compile(context.Background(), pn, nil), "protocol version 73")
			_, err := c.Run(0)
			require.ErrorContains(t, err, "protocol version 73")
		})
	}

	for _, version := range []any{defines.MORPCVersion72, defines.MORPCVersion68, defines.MORPCVersion67, defines.MORPCVersion66,
		defines.MORPCVersion65, defines.MORPCVersion63, int64(62), nil, "68"} {
		rt.SetGlobalVariables(moruntime.MOProtocolVersion, version)
		require.ErrorContains(t, validateHexMySQLNumericProtocol(proc, expr), "protocol version 73")
	...
	require.ErrorContains(t, validateHexMySQLNumericProtocol(nil, expr), "protocol version 73")
	rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion73)
...
func TestHexMigratesLegacyPersistedExpressionsAtVersion73(t *testing.T) {
	proc := testutil.NewProcess(t)
	rt := moruntime.ServiceRuntime(proc.GetService())
	defer rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCLatestVersion)
	...
	for _, version := range []int64{defines.MORPCVersion63, defines.MORPCVersion68, defines.MORPCVersion72} {
		rt.SetGlobalVariables(moruntime.MOProtocolVersion, version)
	...
	require.Equal(t, int32(0), hexExprOverload(implicitBool), "shared input must remain legacy")
	migratedBool := boolTable.Checks[0].Check
	require.Equal(t, int32(2), hexExprOverload(migratedBool))
	require.Equal(t, types.T_int64, types.T(migratedBool.GetF().GetArgs()[0].Typ.Id))
	require.Len(t, migratedBool.GetF().GetArgs()[0].GetF().GetArgs(), 2)
...
func TestHexRemoteProtocolSenderAndReceiver(t *testing.T) {
	...
	rt := moruntime.ServiceRuntime(proc.GetService())
	defer rt.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCLatestVersion)
	for _, id := range []int32{5, 8, 9, 10, 11, 12, 13, 14} {
		...
		if id < function.HexMySQLNumericOverloadStart {
			...
			require.ErrorContains(t, sendErr, "protocol version 73")
			require.ErrorContains(t, localEncodeErr, "protocol version 73")
		}
		...
		if id < function.HexMySQLNumericOverloadStart {
			...
			require.ErrorContains(t, receiveErr, "protocol version 73")
			require.ErrorContains(t, childErr, "protocol version 73")
		}
	}
}
