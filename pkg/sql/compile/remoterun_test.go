...
func TestRemoteExpressionProtocolValidation(t *testing.T) {
	...
	t.Run("statement digest function requires v73", func(t *testing.T) {
		remotePipeline := &pipeline.Pipeline{
			InstructionList: []*pipeline.Instruction{{
	...
	t.Run("statement digest resolver errors before dispatch", func(t *testing.T) {
		// The digest destination gate probes the actual worker before process
		// encoding.  Use the protocol fixture here so this test reaches the
		// resolver path it is intended to exercise instead of being rejected
		// for an unversioned/empty destination.
		...
		resolverErr := moerr.NewInternalErrorNoCtx("max digest resolver failed")
		resolverProc.SetResolveVariableFunc(func(name string, _, _ bool) (interface{}, error) {
		...
		resolverRT.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion73)
		...
		ordinaryScope := makeScope(ordinary)
	...
	t.Run("statement digest sql mode resolver errors before dispatch", func(t *testing.T) {
		...
		resolverProc.Base.SessionInfo.ExplicitZeroTemporalCastReturnsNull = true
		resolverProc.SetResolveVariableFunc(func(name string, _, _ bool) (interface{}, error) {
			switch name {
			case "sql_mode":
				return nil, resolverErr
