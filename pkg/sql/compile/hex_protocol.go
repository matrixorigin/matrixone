...
// admission, cached execution, and remote pipeline boundaries. Legacy catalog
// expressions are migrated before planning by MigrateLegacyHexTableDef.
func validateHexMySQLNumericProtocol(proc *process.Process, value any) error {
	if proc != nil {
		if rt := moruntime.ServiceRuntime(proc.GetService()); rt != nil {
			...
			if ok && valid && version >= defines.MORPCVersion70 {
				return nil
			}
		}
	}
