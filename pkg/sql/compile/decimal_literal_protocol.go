...
// while a rolling cluster still contains workers below MORPC v87. The marker
...
func validateDecimalLiteralDestination(proc *process.Process, p *pipeline.Pipeline) error {
	...
	if !supported {
		return moerr.NewNotSupportedNoCtx(
			"remote destination does not support exact DECIMAL256 literal semantics (MORPC protocol version 87)",
		)
	}
	return nil
}
