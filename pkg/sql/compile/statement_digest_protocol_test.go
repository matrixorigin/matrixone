...
func TestStatementHashDestinationProtocolValidation(t *testing.T) {
	...
	for _, version := range []int64{
		...
		defines.MORPCVersion85, // Integer-parameter support alone is below this feature's v94 gate.
		defines.MORPCVersion86, // Expression-result contracts alone are below the v94 hash gate.
		defines.MORPCVersion87, // Grouping provenance alone is below the v94 hash gate.
...
func TestStatementHashDestinationRejectsUnknownOrFailedProbe(t *testing.T) {
	...
	// A coordinator below v94 is rejected by the coordinator gate before any
...
func TestStatementHashReceiverRejectsProbeThenDowngrade(t *testing.T) {
	c, client := expressionProtocolTestCompile(t)
	oldBuildCommitID := version.BuildCommitID
	version.BuildCommitID = strings.Repeat("a", 40)
	client.version = defines.MORPCVersion94
	...
	require.NoError(t, err)
	require.NotEmpty(t, data)
	require.Positive(t, client.calls, "sender probe must succeed before the receiver is downgraded")
...
func TestStatementHashReceiverRejectsBuildMismatchBeforeScopeExecution(t *testing.T) {
	...
	// A worker can be replaced after the sender probe. The receiver must reject
	// the scope before constructing operators, even if no later batch has an
	// active row to invoke MO_STATEMENT_HASH.
	remoteSession, err := process.ConvertToProcessSessionInfo(pipeline.SessionInfo{
		StatementHashExpectedBuildCommitId: originBuild,
	})
	require.NoError(t, err)
