...
func validateDecimalDivisionDestination(proc *process.Process, p *pipeline.Pipeline) error {
	if p == nil || p.Node == nil {
		return moerr.NewNotSupportedNoCtx("decimal division requires a versioned remote destination")
	}
	...
	if !supported {
		return moerr.NewNotSupportedNoCtx("remote destination does not support decimal division (MORPC version 97)")
	}
	return nil
}
