...
func TestConvBasesPlacementAndActualSender(t *testing.T) {
	...
	for _, version := range []int64{defines.MORPCVersion64, defines.MORPCVersion65} {
		client.version = version
		c.execType = plan2.ExecTypeAP_MULTICN
		c.cnList = engine.Nodes{{Id: "old-worker", Addr: "remote:6001", Mcpu: 4}}
		require.NoError(t, c.constrainConvBasesWorkers(qry))
		data, err := encodeRemoteScope(scope, c.proc)
		if version < defines.MORPCVersion65 {
			...
			require.ErrorContains(t, validateRemoteExpressionPipelineProtocol(c.proc, wire), "version 65")
