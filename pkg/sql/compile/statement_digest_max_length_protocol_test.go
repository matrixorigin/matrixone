...
// TestRemoteStatementDigestTextRejectsMaxDigestLengthResolutionFailure guards the
// sender-side protocol gate. A remote digest-text pipeline must not be dispatched
// with a default token budget when the initiating statement cannot resolve its
// max_digest_length setting.
func TestRemoteStatementDigestTextRejectsMaxDigestLengthResolutionFailure(t *testing.T) {
