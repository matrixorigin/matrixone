...
func pipelineMessageHandle(ctx context.Context, message morpc.Message, cs morpc.ClientSession, messageHelper *messageHandleHelper, cli client.TxnClient) (anaData []byte, err error) {
	...
	// it's a wrapBatch
	if m.GetCmd() == 1 {
		wrapBat := batch.WrapperBatch{}
		if err := wrapBat.UnmarshalBinary(m.Data); err != nil {
			return nil, err
		}
		v, ok := srv.batchMp.Load(wrapBat.Uuid)
		if !ok {
			// we need to deal with a situation that the
			// Chan isn't initialized by the spec scope,
			// so we need to produce one first, and then
			// when the spec scope is generated by CN, we
			// should use the uuid info of this scope to New
			// its process's regs
			srv.batchMp.Store(wrapBat.Uuid, &process.WaitRegister{
				Ctx: nil,
				Ch:  make(chan *batch.Batch, 1),
			})
			v, _ = srv.batchMp.Load(wrapBat.Uuid)
		}
...
func generateScope(proc *process.Process, p *pipeline.Pipeline, ctx *scopeContext,
	...
	for i := range s.uuids {
		...
		v, ok := srv.batchMp.Load(s.uuids[i])
		if !ok {
			srv.batchMp.Store(s.uuids[i], s.Proc.Reg.MergeReceivers[i])
		} else {
			wg := v.(*process.WaitRegister)
			wg.Ctx = s.Proc.Ctx
			srv.batchMp.Store(s.uuids[i], wg)
		}
	}
	{
		for i := range s.Proc.Reg.MergeReceivers {
