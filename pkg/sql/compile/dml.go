...
func (s *Scope) InsertValues(c *Compile, stmt *tree.Insert) (uint64, error) {
	...
	However, case 3 will be a little more subtle because we need to distinguish between the following two insert SQL statements. Consider below table.
	create table t1(
	...
	The bat generated by the above two SQL is no longer distinguishable, so case 3 should be done in build plan stage
	*/
	for i := range bat.Vecs {
		// check for case 1 and case 2
		if (p.ExplicitCols[i].Primary && !p.ExplicitCols[i].AutoIncrement) || (!p.ExplicitCols[i].Default.NullAbility && !p.ExplicitCols[i].AutoIncrement) {
