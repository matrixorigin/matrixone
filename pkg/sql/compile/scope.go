...
/*
func (s *Scope) releaseAllOp(op vm.Operator) {
	numChildren := op.GetOperatorBase().NumChildren()
	if numChildren == 0 {
		return
	}
	for i := 0; i < numChildren; i++ {
		s.releaseAllOp(op.GetOperatorBase().GetChildren(i))
		op.GetOperatorBase().GetChildren(i).Release()
		op.GetOperatorBase().SetChild(nil, i)
	}
} */
...
func (s *Scope) ParallelRun(c *Compile) (err error) {
	...
	defer func() {
		...
		if parallelScope == nil {
			pipeline.NewMerge(s.RootOp, s.Reg).Cleanup(s.Proc, true, err)
		}
	}()
