...
func TestCreateSQLEnvelopeRoundTripAndRejectsParallelMVP(t *testing.T) {
	...
	mapping := TableMapping{
		Connection: "nesr production",
		Database:   "telemetry",
		Collection: "pump/minute",
		Columns:    []ColumnMapping{{Name: "pump", Path: "meta.pump", TypeID: int32(types.T_varchar)}},
	}
...
func TestParseTableMappingSpecIgnoresInternalHiddenColumns(t *testing.T) {
	...
	tableDef := &planpb.TableDef{Cols: []*planpb.ColDef{
		{Name: "value", Typ: planpb.Type{Id: int32(types.T_float64)}},
	...
	require.Equal(t, []ColumnMapping{{
		Name: "value", Path: "reading.value", TypeID: int32(types.T_float64), Conversion: ConversionStrict,
...
func TestConverterTypesMissingTryNullAndRollback(t *testing.T) {
	...
	columns := []ColumnMapping{
		...
		{Name: "pump", Path: "meta.pump", TypeID: int32(types.T_varchar), Width: 32},
	...
	raw, err := bson.Marshal(bson.D{
		...
		{Key: "meta", Value: bson.D{{Key: "pump", Value: "P-01"}}},
	...
	require.Equal(t, "P-01", string(bat.Vecs[1].GetBytesAt(0)))
	...
	badRaw, err := bson.Marshal(bson.D{
		{Key: "_id", Value: objectID},
		{Key: "meta", Value: bson.D{{Key: "pump", Value: "P-02"}}},
