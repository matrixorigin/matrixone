	...
	if literal != nil && !literal.Isnull {
		if value, ok := literal.GetValue().(*plan.Literal_I64Val); ok {
			...
			case types.T_timestamp:
				...
				// Timestamp and Datetime share the internal epoch encoding here,
				// so compare against the corresponding zero-datetime value.
				if timestamp == types.Timestamp(types.ZeroDatetime) {
					return 0, false
				}
