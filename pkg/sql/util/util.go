...
func BuildMoDataBaseFilter(curAccountId uint64) tree.Expr {
	...
	mo_catalogConst := tree.NewNumValWithType2(constant.MakeString(catalog.MO_CATALOG), catalog.MO_CATALOG, catalog.MO_CATALOG, false, tree.P_char)
...
func BuildMoTablesFilter(curAccountId uint64) tree.Expr {
	...
	mo_databaseConst := tree.NewNumValWithType2(constant.MakeString(catalog.MO_DATABASE), catalog.MO_DATABASE, catalog.MO_DATABASE, false, tree.P_char)
	mo_tablesConst := tree.NewNumValWithType2(constant.MakeString(catalog.MO_TABLES), catalog.MO_TABLES, catalog.MO_TABLES, false, tree.P_char)
	mo_columnsConst := tree.NewNumValWithType2(constant.MakeString(catalog.MO_COLUMNS), catalog.MO_COLUMNS, catalog.MO_COLUMNS, false, tree.P_char)
...
func BuildMoColumnsFilter(curAccountId uint64) tree.Expr {
	...
	for _, table := range predefinedTables {
		...
		exprs = append(exprs, tree.NewNumValWithType2(constant.MakeString(table), table, table, false, tree.P_char))
	}
...
func makeStringEqualAst(lColName, rValue string) tree.Expr {
	...
	clusterConst := tree.NewNumValWithType2(constant.MakeString(rValue), rValue, rValue, false, tree.P_char)
...
func makeAccountIdEqualAst(curAccountId uint64) tree.Expr {
	...
	curAccountIdConst := tree.NewNumValWithType2(constant.MakeUint64(curAccountId), curAccountId, strconv.Itoa(int(curAccountId)), false, tree.P_uint64)
