...
// rtrim function's evaluation for arguments: [varchar]
func FdsRTrimVarchar(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
...
// rtrim function's evaluation for arguments: [char]
func FdsRTrimChar(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	origVecCol := vs[0].Col.(*types.Bytes)
