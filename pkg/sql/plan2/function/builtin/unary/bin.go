...
// bin function's evaluation for arguments: [int64]
func FdsBinInt64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	bytesNeed, colLen := calcBytesNeed(vs[0].Col)
	...
	retVec.Col = results
	nulls.Set(retVec.Nsp, vs[0].Nsp)
	vector.SetCol(retVec, toBinary(vs[0].Col, results))
	return retVec, nil
}

// bin function's evaluation for arguments: [uint64]
func FdsBinUInt64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	bytesNeed, colLen := calcBytesNeed(vs[0].Col)
	retVec, err := process.Get(proc, bytesNeed, types.Type{Oid: types.T_varchar, Size: 24})
	if err != nil {
		return nil, err
...
// bin function's evaluation for arguments: [float64]
func FdsBinFloat64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	bytesNeed, colLen := calcBytesNeed(vs[0].Col)
	retVec, err := process.Get(proc, bytesNeed, types.Type{Oid: types.T_varchar, Size: 24})
	if err != nil {
		return nil, err
