...
func AbsUInt64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if origVec.IsConst {
		resultVector := vector.New(types.Type{Oid: types.T_uint64, Size: 8})
		...
		resultVector, err := process.Get2(proc, 8*int64(len(origVecCol)), types.Type{Oid: types.T_uint64, Size: 8})
		if err != nil {
			return nil, err
		}
...
func AbsInt64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if origVec.IsConst {
		resultVector := vector.New(types.Type{Oid: types.T_int64, Size: 8})
		...
		resultVector, err := process.Get2(proc, 8*int64(len(origVecCol)), types.Type{Oid: types.T_int64, Size: 8})
		if err != nil {
			return nil, err
		}
...
func AbsFloat64(vs []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if origVec.IsConst {
		resultVector := vector.New(types.Type{Oid: types.T_float64, Size: 8})
		...
		resultVector, err := process.Get2(proc, 8*int64(len(origVecCol)), types.Type{Oid: types.T_float64, Size: 8})
		if err != nil {
			return nil, err
		}
