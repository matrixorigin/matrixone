...
func TestBindSingleSql(t *testing.T) {
	...
	logicPlan, err := runOneBindStmt(mock, t, sql)
	if err != nil {
		t.Fatalf("%+v", err)
	}
	outPutPlan2(logicPlan, true, t)
}

//Test Query Node Tree
func TestBindNodeTree(t *testing.T) {
...
func TestBindSingleTableSqlBuilder(t *testing.T) {
	mock := NewMockOptimizer()
	...
	runBindTestShouldPass(mock, t, sqls, false, false)
	...
	runBindTestShouldError(mock, t, sqls)
}

//test jion table plan building
func TestBindJoinTableSqlBuilder(t *testing.T) {
	mock := NewMockOptimizer()
...
func TestBindDerivedTableSqlBuilder(t *testing.T) {
	mock := NewMockOptimizer()
	// should pass
	sqls := []string{
...
func TestBindResultColumns(t *testing.T) {
	mock := NewMockOptimizer()
...
func outPutPlan2(logicPlan *Plan, toFile bool, t *testing.T) {
	var json []byte
	switch logicPlan.Plan.(type) {
	case *plan.Plan_Query:
		json = getJsonBytes(logicPlan.GetQuery())
	case *plan.Plan_Tcl:
		json = getJsonBytes(logicPlan.GetTcl())
	case *plan.Plan_Ddl:
		json = getJsonBytes(logicPlan.GetDdl())
	}
	if toFile {
...
func getNewQueryBuilder(typ plan.Query_StatementType, compCtx CompilerContext) *QueryBuilder {
	return &QueryBuilder{
		qry: &Query{
			StmtType: typ,
		},
...
func buildPlanByBinder(ctx CompilerContext, stmt tree.Statement) (*Plan, error) {
	switch stmt := stmt.(type) {
	case *tree.Select:
		builder := getNewQueryBuilder(plan.Query_SELECT, ctx)
...
func runOneBindStmt(opt Optimizer, t *testing.T, sql string) (*Plan, error) {
	stmts, err := mysql.Parse(sql)
	if err != nil {
		t.Fatalf("%+v", err)
	}
	...
	return buildPlanByBinder(ctx, stmts[0])
}

func runBindTestShouldPass(opt Optimizer, t *testing.T, sqls []string, printJson bool, toFile bool) {
	for _, sql := range sqls {
		logicPlan, err := runOneBindStmt(opt, t, sql)
		if err != nil {
			t.Fatalf("%+v, sql=%v", err, sql)
		}
		if printJson {
			outPutPlan2(logicPlan, toFile, t)
		}
	}
}

func runBindTestShouldError(opt Optimizer, t *testing.T, sqls []string) {
	for _, sql := range sqls {
		_, err := runOneBindStmt(opt, t, sql)
		if err == nil {
