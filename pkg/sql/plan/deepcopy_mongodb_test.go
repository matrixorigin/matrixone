...
func TestMongoScanDeepCopyAndCredentialFreeProto(t *testing.T) {
	original := &pb.Node{ExternScan: &pb.ExternScan{
		...
		MongodbScan: &pb.MongoScan{
			...
			Database: "telemetry", Collection: "raw", ProjectedPaths: []string{"meta.pump"},
			Columns:         []*pb.MongoColumnMapping{{Name: "pump", Path: "meta.pump"}},
			PushedPredicate: &pb.MongoPredicate{Op: pb.MongoPredicateOp_MONGO_PREDICATE_EQUAL, Path: "meta.pump", ValueBson: []byte{1, 2, 3}},
		},
	}}
	...
	require.Equal(t, "meta.pump", original.ExternScan.MongodbScan.Columns[0].Path)
	require.Equal(t, "meta.pump", original.ExternScan.MongodbScan.ProjectedPaths[0])
