...
func TestIvfScanWorkUnknownAndBounds(t *testing.T) {
	...
	for _, stats := range []*statsinfo.StatsInfo{
		...
		{TableCnt: 1, BlockNumber: 4, AccurateObjectNumber: -1},
	} {
...
func TestVectorLocalDOPHint(t *testing.T) {
	for _, tc := range []struct {
		hint    string
		enabled bool
	}{
		{"", false}, {"vectorLocalDOP=0", false}, {"vectorLocalDOP=1", true},
		{"vectorLocalDOP=2", false}, {"vectorLocalDOP=-1", false},
		{" vectorLocalDOP=1", false}, {"vectorLocalDOP=bad", false},
		{"vectorLocalDOP=1,vectorLocalDOP=0", false},
		{"vectorLocalDOP=0,vectorLocalDOP=1", true},
		{"vectorLocalDOP=1,vectorLocalDOP=bad", true},
	} {
		t.Run(tc.hint, func(t *testing.T) {
			builder, _, _, _, _ := newIvfIncludeModeTestBuilder(t)
			for _, hint := range strings.Split(tc.hint, ",") {
				handleOptimizerHints(hint, builder)
			}
			require.Equal(t, tc.enabled, builder.optimizerHints != nil && builder.optimizerHints.vectorLocalDOP == 1)
			require.Equal(t, tc.enabled, VectorLocalDOPEnabled(tc.hint))
		})
	}
}
...
func TestIvfRewriteGatesWorkButRequiresScalarDomain(t *testing.T) {
	for _, enabled := range []bool{false, true} {
		...
		c := &scanWorkCompilerContext{CompilerContext: builder.compCtx, t: t, source: scan.ObjRef,
			table: workTestEntries(types.T_array_float32, 768),
			stats: &statsinfo.StatsInfo{TableCnt: 100, BlockNumber: 4, AccurateObjectNumber: 2}}
		builder.compCtx = c
		...
		require.True(t, n.RuntimeFilterProbeList[0].MustApply)
		buildRequired := false
		for _, node := range builder.qry.Nodes {
			for _, rf := range node.RuntimeFilterBuildList {
				if rf.Tag == n.RuntimeFilterProbeList[0].Tag {
					buildRequired = rf.MustApply
				}
			}
		}
		require.True(t, buildRequired)
