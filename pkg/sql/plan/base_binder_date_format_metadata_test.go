...
func TestDateFormatHeadingTraversalHandlesUnsupportedExprNodes(t *testing.T) {
	tests := []struct {
		...
		{
			name:        "format inside scalar subquery heading",
			sql:         "select (select date_format(col2, '%M') from time01 limit 1)",
			wantHeading: "(select date_format(col2, '%M') from time01 limit 1)",
			wantParts:   true,
		},
		{
