...
func (rule *ResetParamRefRule) typedRuntimeParamExpr(pos int) (*Expr, bool, error) {
	...
	if kind != vector.PrepareParamFloat && typ.Oid != types.T_float64 && typ.Oid != types.T_float32 {
		...
		if _, exportSet := rule.exportSetParamPositions[int32(pos)]; exportSet && pos < len(rule.paramValues) {
			if param, ok := rule.paramValues[pos].(ParamValue); ok && !param.ExportSetNumericString {
				// A bare EXPORT_SET text binding uses val_int semantics at its
				// consumer. Inferring DECIMAL here would round "2.5" to 3.
				return nil, false, nil
			}
		}
...
func (rule *ResetParamRefRule) ApplyExpr(e *plan.Expr) (*plan.Expr, error) {
	...
	if fallbackSource != nil {
		if source, ok := preparedNumericFallbackSource(fallbackSource); ok {
			...
			exportSetOwned := false
			for pos := range positions {
				resolved = resolved && rule.hasExportSetResolvedDomain(int(pos))
				_, exportSetPosition := rule.exportSetParamPositions[pos]
				exportSetOwned = exportSetOwned || exportSetPosition
			}
			// The domain-aware pass already rebound this consumer. The fallback
			// source replay would reinstate PREPARE's provisional common casts.
			if len(positions) > 0 && !resolved {
				var bound *Expr
				var changed bool
				var bindErr error
				if exportSetOwned || preparedExprContainsNamedFunction(fallbackSource, "export_set") {
					bound, changed, bindErr = rule.rebindPreparedNumericExpr(source, positions)
				} else {
					bound, changed, bindErr = rule.rebindPreparedNumericExprWithBound(source, rewritten, positions)
				}
				if bindErr != nil {
					return nil, bindErr
				}
	...
	case *plan.Expr_F:
		...
		for i, arg := range exprImpl.F.Args {
			...
			if hasParamPos && preparedFunctionArgUsesSQLExecuteNumericSource(
				...
				switch {
				case hasPreparedBitwiseSource:
					executeNumericSource = preparedBitwiseSource
			...
			if nestedPreparedBitwiseSource != nil {
				...
				compareArgTypes = true
				rule.specialized = true
			} else if executeNumericSource != nil {
				...
				if functionName == "hex" && paramPos >= 0 && paramPos < len(rule.sqlExecuteStringBackedParams) &&
					rule.sqlExecuteStringBackedParams[paramPos] {
					// HEX owns a string operand as bytes. Restore the typed source below
					// the shared numeric-prefix FLOAT64 wrapper.
					if cast := rewrittenArg.GetF(); cast != nil && cast.GetFunc().GetObjName() == "cast" &&
						len(cast.GetArgs()) > 0 {
						rewrittenArg = cast.GetArgs()[0]
					}
				}
			} else {
				var applyErr error
...
func preparedExprContainsNamedFunction(expr *Expr, name string) bool {
	if expr == nil {
		return false
	}
	if fn := expr.GetF(); fn != nil {
		...
		for _, arg := range fn.Args {
			if preparedExprContainsNamedFunction(arg, name) {
				return true
			}
		}
	}
