...
func TestIssue28158SkipsNonPointOuterDomains(t *testing.T) {
	...
	// main already has the duplicate-safe SEMI rewrite for deterministic
	// non-point domains. This PR must not remove that existing optimization;
	// it only adds scan-level point-filter copying.
	require.Equal(t, plan.Node_JOIN, child.NodeType)
	require.Equal(t, plan.Node_SEMI, child.JoinType)
	aggregateScan := issue28158FindTableScanUnder(query, agg.Children[0], "lineitem")
	require.NotNil(t, aggregateScan)
	require.False(t, issue28158HasColumnFilter(aggregateScan.FilterList, "l_partkey"))
}
...
func TestIssue28158SkipsCompositeNonPointDomain(t *testing.T) {
	...
	query := logicPlan.GetQuery()
	child := query.Nodes[agg.Children[0]]
	// Only one component of partsupp's composite primary key is fixed. Keep
	// main's existing SEMI rewrite, but do not add point-filter pushdown.
	require.Equal(t, plan.Node_JOIN, child.NodeType)
	require.Equal(t, plan.Node_SEMI, child.JoinType)
	aggregateScan := issue28158FindTableScanUnder(query, agg.Children[0], "lineitem")
	require.NotNil(t, aggregateScan)
	require.False(t, issue28158HasColumnFilter(aggregateScan.FilterList, "l_partkey"))
	require.False(t, issue28158HasColumnFilter(aggregateScan.FilterList, "l_suppkey"))
}
