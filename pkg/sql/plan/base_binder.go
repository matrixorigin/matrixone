...
func bindFuncExprImplByPlanExpr(name string, args []*Expr) (*plan.Expr, error) {
	...
	case "date":
		...
		if args[0].Typ.Id != plan.T_varchar && args[0].Typ.Id != plan.T_char {
	...
	case "+":
		...
		if args[0].Typ.Id == plan.T_date && args[1].Typ.Id == plan.T_intERVAL {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[0], args[1])
		} else if args[0].Typ.Id == plan.T_intERVAL && args[1].Typ.Id == plan.T_date {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[1], args[0])
		} else if args[0].Typ.Id == plan.T_datetime && args[1].Typ.Id == plan.T_intERVAL {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[0], args[1])
		} else if args[0].Typ.Id == plan.T_intERVAL && args[1].Typ.Id == plan.T_datetime {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[1], args[0])
		} else if args[0].Typ.Id == plan.T_varchar && args[1].Typ.Id == plan.T_intERVAL {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[0], args[1])
		} else if args[0].Typ.Id == plan.T_intERVAL && args[1].Typ.Id == plan.T_varchar {
			name = "date_add"
			args, err = resetDateFunctionArgs(args[1], args[0])
		}
		if err != nil {
	...
	case "unary_minus":
		if args[0].Typ.Id == plan.T_uint64 {
			args[0], err = appendCastBeforeExpr(args[0], &plan.Type{
				Id:       plan.T_decimal128,
				Nullable: false,
			})
			if err != nil {
				return nil, err
	...
	case "variance":
		if args[0].Typ.Id == plan.T_decimal128 || args[0].Typ.Id == plan.T_decimal64 {
			args[0], err = appendCastBeforeExpr(args[0], &plan.Type{
				Id:       plan.T_float64,
				Nullable: false,
			})
...
func (b *baseBinder) bindNumVal(astExpr *tree.NumVal, typ *Type) (*Expr, error) {
	...
	case tree.P_float64:
		...
		if typ != nil && (typ.Id == plan.T_decimal || typ.Id == plan.T_decimal64 || typ.Id == plan.T_decimal128) {
...
func resetDateFunctionArgs(dateExpr *Expr, intervalExpr *Expr) ([]*Expr, error) {
	...
	if firstExpr.Typ.Id == plan.T_varchar || firstExpr.Typ.Id == plan.T_char {
		...
		if dateExpr.Typ.Id == plan.T_date {
			switch returnType {
