...
func TestOnlyFullGroupByNotNullUniqueKey(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "other join binding", sql: "select e.deptno, x.ename, count(*) from constraint_test.emp e join constraint_test.emp x on e.deptno=x.deptno group by e.deptno", wantErr: true},
		{name: "derived relation", sql: "select deptno, ename, sum(sal) from (select deptno,ename,sal from constraint_test.emp) d group by deptno", wantErr: true},
		...
		{name: "nullable filtered", sql: "select deptno, ename, sum(sal) from constraint_test.emp where deptno is not null group by deptno", change: func(_ *pbplan.TableDef, col *pbplan.ColDef, _ *pbplan.IndexDef) { col.Default.NullAbility = true }, wantErr: true},
