...
func buildCreateSource(stmt *tree.CreateSource, ctx CompilerContext) (*Plan, error) {
	...
	if sub, err := ctx.GetSubscriptionMeta(createStream.Database, timestamp.Timestamp{}); err != nil {
		return nil, err
	} else if sub != nil {
...
func buildCreateView(stmt *tree.CreateView, ctx CompilerContext) (*Plan, error) {
	...
	if sub, err := ctx.GetSubscriptionMeta(createTable.Database, timestamp.Timestamp{}); err != nil {
		return nil, err
	} else if sub != nil {
...
func buildDropSequence(stmt *tree.DropSequence, ctx CompilerContext) (*Plan, error) {
	...
	obj, tableDef := ctx.Resolve(dropSequence.Database, dropSequence.Table, timestamp.Timestamp{})
...
func buildAlterSequence(stmt *tree.AlterSequence, ctx CompilerContext) (*Plan, error) {
	...
	if sub, err := ctx.GetSubscriptionMeta(alterSequence.Database, timestamp.Timestamp{}); err != nil {
		return nil, err
	} else if sub != nil {
...
func buildCreateSequence(stmt *tree.CreateSequence, ctx CompilerContext) (*Plan, error) {
	...
	if sub, err := ctx.GetSubscriptionMeta(createSequence.Database, timestamp.Timestamp{}); err != nil {
		return nil, err
	} else if sub != nil {
...
func buildTruncateTable(stmt *tree.TruncateTable, ctx CompilerContext) (*Plan, error) {
	...
	obj, tableDef := ctx.Resolve(truncateTable.Database, truncateTable.Table, timestamp.Timestamp{})
	if tableDef == nil {
...
func buildDropTable(stmt *tree.DropTable, ctx CompilerContext) (*Plan, error) {
	...
	obj, tableDef := ctx.Resolve(dropTable.Database, dropTable.Table, timestamp.Timestamp{})
...
func buildDropDatabase(stmt *tree.DropDatabase, ctx CompilerContext) (*Plan, error) {
	...
	if ctx.DatabaseExists(string(stmt.Name), timestamp.Timestamp{}) {
		databaseId, err := ctx.GetDatabaseId(string(stmt.Name), timestamp.Timestamp{})
		if err != nil {
			return nil, err
		}
...
func buildCreateIndex(stmt *tree.CreateIndex, ctx CompilerContext) (*Plan, error) {
	...
	obj, tableDef := ctx.Resolve(createIndex.Database, tableName, timestamp.Timestamp{})
	if tableDef == nil {
...
func buildDropIndex(stmt *tree.DropIndex, ctx CompilerContext) (*Plan, error) {
	...
	obj, tableDef := ctx.Resolve(dropIndex.Database, dropIndex.Table, timestamp.Timestamp{})
	if tableDef == nil {
...
func buildAlterView(stmt *tree.AlterView, ctx CompilerContext) (*Plan, error) {
	...
	obj, oldViewDef := ctx.Resolve(alterView.Database, viewName, timestamp.Timestamp{})
	if oldViewDef == nil {
...
func buildAlterTableInplace(stmt *tree.AlterTable, ctx CompilerContext) (*Plan, error) {
	...
	_, tableDef := ctx.Resolve(databaseName, tableName, timestamp.Timestamp{})
	if tableDef == nil {
		...
		case *tree.AlterOptionAdd:
			...
			case *tree.ForeignKey:
				...
				if fkData.IsSelfRefer {
					...
					_, parentTableDef := ctx.Resolve(fkData.ParentDbName, fkData.ParentTableName, timestamp.Timestamp{})
...
func buildLockTables(stmt *tree.LockTableStmt, ctx CompilerContext) (*Plan, error) {
	...
	for _, tableLock := range stmt.TableLocks {
		...
		obj, tableDef := ctx.Resolve(schemaName, tblName, timestamp.Timestamp{})
		if tableDef == nil {
...
func getForeignKeyData(ctx CompilerContext, dbName string, tableDef *TableDef, def *tree.ForeignKey) (*FkData, error) {
	...
	_, parentTableDef := ctx.Resolve(parentDbName, parentTableName, timestamp.Timestamp{})
	if parentTableDef == nil {
		enabled, err := IsForeignKeyChecksEnabled(ctx)
		if err != nil {
			return nil, err
...
func buildFkDataOfForwardRefer(ctx CompilerContext,
	...
	_, childTableDef := ctx.Resolve(fkDefs[0].Db, fkDefs[0].Tbl, timestamp.Timestamp{})
