...
func TestBuildShowColumnsRejectsNonCurrentViewMetadata(t *testing.T) {
	base := NewMockCompilerContext(false)
	base.tables["stale_view"] = &plan.TableDef{
		Name:      "stale_view",
		DbName:    "tpch",
		TblId:     42,
		TableType: catalog.SystemViewRel,
		ViewSql:   &plan.ViewDef{View: "{}"},
		Cols: []*plan.ColDef{{
			Name: "value",
			Typ:  plan.Type{Id: int32(types.T_int64)},
		}},
	}
	base.objects["stale_view"] = &plan.ObjectRef{SchemaName: "tpch", ObjName: "stale_view"}
	ctx := &viewMetadataCheckingCompilerContext{
		MockCompilerContext: base,
		err:                 moerr.NewBadView(t.Context(), "tpch", "stale_view"),
	}
	stmt, err := parsers.ParseOne(t.Context(), dialect.MYSQL,
		"show columns from tpch.stale_view", 1)
	require.NoError(t, err)
	defer stmt.Free()
	_, err = BuildPlan(ctx, stmt, false)
