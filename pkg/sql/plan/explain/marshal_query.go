...
func (m MarshalNodeImpl) GetNodeTitle(ctx context.Context, options *ExplainOptions) (string, error) {
	...
	case plan.Node_DELETE:
		if m.node.DeleteCtx != nil {
			first := true
			for _, ctx := range m.node.DeleteCtx.DelRef {
				if !first {
					result += ", "
				}
				result += ctx.SchemaName + "." + ctx.ObjName
...
func (m MarshalNodeImpl) GetNodeLabels(ctx context.Context, options *ExplainOptions) ([]Label, error) {
	...
	case plan.Node_DELETE:
		if m.node.DeleteCtx != nil {
			DeleteNames := GetDeleteLableValue(ctx, m.node.DeleteCtx, options)
			labels = append(labels, Label{
				Name:  "Full table name",
				Value: DeleteNames,
			})
...
func GetDeleteLableValue(ctx context.Context, deleteCtxs *plan.DeleteCtx, options *ExplainOptions) []string {
	if deleteCtxs == nil {
		return make([]string, 0)
	}
	result := make([]string, 0)
	for _, ctx := range deleteCtxs.DelRef {
