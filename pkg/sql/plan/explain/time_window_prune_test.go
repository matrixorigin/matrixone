...
func TestTimeWindowPartitionShape(t *testing.T) {
	t.Run("gapfill row count keeps timestamp and partition input slots aligned", func(t *testing.T) {
		// The MongoDB E2E query has this shape: an outer row count discards all
		// subquery outputs while GAPFILL still needs both the partition key and
		// timestamp. If pruning remaps those inputs independently, TimeWin can
		// read the integer partition vector as a datetime and panic.
		logicPlan, err := buildOneStmt(plan2.NewMockOptimizer(false), t,
			"select count(*) from (select _wstart, id, max(val) from "+twTable+
				" group by id interval(updated_at, 1, minute) gapfill(partition) fill(null)) x")
		require.NoError(t, err)

		var node *plan.Node
		query := logicPlan.GetQuery()
		for _, candidate := range reachablePlanNodes(query) {
			if candidate.NodeType == plan.Node_TIME_WINDOW {
				node = candidate
				break
			}
		}
		...
		tsCol := node.GroupBy[0].GetCol()
		require.NotNil(t, tsCol)
		require.Less(t, int(tsCol.ColPos), len(child.ProjectList))
		require.Equal(t, node.GroupBy[0].Typ.Id, child.ProjectList[tsCol.ColPos].Typ.Id)

		partitionCol := node.TimeWindowPartitionBy[0].GetCol()
		require.NotNil(t, partitionCol)
		require.Less(t, int(partitionCol.ColPos), len(child.ProjectList))
		require.Equal(t, node.TimeWindowPartitionBy[0].Typ.Id, child.ProjectList[partitionCol.ColPos].Typ.Id)
		require.NotEqual(t, tsCol.ColPos, partitionCol.ColPos)
	})
