...
func TestBuildGeneratedExprRejectsStatementDigest(t *testing.T) {
	proc := testutil.NewProcess(t)
	stmt, err := mysql.ParseOne(context.Background(),
		"create table t (g varchar(64) generated always as (statement_digest('SELECT ?')) stored)", 1)
	require.NoError(t, err)
	createTable, ok := stmt.(*tree.CreateTable)
	require.True(t, ok)
	...
	_, err = buildGeneratedExpr(genCol, plan.Type{Id: int32(types.T_varchar), Width: 64}, nil, proc)
	require.Error(t, err)
	require.Contains(t, err.Error(), "non-deterministic function 'statement_digest'")
}
