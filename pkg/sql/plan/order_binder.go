...
func (b *OrderBinder) BindExpr(astExpr tree.Expr) (*plan.Expr, error) {
	...
	if numVal, ok := astExpr.(*tree.NumVal); ok {
		...
		case constant.Int:
			...
			if colPos < 1 || int(colPos) > len(b.ctx.projects) {
				return nil, moerr.NewSyntaxError(b.sysCtx, "ORDER BY position %v is not in select list", colPos)
			}
		...
		default:
			return nil, moerr.NewSyntaxError(b.sysCtx, "non-integer constant in ORDER BY")
		}
	}
	...
	if colPos, ok = b.ctx.projectByExpr[exprStr]; !ok {
		if b.ctx.isDistinct {
			return nil, moerr.NewSyntaxError(b.sysCtx, "for SELECT DISTINCT, ORDER BY expressions must appear in select list")
		}
