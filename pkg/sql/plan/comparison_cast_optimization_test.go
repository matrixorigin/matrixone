	...
	for _, operator := range []string{"=", "<>"} {
		for _, tc := range []struct {
			...
			t.Run(operator+"/"+tc.name, func(t *testing.T) {
				...
				requires, err := plan.RequiresMORPCVersion87DecimalLiteralSemantics(result)
				require.NoError(t, err)
			...
			for _, carrier := range []string{"string", "typed"} {
				for _, op := range []string{"=", "<>", "<", "<=", ">", ">="} {
					for _, reversed := range []bool{false, true} {
						...
						require.NoError(t, err)
						marked, err := plan.RequiresMORPCVersion87DecimalLiteralSemantics(result)
						require.NoError(t, err)
...
func TestDecimalComparisonNegativeDecimal128ZeroSuffixCarriesProtocolMarker(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, int64(defines.MORPCVersion87), requiredVersion)
	...
	require.NoError(t, err)
	positiveRequires, err := plan.RequiresMORPCVersion87DecimalLiteralSemantics(positiveResult)
	require.NoError(t, err)
	...
	require.NoError(t, err)
	nonzeroRequires, err := plan.RequiresMORPCVersion87DecimalLiteralSemantics(nonzeroResult)
	require.NoError(t, err)
