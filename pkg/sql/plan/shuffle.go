...
// shouldUseDistinctKeyPath selects the complement of the complete-group-owner
// topology. A large exact state justifies exchange, while fewer than
// shuffleDistinctGroupMinNDV final-key owners cannot expose enough parallel
// aggregation. In that case the DISTINCT key itself becomes the owner key.
func shouldUseDistinctKeyPath(node *plan.Node, builder *QueryBuilder) bool {
	if node == nil || builder == nil || builder.qry == nil ||
		builder.compCtx == nil || len(node.Children) != 1 ||
		node.Children[0] < 0 || int(node.Children[0]) >= len(builder.qry.Nodes) {
		return false
	}
	child := builder.qry.Nodes[node.Children[0]]
	...
	for i, groupBy := range node.GroupBy {
		...
		estimated := estimateNDVAfterSelection(ndv, child.Stats)
		if estimated > highestGroupNDV {
			highestGroupNDV = estimated
		}
	}
	return highestGroupNDV > 0 &&
		highestGroupNDV < shuffleDistinctGroupMinNDV
}
