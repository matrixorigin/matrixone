...
func determineShuffleForJoin(node *plan.Node, builder *QueryBuilder) {
	...
	case types.T_int64, types.T_int32, types.T_int16, types.T_uint64, types.T_uint32, types.T_uint16, types.T_varchar, types.T_char, types.T_text:
		...
		// These expressions are generated by the planner for secondary index joins and
		// always contain the primary key, so NDV ≈ table row count ≈ HashmapSize.
		// The join condition is created in bind_update.go before stats are available,
		// so Ndv is left unset (-1). We set it here using HashmapSize as a reliable proxy.
		if isExprBasedShuffle && node.OnList[idx].Ndv < 0 {
