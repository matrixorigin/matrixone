...
func (b *HavingBinder) BindExpr(astExpr tree.Expr, depth int32, isRoot bool) (*plan.Expr, error) {
	...
	if colPos, ok := b.ctx.aggregateByAst[astStr]; ok {
		if !b.insideAgg {
			...
			return nil, moerr.NewInvalidInput(b.sysCtx, "nestted aggregate function")
		}
	}
...
func (b *HavingBinder) BindColRef(astExpr *tree.UnresolvedName, depth int32, isRoot bool) (*plan.Expr, error) {
	if b.insideAgg {
		...
		if _, ok := expr.Expr.(*plan.Expr_Corr); ok {
			return nil, moerr.NewNYI(b.sysCtx, "correlated columns in aggregate function")
		}

		return expr, nil
	} else {
		return nil, moerr.NewSyntaxError(b.sysCtx, "column %q must appear in the GROUP BY clause or be used in an aggregate function", tree.String(astExpr, dialect.MYSQL))
	}
}

func (b *HavingBinder) BindAggFunc(funcName string, astExpr *tree.FuncExpr, depth int32, isRoot bool) (*plan.Expr, error) {
	if b.insideAgg {
		return nil, moerr.NewSyntaxError(b.sysCtx, "aggregate function %s calls cannot be nested", funcName)
	}
...
func (b *HavingBinder) BindWinFunc(funcName string, astExpr *tree.FuncExpr, depth int32, isRoot bool) (*plan.Expr, error) {
	if b.insideAgg {
		return nil, moerr.NewSyntaxError(b.sysCtx, "aggregate function calls cannot contain window function calls")
	} else {
		return nil, moerr.NewSyntaxError(b.sysCtx, "window %s functions not allowed in having clause", funcName)
	}
}
