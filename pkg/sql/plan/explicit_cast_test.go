...
func TestUseExplicitCastOverload(t *testing.T) {
	tests := []struct {
		...
		{name: "text", typ: tree.InternalType{Oid: uint32(defines.MYSQL_TYPE_TEXT), FamilyString: "text"}, want: true},
		...
		{name: "date", typ: tree.InternalType{Oid: uint32(defines.MYSQL_TYPE_DATE), FamilyString: "date"}, want: true},
		{name: "datetime", typ: tree.InternalType{Oid: uint32(defines.MYSQL_TYPE_DATETIME), FamilyString: "datetime"}, want: true},
		{name: "timestamp", typ: tree.InternalType{Oid: uint32(defines.MYSQL_TYPE_TIMESTAMP), FamilyString: "timestamp"}, want: true},
