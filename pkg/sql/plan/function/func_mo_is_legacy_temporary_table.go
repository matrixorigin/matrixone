...
func builtInIsLegacyTemporaryTable(
	...
	for row := uint64(0); row < uint64(length); row++ {
		...
		if nullKind || nullName || nullDatabase || nullCreateSQL {
			if err := results.Append(false, true); err != nil {
				return err
			}
			continue
...
func isLegacyTemporaryTable(ctx context.Context, relKind, relName, database, createSQL string) bool {
	...
	for _, statement := range statements {
		...
		if !ok || !createTable.Temporary {
			continue
		}
		if !strings.EqualFold(string(createTable.Table.ObjectName), logicalName) {
			continue
		}
		schemaName := string(createTable.Table.SchemaName)
		if schemaName == "" || strings.EqualFold(schemaName, database) {
			return true
		}
	}
	return false
