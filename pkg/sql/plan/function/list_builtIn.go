...
func binaryStringTransformReturnType(parameters []types.Type) types.Type {
	if len(parameters) == 0 {
...
func concatReturnType(parameters []types.Type) types.Type {
	...
	for _, parameter := range parameters {
		...
		case types.T_char, types.T_varchar:
			width += int64(parameter.Width) * utf8.UTFMax
		}
	}
	if !hasBinary {
		result := types.T_varchar.ToType()
		result.Charset = types.MergeStringCharset(parameters, result.Charset)
		return result
	}
	if unbounded {
		return types.T_blob.ToType()
...
func charReturnType(parameters []types.Type) types.Type {
	width := int32(len(parameters) * utf8.UTFMax)
	if width > types.MaxVarBinaryLen {
		return types.T_blob.ToType()
	}
	result := types.New(types.T_varbinary, width, 0)
	result.Charset = types.CharsetBinary
	return result
}
