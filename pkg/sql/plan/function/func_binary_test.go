...
func initEncodeTestCase() []tcTemp {
	regularCases := []struct {
	...
	}{
		{
			...
			data:   []string{"", "abc", "a\nb", `a\nb`, "a\"b"},
			format: []string{"hex", "hex", "hex", "hex", "hex"},
			wants:  []string{"", "616263", "610a62", "615c6e62", "612262"},
		},
		{
			info:   "test encode - string to base64",
			data:   []string{"", "abc", "a\nb", `a\nb`, "a\"b"},
			format: []string{"base64", "base64", "base64", "base64", "base64"},
	...
	var testInputs = make([]tcTemp, 0, len(regularCases)+len(arrayF32Cases)+len(arrayF64Cases))
	for _, c := range regularCases {
		...
		testInputs = append(testInputs, tcTemp{
			...
			inputs: []testutil.FunctionTestInput{
				...
				testutil.NewFunctionTestInput(types.T_varchar.ToType(), c.format, []bool{}),
			},
...
func TestEncode(t *testing.T) {
	...
	proc := testutil.NewProcess()
	for _, tc := range testCases {
		fcTC := testutil.NewFunctionTestCase(proc, tc.inputs, tc.expect, Encode)
		s, info := fcTC.Run()
		require.True(t, s, fmt.Sprintf("case is '%s', err info is '%s'", tc.info, info))
	}
}
...
func initDecodeTestCase() []tcTemp {
	regularCases := []struct {
	...
	}{
		{
			...
			data:   []string{"", "616263", "610a62", "615c6e62", "612262"},
			format: []string{"hex", "hex", "hex", "hex", "hex"},
			wants:  []string{"", "abc", "a\nb", `a\nb`, "a\"b"},
		},
		{
			...
			format: []string{"base64", "base64", "base64", "base64"},
			wants:  []string{"", "abc", "a\nb", "a\"b"},
		},
		{
...
func TestDecode(t *testing.T) {
	...
	proc := testutil.NewProcess()
	for _, tc := range testCases {
		fcTC := testutil.NewFunctionTestCase(proc, tc.inputs, tc.expect, Decode)
		s, info := fcTC.Run()
		require.True(t, s, fmt.Sprintf("case is '%s', err info is '%s'", tc.info, info))
	}
}
	...
	return []tcTemp{
		{
			...
			expect: testutil.NewFunctionTestResult(types.T_float64.ToType(), false,
				[]float64{0, 0.0003542304039001465},
				[]bool{false, false}),
		},
		{
