...
func geometryContainsImpl(container, target []byte, containerType, targetType string) (bool, error) {
	...
	case "POLYGON":
		...
		case "LINESTRING":
			...
			return lineStringCoveredByPolygon(targetLine, containerPolygon) && !lineStringTouchesPolygon(targetLine, containerPolygon), nil
		case "POLYGON":
			containerPolygon, err := polygonSingleRingPointsFromPayload(container)
			if err != nil {
				return false, err
...
func geometryIntersectsImpl(left, right []byte, leftType, rightType string) (bool, error) {
	...
	case "POLYGON":
		...
		case "LINESTRING":
			...
			return lineStringIntersectsPolygon(rightPoints, leftPolygon), nil
		case "POLYGON":
			leftPolygon, err := polygonSingleRingPointsFromPayload(left)
			if err != nil {
				return false, err
...
func geometryCoversImpl(container, target []byte, containerType, targetType string) (bool, error) {
	...
	case "POLYGON":
		...
		case "POINT":
			...
			return pointIntersectsPolygonGeometry(geometryPoint2D{x: targetX, y: targetY}, containerPolygon), nil
		case "LINESTRING":
			containerPolygon, err := polygonSingleRingPointsFromPayload(container)
			if err != nil {
				return false, err
