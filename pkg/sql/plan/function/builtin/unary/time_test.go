...
func makeVectorForTimeTest(str string, precision int32, isConst bool, typ types.Type, proc *process.Process) ([]*vector.Vector, uint16) {
	...
	if isConst {
		...
		case types.T_int64:
			...
			vec[0] = vector.New(vector.CONSTANT, types.T_int64.ToType(), 1, data, testutil.TestUtilMp)
		case types.T_decimal128:
			data, err := types.ParseStringToDecimal128(str, 34, precision, false)
			if err != nil {
				return nil, moerr.ErrInvalidInput
			}
			vec[0] = vector.New(vector.CONSTANT, types.T_decimal128.ToType(), 1, data, testutil.TestUtilMp)
		case types.T_date:
			data, err := types.ParseDateCast(str)
			if err != nil {
				return nil, moerr.ErrInvalidInput
			}
			vec[0] = vector.New(vector.CONSTANT, types.T_date.ToType(), 1, data, testutil.TestUtilMp)
		case types.T_datetime:
			data, err := types.ParseDatetime(str, precision)
			if err != nil {
				return nil, moerr.ErrInvalidInput
			}
			vec[0] = vector.New(vector.CONSTANT, types.T_datetime.ToType(), 1, data, testutil.TestUtilMp)
