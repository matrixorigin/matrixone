...
func LLMChunk(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	...
	for i := uint64(0); i < rowCount; i++ {
		...
		defer r.Close()
Contributor
@cpegeric cpegeric on Sep 20
defer r.Close() is buggy inside the for loop. variable r only close when end of the function but not each block. use func() { defer r.Close() } to make sure r is closed at the end of func() {}... You may run out of file descriptors when rowCount is large. (see sql/plan/function/func_unary.go:WriteFileDatalink() for details)
