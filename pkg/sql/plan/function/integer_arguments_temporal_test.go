...
func TestIntegerArgumentEnumDomains(t *testing.T) {
	ctx := context.Background()
	_, err := GetFunctionByName(ctx, "left", []types.Type{types.T_varchar.ToType(), types.T_enum.ToType()})
	require.NoError(t, err)
	_, err = GetFunctionByName(ctx, "random_bytes", []types.Type{types.T_enum.ToType()})
	require.NoError(t, err)
	_, err = GetFunctionByName(ctx, "conv", []types.Type{types.T_varchar.ToType(), types.T_enum.ToType(), types.T_int64.ToType()})
	require.NoError(t, err)
	_, err = GetFunctionByNameWithOverload(ctx, "cast", []types.Type{types.T_uuid.ToType(), types.T_int64.ToType()}, TemporalIntegerArgumentCastOverload)
	require.NoError(t, err)
	_, err = GetFunctionByNameWithOverload(ctx, "cast", []types.Type{types.T_uuid.ToType(), types.T_int64.ToType()}, IntegerArgumentCastOverload)
	require.Error(t, err)
	_, err = GetFunctionByNameWithOverload(ctx, "cast", []types.Type{types.T_time.ToType(), types.T_uint64.ToType()}, TemporalIntegerArgumentCastOverload)
	require.Error(t, err)
}

func TestIntegerArgumentTemporalRoles(t *testing.T) {
	for _, source := range []types.T{types.T_date, types.T_time, types.T_datetime, types.T_timestamp, types.T_uuid} {
		args := []types.Type{types.T_varchar.ToType(), source.ToType()}
		result, err := GetFunctionByName(context.Background(), "sha2", args)
		require.NoError(t, err)
		_, overload := DecodeOverloadID(result.GetEncodedOverloadID())
		require.Zero(t, overload)
		require.Equal(t, source, args[1].Oid)
		_, err = GetFunctionByName(context.Background(), "left", args)
		require.Error(t, err)
	}
}
