...
func statementDigestSQLMode(proc *process.Process) (string, digest.SQLMode, error) {
	...
	// A remote process must honor an explicitly empty sql_mode captured by its
	// coordinator. Its resolver can be an internal compiled default from a
	// previous hop and is not allowed to overwrite that sentinel.
	if sqlMode == process.EmptySqlModeSentinel && !proc.Base.IsFrontend {
		return "", 0, nil
	}
