...
func TestStatementDigestSQLModeSnapshotWinsOnRemoteProcess(t *testing.T) {
	proc := testutil.NewProcess(t)
	...
	require.NoError(t, err)
	require.Equal(t, "STRICT_TRANS_TABLES", parserMode)
...
func TestStatementDigestHonorsParserSQLMode(t *testing.T) {
	...
	parserMode, digestMode, err = statementDigestSQLMode(proc)
	require.NoError(t, err)
	require.Equal(t, "ANSI", parserMode)
	require.NotZero(t, digestMode)
	require.True(t, digestMode&digest.ModeANSIQuotes != 0)
	...
	proc.Base.SessionInfo.SqlMode = ""
	proc.SetResolveVariableFunc(func(string, bool, bool) (any, error) { return nil, errors.New("resolve sql_mode") })
...
func TestStatementDigestHonorsMaxDigestLength(t *testing.T) {
	...
	proc.SetResolveVariableFunc(func(name string, _, _ bool) (any, error) {
		...
		return uint64(1048576), nil
	})
	maxLength, err = statementDigestMaxLength(proc)
	require.NoError(t, err)
	require.Equal(t, 1048576, maxLength)
	...
	_, err = statementDigestMaxLength(proc)
	require.EqualError(t, err, "internal error: max_digest_length is out of range: 18446744073709551615")
