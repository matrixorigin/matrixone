...
func Test_fixedTypeCastRule2(t *testing.T) {
	inputs := []struct {
		...
		},
		{
			shouldCast: true,
			in:         [2]types.Type{types.T_int64.ToType(), types.T_int64.ToType()},
			want:       [2]types.Type{types.New(types.T_decimal128, 38, 18), types.New(types.T_decimal128, 38, 18)},
		},
		{
			shouldCast: true,
...
func Test_GetFunctionByName(t *testing.T) {
	...
	cs := []fInput{
...

		{
			...
			shouldErr:  false,
			requireFid: DIV, requireOid: 0,
			shouldCast: true, requireTyp: []types.Type{types.New(types.T_decimal128, 38, 18), types.New(types.T_decimal128, 38, 18)},
			requireRet: types.New(types.T_decimal128, 38, 18),
		},
