...
func (s *sAggBitAnd[T]) Fill(groupNumber int64, value T, lastResult uint64, count int64, isEmpty bool, isNull bool) (uint64, bool, error) {
	if !isNull {
		...
		if vv > math.MaxUint64 {
			return math.MaxUint64 & lastResult, false, nil
		}
		if vv < 0 {
...
func (s *sAggBitAnd[T]) Eval(lastResult []uint64, _ error) ([]uint64, error) {
...
func (s *sAggBinaryBitAnd) Eval(lastResult [][]byte, _ error) ([][]byte, error) {
	return lastResult, nil
}
