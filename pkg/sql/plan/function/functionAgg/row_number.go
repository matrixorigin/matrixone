...
func NewWinRowNumber(overloadID int64, dist bool, inputTypes []types.Type, outputType types.Type, _ any, _ any) (agg.Agg[any], error) {
	winPriv := &sWindowBase{}
...
func (s *sWindowBase) EvalRowNumber(result []int64, _ any) ([]int64, error) {
	idx := 0
	for _, p := range s.ps {
		n := p[len(p)-1] - p[0]
		for i := int64(1); i <= n; i++ {
