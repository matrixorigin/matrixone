...
func regexpByteStringAt(vec *vector.Vector, row int) bool {
	return isBinaryRegexpVector(vec) || vec.GetIsBinaryStringAt(row) &&
		vec.GetPrepareParamKindAt(row) != vector.PrepareParamBinaryProtocol
}

// MySQL chooses the byte regexp implementation from the pattern item.  A
// binary subject paired with a text pattern is legal, but the subject is
// exposed to the text engine through its latin1 representation.  It must not
// silently turn the text pattern into a byte pattern.
func regexpBytesForMode(vec *vector.Vector, row int, value []byte, binaryMode bool) []byte {
	if binaryMode || !isBinaryRegexpVector(vec) && !vec.GetIsBinaryStringAt(row) {
		return value
	}
	return []byte(binaryBytesToRegexpString(value))
}

func regexpTextBytePosition(value []byte, position int64) int64 {
	if position <= 1 {
		return position
	}
	byteOffset := 0
	for runeIndex := int64(1); runeIndex < position && byteOffset < len(value); runeIndex++ {
		_, width := utf8.DecodeRune(value[byteOffset:])
		byteOffset += width
	}
	return int64(byteOffset + 1)
}

func regexpTextCharacterPosition(value []byte, position int64) int64 {
	if position <= 1 {
		return position
	}
	end := int(position - 1)
	...
	return int64(utf8.RuneCount(value[:end]) + 1)
}
...
func (op *opBuiltInRegexp) likeFn(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
			...
			func(value, pattern []byte, binaryString bool) (bool, error) {
				if binaryString {
					return op.regMap.regularMatchForBinaryLikeOp(
						pattern, value, []byte{DefaultEscapeChar}, true)
				}
				return op.regMap.regularMatchForLikeOp(pattern, value)
			}, selectList)
	}
	...
	return opBinaryBytesBytesToFixedWithErrorCheck[bool](parameters, result, proc, length, func(v1, v2 []byte) (bool, error) {
		if binaryInput {
			return op.regMap.regularMatchForBinaryLikeOp(v2, v1, []byte{DefaultEscapeChar}, true)
		}
	...
	if binaryRows {
		return opBinaryBytesBytesToFixedByAnyBinaryRow[bool](parameters, result, length,
			func(value, pattern []byte, binaryString bool) (bool, error) {
				...
				return op.regMap.regularMatchForLikeOpWithEscape(
					pattern, value, escape, escapeEnabled, caseInsensitive)
			}, selectList)
	}
...
func (op *opBuiltInRegexp) builtInRegMatch(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	return op.regMatchWithMode(parameters, result, length, selectList, false)
}
...
func (op *opBuiltInRegexp) builtInNotRegMatch(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	return op.regMatchWithMode(parameters, result, length, selectList, true)
}

func (op *opBuiltInRegexp) regMatchWithMode(parameters []*vector.Vector, result vector.FunctionResultWrapper, length int, selectList *FunctionSelectList, negate bool) error {
	p1 := vector.GenerateFunctionStrParameter(parameters[0])
	p2 := vector.GenerateFunctionStrParameter(parameters[1])
	rs := vector.MustFunctionResult[bool](result)
	for i := 0; i < length; i++ {
		expr, null1 := p1.GetStrValue(uint64(i))
		pat, null2 := p2.GetStrValue(uint64(i))
		if null1 || null2 || selectList != nil && selectList.Contains(uint64(i)) {
			if err := rs.Append(false, true); err != nil {
				return err
			}
			continue
		}
		binaryMode := regexpByteStringAt(parameters[1], i)
		expr = regexpBytesForMode(parameters[0], i, expr, binaryMode)
		pat = regexpBytesForMode(parameters[1], i, pat, binaryMode)
		reg, err := op.regMap.getRegularMatcherForMatch(functionUtil.QuickBytesToStr(pat))
		if err != nil {
			return err
		}
		matched := reg.MatchString(functionUtil.QuickBytesToStr(expr))
...
func (op *opBuiltInRegexp) builtInRegexpSubstr(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	...
	regularSubstr := func(
		...
		binaryInput, binaryPattern bool,
	) (bool, []byte, error) {
		if binaryPattern {
	...
	case 2:
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || len(v2) == 0 {
				...
				binaryPattern := regexpByteStringAt(parameters[1], int(i))
				v1 = regexpBytesForMode(parameters[0], int(i), v1, binaryPattern)
				v2 = regexpBytesForMode(parameters[1], int(i), v2, binaryPattern)
				binaryInput := binaryPattern
	...
	case 3:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 || len(v2) == 0 {
				...
				binaryInput := binaryPattern
				if !binaryPattern {
					pos = regexpTextBytePosition(v1, pos)
				}
...
func (op *opBuiltInRegexp) builtInRegexpInstr(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	...
	regularInstr := func(expr, pat []byte, pos, occurrence int64, retOption int8, binaryInput bool) (int64, error) {
		if binaryInput {
			return op.regMap.regularInstrBinary(pat, expr, pos, occurrence, retOption)
		}
		return op.regMap.regularInstr(functionUtil.QuickBytesToStr(pat), functionUtil.QuickBytesToStr(expr), pos, occurrence, retOption)
	}
	switch len(parameters) {
	case 2:
		for i := uint64(0); i < uint64(length); i++ {
			v1, null1 := p1.GetStrValue(i)
			v2, null2 := p2.GetStrValue(i)
			if null1 || null2 || selectList != nil && selectList.Contains(i) {
				if err := rs.Append(0, true); err != nil {
					return err
				}
				continue
			}
			binaryMode := regexpByteStringAt(parameters[1], int(i))
			textMode := !binaryMode
			v1 = regexpBytesForMode(parameters[0], int(i), v1, binaryMode)
			v2 = regexpBytesForMode(parameters[1], int(i), v2, binaryMode)
			index, err := regularInstr(v1, v2, 1, 1, 0, binaryMode)
			if textMode {
				index = regexpTextCharacterPosition(v1, index)
			}
			if err != nil {
				return err
			}
	...
	case 3:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 {
				...
				index, err := regularInstr(v1, v2, pos, 1, 0, binaryMode)
				if textMode {
					index = regexpTextCharacterPosition(v1, index)
				}
				if err != nil {
	...
	case 4:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 || null4 {
				...
				index, err := regularInstr(v1, v2, pos, ocur, 0, binaryMode)
				if textMode {
					index = regexpTextCharacterPosition(v1, index)
				}
				if err != nil {
	...
	case 5:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 || null4 || null5 {
				...
				index, err := regularInstr(v1, v2, pos, ocur, resOp, binaryMode)
				if textMode {
					index = regexpTextCharacterPosition(v1, index)
				}
				if err != nil {
...
func (op *opBuiltInRegexp) builtInRegexpLike(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	...
	p2 := vector.GenerateFunctionStrParameter(parameters[1])
	rs := vector.MustFunctionResult[bool](result)
	regularLike := func(expr, pat, matchType []byte, binaryInput bool) (bool, error) {
		exprString, patString := functionUtil.QuickBytesToStr(expr), functionUtil.QuickBytesToStr(pat)
		if binaryInput {
			exprString, patString = binaryBytesToRegexpString(expr), binaryBytesToRegexpString(pat)
		}
		return op.regMap.regularLike(patString, exprString, functionUtil.QuickBytesToStr(matchType))
	}
	...
	if len(parameters) == 2 {
		for i := uint64(0); i < uint64(length); i++ {
			...
			binaryMode := regexpByteStringAt(parameters[1], int(i))
			expr = regexpBytesForMode(parameters[0], int(i), expr, binaryMode)
			pat = regexpBytesForMode(parameters[1], int(i), pat, binaryMode)
			match, err := regularLike(expr, pat, []byte("c"), binaryMode)
			if err != nil {
				return err
			}
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 {
				...
				expr = regexpBytesForMode(parameters[0], int(i), expr, binaryMode)
				pat = regexpBytesForMode(parameters[1], int(i), pat, binaryMode)
				match, err := regularLike(expr, pat, mt, binaryMode)
				if err != nil {
					return err
				}
				if err = rs.Append(match, false); err != nil {
...
func (op *opBuiltInRegexp) builtInRegexpReplace(parameters []*vector.Vector, result vector.FunctionResultWrapper, proc *process.Process, length int, selectList *FunctionSelectList) error {
	...
	rs := vector.MustFunctionResult[types.Varlena](result)
	regularReplace := func(expr, pat, repl []byte, pos, occurrence int64, binaryMode bool) ([]byte, error) {
		if binaryMode {
			return op.regMap.regularReplaceBinary(pat, expr, repl, pos, occurrence)
		}
		value, err := op.regMap.regularReplace(functionUtil.QuickBytesToStr(pat), functionUtil.QuickBytesToStr(expr), functionUtil.QuickBytesToStr(repl), pos, occurrence)
		return functionUtil.QuickStrToBytes(value), err
	}
	...
	case 3:
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 {
				...
				v1 = regexpBytesForMode(parameters[0], int(i), v1, binaryMode)
				v2 = regexpBytesForMode(parameters[1], int(i), v2, binaryMode)
				v3 = regexpBytesForMode(parameters[2], int(i), v3, binaryMode)
				val, err := regularReplace(v1, v2, v3, 1, 0, binaryMode)
				if err != nil {
					return err
				}
				...
				if err = setBinaryStringResultAt(result, int(i), binaryMode, proc); err != nil {
					return err
				}
			}
		}
	...
	case 4:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 || null4 {
				...
				v3 = regexpBytesForMode(parameters[2], int(i), v3, binaryMode)
				if !binaryMode {
					v4 = regexpTextBytePosition(v1, v4)
				}
				val, err := regularReplace(v1, v2, v3, v4, 0, binaryMode)
				if err != nil {
					return err
				}
				if err = rs.AppendBytes(val, false); err != nil {
	...
	case 5:
		...
		for i := uint64(0); i < uint64(length); i++ {
			...
			if null1 || null2 || null3 || null4 || null5 {
				...
				val, err := regularReplace(v1, v2, v3, v4, v5, binaryMode)
				if err != nil {
					return err
				}
				if err = rs.AppendBytes(val, false); err != nil {
...
func (rs *regexpSet) regularMatchForBinaryLikeOp(
	...
	for i := 0; i < len(pat); i++ {
		...
		switch {
		case escapeEnabled && len(escape) > 0 && bytes.HasPrefix(pat[i:], escape):
			escaped = true
			i += len(escape) - 1
		case b == '_':
			pattern.WriteByte('.')
	...
	if escaped {
		pattern.WriteString(regexp.QuoteMeta(binaryBytesToRegexpString(escape)))
	}
	pattern.WriteString(")$")
...
func (rs *regexpSet) regularInstrBinary(
	pat, str []byte,
	pos, occurrence int64,
	retOption int8,
) (int64, error) {
	if pos < 1 || pos > int64(len(str)) {
		return 0, moerr.NewInvalidInputNoCtxf(
			"regexp_instr: Index out of bounds in regular expression search. Search start position: %d, Search string length: %d",
			pos, len(str))
	}
	if occurrence < 1 {
		return 0, moerr.NewInvalidInputNoCtxf(
			"regexp_instr have Index out of bounds in regular expression search, return occurrence %d", occurrence)
	}
	if retOption > 1 {
		return 0, moerr.NewInvalidInputNoCtxf(
			"regexp_instr have Index out of bounds in regular expression search, return option %d", retOption)
	}
	matcher, err := rs.getRegularMatcher(binaryBytesToRegexpString(pat))
	if err != nil {
		return 0, moerr.NewInvalidArgNoCtx(
			"regexp_instr have invalid regexp pattern arg", "["+string(pat)+"]")
	}
	value := binaryBytesToRegexpString(str[pos-1:])
	matches := matcher.FindAllStringIndex(value, -1)
	if int64(len(matches)) < occurrence {
		return 0, nil
	}
	matchIndex := matches[occurrence-1][retOption]
	return int64(utf8.RuneCountInString(value[:matchIndex])) + pos, nil
}

func (rs *regexpSet) regularReplaceBinary(
	pat, str, repl []byte,
	pos, occurrence int64,
) ([]byte, error) {
	if pos < 1 || pos > int64(len(str)) {
		return nil, moerr.NewInvalidInputNoCtxf(
			"regexp_replace: Index out of bounds in regular expression search. Search start position: %d, Search string length: %d",
			pos, len(str))
	}
	if occurrence < 0 {
		return nil, moerr.NewInvalidInputNoCtxf(
			"regexp_replace have Index out of bounds in regular expression search, return occurrence %d", occurrence)
	}
	matcher, err := rs.getRegularMatcher(binaryBytesToRegexpString(pat))
	if err != nil {
		return nil, moerr.NewInvalidArgNoCtx(
			"regexp_replace have invalid regexp pattern arg", "["+string(pat)+"]")
	}
	prefix := binaryBytesToRegexpString(str[:pos-1])
	value := binaryBytesToRegexpString(str[pos-1:])
	replacement := binaryBytesToRegexpString(repl)
	matches := matcher.FindAllStringIndex(value, -1)
	if len(matches) == 0 || occurrence > int64(len(matches)) {
		return append([]byte(nil), str...), nil
	}
	if occurrence == 0 {
		return binaryRegexpStringToBytes(prefix + matcher.ReplaceAllLiteralString(value, replacement)), nil
	}
	match := matches[occurrence-1]
	result := prefix + value[:match[0]] + replacement + value[match[1]:]
	return binaryRegexpStringToBytes(result), nil
}
...
func (rs *regexpSet) regularReplace(pat string, str string, repl string, pos, occurrence int64) (r string, err error) {
	...
	if occurrence == 0 {
		start := int(pos - 1)
		return str[:start] + reg.ReplaceAllLiteralString(str[start:], repl), nil
