...
func TestCastSpecial4(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, c.wantValues, vector.MustTCols[types.Decimal128](decimalres))
...
func TestCastFloatAsDecimal(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, c.wantBytes, vector.MustTCols[types.Decimal128](result))
		})
	}
}
...
func TestCastDecimal64AsDecimal128(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, c.wantBytes, vector.MustTCols[types.Decimal64](castRes))
...
func TestCastDecimal128AsDecimal128(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			require.Equal(t, c.wantBytes, vector.MustTCols[types.Decimal128](castRes))
			require.Equal(t, c.wantType.Oid, castRes.GetType().Oid)
			require.Equal(t, c.wantScalar, castRes.IsConst())
		})
	}
...
func TestCastDateAsDatetimeAndString(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			proc:       procs,
			wantValues: []types.Datetime{types.DatetimeFromClock(1992, 1, 1, 0, 0, 0, 0)},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if castRes.GetType().IsVarlen() {
				...
				require.Equal(t, c.wantValues, vector.MustStrCols(castRes))
			}
			require.Equal(t, c.wantType, castRes.GetType().Oid)
			require.Equal(t, c.wantScalar, castRes.IsConst())
		})
...
func TestCastDatetimeAsDateAndString(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			proc:       procs,
			wantValues: []types.Date{types.DateFromCalendar(1992, 1, 1)},
...
func TestCastStringToTime(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			castRes, err := Cast(c.vecs, c.proc)
			require.NoError(t, err)
			require.Equal(t, c.wantValues, vector.MustTCols[types.Time](castRes))
			require.Equal(t, c.wantType, castRes.GetType().Oid)
			require.Equal(t, c.wantScalar, castRes.IsConst())
		})
	}
...
func TestCastDateAndDatetimeToInt(t *testing.T) {
	makeTempVectors := func(src string, srcIsConst bool, precision int32, srcType, dstType types.T) []*vector.Vector {
		...
		case types.T_timestamp:
			datetime, err := types.ParseTimestamp(time.UTC, src, 6)
			require.NoError(t, err)
			vectors[0] = makeVector(datetime, srcIsConst)
		default:
			panic("wrong input test type")
		}
		vectors[1] = makeTypeVector(dstType)
		return vectors
	}
...
func TestCastJsonToString(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			got := vector.MustStrCols(castRes)
			require.Equal(t, c.wantValues, got)
			require.Equal(t, types.T_varchar, castRes.GetType().Oid)
			require.Equal(t, c.wantScalar, castRes.IsConst())
		})
	}
}
...
func makeScalarNullVector(srcType types.T) *vector.Vector {
	nullVector := vector.New(vector.CONSTANT, srcType.ToType())
	nulls.Add(nullVector.GetNulls(), 0)
	return nullVector
}
...
func makeJsonVectors(src string, isConst bool, t types.T) []*vector.Vector {
	...
	r, _ := dt.Marshal()
	if isConst {
		vec := vector.New(vector.CONSTANT, types.T_json.ToType())
		vector.AppendBytes(vec, r, false, testutil.TestUtilMp)
		return []*vector.Vector{vec, makeTypeVector(t)}
	} else {
		vec := vector.New(vector.CONSTANT, types.T_json.ToType())
		vector.AppendBytesList(vec, [][]byte{r}, nil, testutil.TestUtilMp)
		return []*vector.Vector{vec, makeTypeVector(t)}
	}
}
