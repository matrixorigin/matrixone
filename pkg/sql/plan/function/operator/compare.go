...
func CompareString(vs []*vector.Vector, fn compStringFn, proc *process.Process) (*vector.Vector, error) {
	...
	if v1.IsScalar() {
		...
		veccol := vec.Col.([]bool)
		for i := range veccol {
			veccol[i] = fn(col1[0], (&col2[i]).GetByteSlice(area), v1.Typ.Scale, v2.Typ.Scale)
		}
	...
	if v2.IsScalar() {
		...
		veccol := vec.Col.([]bool)
		for i := range veccol {
			veccol[i] = fn((&col1[i]).GetByteSlice(area), col2[0], v1.Typ.Scale, v2.Typ.Scale)
		}
	...
	veccol := vec.Col.([]bool)
	for i := range veccol {
		veccol[i] = fn((&col1[i]).GetByteSlice(area1), (&col2[i]).GetByteSlice(area2), v1.Typ.Scale, v2.Typ.Scale)
	}
