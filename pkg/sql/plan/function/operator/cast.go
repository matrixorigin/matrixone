...
func CastTimestampAsDate(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if _, err := binary.TimestampToDatetime(proc.Ctx, t, lvs, rs2); err != nil {
		return nil, err
	}
	for i := 0; i < len(rs2); i++ {
		rs[i] = rs2[i].ToDate()
...
func CastSpecialIntToDecimal[T constraints.Integer](
	lv, rv *vector.Vector,
	i2d func(ctx context.Context, xs []T, rs []types.Decimal128, width, scale int32) ([]types.Decimal128, error),
...
func CastSpecialIntToDecimal64[T constraints.Integer](
	lv, rv *vector.Vector,
	i2d func(ctx context.Context, xs []T, rs []types.Decimal64, width, scale int32) ([]types.Decimal64, error),
	proc *process.Process) (*vector.Vector, error) {
...
func CastFloatAsDecimal128[T constraints.Float](lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		rs := make([]types.Decimal128, 1)
		decimal128, err := types.ParseDecimal128(srcStr, resultType.Width, resultType.Scale)
		if err != nil {
			return nil, err
		}
		rs[0] = decimal128
	...
	for i := 0; i < len(vs); i++ {
		...
		decimal128, err2 := types.ParseDecimal128(strValue, resultType.Width, resultType.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal128
...
func CastFloatAsDecimal64[T constraints.Float](lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(resultType)
		rs := make([]types.Decimal64, 1)
		rs[0], err = types.Decimal64FromFloat64(float64(vs[0]), resultType.Width, resultType.Scale)
		if err != nil {
			return nil, err
		}
		nulls.Reset(vec.Nsp)
	...
	for i := 0; i < len(vs); i++ {
		...
		rs[i], err = types.Decimal64FromFloat64(float64(vs[i]), resultType.Width, resultType.Scale)
		if err != nil {
			return nil, err
		}
	}
...
func CastVarcharAsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		data, err2 := types.ParseTime(vs[0], rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		return vector.NewConstFixed(rv.Typ, lv.Length(), data, proc.Mp()), nil
	...
	for i, str := range vs {
		...
		data, err2 := types.ParseTime(str, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = data
...
func CastVarcharAsDatetime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		data, err2 := types.ParseDatetime(vs[0], rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		return vector.NewConstFixed(rv.Typ, lv.Length(), data, proc.Mp()), nil
	...
	for i, str := range vs {
		...
		data, err2 := types.ParseDatetime(str, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = data
...
func CastVarcharAsTimestamp(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		data, err := types.ParseTimestamp(t, vs[0], rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
		return vector.NewConstFixed(rv.Typ, lv.Length(), data, proc.Mp()), nil
	...
	for i, str := range vs {
		...
		data, err := types.ParseTimestamp(t, str, rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
		rs[i] = data
...
func castTimestampAsVarchar(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		rs := make([]string, 1)
		if _, err := binary.TimestampToVarchar(proc.Ctx, t, lvs, rs, scale); err != nil {
			return nil, err
		}
...
func CastStringAsDecimal64(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		decimal64, err := types.ParseDecimal64(vs[0], rv.Typ.Width, rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
	...
	for i, str := range vs {
		...
		decimal64, err2 := types.ParseDecimal64(str, rv.Typ.Width, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal64
...
func CastStringAsDecimal128(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		decimal128, err := types.ParseDecimal128(vs[0], rv.Typ.Width, rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
	...
	for i := range vs {
		...
		decimal128, err2 := types.ParseDecimal128(vs[i], rv.Typ.Width, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal128
...
func CastDatetimeAsString(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		if col, err = binary.DatetimeToBytes(proc.Ctx, lvs, col, lv.Typ.Scale); err != nil {
			return nil, err
		}
		return vector.NewConstString(rv.Typ, lv.Length(), col[0], proc.Mp()), nil
	}
...
func CastDecimal64AsTimestamp(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Timestamp, 1)
		if _, err := binary.Decimal64ToTimestamp(lvs, lv.Typ.Scale, rs); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastDecimal128AsTimestamp(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Timestamp, 1)
		if _, err := binary.Decimal128ToTimestamp(lvs, lv.Typ.Scale, rs); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastTimeAsString(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		if col, err = binary.TimeToBytes(proc.Ctx, lvs, col, lv.Typ.Scale); err != nil {
			return nil, err
		}
		return vector.NewConstString(rv.Typ, lv.Length(), col[0], proc.Mp()), nil
	}
...
func CastTimeAsDatetime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		rs := make([]types.Datetime, 1)
		if _, err := binary.TimeToDatetime(proc.Ctx, lvs, rs, rv.Typ.Scale); err != nil {
			return nil, err
		}
		return vector.NewConstFixed(rv.Typ, lv.Length(), rs[0], proc.Mp()), nil
	}
...
func CastDatetimeAsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		rs := make([]types.Time, 1)
		if _, err := binary.DatetimeToTime(proc.Ctx, lvs, rs, scale); err != nil {
			return nil, err
		}
		return vector.NewConstFixed(rv.Typ, lv.Length(), rs[0], proc.Mp()), nil
	}
...
func CastIntAsTime[T constraints.Signed](lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		if _, err := binary.NumericToTime(proc.Ctx, lvs, rs, rv.Typ.Scale); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastDecimal64AsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Time, 1)
		if _, err := binary.Decimal64ToTime(proc.Ctx, lvs, rs, lv.Typ.Scale, rv.Typ.Scale); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastDecimal128AsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Time, 1)
		if _, err := binary.Decimal128ToTime(proc.Ctx, lvs, rs, lv.Typ.Scale, rv.Typ.Scale); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastTimeAsDecimal64(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Decimal64, 1)
		if _, err := binary.TimeToDecimal64(proc.Ctx, lvs, rs, rv.Typ.Width, lv.Typ.Scale); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastTimeAsDecimal128(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Decimal128, 1)
		if _, err := binary.TimeToDecimal128(proc.Ctx, lvs, rs, rv.Typ.Width, lv.Typ.Scale); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
