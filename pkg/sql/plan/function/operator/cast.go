...
func CastFloatAsDecimal128[T constraints.Float](lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		rs := make([]types.Decimal128, 1)
		decimal128, err := types.ParseDecimal128(srcStr, resultType.Precision, resultType.Scale)
		if err != nil {
			return nil, err
		}
		rs[0] = decimal128
	...
	for i := 0; i < len(vs); i++ {
		...
		decimal128, err2 := types.ParseDecimal128(strValue, resultType.Precision, resultType.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal128
...
func CastFloatAsDecimal64[T constraints.Float](lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(resultType)
		rs := make([]types.Decimal64, 1)
		rs[0], err = types.Decimal64FromFloat64(float64(vs[0]), resultType.Precision, resultType.Scale)
		if err != nil {
			return nil, err
		}
		nulls.Reset(vec.Nsp)
	...
	for i := 0; i < len(vs); i++ {
		...
		rs[i], err = types.Decimal64FromFloat64(float64(vs[i]), resultType.Precision, resultType.Scale)
		if err != nil {
			return nil, err
		}
	}
...
func CastStringAsDecimal64(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		decimal64, err := types.ParseDecimal64(vs[0], rv.Typ.Precision, rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
	...
	for i, str := range vs {
		...
		decimal64, err2 := types.ParseDecimal64(str, rv.Typ.Precision, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal64
...
func CastStringAsDecimal128(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		...
		decimal128, err := types.ParseDecimal128(vs[0], rv.Typ.Precision, rv.Typ.Scale)
		if err != nil {
			return nil, err
		}
	...
	for i := range vs {
		...
		decimal128, err2 := types.ParseDecimal128(vs[i], rv.Typ.Precision, rv.Typ.Scale)
		if err2 != nil {
			return nil, err2
		}
		rs[i] = decimal128
...
func CastDecimal64AsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Time, 1)
		if _, err := binary.Decimal64ToTime(proc.Ctx, lvs, rs, rv.Typ.Scale, rv.Typ.Precision); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
...
func CastDecimal128AsTime(lv, rv *vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	if lv.IsScalar() {
		vec := proc.AllocScalarVector(rv.Typ)
		rs := make([]types.Time, 1)
		if _, err := binary.Decimal128ToTime(proc.Ctx, lvs, rs, lv.Typ.Scale, rv.Typ.Precision); err != nil {
			return nil, err
		}
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, rs)
	...
	rs := vector.MustTCols[types.Time](vec)
	if _, err := binary.Decimal128ToTime(proc.Ctx, lvs, rs, rv.Typ.Scale, rv.Typ.Precision); err != nil {
		return nil, err
	}
	return vec, nil
}
