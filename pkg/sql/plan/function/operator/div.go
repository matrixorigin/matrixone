...
func Div[T constraints.Float](vectors []*vector.Vector, proc *process.Process, typ types.Type) (*vector.Vector, error) {
	...
	case lv.IsScalar() && rv.IsScalar():
		...
		if !nulls.Any(rv.Nsp) {
			...
			vector.SetCol(vec, div2.NumericDiv(lvs, rvs, rs))
			return vec, nil
		}
		...
		vector.SetCol(vec, div2.NumericDivSels(lvs, rvs, rs, sels))
		return vec, nil
	case lv.IsScalar() && !rv.IsScalar():
		if !nulls.Any(rv.Nsp) {
			for _, v := range rvs {
			...
			vector.SetCol(vec, div2.NumericDivScalar(lvs[0], rvs, rs))
			return vec, nil
		}
		sels := process.GetSels(proc)
		defer process.PutSels(sels, proc)
		...
		rs := encoding.DecodeFixedSlice[T](vec.Data, rtl)
		nulls.Set(vec.Nsp, rv.Nsp)
		vector.SetCol(vec, div2.NumericDivScalarSels(lvs[0], rvs, rs, sels))
		return vec, nil
	case !lv.IsScalar() && rv.IsScalar():
		if rvs[0] == 0 {
			return nil, ErrDivByZero
		...
		vector.SetCol(vec, div2.NumericDivByScalar(rvs[0], lvs, rs))
		return vec, nil
	}
	vec, err := proc.AllocVector(lv.Typ, int64(rtl)*int64(len(lvs)))
	if err != nil {
...
func DivDecimal64(vectors []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	case lv.IsScalar() && rv.IsScalar():
		...
		nulls.Or(lv.Nsp, rv.Nsp, vec.Nsp)
		if !nulls.Any(rv.Nsp) {
			vector.SetCol(vec, div2.Decimal64Div(lvs, rvs, lvScale, rvScale, rs))
			vec.Typ = resultTyp
			return vec, nil
		}
		sels := process.GetSels(proc)
		...
		vector.SetCol(vec, div2.Decimal64DivSels(lvs, rvs, lvScale, rvScale, rs, sels))
		vec.Typ = resultTyp
		return vec, nil
	case lv.IsScalar() && !rv.IsScalar():
		sels := process.GetSels(proc)
		...
		rs = rs[:len(rvs)]
		nulls.Set(vec.Nsp, rv.Nsp)
		vector.SetCol(vec, div2.Decimal64DivScalarSels(lvs[0], rvs, lvScale, rvScale, rs, sels))
		vec.Typ = resultTyp
		return vec, nil
	case !lv.IsScalar() && rv.IsScalar():
		...
		rs = rs[:len(lvs)]
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, div2.Decimal64DivByScalar(rvs[0], lvs, rvScale, lvScale, rs))
		vec.Typ = resultTyp
		return vec, nil
	}
	vec, err := proc.AllocVector(resultTyp, int64(resultTyp.Size)*int64(len(lvs)))
...
func DivDecimal128(vectors []*vector.Vector, proc *process.Process) (*vector.Vector, error) {
	...
	case lv.IsScalar() && rv.IsScalar():
		...
		if !nulls.Any(rv.Nsp) {
			...
			vector.SetCol(vec, div2.Decimal128Div(lvs, rvs, lvScale, rvScale, rs))
			vec.Typ = resultTyp
			return vec, nil
		}
		sels := process.GetSels(proc)
		...
		vector.SetCol(vec, div2.Decimal128DivSels(lvs, rvs, lvScale, rvScale, rs, sels))
		vec.Typ = resultTyp
		return vec, nil
	case lv.IsScalar() && !rv.IsScalar():
		if !nulls.Any(rv.Nsp) {
			...
			rs = rs[:len(rvs)]
			nulls.Set(vec.Nsp, rv.Nsp)
			vector.SetCol(vec, div2.Decimal128DivScalar(lvs[0], rvs, lvScale, rvScale, rs))
			vec.Typ = resultTyp
			return vec, nil
		}
		sels := process.GetSels(proc)
		...
		rs = rs[:len(rvs)]
		nulls.Set(vec.Nsp, rv.Nsp)
		vector.SetCol(vec, div2.Decimal128DivScalarSels(lvs[0], rvs, lvScale, rvScale, rs, sels))
		vec.Typ = resultTyp
		return vec, nil
	case !lv.IsScalar() && rv.IsScalar():
		...
		rs = rs[:len(lvs)]
		nulls.Set(vec.Nsp, lv.Nsp)
		vector.SetCol(vec, div2.Decimal128DivByScalar(rvs[0], lvs, rvScale, lvScale, rs))
		vec.Typ = resultTyp
		return vec, nil
	}
	vec, err := proc.AllocVector(lv.Typ, int64(resultTyp.Size)*int64(len(lvs)))
