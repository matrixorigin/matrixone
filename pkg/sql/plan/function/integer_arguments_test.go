...
func TestIntegerArgumentRegisteredConsumers(t *testing.T) {
	for _, name := range []string{"substring_index", "period_add", "period_diff", "left", "right", "substring", "substr", "mid", "lpad", "rpad", "insert", "locate", "repeat", "space", "elt"} {
		t.Run(name, func(t *testing.T) {
			...
			for _, arg := range f.Overloads[index].args {
				inputs = append(inputs, arg.ToType())
			}
			if name == "elt" {
...
func TestIntegerArgumentTimestampAddDomains(t *testing.T) {
	ctx := context.Background()
	for _, valueType := range []types.T{types.T_date, types.T_datetime, types.T_timestamp, types.T_varchar} {
		inputs := []types.Type{types.T_varchar.ToType(), types.T_int64.ToType(), valueType.ToType()}
		control, err := GetFunctionByName(ctx, "timestampadd", inputs)
		require.NoError(t, err)
		for _, countType := range []types.T{types.T_float64, types.T_decimal128, types.T_varchar} {
			args := append([]types.Type(nil), inputs...)
			args[1] = countType.ToType()
			got, err := GetFunctionByName(ctx, "timestampadd", args)
			require.NoError(t, err)
			require.Equal(t, control.GetEncodedOverloadID(), got.GetEncodedOverloadID())
			require.Equal(t, control.GetReturnType(), got.GetReturnType())
		}
	}
	for _, countType := range []types.T{types.T_date, types.T_time, types.T_datetime, types.T_timestamp} {
		_, err := GetFunctionByName(ctx, "timestampadd", []types.Type{types.T_varchar.ToType(), countType.ToType(), types.T_varchar.ToType()})
		require.Error(t, err, "preserve existing rejected count sources")
	}
}

func TestIntegerArgumentMixedDomains(t *testing.T) {
	ctx := context.Background()
	for _, tc := range []struct {
		...
		{types.T_varchar, 0, types.T_varchar}, {types.T_uint64, 3, types.T_uint64},
		{types.T_uint8, 3, types.T_uint64}, {types.T_int8, 2, types.T_int64},
		{types.T_float64, 2, types.T_int64}, {types.T_decimal256, 2, types.T_int64},
		{types.T_array_float32, 6, types.T_array_float32},
	} {
		result, err := GetFunctionByName(ctx, "hex", []types.Type{tc.source.ToType()})
		require.NoError(t, err)
		_, id := DecodeOverloadID(result.GetEncodedOverloadID())
		require.Equal(t, tc.id, id)
		targets, cast := result.ShouldDoImplicitTypeCast()
		if cast {
			require.Equal(t, tc.target, targets[0].Oid)
		} else {
			require.Equal(t, tc.target, tc.source)
		}
	}
	for _, id := range []int32{4, 5} {
		_, err := GetFunctionByNameWithOverload(ctx, "hex", []types.Type{types.T_float64.ToType()}, id)
		require.ErrorContains(t, err, "legacy execution only")
		_, err = GetFunctionById(ctx, encodeOverloadID(HEX, id))
		require.NoError(t, err)
	}
	inputs := []types.Type{types.T_decimal128.ToType(), types.T_uint64.ToType(), types.T_varchar.ToType()}
	original := append([]types.Type(nil), inputs...)
	result, err := GetFunctionByName(ctx, "char", inputs)
	require.NoError(t, err)
	targets, cast := result.ShouldDoImplicitTypeCast()
	require.True(t, cast)
	require.Equal(t, []types.Type{types.T_int64.ToType(), types.T_uint64.ToType(), types.T_uint64.ToType()}, targets)
	require.Equal(t, original, inputs)
	for _, name := range []string{"bin", "oct", "conv"} {
		_, applies := IntegerArgumentTargetForSource(name, 0, types.T_decimal128, false)
		require.False(t, applies, "radix operands are not val_int contexts")
	}
	target, ok := IntegerArgumentTargetForSource("char", 7, types.T_varchar, true)
	require.True(t, ok)
	require.Equal(t, types.T_uint64, target)
	_, ok = IntegerArgumentTargetForSource("hex", 0, types.T_varchar, true)
	require.False(t, ok)
	target, ok = IntegerArgumentTargetForSource("export_set", 4, types.T_uint64, false)
	require.True(t, ok)
	require.Equal(t, types.T_int64, target)
}
