...
func TestHandleSetProtocolVersionDispatchesCompleteTargetSetConcurrently(t *testing.T) {
	...
	result, err := handleSetProtocolVersion(
		proc, cn, strings.Join(targets, ",")+":44", nil)
	require.NoError(t, err)
	require.Equal(t, "activation-cn-1:44, activation-cn-2:44", result.Data)
	...
	_, err = handleSetProtocolVersion(proc, cn, "activation-cn-1,activation-cn-1:44", nil)
...
func TestHandleSetProtocolVersionRejectsOmittedAuthoritativeCN(t *testing.T) {
	...
	_, err := handleSetProtocolVersion(proc, cn, "new-cn:44", nil)
...
func TestTransferToCNAllowsActivationFence(t *testing.T) {
	...
	started := time.Now()
	_, err := transferToCN(qcli, serviceID, defines.MORPCVersion44, []string{serviceID})
	require.Error(t, err)
