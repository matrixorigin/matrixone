...
func TestBindRegexpAllowsParamsAndPlainNull(t *testing.T) {
...
func TestBindRegexpUsesMySQLBinaryCompatibilityPairs(t *testing.T) {
	...
	binaryCharset := regexpTestExpr(types.T_varchar, "a")
	binaryCharset.Typ.Charset = uint32(types.CharsetBinary)
	_, err = BindFuncExprImplByPlanExpr(context.Background(), "regexp_like", []*Expr{binaryCharset, text})
	require.Error(t, err)
	require.ErrorAs(t, err, &moErr)
	require.Equal(t, uint16(moerr.ER_CHARACTER_SET_MISMATCH), moErr.MySQLCode())
...
func TestBuildRegexpUsesMySQLBinaryCompatibilityContract(t *testing.T) {
	for _, sql := range []string{
		...
		var moErr *moerr.Error
		require.ErrorAs(t, err, &moErr, sql)
		require.Equal(t, uint16(moerr.ER_CHARACTER_SET_MISMATCH), moErr.MySQLCode(), sql)
	}

	for _, sql := range []string{
		"select _binary 'abc' regexp _binary 'a'",
		"select null regexp 'a'",
		`prepare s from "select ? regexp 'a'"`,
	} {
		stmt, err := parsers.ParseOne(context.Background(), dialect.MYSQL, sql, 1)
		require.NoError(t, err, sql)
		_, err = BuildPlan(NewMockCompilerContext(true), stmt, false)
