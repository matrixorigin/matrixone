...
func DeepCopyPlanWithParamVals(pl *Plan, paramVals []any) *Plan {
	switch p := pl.Plan.(type) {
	case *Plan_Query:
		return &Plan{
			Plan: &plan.Plan_Query{
				Query: DeepCopyQueryWithParamVals(p.Query, paramVals),
			},
			IsPrepare:   pl.IsPrepare,
			TryRunTimes: pl.TryRunTimes,
		}
...
func DeepCopyQueryWithParamVals(qry *plan.Query, paramVals []any) *plan.Query {
	newQry := &plan.Query{
		StmtType: qry.StmtType,
		Steps:    qry.Steps,
		Nodes:    make([]*plan.Node, len(qry.Nodes)),
	...
	for idx, param := range qry.Params {
		newQry.Params[idx] = DeepCopyExprWithParamVals(param, paramVals)
	}
	for idx, node := range qry.Nodes {
		newQry.Nodes[idx] = DeepCopyNodeWithParamVals(node, paramVals)
	}
	return newQry
}

func DeepCopyNodeWithParamVals(node *plan.Node, paramVals []any) *plan.Node {
	newNode := &Node{
		NodeType:        node.NodeType,
		NodeId:          node.NodeId,
		ExtraOptions:    node.ExtraOptions,
		...
		BuildOnLeft:     node.BuildOnLeft,
		BindingTags:     make([]int32, len(node.BindingTags)),
		Limit:           DeepCopyExprWithParamVals(node.Limit, paramVals),
		Offset:          DeepCopyExprWithParamVals(node.Offset, paramVals),
		ProjectList:     make([]*plan.Expr, len(node.ProjectList)),
		OnList:          make([]*plan.Expr, len(node.OnList)),
		FilterList:      make([]*plan.Expr, len(node.FilterList)),
		BlockFilterList: make([]*plan.Expr, len(node.BlockFilterList)),
	...
	for idx, expr := range node.ProjectList {
		newNode.ProjectList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.OnList {
		newNode.OnList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.FilterList {
		newNode.FilterList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.BlockFilterList {
		newNode.BlockFilterList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.GroupBy {
		newNode.GroupBy[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.GroupingSet {
		newNode.GroupingSet[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	for idx, expr := range node.AggList {
		newNode.AggList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}
	...
	for idx, expr := range node.OnUpdateExprs {
		newNode.OnUpdateExprs[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}
	...
	if node.WinSpecList != nil {
		newNode.WinSpecList = make([]*Expr, len(node.WinSpecList))
		for i, w := range node.WinSpecList {
			newNode.WinSpecList[i] = DeepCopyExprWithParamVals(w, paramVals)
		}
	}
	...
	for idx, expr := range node.TblFuncExprList {
		newNode.TblFuncExprList[idx] = DeepCopyExprWithParamVals(expr, paramVals)
	}

	return newNode
}

func DeepCopyExprWithParamVals(expr *Expr, paramVals []any) *Expr {
	if expr == nil {
		return nil
	}
	newExpr := &Expr{
	...
	case *plan.Expr_P:
		...
		if item.P.GetPos() < int32(len(paramVals)) {
			val := paramVals[item.P.GetPos()]
			reflect.TypeOf(val)

			pc := &plan.Literal{}
			pc.Value = &plan.Literal_Sval{Sval: fmt.Sprintf("%v", paramVals[item.P.GetPos()])}
			newExpr.Expr = &plan.Expr_Lit{
				Lit: pc,
			}
		} else {
	...
	case *plan.Expr_F:
		newArgs := make([]*Expr, len(item.F.Args))
		for idx, arg := range item.F.Args {
			newArgs[idx] = DeepCopyExprWithParamVals(arg, paramVals)
		}
		newExpr.Expr = &plan.Expr_F{
			F: &plan.Function{
				Func: DeepCopyObjectRef(item.F.Func),
	...
	case *plan.Expr_W:
		ps := make([]*Expr, len(item.W.PartitionBy))
		for i, p := range item.W.PartitionBy {
			ps[i] = DeepCopyExprWithParamVals(p, paramVals)
		}
		os := make([]*OrderBySpec, len(item.W.OrderBy))
		for i, o := range item.W.OrderBy {
			os[i] = DeepCopyOrderBy(o)
		...
		newExpr.Expr = &plan.Expr_W{
			W: &plan.WindowSpec{
				WindowFunc:  DeepCopyExprWithParamVals(item.W.WindowFunc, paramVals),
				PartitionBy: ps,
				OrderBy:     os,
				Name:        item.W.Name,
				Frame: &plan.FrameClause{
					...
					Start: &plan.FrameBound{
						Type:      f.Start.Type,
						UnBounded: f.Start.UnBounded,
						Val:       DeepCopyExprWithParamVals(f.Start.Val, paramVals),
					},
					End: &plan.FrameBound{
						Type:      f.End.Type,
						UnBounded: f.End.UnBounded,
						Val:       DeepCopyExprWithParamVals(f.End.Val, paramVals),
					},
				},
			},
		}
	...
	case *plan.Expr_Sub:
		newExpr.Expr = &plan.Expr_Sub{
			Sub: &plan.SubqueryRef{
				...
				Op:      item.Sub.Op,
				RowSize: item.Sub.RowSize,
				Child:   DeepCopyExprWithParamVals(item.Sub.Child, paramVals),
			},
		}
	...
	case *plan.Expr_List:
		...
		for i, ie := range item.List.List {
			e.List[i] = DeepCopyExprWithParamVals(ie, paramVals)
		}
		newExpr.Expr = &plan.Expr_List{
			List: e,
		}
