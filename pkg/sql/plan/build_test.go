...
func TestSingleSQL(t *testing.T) {
	...
	//sql := "select n_nationkey % 5 from NATION"
	//sql := `select case when n_name = 'abc' then 1 when n_name = 'bcd' then 2 else 3 end from NATION`
	sql := "select abs(n_nationkey) from NATION"
