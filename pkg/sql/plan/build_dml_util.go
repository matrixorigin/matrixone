...
type dmlPlanCtx struct {
	...
	// replaceOldRowsStep identifies the complete old-row ownership set of the
	// outer REPLACE. Recursive CASCADE actions use it to avoid revisiting a row
	// whose delete and rebuild are owned by the main MULTI_UPDATE.
	replaceOldRowsStep   int32
	replaceTargetTableID uint64
...
func buildUpdatePlans(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, updatePlanCtx *dmlPlanCtx, addAffectedRows bool) error {
	...
	if updatePlanCtx.isFkRecursionCall {
		builder.fkUpdatedTables[updatePlanCtx.tableDef.TblId] = struct{}{}
	}
...
func buildDeletePlans(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, delCtx *dmlPlanCtx) error {
	...
	if !delCtx.skipTargetDelete {
		selfActionCount := 0
		for _, fk := range delCtx.tableDef.Fkeys {
			if fk.ForeignTbl == 0 &&
				(fk.OnDelete == plan.ForeignKeyDef_CASCADE || fk.OnDelete == plan.ForeignKeyDef_SET_NULL) {
				selfActionCount++
			}
		}
		if selfActionCount > 1 {
			// Each self action owns an independent recursive source. Sharing one
			// deleteNode across different FK edges either drops action-specific
			// columns (SET NULL versus CASCADE) or creates a sink wait cycle.
		} else if sinkNodeId, ok := builder.deleteNode[delCtx.tableDef.TblId]; ok {
	...
	if enabled && len(delCtx.tableDef.RefChildTbls) > 0 {
		...
		for _, tableId := range delCtx.tableDef.RefChildTbls {
			...
			if delCtx.skipTargetDelete || (!isUpdate && delCtx.sourceTag != 0) {
				...
				// Recursive FK actions also carry a source tag even when the
				// target delete itself is not skipped.
			...
			appendExcludeCascadeOwnedRows := func(inputNodeID, candidateTag int32) (int32, int32, error) {
				if !delCtx.skipTargetDelete || isUpdate {
					return inputNodeID, candidateTag, nil
				}
				...
				antiNodeID := builder.appendNode(&plan.Node{
				...
				projectNodeID := builder.appendNode(&plan.Node{
					NodeType: plan.Node_PROJECT, Children: []int32{antiNodeID},
					ProjectList: candidateProjection, BindingTags: []int32{outputTag},
				}, bindCtx)
				return projectNodeID, outputTag, nil
			}
			...
			if !isUpdate {
				...
				if len(setNullFks) > 1 {
					...
					combinedNodeID, combinedTag, err = appendExcludeCascadeOwnedRows(combinedNodeID, combinedTag)
					if err != nil {
						return err
					}
					...
					upPlanCtx.fkSetNullColumns = make(map[string]struct{}, len(markerByColumn))
					for columnName := range markerByColumn {
			...
			for _, fk := range childTableDef.Fkeys {
				...
				if fk.ForeignTbl == delCtx.tableDef.TblId || fkSelfReferCond {
					...
					case plan.ForeignKeyDef_NO_ACTION, plan.ForeignKeyDef_RESTRICT, plan.ForeignKeyDef_SET_DEFAULT:
						...
						*/
						...
						if !taggedActionInput {
							joinNode.ProjectList = joinProjection
						}
					...
					case plan.ForeignKeyDef_SET_NULL:
						...
						lastNodeId, childScanTag, err = appendExcludeCascadeOwnedRows(lastNodeId, childScanTag)
						if err != nil {
							return err
						}
						if fkSelfReferCond && delCtx.skipTargetDelete {
							lastNodeId, err = appendExcludeReplaceOldRows(
								builder, bindCtx, lastNodeId,
								childScanTag, int32(childRowIdPos),
								parentActionStep, parentActionTag, int32(len(fk.Cols)))
							if err != nil {
								return err
							}
						}
						// inner join cannot dealwith null expr in projectList. so we append a project node
						taggedSetNullSource := delCtx.skipTargetDelete || (!isUpdate && delCtx.sourceTag != 0)
						projectProjection := getProjectionByLastNode(builder, lastNodeId)
						if taggedSetNullSource {
						...
						upPlanCtx.preserveUpdateSourceProjection = taggedSetNullSource
					...
					case plan.ForeignKeyDef_CASCADE:
						...
						// action path must explicitly collect self-referencing descendants.
						if !fkSelfReferCond || delCtx.skipTargetDelete {
							builder.qry.HasForeignKeyAction = true
							if isUpdate {
								...
								lastNodeId = appendAggNodeForFkJoin(
									builder, bindCtx, lastNodeId, fkSelfReferCond && delCtx.skipTargetDelete)
								newSourceStep := builder.appendStep(lastNodeId)
								...
								if childTableDef.TblId == delCtx.replaceTargetTableID &&
									!(fkSelfReferCond && delCtx.skipTargetDelete) {
									lastNodeId, err = appendExcludeReplaceOldRows(
										builder, bindCtx, lastNodeId,
										childScanTag, int32(childRowIdPos),
										delCtx.replaceOldRowsStep, 0, delCtx.replaceOldRowIDPos)
									if err != nil {
										return err
									}
									if tags := builder.qry.Nodes[lastNodeId].BindingTags; len(tags) == 1 {
										childActionTag = tags[0]
									}
								}
								if fkSelfReferCond && delCtx.skipTargetDelete {
								...
								upPlanCtx.isFkRecursionCall = true
								upPlanCtx.replaceOldRowsStep = delCtx.replaceOldRowsStep
								upPlanCtx.replaceTargetTableID = delCtx.replaceTargetTableID
								upPlanCtx.replaceOldRowIDPos = delCtx.replaceOldRowIDPos
...
// appendExcludeReplaceOldRows keeps cascade DELETE ownership disjoint from the
// old rows deleted by the main REPLACE MULTI_UPDATE.
func appendExcludeReplaceOldRows(
	builder *QueryBuilder,
	bindCtx *BindContext,
	inputNodeID int32,
	candidateTag int32,
	...
	candidateProjection := make([]*Expr, len(inputProject))
	for i, e := range inputProject {
		candidateProjection[i] = &Expr{
			Typ: e.Typ,
			Expr: &plan.Expr_Col{Col: &plan.ColRef{
				RelPos: candidateTag,
				ColPos: int32(i),
	...
	antiJoinID := builder.appendNode(&Node{
		NodeType: plan.Node_JOIN,
	...
	return builder.appendNode(&Node{
		NodeType:    plan.Node_PROJECT,
		Children:    []int32{antiJoinID},
		ProjectList: candidateProjection,
		BindingTags: []int32{candidateTag},
	}, bindCtx), nil
}
...
func appendAggNodeForFkJoin(
	...
	if preserveOutputTag {
		outputTag := builder.genNewBindTag()
		lastNodeId = builder.appendNode(&Node{
			NodeType: plan.Node_PROJECT,
			Children: []int32{lastNodeId},
			ProjectList: getProjectionByLastNodeWithTag(
				builder, lastNodeId, builder.qry.Nodes[lastNodeId].BindingTags[0]),
			BindingTags: []int32{outputTag},
		}, bindCtx)
		return appendSinkNodeWithTag(builder, bindCtx, lastNodeId, outputTag)
	}
	lastNodeId = appendSinkNode(builder, bindCtx, lastNodeId)
...
// complete descendant set for a self-referencing ON DELETE CASCADE. Recursive
// expansion keeps rows that are also replaced by the main REPLACE because they
// can still be parents of later descendants. Those rows are removed from the
// final delete stream after the descendant closure has been computed.
func appendSelfReferCascadeSource(
	...
	seedTag := builder.genNewBindTag()
	seedProject := make([]*Expr, len(builder.qry.Nodes[initialNodeID].ProjectList))
	for i, e := range builder.qry.Nodes[initialNodeID].ProjectList {
		seedProject[i] = &Expr{Typ: e.Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
			RelPos: seedTag, ColPos: int32(i),
		}}}
	}
	initialNodeID = builder.appendNode(&Node{
		NodeType: plan.Node_PROJECT, Children: []int32{initialNodeID},
		ProjectList: DeepCopyExprList(builder.qry.Nodes[initialNodeID].ProjectList),
		BindingTags: []int32{seedTag},
	}, bindCtx)
	oldRowsScanID := appendSinkScanNodeWithTag(builder, bindCtx, oldRowsSourceStep, oldRowsSourceTag)
	ownedTag := builder.genNewBindTag()
	oldRowsScanID = builder.appendNode(&Node{
		NodeType: plan.Node_PROJECT, Children: []int32{oldRowsScanID},
		ProjectList: DeepCopyExprList(builder.qry.Nodes[oldRowsScanID].ProjectList),
		BindingTags: []int32{ownedTag},
	}, bindCtx)
	seedRowID := &Expr{Typ: seedProject[rowIDPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
		RelPos: seedTag, ColPos: rowIDPos,
	}}}
	ownedRowID := &Expr{Typ: builder.qry.Nodes[oldRowsScanID].ProjectList[oldRowIDPos].Typ,
		Expr: &plan.Expr_Col{Col: &plan.ColRef{RelPos: ownedTag, ColPos: oldRowIDPos}}}
	ownedEqual, err := BindFuncExprImplByPlanExpr(
		builder.GetContext(), "=", []*Expr{seedRowID, ownedRowID})
	if err != nil {
		return 0, err
	}
	initialNodeID, emitInitial, err := builder.insertMarkJoin(
		initialNodeID, oldRowsScanID, []*Expr{ownedEqual}, nil, true, bindCtx)
	if err != nil {
		return 0, err
	}
	seedProject = append(seedProject, emitInitial)
	cteTag := builder.genNewBindTag()
	initialNodeID = builder.appendNode(&Node{
		NodeType:    plan.Node_PROJECT,
		Children:    []int32{initialNodeID},
		ProjectList: seedProject,
	...
	descendantJoinProject := make([]*Expr, len(childTableDef.Cols), len(childTableDef.Cols)+1)
	for i, col := range childTableDef.Cols {
	...
	descendantJoinProject = append(descendantJoinProject, makePlan2BoolConstExprWithType(true))
	...
	var recursiveCond *Expr
	for _, cascadeFK := range childTableDef.Fkeys {
		if cascadeFK.ForeignTbl != 0 || cascadeFK.OnDelete != plan.ForeignKeyDef_CASCADE ||
			len(cascadeFK.Cols) == 0 || len(cascadeFK.Cols) != len(cascadeFK.ForeignCols) {
			continue
		}
		var fkCond *Expr
		for i, childColID := range cascadeFK.Cols {
			childName := colIDToName(childTableDef, childColID)
			parentName := colIDToName(childTableDef, cascadeFK.ForeignCols[i])
			...
			leftExpr := &Expr{Typ: childTableDef.Cols[parentPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
				RelPos: cteTag, ColPos: parentPos, Name: parentName,
			}}}
			rightExpr := &Expr{Typ: childTableDef.Cols[childPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
				RelPos: descendantTag, ColPos: childPos, Name: childName,
			}}}
			cond, bindErr := BindFuncExprImplByPlanExpr(
				builder.GetContext(), "=", []*Expr{leftExpr, rightExpr})
			if bindErr != nil {
				return 0, bindErr
			}
			if fkCond == nil {
				...
				fkCond, bindErr = BindFuncExprImplByPlanExpr(
					builder.GetContext(), "and", []*Expr{fkCond, cond})
				if bindErr != nil {
					return 0, bindErr
				}
			}
		}
		if recursiveCond == nil {
			recursiveCond = fkCond
		} else {
			var bindErr error
			recursiveCond, bindErr = BindFuncExprImplByPlanExpr(
				builder.GetContext(), "or", []*Expr{recursiveCond, fkCond})
			if bindErr != nil {
				return 0, bindErr
			}
		}
	...
	if recursiveCond == nil {
		return 0, moerr.NewInternalErrorf(
			builder.GetContext(), "self-referencing cascade has no recursive edge")
	}
	recursiveJoinID := builder.appendNode(&Node{
		NodeType:    plan.Node_JOIN,
		Children:    []int32{recursiveScanID, descendantScanID},
		JoinType:    plan.Node_INNER,
		OnList:      []*Expr{recursiveCond},
		ProjectList: descendantJoinProject,
	}, bindCtx)
	recursiveJoinID, err = appendExcludeReplaceOldRows(
	...
	builder.preserveScanProjection[lastNodeID] = struct{}{}
	emitFilter := &Expr{Typ: plan.Type{Id: int32(types.T_bool), NotNullable: true},
		Expr: &plan.Expr_Col{Col: &plan.ColRef{RelPos: cteTag, ColPos: int32(len(childTableDef.Cols))}}}
	lastNodeID = builder.appendNode(&Node{
		NodeType: plan.Node_FILTER, Children: []int32{lastNodeID},
		FilterList:  []*Expr{emitFilter},
		ProjectList: getProjectionByLastNodeWithTag(builder, lastNodeID, cteTag),
		BindingTags: []int32{cteTag},
	}, bindCtx)
	lastNodeID = builder.appendNode(&Node{
		NodeType:    plan.Node_PROJECT,
	...
	groupByList := getProjectionByLastNodeWithTag(builder, lastNodeID, cteTag)
	aggProject := make([]*Expr, len(groupByList))
	for i, e := range groupByList {
		aggProject[i] = &Expr{
			Typ: e.Typ,
	...
	filteredTag := builder.genNewBindTag()
	lastNodeID = builder.appendNode(&Node{
		NodeType: plan.Node_PROJECT,
		Children: []int32{lastNodeID},
		ProjectList: getProjectionByLastNodeWithTag(
			builder, lastNodeID, builder.qry.Nodes[lastNodeID].BindingTags[0]),
		BindingTags: []int32{filteredTag},
	}, bindCtx)
	return appendSinkNodeWithTag(builder, bindCtx, lastNodeID, filteredTag), nil
}
...
func makeOneInsertPlan(
	...
	if ifCheckPkDup {
		if err = appendPrimaryConstraintPlan(builder, bindCtx, tableDef, objRef, partitionExpr, pkFilterExprs,
			indexSourceColTypes, sourceStep, updateColLength > 0, updatePkCol, ifInsertFromUnique, fuzzymessage,
			isFkRecursionCall); err != nil {
			return err
		}
	}
...
func makeOneDeletePlan(
	...
	inputTag := int32(0)
	if input := builder.qry.Nodes[lastNodeId]; len(input.BindingTags) == 1 {
		inputTag = input.BindingTags[0]
	}
	if isUK || isSK {
	...
	if (isUK || isSK) && inputTag != 0 {
		if builder.positionalDeleteInputRefs == nil {
			builder.positionalDeleteInputRefs = make(map[int32]deleteInputRefs)
		}
		builder.positionalDeleteInputRefs[lastNodeId] = deleteInputRefs{
			rowID:      [2]int32{inputTag, int32(delNodeInfo.deleteIndex)},
			primaryKey: [2]int32{inputTag, int32(delNodeInfo.pkPos)},
		}
	}
...
func getProjectionByLastNodeForRightJoin(builder *QueryBuilder, lastNodeId int32) []*Expr {
	...
	if len(lastNode.ProjectList) == 0 {
		return getProjectionByLastNodeForRightJoin(builder, lastNode.Children[0])
	}
	projection := make([]*Expr, len(lastNode.ProjectList))
	for i, expr := range lastNode.ProjectList {
		name := ""
		...
		projection[i] = &plan.Expr{Typ: expr.Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
			RelPos: 1, ColPos: int32(i), Name: name,
		}}}
	}
	return projection
}
...
func appendLegacyDeleteIndexTablePlan(
	builder *QueryBuilder,
	bindCtx *BindContext,
	...
	var rowIDPos, pkPos int32 = -1, -1
	scanProjection := make([]*Expr, len(uniqueTableDef.Cols))
	for i, col := range uniqueTableDef.Cols {
		if col.Name == catalog.Row_ID {
			rowIDPos = int32(i)
		} else if col.Name == catalog.IndexTableIndexColName {
			pkPos = int32(i)
		}
		scanProjection[i] = &plan.Expr{Typ: col.Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
			ColPos: int32(i), Name: col.Name,
		}}}
	}
	leftScan := &plan.Node{
		NodeType: plan.Node_TABLE_SCAN, Stats: &plan.Stats{}, ObjRef: uniqueObjRef,
		TableDef: uniqueTableDef, ProjectList: scanProjection,
	}
	leftID := builder.appendNode(leftScan, bindCtx)
	projectList = append(projectList,
		&plan.Expr{Typ: uniqueTableDef.Cols[rowIDPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
			RelPos: 0, ColPos: rowIDPos, Name: catalog.Row_ID,
		}}},
		&plan.Expr{Typ: uniqueTableDef.Cols[pkPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
			RelPos: 0, ColPos: pkPos, Name: catalog.IndexTableIndexColName,
		}}},
	)
	rightExpr := &plan.Expr{Typ: uniqueTableDef.Cols[pkPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
		RelPos: 0, ColPos: pkPos, Name: catalog.IndexTableIndexColName,
	}}}
	...
	if len(indexdef.Parts) == 1 {
		...
		leftExpr, err = builder.makeIndexPartExpr(1, int32(posMap[name]), name, typMap[name], prefixLengths)
	} else {
		args := make([]*Expr, len(indexdef.Parts))
		for i, part := range indexdef.Parts {
			name := catalog.ResolveAlias(part)
			args[i], err = builder.makeIndexPartExpr(1, int32(posMap[name]), name, typMap[name], prefixLengths)
			if err != nil {
				return -1, err
			}
		}
		...
		leftExpr, err = BindFuncExprImplByPlanExpr(builder.GetContext(), fn, args)
	}
	if err != nil {
		return -1, err
	}
	cond, err := BindFuncExprImplByPlanExpr(builder.GetContext(), "=", []*Expr{rightExpr, leftExpr})
	if err != nil {
		return -1, err
	}
	probeExpr := &plan.Expr{Typ: uniqueTableDef.Cols[pkPos].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{
		Name: uniqueTableDef.Pkey.PkeyColName,
	}}}
	buildExpr := &plan.Expr{Typ: cond.GetF().Args[1].Typ, Expr: &plan.Expr_Col{Col: &plan.ColRef{ColPos: 0}}}
	probeSpec, buildSpec, hasRuntimeFilter := builder.makeExactRuntimeFilterPair(
		rfTag, false,
		GetInFilterCardLimitOnPK(builder.compCtx.GetProcess().GetService(), leftScan.Stats.TableCnt),
		probeExpr, buildExpr, false,
	)
	if disableRuntimeFilter {
	...
	joinNode := &plan.Node{
		NodeType: plan.Node_JOIN, Children: []int32{leftID, lastNodeId}, JoinType: plan.Node_RIGHT,
		IsRightJoin: true, OnList: []*Expr{cond}, ProjectList: projectList,
	}
	if hasRuntimeFilter {
	...
	lastNodeId = builder.appendNode(joinNode, bindCtx)
	if hasRuntimeFilter {
		recalcStatsByRuntimeFilter(leftScan, joinNode, builder)
	}
	return lastNodeId, nil
}
...
func buildDeleteRegularIndex(ctx CompilerContext, builder *QueryBuilder, bindCtx *BindContext, delCtx *dmlPlanCtx,
	...
	********/
	...
	var uniqueDeleteRefs deleteInputRefs
	...
	if delCtx.isDeleteWithoutFilters {
		...
		lastNodeId = appendSinkScanNode(builder, bindCtx, delCtx.sourceStep)
		if delCtx.sourceTag != 0 || delCtx.preserveUpdateSourceProjection {
			lastNodeId, err = appendDeleteIndexTablePlan(
				builder, bindCtx, uniqueObjRef, uniqueTableDef, indexdef, typMap, posMap, lastNodeId, isUk,
			)
		} else {
			lastNodeId, err = appendLegacyDeleteIndexTablePlan(
				builder, bindCtx, uniqueObjRef, uniqueTableDef, indexdef, typMap, posMap, lastNodeId, isUk,
				delCtx.isFkRecursionCall,
			)
		}
		projectionLen := len(builder.qry.Nodes[lastNodeId].ProjectList)
		if projectionLen < 2 {
			return moerr.NewInternalErrorf(builder.GetContext(),
				"hidden-index delete projection is missing rowid and primary-key columns")
		}
		uniqueDeleteIdx = projectionLen - 2
		uniqueTblPkPos = projectionLen - 1
		uniqueTblPkTyp = uniqueTableDef.Cols[0].Typ
		if rowIDCol := builder.qry.Nodes[lastNodeId].ProjectList[uniqueDeleteIdx].GetCol(); rowIDCol != nil {
			uniqueDeleteRefs.rowID = [2]int32{rowIDCol.RelPos, rowIDCol.ColPos}
		}
		if primaryKeyCol := builder.qry.Nodes[lastNodeId].ProjectList[uniqueTblPkPos].GetCol(); primaryKeyCol != nil {
			uniqueDeleteRefs.primaryKey = [2]int32{primaryKeyCol.RelPos, primaryKeyCol.ColPos}
		}
	}
	if err != nil {
		return err
	...
	if isUpdate {
		...
		// do it like simple update
		indexDeleteTag := int32(0)
		if input := builder.qry.Nodes[lastNodeId]; len(input.BindingTags) == 1 {
			lastNodeId = appendSinkNodeWithTag(builder, bindCtx, lastNodeId, input.BindingTags[0])
			indexDeleteTag = builder.genNewBindTag()
		} else {
			lastNodeId = appendSinkNode(builder, bindCtx, lastNodeId)
		}
		newSourceStep := builder.appendStep(lastNodeId)
		// delete uk plan
		{
			//sink_scan -> lock -> delete
			if indexDeleteTag != 0 {
				lastNodeId = builder.appendTaggedSinkScan(bindCtx, newSourceStep, indexDeleteTag)
			} else {
				lastNodeId = appendSinkScanNode(builder, bindCtx, newSourceStep)
			}
		...
		// A UNIQUE key containing an unconditionally SET NULL column has no
		// replacement hidden-index row.  Omitting the empty insert/check fanout
		// also avoids creating a sink step with no physical consumer.
		if !skipIndexInsert {
			lastNodeId = appendSinkScanNode(builder, bindCtx, newSourceStep)
			lastProject := builder.qry.Nodes[lastNodeId].ProjectList
		...
		if delCtx.sourceTag != 0 && uniqueDeleteRefs.rowID != [2]int32{} && uniqueDeleteRefs.primaryKey != [2]int32{} {
			deleteRefs := builder.positionalDeleteInputRefs[lastNodeId]
			deleteRefs.sourceRowID = uniqueDeleteRefs.rowID
			deleteRefs.sourcePrimaryKey = uniqueDeleteRefs.primaryKey
			builder.positionalDeleteInputRefs[lastNodeId] = deleteRefs
		}
		builder.appendStep(lastNodeId)
	}
