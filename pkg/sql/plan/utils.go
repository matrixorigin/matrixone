...
func FillValuesOfParamsInPlanWithSpecialization(
	...
	return fillValuesOfParamsInPlanWithSpecializationSelected(ctx, preparePlan, paramVals, nil)
}

// FillValuesOfParamsInPlanWithSpecializationAtPositions limits execute-time
// rebinding to the supplied parameter positions. This keeps later direct-result
// metadata fixes independent from the reverted generic specialization gate.
func FillValuesOfParamsInPlanWithSpecializationAtPositions(
	...
	return fillValuesOfParamsInPlanWithSpecializationSelected(ctx, preparePlan, paramVals, selected)
}

// FillValuesOfParamsInPlanWithPreparedNumericOverload preserves the later
// prepared numeric-overload fix while using the pre-#27493 specialization walk.
...
func fillValuesOfParamsInPlanWithSpecializationSelected(
	...
	case *plan.Plan_Ddl:
		if pp.Ddl.Query != nil {
			...
			specialized, err := replaceParamValsWithSelection(
				ctx, queryPlan, effectiveParamVals, selected)
			if err != nil {
				return nil, false, err
			}
	...
	case *plan.Plan_Query, *plan.Plan_Dcl:
		specialized, err := replaceParamValsWithSelection(
			ctx, copied, effectiveParamVals, selected)
		if err != nil {
			return nil, false, err
		}
...
func replaceParamVals(
	...
	return replaceParamValsWithSelection(ctx, plan0, paramVals, nil)
}
