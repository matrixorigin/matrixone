...
// PreparedPlanRuntimeTextComparisonParamPositions returns the parameter
// positions whose binary runtime type can turn a prepared comparison into a
// text-vs-numeric comparison. The result is intended to be computed once per
// prepared-plan generation. Executions whose values at these positions are
// non-text can stay on the cached compile path without walking the plan.
func PreparedPlanRuntimeTextComparisonParamPositions(
	preparePlan *Plan,
	paramCount int,
) []int32 {
	if preparePlan == nil || paramCount <= 0 {
		return nil
	}

	// Probe one text position at a time while treating the other markers as
	// numeric. This distinguishes a numeric-column/marker or marker/marker
	// comparison from an ordinary string-column/marker comparison, and reuses
	// the exact admission rules used by execute-time specialization.
	runtimeParamTypes := make([]types.Type, paramCount)
	for i := range runtimeParamTypes {
		runtimeParamTypes[i] = types.T_int64.ToType()
	}
	positions := make([]int32, 0)
	for candidate := range runtimeParamTypes {
		runtimeParamTypes[candidate] = types.T_text.ToType()
		if PreparedPlanNeedsRuntimeTextComparisonSpecialization(preparePlan, runtimeParamTypes) {
			positions = append(positions, int32(candidate))
		}
		runtimeParamTypes[candidate] = types.T_int64.ToType()
	}
	return positions
}
