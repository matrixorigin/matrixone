...
func TestBindSQLUDFUsesStoredNoUnsignedSubtractionMode(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// Binding a stored UDF must not leak its SQL mode into the caller's
			// binder.  Rebind the same unsigned subtraction after the UDF body and
			// verify that the caller mode is restored in both directions.
			stmt, err := mysql.ParseOne(context.Background(), "select cast(0 as unsigned) - 1", 1)
			require.NoError(t, err)
			defer stmt.Free()
			direct, err := binder.BindExpr(stmt.(*tree.Select).Select.(*tree.SelectClause).Exprs[0].Expr, 0, false)
			require.NoError(t, err)
			callerWant := types.T_uint64
			if tc.callerMode {
				callerWant = types.T_int64
			}
			require.Equal(t, int32(callerWant), direct.Typ.Id)
		})
	}
}
