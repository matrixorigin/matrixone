...
func TestPersistedExpressionProtocolAdmissionForStatementHash(t *testing.T) {
	proc := testutil.NewProcess(t)
	rt := moruntime.ServiceRuntime(proc.GetService())
	oldProtocol, hadProtocol := rt.GetGlobalVariables(moruntime.MOProtocolVersion)
	oldReadFloor, hadReadFloor := rt.GetGlobalVariables(moruntime.PersistedExpressionProtocolFloor)
	...
	expr := &planpb.Expr{
		...
		Expr: &planpb.Expr_F{F: &planpb.Function{Func: &planpb.ObjectRef{
			Obj:     int64(function.MO_STATEMENT_HASH) << 32,
	...
	required, err := RequiredPersistedExpressionProtocolVersion(expr)
	require.NoError(t, err)
	require.Equal(t, int64(defines.MORPCVersion92), required)

	// A catalog-bound view/default must retain the hash feature's v92 floor;
	...
	require.ErrorContains(t,
		RequirePersistedExpressionProtocol(proc.Ctx, proc, owner), "protocol version 92")
	require.ErrorContains(t,
		RequirePersistedExpressionProtocolForAuthoring(proc.Ctx, proc, owner), "protocol version 92")
