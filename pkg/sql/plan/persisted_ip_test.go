...
func TestPersistedDecimalLiteralTargetTypedDefaultAdmission(t *testing.T) {
	...
	// The provenance marker must still fence the persisted default from v83
	// readers and writers.
...
func TestPersistedDecimalLiteralUsesDedicatedEpochInMixedOwner(t *testing.T) {
	...
	ipExpr := &planpb.Expr{
		...
		Expr: &planpb.Expr_F{F: &planpb.Function{Func: &planpb.ObjectRef{
			Obj: function.EncodeOverloadID(function.INET_NTOA, 9),
		}}},
	}
	...
	for _, tc := range []struct {
		...
		{"IP only", []*planpb.Expr{ipExpr}, defines.MORPCVersion86},
		{"decimal only", []*planpb.Expr{decimalExpr}, defines.MORPCVersion87},
		{"IP then decimal", []*planpb.Expr{ipExpr, decimalExpr}, defines.MORPCVersion87},
		{"decimal then IP", []*planpb.Expr{decimalExpr, ipExpr}, defines.MORPCVersion87},
		...
		{"IP decimal spatial", []*planpb.Expr{ipExpr, decimalExpr, spatialExpr}, defines.MORPCVersion88},
		{"spatial decimal IP", []*planpb.Expr{spatialExpr, decimalExpr, ipExpr}, defines.MORPCVersion88},
	} {
