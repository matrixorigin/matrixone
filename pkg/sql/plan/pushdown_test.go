...
// setupDedupJoinBase builds a two-scan DEDUP join. When withUpdateCtx is true the
// join carries a DedupJoinCtx, marking it an ON DUPLICATE KEY UPDATE / REPLACE
// update-dedup that NULL-extends its left (old-table) side.
func setupDedupJoinBase(t *testing.T, withUpdateCtx bool) (*MockCompilerContext, *QueryBuilder, *plan.Expr) {
	t.Helper()
	...
	leftValCol := &plan.Expr{
		Typ:  intType,
		Expr: &plan.Expr_Col{Col: &plan.ColRef{RelPos: leftTag, ColPos: 1}},
	}
	rightIDCol := &plan.Expr{
		Typ:  intType,
	...
	joinNode := &plan.Node{
		...
		OnList:   []*plan.Expr{onExpr},
		ProjectList: []*plan.Expr{
			DeepCopyExpr(leftIDCol), DeepCopyExpr(leftValCol), DeepCopyExpr(rightIDCol),
		},
	}
	if withUpdateCtx {
		joinNode.DedupJoinCtx = &plan.DedupJoinCtx{
			OldColList: []plan.ColRef{{RelPos: leftTag, ColPos: 0}},
		}
	}
	...
	builder.qry.Nodes = []*plan.Node{
		{
			NodeType:    plan.Node_TABLE_SCAN,
			BindingTags: []int32{leftTag},
			ProjectList: []*plan.Expr{DeepCopyExpr(leftIDCol), DeepCopyExpr(leftValCol)},
		},
		{
			NodeType:    plan.Node_TABLE_SCAN,
			BindingTags: []int32{rightTag},
			ProjectList: []*plan.Expr{DeepCopyExpr(rightIDCol)},
		},
		joinNode,
	}

	// Left-only filter (references only the old-table side), like the ODKU no-op
	// guard when the update writes a column to a function of its old value.
	leftFilter, err := BindFuncExprImplByPlanExpr(ctx.GetContext(), "=", []*plan.Expr{
		DeepCopyExpr(leftValCol),
		{Typ: intType, Expr: &plan.Expr_Lit{Lit: &plan.Literal{Value: &plan.Literal_I64Val{I64Val: 11}}}},
	})
	require.NoError(t, err)

	return ctx, builder, leftFilter
}

// TestDedupUpdateJoinKeepsLeftFilterAboveJoin verifies that a left-only filter
// is NOT pushed below a DEDUP-update join (which NULL-extends its old-table
// side), but is still pushed below a plain DEDUP join.
func TestDedupUpdateJoinKeepsLeftFilterAboveJoin(t *testing.T) {
	t.Run("DedupJoinCtx blocks left-filter pushdown", func(t *testing.T) {
		_, builder, leftFilter := setupDedupJoinBase(t, true)
		nodeID, cantPushdown := builder.pushdownFilters(2, []*plan.Expr{leftFilter}, false)
		require.Equal(t, int32(2), nodeID)
		require.Len(t, cantPushdown, 1, "left filter must stay above the dedup-update join")
		require.Same(t, leftFilter, cantPushdown[0])
		require.Empty(t, builder.qry.Nodes[0].FilterList, "left scan must not receive the filter")
	})

	t.Run("plain DEDUP still pushes left filter down", func(t *testing.T) {
		_, builder, leftFilter := setupDedupJoinBase(t, false)
		_, cantPushdown := builder.pushdownFilters(2, []*plan.Expr{leftFilter}, false)
		require.Empty(t, cantPushdown)
		require.Len(t, builder.qry.Nodes[0].FilterList, 1, "plain dedup join keeps left-filter pushdown")
	})
}
