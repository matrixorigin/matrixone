...
func (builder *QueryBuilder) buildSelect(stmt *tree.Select, ctx *BindContext, isRoot bool) (int32, error) {
		...
		if !ctx.isTryBindingCTE && ctx.recSelect {
			f := &tree.FuncExpr{
				...
				Exprs: tree.Exprs{tree.NewComparisonExpr(
					...
					tree.NewNumValWithType2(constant.MakeInt64(moDefaultRecursionMax), int64(moDefaultRecursionMax), fmt.Sprintf("%d", moDefaultRecursionMax), false, tree.P_int64),
				)},
			}
			if clause.Where != nil {
	...
	if astTimeWindow != nil {
		...
		arg2 := tree.NewNumValWithType2(constant.MakeString(astTimeWindow.Interval.Unit), astTimeWindow.Interval.Unit, astTimeWindow.Interval.Unit, false, tree.P_char)
		itr := &tree.FuncExpr{
		...
		if astTimeWindow.Sliding != nil {
			arg2 = tree.NewNumValWithType2(constant.MakeString(astTimeWindow.Sliding.Unit), astTimeWindow.Sliding.Unit, astTimeWindow.Sliding.Unit, false, tree.P_char)
			sld := &tree.FuncExpr{
				Func:  tree.FuncName2ResolvableFunctionReference(name),
			...
			if astTimeWindow.Fill.Mode == tree.FillLinear {
				for i, e := range ctx.timeAsts {
					b := &tree.BinaryExpr{
						...
						Right: tree.NewNumValWithType2(constant.MakeInt64(2), int64(2), "2", false, tree.P_int64),
					}
		...
		if midNode.NodeType == plan.Node_TABLE_SCAN {
			...
			if currentAccountID != catalog.System_Account {
				...
				if dbName == catalog.MO_CATALOG && tableName == catalog.MO_DATABASE {
					...
					right := tree.NewNumValWithType2(constant.MakeUint64(uint64(currentAccountID)), uint64(currentAccountID), strconv.Itoa(int(currentAccountID)), false, tree.P_uint64)
