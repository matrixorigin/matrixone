...
// genSqlForCheckFKConstraintsOnReplaceRows narrows the post-action FK check to
// rows produced by a literal REPLACE VALUES source. Historical orphan rows are
// outside that statement's validation domain and must not reject an unrelated
// replacement. Dynamic sources fall back to the caller's regular check because
// their evaluated row image is not available as SQL text.
func genSqlForCheckFKConstraintsOnReplaceRows(
	ctx context.Context,
	fkey *plan.ForeignKeyDef,
	childDbName, childTblName string,
	childTableDef *plan.TableDef,
	parentDbName, parentTblName string,
	parentCols []*plan.ColDef,
	stmt *tree.Replace,
) (string, bool, error) {
	if stmt.Rows == nil || childTableDef.Pkey == nil ||
		childTableDef.Pkey.PkeyColName == catalog.FakePrimaryKeyColName ||
		len(childTableDef.Pkey.Names) == 0 {
		return "", false, nil
	}
	...
	columnPositions := make(map[string]int, len(childTableDef.Cols))
	if len(stmt.Columns) > 0 {
		for pos, col := range stmt.Columns {
			columnPositions[strings.ToLower(string(col))] = pos
		}
	} else {
		pos := 0
		for _, col := range childTableDef.Cols {
			...
			columnPositions[strings.ToLower(col.Name)] = pos
			pos++
		}
	}

	isLiteral := func(expr tree.Expr) bool {
		switch expr.(type) {
	...
	rowPredicates := make([]string, 0, len(valuesClause.Rows))
	for _, row := range valuesClause.Rows {
		parts := make([]string, 0, len(childTableDef.Pkey.Names))
		for _, pkName := range childTableDef.Pkey.Names {
			pos, exists := columnPositions[strings.ToLower(catalog.ResolveAlias(pkName))]
			if !exists || pos >= len(row) || !isLiteral(row[pos]) {
				return "", false, nil
			}
			parts = append(parts, fmt.Sprintf("`%s`.`%s` = %s",
				childTblName, catalog.ResolveAlias(pkName), tree.String(row[pos], dialect.MYSQL)))
		}
		rowPredicates = append(rowPredicates, "("+strings.Join(parts, " and ")+")")
	}

	fkCols, err := colIdsToNames(ctx, fkey.Cols, childTableDef.Cols)
	if err != nil {
		return "", false, err
	}
	referCols, err := colIdsToNames(ctx, fkey.ForeignCols, parentCols)
	if err != nil {
		return "", false, err
	}
	childTableClause := fmt.Sprintf("`%s`.`%s`", childDbName, childTblName)
	parentTableClause := fmt.Sprintf("`%s`.`%s`", parentDbName, parentTblName)
		...
		"select distinct %s from %s where (%s) and %s except select distinct %s from %s",
		makeSelectList(childTblName, fkCols), childTableClause,
		strings.Join(rowPredicates, " or "), makeWhere(childTblName, fkCols),
		makeSelectList(parentTblName, referCols), parentTableClause)
	return strings.Join([]string{
		"select count(*) = 0 from (", except, ") as __mo_fk_check_source",
	}, " "), true, nil
}
