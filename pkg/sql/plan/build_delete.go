...
func deleteToSelect(node *tree.Delete, dialectType dialect.DialectType, info *tableInfo, rewriteInfo *rewriteSelectInfo) string {
	...
	prefix := ""
	var column string
	returnAll := rewriteInfo.projects != ""
	for _, tbl := range info.tableNames {
		ctx.WriteString(prefix)
		if returnAll {
			column = fmt.Sprintf("%s.*", tbl) //有bug，名称重复问题
		} else {
			column = fmt.Sprintf("%s.%s", tbl, catalog.Row_ID)
		}
...
const derivedTableName = "_t_"

type rewriteSelectInfo struct {
	projects              string
	leftJoins             string
	idx                   int
	onDeleteIdx           []int
...
func buildSingleTableDelete2(ctx CompilerContext, stmt *tree.Delete) (*Plan, error) {
	tblInfo, err := getTableInfoForUpdate(stmt.Tables, ctx)
	if err != nil {
		return nil, err
	}
	...
	rewriteInfo := &rewriteSelectInfo{
		idx: len(tblInfo.tableNames),
	}
	for _, tableDef := range tblInfo.tableDefs {
		err := rewriteDeleteSelect(ctx, tableDef, rewriteInfo)
		if err != nil {
			return nil, err
		}
	}

	sql := deleteToSelect(stmt, dialect.MYSQL, tblInfo, rewriteInfo)
	if rewriteInfo.projects != "" {
		project := "select"
		for range tblInfo.tableNames {
			project = project + " " + derivedTableName + "." + catalog.Row_ID //这里有bug，多表的问题
		}
		sql = fmt.Sprintf("%s %s from (%s) %s %s", project, rewriteInfo.projects, sql, derivedTableName, rewriteInfo.leftJoins)
	}

	stmts, err := mysql.Parse(ctx.GetContext(), sql)
	if err != nil {
		return nil, err
	}
	if len(stmts) > 1 {
	...
	rootId, err := builder.buildSelect(stmts[0].(*tree.Select), bindCtx, false)
