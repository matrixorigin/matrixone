...
func (b *baseBinder) bindIntegerSourceAst(ast tree.Expr, depth int32, target types.T, name string, position int) (*Expr, error) {
	...
	if name != "" {
		return appendIntegerDomainArgument(b.GetContext(), name, position, source, false)
	}
...
func (b *baseBinder) bindIntegerDomainArgumentAst(name string, position int, ast tree.Expr, depth int32) (*Expr, error) {
	if function.IntegerArgumentUsesBitSources(name, position) || function.IntegerArgumentHasSourceRestrictions(name, position) {
		return b.bindIntegerSourceAst(ast, depth, 0, name, position)
	}
	previous := b.integerArgumentSourceContext
	b.integerArgumentSourceContext = true
	defer func() { b.integerArgumentSourceContext = previous }()
	source, err := b.impl.BindExpr(ast, depth, false)
	if err != nil {
		return nil, err
	}
	_, applies := function.IntegerArgumentTargetForSource(name, position, types.T(source.Typ.Id), isBinaryNumericLiteral(source))
	if !applies {
		return source, nil
	}
	...
	case *tree.CaseExpr:
		selectBranches = true
	case *tree.FuncExpr:
		fn := numericAstFunctionName(value)
		selectBranches = fn == "if" || fn == "iff" || fn == "nullif"
	}
	return appendIntegerDomainArgument(b.GetContext(), name, position, source, selectBranches)
}

// rebindIntegerDomainArgument restores only this consumer's integer value
// flow. Private conversions inside arithmetic or another function retain that
// function's own contract. An outer private CAST remains an opaque value
// boundary (notably IFNULL/COALESCE), even when its result domain changes.
func (rule *ResetParamRefRule) rebindIntegerDomainArgument(name string, position int, expr *Expr) (*Expr, error) {
	if function.IntegerArgumentUsesBitSources(name, position) {
		if fn := expr.GetF(); fn != nil && fn.Func != nil && len(fn.Args) == 2 && !fn.SyntaxExplicitCast && types.T(expr.Typ.Id) == types.T_uint64 && types.T(fn.Args[0].Typ.Id) == types.T_int64 {
			id, overload := function.DecodeOverloadID(fn.Func.Obj)
			if id == function.CAST && overload == 1 && isIntegerArgumentCast(fn.Args[0]) {
				return rule.rebindIntegerDomainArgument(name, position, fn.Args[0])
			}
		}
	}
	if isIntegerArgumentCast(expr) {
		source, err := rule.integerArgumentRuntimeSource(expr.GetF().Args[0])
		if err != nil {
			return nil, err
		}
		...
		target, applies := function.IntegerArgumentTargetForSource(name, position, types.T(source.Typ.Id), isBinaryNumericLiteral(source))
		if !applies {
			return source, nil
		}
		if !function.IntegerArgumentSourceAllowed(name, position, types.T(source.Typ.Id)) {
			return nil, moerr.NewInvalidInputf(rule.ctx, "unsupported integer argument source %s for %s", types.T(source.Typ.Id), name)
		}
		_, overload := function.DecodeOverloadID(expr.GetF().Func.Obj)
		if overload == function.TextIntegerBitsCastOverload {
			overload = function.IntegerArgumentCastOverload
		}
		if function.IntegerArgumentUsesBitSources(name, position) && types.T(source.Typ.Id).IsMySQLString() {
			overload = function.TextIntegerBitsCastOverload
		}
		if function.IntegerArgumentUsesExtendedSources(name, position) && overload != function.TruncatedIntegerArgumentCastOverload {
			overload = function.TemporalIntegerArgumentCastOverload
		}
		typ := target.ToType()
		return appendCastBeforeExprWithOverload(rule.ctx, source, makePlan2Type(&typ), overload)
	}
	if fn := expr.GetF(); fn != nil && fn.Func != nil && (fn.Func.ObjName == "case" || fn.Func.ObjName == "if" || fn.Func.ObjName == "iff") {
		...
		for i, arg := range fn.Args {
			...
			if i%2 == 0 && i != len(args)-1 {
				...
				args[i], err = rule.rebindIntegerDomainArgument(name, position, arg)
				if err == nil && args[i].GetLit().GetIsnull() {
					// A prepare-time NULL envelope must not force the live
					// unsigned value branch back into a signed/text domain.
					args[i] = makePlan2NullConstExprWithType()
				}
			}
			if err != nil {
				return nil, err
		...
		return bindIntegerSelector(rule.ctx, args, function.IntegerArgumentUsesBitSources(name, position))
	}
	if isImplicitPreparedParamCast(expr) {
		return rule.integerArgumentRuntimeSource(expr.GetF().Args[0])
	}
	return rule.integerArgumentRuntimeSource(expr)
}
...
func appendIntegerDomainArgument(ctx context.Context, name string, position int, source *Expr, selectBranches bool) (*Expr, error) {
	target, ok := function.IntegerArgumentTargetForSource(name, position, types.T(source.Typ.Id), isBinaryNumericLiteral(source))
	if !ok {
		return source, nil
	}
	if raw, direct := storedMySQLSpecialTypeExpr(source); direct {
		source = raw
		target, _ = function.IntegerArgumentTargetForSource(name, position, types.T(source.Typ.Id), isBinaryNumericLiteral(source))
	}
	if !function.IntegerArgumentSourceAllowed(name, position, types.T(source.Typ.Id)) {
		return nil, moerr.NewInvalidInputf(ctx, "unsupported integer argument source %s for %s", types.T(source.Typ.Id), name)
	}
	if function.IntegerArgumentUsesBitSources(name, position) && types.T(source.Typ.Id).IsMySQLString() {
	...
	if function.IntegerArgumentHasSourceRestrictions(name, position) && types.T(source.Typ.Id) != target {
		if fn := source.GetF(); selectBranches && fn != nil && fn.Func != nil && (fn.Func.ObjName == "case" || fn.Func.ObjName == "if" || fn.Func.ObjName == "iff") {
			args := append([]*Expr(nil), fn.Args...)
			for i, arg := range args {
				if i%2 == 0 && i != len(args)-1 {
					continue
				}
				bound, err := appendIntegerDomainArgument(ctx, name, position, stripIntegerSelectionReconciliation(arg), true)
				if err != nil {
					return nil, err
				}
				args[i] = bound
			}
			return bindIntegerSelector(ctx, args, false)
		}
		overload := integerSourceCastOverload(source)
		if overload == function.IntegerArgumentCastOverload && function.IntegerArgumentUsesExtendedSources(name, position) {
			overload = function.TemporalIntegerArgumentCastOverload
		}
		typ := target.ToType()
		return appendCastBeforeExprWithOverload(ctx, source, makePlan2Type(&typ), overload)
	}
	return appendIntegerArgument(ctx, source, target, selectBranches)
}
...
func bindIntegerFunctionArguments(ctx context.Context, name string, args []*Expr) ([]*Expr, error) {
	...
	for i, arg := range args {
		...
		_, ok := function.IntegerArgumentTargetForSource(name, i, types.T(arg.Typ.Id), isBinaryNumericLiteral(arg))
		if !ok {
			continue
		}
		bound, err := appendIntegerDomainArgument(ctx, name, i, arg, true)
		if err != nil {
			return nil, err
		}
		if bound == arg {
