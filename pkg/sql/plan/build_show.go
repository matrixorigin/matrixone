...
func buildShowTables(stmt *tree.ShowTables, ctx CompilerContext) (*Plan, error) {
	...
	sql = fmt.Sprintf("SELECT relname as `Tables_in_%s` %s FROM %s.mo_tables %s WHERE reldatabase = '%s' and relname != '%s' and relname not like '%s' and relname not like '%s' and relkind != '%s' and (%s)",
		subName, tableType, MO_CATALOG_DB_NAME, snapshotSpec, dbName, catalog.MOAutoIncrTable, catalog.IndexTableNamePrefix+"%", catalog.HiddenTableNamePrefix+"%", catalog.SystemPartitionRel, accountClause)
...
func buildShowTableNumber(stmt *tree.ShowTableNumber, ctx CompilerContext) (*Plan, error) {
	...
	clusterTable := fmt.Sprintf(" or relkind = '%s'", catalog.SystemClusterRel)
	accountClause := fmt.Sprintf("account_id = %v or (account_id = 0 and (%s))", accountId, mustShowTable+clusterTable)
	sql := fmt.Sprintf("SELECT count(relname) `Number of tables in %s` FROM %s.mo_tables WHERE reldatabase = '%s' and relname != '%s' and relname not like '%s' and relname not like '%s' and (%s)",
		subName, MO_CATALOG_DB_NAME, dbName, catalog.MOAutoIncrTable, catalog.IndexTableNamePrefix+"%", catalog.HiddenTableNamePrefix+"%", accountClause)
...
func buildShowTableStatus(stmt *tree.ShowTableStatus, ctx CompilerContext) (*Plan, error) {
	...
	sql := `select
			...
			where
				...
				and relname not like '%s'
				and (%s)`
	sql = fmt.Sprintf(sql, MO_CATALOG_DB_NAME, dbName, catalog.SystemPartitionRel, catalog.MOAutoIncrTable, catalog.IndexTableNamePrefix+"%", catalog.HiddenTableNamePrefix+"%", accountClause)
