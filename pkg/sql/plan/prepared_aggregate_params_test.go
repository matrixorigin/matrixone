...
func TestPreparedDMLRuntimeSpecializationPreservesWriteParameters(t *testing.T) {
	...
	require.Equal(t, []int32{0, 1},
		PreparedPlanRuntimeTextComparisonParamPositions(predicateOnly.Plan, 2))
	...
	require.Equal(t, []int32{1, 2},
		PreparedPlanRuntimeTextComparisonParamPositions(withWriteParameter.Plan, 3),
		"the assignment marker must not expand text-comparison admission")
	...
	require.Equal(t, []int32{1, 2},
		PreparedPlanRuntimeTextComparisonParamPositions(columnBoundPredicate.Plan, 3))
	textColumnPredicate := buildPreparedAggregatePlan(t,
		"update nation set n_comment = n_comment where n_comment = ?")
	require.Empty(t,
		PreparedPlanRuntimeTextComparisonParamPositions(textColumnPredicate.Plan, 1),
		"ordinary string comparisons must stay on the cached compile path")
