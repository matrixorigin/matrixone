...
func TestMinMaxJSONLeastGreatestPreservesBinaryCollation(t *testing.T) {
	tests := []struct {
		name         string
		query        string
		argumentOIDs []types.T
	}{
		{
			name:         "json mixed string path",
			query:        "select min(least(b, c)), max(greatest(c, b)) from select_test.bind_select",
			argumentOIDs: []types.T{types.T_varchar, types.T_varchar},
		},
		{
			name:         "json date path",
			query:        "select min(least(b, c, a)), max(greatest(b, c, a)) from select_test.bind_select",
			argumentOIDs: []types.T{types.T_varchar, types.T_varchar, types.T_date},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			mock.ctxt.tables["bind_select"].Cols[2].Typ = planpb.Type{Id: int32(types.T_json)}
			...
			require.Len(t, aggregates, 2)
			for _, aggregate := range aggregates {
				require.Equal(t, int32(types.T_varchar), aggregate.Typ.Id)
				require.Equal(t, uint32(types.CharsetUTF8MB4Bin), aggregate.Typ.Charset)
				require.Len(t, aggregate.GetF().Args, 1)

				leastGreatest := aggregate.GetF().Args[0]
				require.Equal(t, int32(types.T_varchar), leastGreatest.Typ.Id)
				require.Equal(t, uint32(types.CharsetUTF8MB4Bin), leastGreatest.Typ.Charset)
				require.Contains(t, []string{"least", "greatest"}, leastGreatest.GetF().Func.ObjName)
				require.Len(t, leastGreatest.GetF().Args, len(test.argumentOIDs))
				for i, argument := range leastGreatest.GetF().Args {
					require.Equal(t, int32(test.argumentOIDs[i]), argument.Typ.Id)
					if test.argumentOIDs[i].IsMySQLString() {
						require.Equal(t, uint32(types.CharsetUTF8MB4Bin), argument.Typ.Charset)
					}
				}
			}
