...
var (
	kCAGRACreateFuncName = "cagra_create"
	kCAGRASearchFuncName = "cagra_search"

	kCAGRABuildIndexColDefs = []*plan.ColDef{
		{
			Name: "status",
			Typ: plan.Type{
	...
	kCAGRASearchColDefs = []*plan.ColDef{
		{
			Name: "pkid",
			Typ: plan.Type{
...
func (builder *QueryBuilder) buildCagraCreate(tbl *tree.TableFunction, ctx *BindContext, exprs []*plan.Expr, children []int32) (int32, error) {
	if len(exprs) < 4 {
		return 0, moerr.NewInvalidInput(builder.GetContext(), "Invalid number of arguments (NARGS < 4).")
	}

	colDefs := DeepCopyColDefList(kCAGRABuildIndexColDefs)
	params, err := builder.getCagraParams(tbl.Func)
	if err != nil {
		return 0, err
	}
...
func (builder *QueryBuilder) buildCagraSearch(tbl *tree.TableFunction, ctx *BindContext, exprs []*plan.Expr, children []int32) (int32, error) {
	...
	colDefs := DeepCopyColDefList(kCAGRASearchColDefs)
...
func (builder *QueryBuilder) getCagraParams(fn *tree.FuncExpr) (string, error) {
