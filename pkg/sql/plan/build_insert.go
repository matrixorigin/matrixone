...
func buildInsert(stmt *tree.Insert, ctx CompilerContext, isReplace bool, isPrepareStmt bool) (p *Plan, err error) {
	...
	skipDupCheckForAutoPk, insertWithoutUniqueKeyMap, ifInsertFromUniqueColMap, err := initInsertStmt(builder, bindCtx, stmt, rewriteInfo)
	if err != nil {
		return nil, err
	}
	...
	if len(rewriteInfo.onDuplicateIdx) > 0 {
		...
		err = buildInsertPlans(ctx, builder, bindCtx, stmt, objRef, tableDef, rewriteInfo.rootId, skipDupCheckForAutoPk, insertWithoutUniqueKeyMap, ifInsertFromUniqueColMap)
		if err != nil {
			return nil, err
		}
