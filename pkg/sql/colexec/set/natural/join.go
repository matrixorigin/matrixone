			...
			if gs, ok := ctr.groups[h]; ok {
				for k := 0; k < len(gs); k++ {
					...
					if sel >= 0 {
						...
						if err := ctr.product(len(vecs), sel, g, bat, proc); err != nil {
							return err
						}
					}
					copy(ctr.diffs[:len(remaining)], ZeroBools[:len(remaining)])
...
func (ctr *Container) product(n int, sel int64, g *hash.SetGroup, bat *batch.Batch, proc *process.Process) error {
	{
