...
func getRelationByObjRef(ctx context.Context, proc *process.Process, eg engine.Engine, ref *plan.ObjectRef) (engine.Relation, error) {
	dbSource, err := eg.Database(ctx, ref.SchemaName, proc.GetCloneTxnOperator())
	if err != nil {
		return nil, err
	}
	...
	dbSource, err = eg.Database(ctx, defines.TEMPORARY_DBNAME, proc.GetCloneTxnOperator())
	if err != nil {
...
func GetRelAndPartitionRelsByObjRef(
	...
	if len(partitionTableNames) > 0 {
		...
		dbSource, err = eng.Database(proc.Ctx, ref.SchemaName, proc.GetCloneTxnOperator())
		if err != nil {
			return
		}
