...
func init() {
	tcs = []joinTestCase{
		newTestCase(testutil.NewMheap(), []bool{false}, []types.Type{{Oid: types.T_int8}}, []colexec.ResultPos{{Rel: 0, Pos: 0}},
			[][]*plan.Expr{
				{
		...
		newTestCase(testutil.NewMheap(), []bool{true}, []types.Type{{Oid: types.T_int8}}, []colexec.ResultPos{{Rel: 0, Pos: 0}, {Rel: 1, Pos: 0}},
			[][]*plan.Expr{
				{
					newExpr(0, types.Type{Oid: types.T_int8}),
				},
...
func BenchmarkJoin(b *testing.B) {
	for i := 0; i < b.N; i++ {
		tcs = []joinTestCase{
			newTestCase(testutil.NewMheap(), []bool{false}, []types.Type{{Oid: types.T_int8}}, []colexec.ResultPos{{Rel: 0, Pos: 0}, {Rel: 1, Pos: 0}},
				[][]*plan.Expr{
					{
						newExpr(0, types.Type{Oid: types.T_int8}),
					},
