...
func TestMarshalRemoteBatchBinaryStringProtocolGate(t *testing.T) {
	...
	runtime.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion34)
	buf.Reset()
	encoded, err = marshalRemoteBatch(proc, dynamic, buf)
	require.NoError(t, err)
	...
	runtime.SetGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion35)
	buf.Reset()
	encoded, err = marshalRemoteBatch(proc, dynamic, buf)
	require.NoError(t, err)
