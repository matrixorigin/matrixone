...
func TestDecimal128Avg(t *testing.T) {
	...
	vec2 := testutil.MakeDecimal128Vector(input2, nil, testTyp)
	{
		...
		v, err := agg.Eval(m)
		require.NoError(t, err)
		require.Equal(t, testutil.MakeDecimal128ArrByFloat64Arr([]float64{4.5}), vector.GetColumn[types.Decimal128](v))
		v.Free(m)
	}
	{
		// test two agg with Merge function
		...
		agg0.Merge(agg1, 0, 0)
		{
			v, err := agg0.Eval(m)
			require.NoError(t, err)
			require.Equal(t, testutil.MakeDecimal128ArrByFloat64Arr([]float64{9.5}), vector.GetColumn[types.Decimal128](v))
			v.Free(m)
		}
		{
			v, err := agg1.Eval(m)
			require.NoError(t, err)
			require.Equal(t, testutil.MakeDecimal128ArrByFloat64Arr([]float64{14.5}), vector.GetColumn[types.Decimal128](v))
			v.Free(m)
		}
	}
	vec.Free(m)
