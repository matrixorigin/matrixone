...
func CompareResult(t *testing.T, typ types.Type, expected any, vec *vector.Vector, hasDecimalResult bool) bool {
	switch typ.Oid {
	case types.T_bool:
		require.Equal(t, expected.([]bool), vector.MustTCols[bool](vec))
	case types.T_int8:
		require.Equal(t, expected.([]int8), vector.MustTCols[int8](vec))
	case types.T_int16:
		require.Equal(t, expected.([]int16), vector.MustTCols[int16](vec))
	case types.T_int32:
		require.Equal(t, expected.([]int32), vector.MustTCols[int32](vec))
	case types.T_int64:
		require.Equal(t, expected.([]int64), vector.MustTCols[int64](vec))
	case types.T_uint8:
		require.Equal(t, expected.([]uint8), vector.MustTCols[uint8](vec))
	case types.T_uint16:
		require.Equal(t, expected.([]uint16), vector.MustTCols[uint16](vec))
	case types.T_uint32:
		require.Equal(t, expected.([]uint32), vector.MustTCols[uint32](vec))
	case types.T_uint64:
		require.Equal(t, expected.([]uint64), vector.MustTCols[uint64](vec))
	case types.T_float32:
		require.Equal(t, expected.([]float32), vector.MustTCols[float32](vec))
	case types.T_float64:
		require.Equal(t, expected.([]float64), vector.MustTCols[float64](vec))
	case types.T_char:
		require.Equal(t, expected.([]string), vector.MustStrCols(vec))
	case types.T_varchar:
		require.Equal(t, expected.([]string), vector.MustStrCols(vec))
	case types.T_date:
		require.Equal(t, expected.([]types.Date), vector.MustTCols[types.Date](vec))
	case types.T_time:
		require.Equal(t, expected.([]types.Time), vector.MustTCols[types.Datetime](vec))
	case types.T_datetime:
		require.Equal(t, expected.([]types.Datetime), vector.MustTCols[types.Datetime](vec))
	case types.T_timestamp:
		require.Equal(t, expected.([]types.Timestamp), vector.MustTCols[types.Timestamp](vec))
	case types.T_decimal64:
		if hasDecimalResult {
			result := testutil.MakeDecimal64ArrByFloat64Arr(expected.([]float64))
			require.Equal(t, result, vector.MustTCols[types.Decimal64](vec))
		} else {
	...
	case types.T_decimal128:
		if hasDecimalResult {
			...
			require.Equal(t, result, vector.MustTCols[types.Decimal128](vec))
		} else {
			result := testutil.MakeDecimal128ArrByInt64Arr(expected.([]int64))
			require.Equal(t, result, vector.MustTCols[types.Decimal128](vec))
		}
	case types.T_uuid:
		result := vector.MustTCols[types.Uuid](testutil.MakeUuidVectorByString(expected.([]string), nil))
		require.Equal(t, result, vector.MustTCols[types.Uuid](vec))
	default:
		return false
	}
	return true
