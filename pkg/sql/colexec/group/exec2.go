	...
	case vm.Build:
		...
		for {
			...
			if needSpill {
				...
				if group.NeedEval {
					group.ctr.spillDataToDisk(proc, &group.ctr.spillBkt)
		...
		if group.ctr.isSpilling() {
			if err := group.ctr.spillDataToDisk(proc, &group.ctr.spillBkt); err != nil {
				return vm.CancelResult, err
			}
...
func (group *Group) buildOneBatch(proc *process.Process, bat *batch.Batch) (bool, error) {
	...
	if group.ctr.mtyp == H0 {
		...
		// check size
		return group.ctr.memUsed() > group.SpillMem, nil
	}
}
