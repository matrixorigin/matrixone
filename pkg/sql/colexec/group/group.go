...
func (group *Group) spillToDisk(proc *process.Process) error {
	...
	logutil.Debugf("Group: Preparing %d batches and %d aggregators for spilling", len(groups), len(group.ctr.result1.AggList))
...
func (group *Group) mergeSpilledData(proc *process.Process) error {
	...
	for _, filePath := range group.ctr.spillManager.spillFiles {
		logutil.Debugf("Group: Merging spilled data from file %s", filePath)
