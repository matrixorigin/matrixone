...
func (mergeGroup *MergeGroup) Prepare(proc *process.Process) error {
	...
	if mergeGroup.SpillMem == 0 {
		// TODO: should auto tune this number with available memory.
		mergeGroup.SpillMem = common.GiB
	}

	// but be sane, at least use 1MB, and at most 16GB.
	mergeGroup.SpillMem = min(max(mergeGroup.SpillMem, common.MiB), common.GiB*16)
	return nil
}
...
func (mergeGroup *MergeGroup) Call(proc *process.Process) (vm.CallResult, error) {
	...
	case vm.Build:
		...
		for {
			...
			if needSpill {
				mergeGroup.ctr.spillDataToDisk(proc, &mergeGroup.ctr.spillBkt)
			}
		}

		if mergeGroup.ctr.isSpilling() {
			if err := mergeGroup.ctr.spillDataToDisk(proc, &mergeGroup.ctr.spillBkt); err != nil {
				return vm.CancelResult, err
			}
