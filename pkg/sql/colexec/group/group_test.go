...
type cancelingFinalBatchConsumer struct {
	cancel  context.CancelFunc
	batches int
}

type countingFinalBatchConsumer struct {
	batches int
	rows    int
}

func (consumer *countingFinalBatchConsumer) ConsumeFinalizedBatch(
	_ *process.Process,
	bat *batch.Batch,
) error {
	consumer.batches++
	consumer.rows += bat.RowCount()
	return nil
}

func (consumer *cancelingFinalBatchConsumer) ConsumeFinalizedBatch(
	_ *process.Process,
	_ *batch.Batch,
) error {
	consumer.batches++
...
func TestFinalizedTopCancellationFreesTransferredPrefixAndOwnedSuffix(t *testing.T) {
	...
	group := NewArgument()
	group.NeedEval = true
	group.GroupBy = []*plan.Expr{colExpr(0, types.T_int32)}
	group.Aggs = []aggexec.AggFuncExecExpression{countStarAgg()}
	group.AppendChild(child)
	require.NoError(t, vm.Prepare(group, proc))

	consumer := &cancelingFinalBatchConsumer{cancel: cancel}
	token, attached := group.TryAttachFinalizedBatchConsumer(consumer)
	require.True(t, attached)
	require.NotZero(t, token)

	_, err := vm.Exec(group, proc)
	require.ErrorIs(t, err, context.Canceled)
	require.Equal(t, 1, consumer.batches)
...
func TestFinalizedTopSingleChunkUsesOrdinaryPullPath(t *testing.T) {
	mp := mpool.MustNewZero()
	proc := testutil.NewProcessWithMPool(t, "", mp)
	const rows = 128
	input := batch.NewWithSize(1)
	...
	require.Zero(t, consumer.rows)
	extra := group.OpAnalyzer.GetOpStats().ExtraStats
	require.Equal(t, int64(1), extra["GroupTopKFallbackSingleChunk"])
	require.Zero(t, extra["GroupTopKFusionUsed"])
...
func TestMergeGroupClosesProvisionalFinalizationGateBeforeAdmission(t *testing.T) {
	mp := mpool.MustNewZero()
	proc := testutil.NewProcessWithMPool(t, "", mp)

	makeSource := func(base int32, rows int) *batch.Batch {
		bat := batch.NewWithSize(2)
		first := make([]int32, rows)
		second := make([]int32, rows)
		for i := range first {
			first[i] = base + int32(i)
			second[i] = 1
		}
		bat.Vecs[0] = testutil.MakeInt32Vector(first, nil, mp)
		bat.Vecs[1] = testutil.MakeInt32Vector(second, nil, mp)
		bat.SetRowCount(rows)
		return bat
	}
	sources := []*batch.Batch{
		makeSource(0, aggexec.AggBatchSize/2+1),
		makeSource(aggexec.AggBatchSize/2+1, aggexec.AggBatchSize/2+1),
	}
	partials := buildPartialGroupBatches(t, proc, sources, false)
	...
	require.NoError(t, vm.Prepare(merge, proc))
	require.Empty(t, merge.ctr.aggList)

	consumer := &countingFinalBatchConsumer{}
	_, attached := merge.TryAttachFinalizedBatchConsumer(consumer)
	require.True(t, attached)
	result, err := vm.Exec(merge, proc)
	require.NoError(t, err)
	require.Nil(t, result.Batch)
	require.Equal(t, aggexec.AggBatchSize+2, consumer.rows)
	require.Equal(t, 2, consumer.batches)
	...
	require.Equal(t, int64(aggexec.AggBatchSize+2),
		extra["GroupTopKFinalizedRows"])
...
func TestFinalizationEligibilityRejectsSemanticBoundaryChanges(t *testing.T) {
	require.True(t, supportedFinalizationAggregates(nil))
	require.True(t, supportedFinalizationAggregates(
		[]aggexec.AggFuncExecExpression{countStarAgg()}))
	require.False(t, supportedFinalizationAggregates(
		[]aggexec.AggFuncExecExpression{countDistinctAgg(0)}))
	require.False(t, supportedFinalizationAggregates(
		[]aggexec.AggFuncExecExpression{
			aggexec.MakeAggFunctionExpression(
				aggexec.AggIdOfAvg, false,
				[]*plan.Expr{colExpr(0, types.T_int32)}, nil),
		}))
	require.False(t, supportedFinalizationAggregates(
		[]aggexec.AggFuncExecExpression{
			aggexec.MakeAggFunctionExpression(
				aggexec.AggIdOfCountStar, false,
				[]*plan.Expr{colExpr(0, types.T_int32)}, []byte{1},
	...
	require.False(t, stableFinalizationExpression(volatile))
	require.False(t, stableFinalizationExpression(&plan.Expr{
		Expr: &plan.Expr_F{F: &plan.Function{
	...
	require.NoError(t, err)
	require.True(t, stableFinalizationExpression(&plan.Expr{
		Expr: &plan.Expr_F{F: &plan.Function{
			Func: &plan.ObjectRef{
				Obj: absFn.GetEncodedOverloadID(), ObjName: "abs",
			},
			Args: []*plan.Expr{colExpr(0, types.T_int64)},
		}},
	}))
	require.True(t, stableFinalizationExpression(colExpr(0, types.T_int32)))
}

func TestFinalizedConsumerTokenSurvivesSourceResetGeneration(t *testing.T) {
	...
	firstConsumer := &countingFinalBatchConsumer{}
	firstToken, attached := group.TryAttachFinalizedBatchConsumer(firstConsumer)
	require.True(t, attached)

	group.Reset(proc, false, nil)
	require.Nil(t, group.ctr.finalConsumer)
	require.NoError(t, group.Prepare(proc))
	secondConsumer := &countingFinalBatchConsumer{}
	secondToken, attached := group.TryAttachFinalizedBatchConsumer(secondConsumer)
	require.True(t, attached)
	require.Greater(t, secondToken, firstToken)
	group.DetachFinalizedBatchConsumer(firstToken)
	require.Same(t, secondConsumer, group.ctr.finalConsumer)
...
func TestFinalizedTopConsumerPersistsAcrossGroupSpillReloads(t *testing.T) {
	mp := mpool.MustNewZero()
	proc := testutil.NewProcessWithMPool(t, "", mp)
	const rows = 1025
	input := batch.NewWithSize(1)
	...
	// Values below 10K are interpreted as a group-count threshold.
	group.SpillMem = 128
	group.AppendChild(child)
	require.NoError(t, vm.Prepare(group, proc))
	consumer := &countingFinalBatchConsumer{}
