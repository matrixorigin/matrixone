...
func TestBug_NilHashPointerDereference(t *testing.T) {
	// Create a ResHashRelated with nil Hash
	hr := &ResHashRelated{
		Hash: nil, // This will cause the original bug
		Itr:  nil,
	}
	...
	insertList, insertCount := hr.GetBinaryInsertList(vals, before)

	require.NotNil(t, insertList)
	require.Equal(t, uint64(0), insertCount) // Should return 0 when hash is nil
	require.Equal(t, len(vals), len(insertList))
}
