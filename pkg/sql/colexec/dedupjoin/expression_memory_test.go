...
func TestDedupJoinResetReleasesProbeExpressionLease(t *testing.T) {
...
func TestDedupJoinResetReleasesAccountedProbeExpressions(t *testing.T) {
	proc := testutil.NewProcessWithMPool(t, "", mpool.MustNewZero())
	defer proc.Free()
	...
	require.NoError(t, err)
	arg := &DedupJoin{allocationAccount: account}
	arg.ctr.evecs = []evalVector{{executor: executors[0]}}
	arg.ctr.vecs = make([]*vector.Vector, len(executors))
