...
func (productl2 *Productl2) build(proc *process.Process, anal process.Analyzer) error {
	ctr := productl2.ctr
	start := time.Now()
...
func (ctr *container) probe(ap *Productl2, proc *process.Process, analyzer process.Analyzer, result *vm.CallResult) error {
	if ctr.rbat != nil {
