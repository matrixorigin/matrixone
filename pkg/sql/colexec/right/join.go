...
func (rightJoin *RightJoin) build(anal process.Analyzer, proc *process.Process) {
	ctr := rightJoin.ctr
	start := time.Now()
...
func (ctr *container) sendLast(ap *RightJoin, proc *process.Process, analyze process.Analyzer, result *vm.CallResult) (bool, error) {
...
func (ctr *container) probe(ap *RightJoin, proc *process.Process, anal process.Analyzer, result *vm.CallResult) error {
