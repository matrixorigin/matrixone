...
func TestMongoScanExplicitQueryRejectsRolledBackProtocolBeforeMongoCall(t *testing.T) {
	...
	t.Cleanup(func() {
		if hadPrevious {
			...
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion34)
		}
	})
	...
	scan := NewArgument().WithScan(spec)
	scan.Dependencies = deps
	require.ErrorContains(t, scan.Prepare(proc), "MORPC protocol version 35")
...
func TestMongoScanExplicitQueryDeadlineRejectsBufferedDocument(t *testing.T) {
	...
	t.Cleanup(func() {
		if hadPrevious {
			...
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion35)
		}
	})
