...
func TestMongoScanExplicitQueryRejectsRolledBackProtocolBeforeMongoCall(t *testing.T) {
	...
	t.Cleanup(func() {
		if hadPrevious {
			...
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion42)
		}
	})
	...
	scan := NewArgument().WithScan(spec)
	scan.Dependencies = deps
	require.ErrorContains(t, scan.Prepare(proc), "MORPC protocol version 43")
...
func TestMongoScanExplicitQueryDeadlineRejectsBufferedDocument(t *testing.T) {
	...
	t.Cleanup(func() {
		if hadPrevious {
			...
			rt.CompareAndDeleteGlobalVariables(moruntime.MOProtocolVersion, defines.MORPCVersion43)
		}
	})
...
func TestMongoScanPrepareRejectsV43SemanticsOnV42(t *testing.T) {
	for name, spec := range map[string]*plan.MongoScan{
		...
		t.Run(name, func(t *testing.T) {
			...
			require.ErrorContains(t, err, "MORPC protocol version 43")
			proc.Free()
		})
	}
}
