...
func (u *fulltext2CreateState) sealSegment(proc *process.Process) (err error) {
	...
	u.segIdx++
	// The compile lifecycle transaction has already advanced the reserved durable
	// generation marker. Keep the wall-clock build id out of generation ordering.
	sqls, cleanup, err := seg.ToInsertSqls(sqlproc, u.tblcfg, 0, 0 /* tag=0 base */)
	if err != nil {
		return err
	}
...
func (u *fulltext2CreateState) end(tf *TableFunction, proc *process.Process) error {
	...
	accountID, err := sqlexec.NewSqlProcess(proc).GetAccountID()
	if err != nil {
		return err
	}
	u.tblcfg.AccountID = accountID
	fulltext2.NewFulltext2SearchForAccount(u.tblcfg, accountID).OnCacheInvalidated(string(fulltext2.LoadMissRebuild))
