...
// json columns to their flattened values; a SQL NULL column contributes no terms
// while other indexed columns in the row continue to contribute terms.
func (u *fulltext2CreateState) rowTerms(tf *TableFunction, proc *process.Process, nthRow int) ([]fulltext2.WordPos, error) {
	...
	// contributes no terms; a NULL INCLUDE column is fine (stored as NULL).
