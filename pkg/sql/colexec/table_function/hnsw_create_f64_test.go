...
func newHnswCreateTestCaseF64(t *testing.T, m *mpool.MPool, attrs []string, param string) hnswCreateTestCase {
...
func TestHnswCreateF64(t *testing.T) {

	hnsw_runSql = mock_hnsw_runSql

	param := "{\"op_type\": \"vector_l2_ops\"}"
	ut := newHnswCreateTestCaseF64(t, mpool.MustNewZero(), hnswcreatedefaultAttrs, param)

	inbat := makeBatchHnswCreateF64(ut.proc)

	ut.arg.Args = makeConstInputExprsHnswCreateF64()
...
func TestHnswCreateParamFailF64(t *testing.T) {
...
func TestHnswCreateIndexTableConfigFailF64(t *testing.T) {

	hnsw_runSql = mock_hnsw_runSql
	param := "{\"op_type\": \"vector_l2_ops\"}"

	ut := newHnswCreateTestCaseF64(t, mpool.MustNewZero(), hnswcreatedefaultAttrs, param)
	failbatches := makeBatchHnswCreateFailF64(ut.proc)
...
func makeConstInputExprsHnswCreateF64() []*plan.Expr {
...
func makeBatchHnswCreateF64(proc *process.Process) *batch.Batch {
...
func makeBatchHnswCreateFailF64(proc *process.Process) []failBatch {
	...
	}
	{
		...
		ret := []*plan.Expr{
			...
			planner.MakePlan2Vecf64ConstExprWithType("[0,1]", 3),
		}
