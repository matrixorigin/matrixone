	...
	if !u.inited {
		...
		if narrowStorage {
			...
			for _, d := range vimemory.DeviceDistinct(u.devices) {
				r, rerr := cuvs.QuantizerStagingRows(d, perTrainRow, u.idxcfg.CuvsIvfpq.QuantizerTrainLimit, u.idxcfg.Type)
				if rerr != nil || r == 0 {
					...
					return moerr.NewInternalErrorf(proc.Ctx,
						"ivfpq: cannot size the quantizer training sample on device %d: %v", d, rerr)
				}
