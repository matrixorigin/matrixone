...
const groupConcatWarningRetentionLimit = process.WarningDiagnosticDefaultRetentionLimit
...
func ReportGroupConcatWarnings(agg AggFuncExec, session any) {
	...
	if !hasBatchSink && !hasDiagnosticSink && !hasCountSink {
		return
	}
	...
	codes := make([]uint16, 0, len(warnings))
	...
	if uint64(len(codes)) > total {
		codes = codes[:int(total)]
		messages = messages[:int(total)]
	}
	if hasCountSink && total > uint64(len(codes)) {
		counter.AppendWarningCount(total - uint64(len(codes)))
	}
	for i := range codes {
...
func (exec *groupConcatExec) recordTruncation(row uint64) {
	...
	if len(exec.truncationRows) < exec.warningRetentionLimit {
