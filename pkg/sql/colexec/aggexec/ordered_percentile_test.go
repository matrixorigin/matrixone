...
func TestMakeSpecialOrderedPercentileExec(t *testing.T) {
	...
	for _, id := range []int64{AggIdOfPercentileCont, AggIdOfPercentileDisc} {
		exec, ok, err := makeSpecialAggExec(mp, id, false, false, false, false, types.T_int64.ToType())
		require.True(t, ok)
		require.NoError(t, err)
		require.NotNil(t, exec)
		exec.Free()

		_, ok, err = makeSpecialAggExec(mp, id, false, false, false, false)
		require.True(t, ok)
		require.Error(t, err)
		_, ok, err = makeSpecialAggExec(mp, id, false, false, false, false, types.T_int64.ToType(), types.T_int64.ToType())
		require.True(t, ok)
		require.Error(t, err)
	}
}
