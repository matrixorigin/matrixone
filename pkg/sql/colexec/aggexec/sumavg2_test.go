...
func TestSumTwResultKeepsPartialSumType(t *testing.T) {
	mp := mpool.MustNewZero()
	defer mpool.DeleteMPool(mp)
	...
	cases := []struct {
		...
		{
			name: "int64 partial sum",
			typ:  types.T_int64.ToType(),
			vec:  testutil.NewInt64Vector(3, types.T_int64.ToType(), mp, false, nil, []int64{1, 2, 3}),
			want: int64(6),
		},
		{
			name: "uint64 partial sum",
			typ:  types.T_uint64.ToType(),
			vec:  testutil.NewUInt64Vector(3, types.T_uint64.ToType(), mp, false, nil, []uint64{1, 2, 3}),
			want: uint64(6),
		},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			exec, err := MakeAgg(mp, AggIdOfSumTwResult, false, tc.typ)
			require.NoError(t, err)
			require.NoError(t, exec.GroupGrow(1))
			require.NoError(t, exec.BatchFill(0, []uint64{1, 1, 1}, []*vector.Vector{tc.vec}))
			...
			require.Len(t, results, 1)
			require.Equal(t, tc.typ.Oid, results[0].GetType().Oid)
			switch tc.typ.Oid {
			case types.T_int64:
				require.Equal(t, tc.want, vector.MustFixedColNoTypeCheck[int64](results[0])[0])
			case types.T_uint64:
				require.Equal(t, tc.want, vector.MustFixedColNoTypeCheck[uint64](results[0])[0])
			}
		})
	}
}
