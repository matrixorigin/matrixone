...
func TestJsonMinMaxUsesJsonComparisonOrder(t *testing.T) {
	jsonValues := []bytejson.ByteJson{
		mustParseMinMaxJSON(t, "null"),
		mustParseMinMaxJSON(t, "0"),
		mustParseMinMaxJSON(t, `""`),
		mustParseMinMaxJSON(t, `{}`),
		mustParseMinMaxJSON(t, `[]`),
		mustParseMinMaxJSON(t, `false`),
		mustParseMinMaxJSON(t, `true`),
		minMaxTypedJSON(bytejson.TpCodeDate, []byte("2024-01-01")),
		minMaxTypedJSON(bytejson.TpCodeTime, []byte("12:34:56")),
		minMaxTypedJSON(bytejson.TpCodeDatetime, []byte("2024-01-01 12:34:56")),
		minMaxTypedJSON(bytejson.TpCodeBit, []byte{0x01}),
		minMaxTypedJSON(bytejson.TpCodeOpaque, []byte{0x01}),
	}

	for _, mode := range []string{"fill", "bulk", "batch", "merge"} {
		for _, tc := range []struct {
			name string
			id   int64
			want bytejson.ByteJson
		}{
			{name: "min", id: AggIdOfMin, want: jsonValues[0]},
			{name: "max", id: AggIdOfMax, want: jsonValues[len(jsonValues)-1]},
		} {
			t.Run(mode+"/"+tc.name, func(t *testing.T) {
				mp := mpool.MustNewZero()
				exec := makeMinMaxExec(mp, tc.id, tc.id == AggIdOfMin, types.T_json.ToType())
				require.NoError(t, exec.GroupGrow(1))

				if mode == "merge" {
					left := newMinMaxJSONVector(t, mp, jsonValues[:len(jsonValues)/2])
					right := newMinMaxJSONVector(t, mp, jsonValues[len(jsonValues)/2:])
					require.NoError(t, exec.BulkFill(0, []*vector.Vector{left}))
					other := makeMinMaxExec(mp, tc.id, tc.id == AggIdOfMin, types.T_json.ToType())
					require.NoError(t, other.GroupGrow(1))
					require.NoError(t, other.BulkFill(0, []*vector.Vector{right}))
					require.NoError(t, exec.Merge(other, 0, 0))
					other.Free()
					left.Free(mp)
					right.Free(mp)
				} else {
					input := newMinMaxJSONVector(t, mp, jsonValues)
					switch mode {
					case "fill":
						for row := range jsonValues {
							require.NoError(t, exec.Fill(0, row, []*vector.Vector{input}))
						}
					case "bulk":
						require.NoError(t, exec.BulkFill(0, []*vector.Vector{input}))
					case "batch":
						groups := make([]uint64, len(jsonValues))
						for i := range groups {
							groups[i] = 1
						}
				...
				got := types.DecodeJson(results[0].GetBytesAt(0))
				require.Zero(t, bytejson.CompareByteJson(tc.want, got))
				results[0].Free(mp)
				exec.Free()
...
func minMaxTypedJSON(tp bytejson.TpCode, payload []byte) bytejson.ByteJson {
	data := make([]byte, binary.MaxVarintLen64+len(payload))
	n := binary.PutUvarint(data, uint64(len(payload)))
	copy(data[n:], payload)
	return bytejson.ByteJson{Type: tp, Data: data[:n+len(payload)]}
}

func newMinMaxJSONVector(
	t *testing.T, mp *mpool.MPool, values []bytejson.ByteJson,
) *vector.Vector {
	t.Helper()
