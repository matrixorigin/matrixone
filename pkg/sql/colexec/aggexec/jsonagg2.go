...
func jsonAggregateNumberSize(value string) (int, error) {
	return 1 + jsonUvarintSize(uint64(len(value))) + len(value), nil
}
...
func buildValueByteJson(vec *vector.Vector, row uint64) (bytejson.ByteJson, error) {
	...
	case types.T_decimal64:
		...
		return makeJSONAggregateDecimal(val.Format(typ.Scale)), nil
	case types.T_decimal128:
...
func makeJSONAggregateDecimal(value string) bytejson.ByteJson {
