...
func (exec *clusterCentersExec) Merge(next AggFuncExec, groupIdx1 int, groupIdx2 int) error {
	...
	typ := *other.groupData[groupIdx2].GetType()
	if err := vector.GetUnionAllFunction(typ, exec.ret.mp)(exec.groupData[groupIdx1], other.groupData[groupIdx2]); err != nil {
		return err
	}
	other.groupData[groupIdx2] = nil
...
func (exec *clusterCentersExec) flushArray32() error {
	for i, group := range exec.groupData {
		...
		for i := range f64s {
			f32s := types.BytesToArray[float32](group.GetBytesAt(i))
...
func (exec *clusterCentersExec) flushArray64() error {
	for i, group := range exec.groupData {
		...
		f64s := make([][]float64, group.Length())
		for i := range f64s {
			f64s[i] = types.BytesToArray[float64](group.GetBytesAt(i))
		}
