	...
	for {
		...
		case Eval:
			if ctr.insertBat != nil {
				...
				logutil.Infof("Table[%s] on duplicate key operator, ptr:[%p], output batch: %s, newBatch ptr[%p] cnt %d", arg.TableDef.Name, arg, ctr.insertBat.PrintBatch(), ctr.insertBat, ctr.insertBat.Cnt)
				ctr.insertBat = nil
				ctr.state = End
