...
func shuffleConstVectorByHash(ap *ShuffleV2, bat *batch.Batch) uint64 {
	...
	case types.T_int64:
		...
		return planner.SimpleInt64HashToRange(uint64(groupByCol[0]), lenRegs)
	case types.T_int32:
	...
	case types.T_char, types.T_varchar, types.T_text:
		...
		return planner.SimpleCharHashToRange(groupByCol[0].GetByteSlice(area), lenRegs)
	default:
...
func getShuffledSelsByHashWithNull(ap *ShuffleV2, bat *batch.Batch) [][]int32 {
	...
	case types.T_int64:
		...
		for row, v := range groupByCol {
			...
			if !groupByVec.IsNull(uint64(row)) {
				regIndex = planner.SimpleInt64HashToRange(uint64(v), lenRegs)
			}
	...
	case types.T_uint64:
		...
		for row, v := range groupByCol {
			var regIndex uint64 = 0
			if !groupByVec.IsNull(uint64(row)) {
				regIndex = planner.SimpleInt64HashToRange(v, lenRegs)
			}
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	case types.T_uint32:
	...
	case types.T_char, types.T_varchar, types.T_text:
		...
		for row := range groupByCol {
			var regIndex uint64 = 0
			if !groupByVec.IsNull(uint64(row)) {
				regIndex = planner.SimpleCharHashToRange(groupByCol[row].GetByteSlice(area), lenRegs)
			}
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	default:
...
func getShuffledSelsByHashWithoutNull(ap *ShuffleV2, bat *batch.Batch) [][]int32 {
	...
	case types.T_int64:
		groupByCol := vector.MustFixedColNoTypeCheck[int64](groupByVec)
		for row, v := range groupByCol {
			regIndex := planner.SimpleInt64HashToRange(uint64(v), bucketNum)
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	case types.T_int32:
		groupByCol := vector.MustFixedColNoTypeCheck[int32](groupByVec)
	...
	case types.T_uint64:
		groupByCol := vector.MustFixedColNoTypeCheck[uint64](groupByVec)
		for row, v := range groupByCol {
			regIndex := planner.SimpleInt64HashToRange(v, bucketNum)
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	case types.T_uint32:
		groupByCol := vector.MustFixedColNoTypeCheck[uint32](groupByVec)
	...
	case types.T_decimal128:
		...
		for row, v := range groupByCol {
			regIndex := planner.SimpleInt64HashToRange(uint64(v.B0_63^v.B64_127), bucketNum)
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	case types.T_char, types.T_varchar, types.T_text:
		groupByCol, area := vector.MustVarlenaRawData(groupByVec)
		for row := range groupByCol {
			regIndex := planner.SimpleCharHashToRange(groupByCol[row].GetByteSlice(area), bucketNum)
			sels[regIndex] = append(sels[regIndex], int32(row))
		}
	default:
...
func allBatchInOneRange(ap *ShuffleV2, bat *batch.Batch) (bool, uint64) {
	...
	case types.T_char, types.T_varchar, types.T_text:
		groupByCol, area := vector.MustVarlenaRawData(groupByVec)
		firstValueUnsigned = planner.VarlenaToUint64(&groupByCol[0], area)
		if groupByVec.IsConst() {
			lastValueUnsigned = firstValueUnsigned
		} else {
			lastValueUnsigned = planner.VarlenaToUint64(&groupByCol[groupByVec.Length()-1], area)
		}
	default:
		panic("unsupported shuffle type, wrong plan!") //something got wrong here!
	}
	...
	if ap.ShuffleRangeInt64 != nil {
		regIndexFirst = planner.GetRangeShuffleIndexSignedSlice(ap.ShuffleRangeInt64, firstValueSigned)
		regIndexLast = planner.GetRangeShuffleIndexSignedSlice(ap.ShuffleRangeInt64, lastValueSigned)
	} else if ap.ShuffleRangeUint64 != nil {
		regIndexFirst = planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, firstValueUnsigned)
		regIndexLast = planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, lastValueUnsigned)
	} else if signed {
		regIndexFirst = planner.GetRangeShuffleIndexSignedMinMax(ap.ShuffleColMin, ap.ShuffleColMax, firstValueSigned, bucketNum)
		regIndexLast = planner.GetRangeShuffleIndexSignedMinMax(ap.ShuffleColMin, ap.ShuffleColMax, lastValueSigned, bucketNum)
	} else {
		regIndexFirst = planner.GetRangeShuffleIndexUnsignedMinMax(uint64(ap.ShuffleColMin), uint64(ap.ShuffleColMax), firstValueUnsigned, bucketNum)
		regIndexLast = planner.GetRangeShuffleIndexUnsignedMinMax(uint64(ap.ShuffleColMin), uint64(ap.ShuffleColMax), lastValueUnsigned, bucketNum)
	}
...
func getShuffledSelsByRangeWithoutNull(ap *ShuffleV2, bat *batch.Batch) [][]int32 {
	...
	case types.T_int64:
		...
		if ap.ShuffleRangeInt64 != nil {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexSignedSlice(ap.ShuffleRangeInt64, v)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		} else {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexSignedMinMax(ap.ShuffleColMin, ap.ShuffleColMax, v, bucketNum)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		}
	case types.T_int32:
		...
		if ap.ShuffleRangeInt64 != nil {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexSignedSlice(ap.ShuffleRangeInt64, int64(v))
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		} else {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexSignedMinMax(ap.ShuffleColMin, ap.ShuffleColMax, int64(v), bucketNum)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		}
	case types.T_int16:
	...
	case types.T_uint64:
		...
		if ap.ShuffleRangeUint64 != nil {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, v)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		} else {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexUnsignedMinMax(uint64(ap.ShuffleColMin), uint64(ap.ShuffleColMax), v, bucketNum)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		}
	case types.T_uint32:
		...
		if ap.ShuffleRangeUint64 != nil {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, uint64(v))
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		} else {
			for row, v := range groupByCol {
				regIndex := planner.GetRangeShuffleIndexUnsignedMinMax(uint64(ap.ShuffleColMin), uint64(ap.ShuffleColMax), uint64(v), bucketNum)
				sels[regIndex] = append(sels[regIndex], int32(row))
			}
		}
	case types.T_uint16:
	...
	case types.T_char, types.T_varchar, types.T_text:
		...
		if area == nil {
			if ap.ShuffleRangeUint64 != nil {
				for row := range groupByCol {
					v := planner.VarlenaToUint64Inline(&groupByCol[row])
					regIndex := planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, v)
					sels[regIndex] = append(sels[regIndex], int32(row))
				}
			} else {
			...
			if ap.ShuffleRangeUint64 != nil {
				for row := range groupByCol {
					v := planner.VarlenaToUint64(&groupByCol[row], area)
					regIndex := planner.GetRangeShuffleIndexUnsignedSlice(ap.ShuffleRangeUint64, v)
					sels[regIndex] = append(sels[regIndex], int32(row))
				}
			} else {
