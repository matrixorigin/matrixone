...
func (r *Reader) ReadBytes() (record [][]byte, err error) {
	if r.ReuseRecord {
		record, err = r.readRecordBytes(r.lastRecordBytes)
		r.lastRecordBytes = record
	} else {
		record, err = r.readRecordBytes(nil)
	}
	return record, err
}
...
func (r *Reader) readRecordBytes(dst [][]byte) ([][]byte, error) {
	if r.Comma == r.Comment || !validDelim(r.Comma) || (r.Comment != 0 && !validDelim(r.Comment)) {
		return nil, errInvalidDelim
	}
...
parseField:
	...
	var preIdx int
	for i, idx := range r.fieldIndexes {
		dst[i] = r.recordBuffer[preIdx:idx]
		preIdx = idx
	}
