...
// Some branches encode MySQL's zero datetime as a negative sentinel, distinct
// from the valid 0001-01-01 epoch. Older encodings use zero for that epoch and
// do not support a distinct zero-date value, so they must use normal window
// arithmetic for it.
const hasDistinctZeroDatetimeSentinel = types.ZeroDatetime < types.Datetime(0)
...
func (ctr *container) firstWindow(t *TimeWin) error {
	...
	if hasDistinctZeroDatetimeSentinel && val == types.ZeroDatetime {
