...
func TestTimeWinSlidingKeepsDatetimeEpoch(t *testing.T) {
	proc := testutil.NewProcessWithMPool(t, "", mpool.MustNewZero())
	sliding, err := calcDatetime(5, types.Second)
	require.NoError(t, err)

	in := makePartInput(t, proc.Mp(), []row{
		{"0001-01-01 00:00:00", 10, 1},
		{"0001-01-01 00:00:04", 20, 1},
		{"0001-01-01 00:00:05", 30, 1},
	})
	arg := newPartArg(t, proc, sliding, false)
	starts, sums, _ := runPartArg(t, arg, proc, in)

	require.Equal(t, []types.Datetime{
		mustDatetime(t, "0001-01-01 00:00:00"),
		mustDatetime(t, "0001-01-01 00:00:05"),
	}, starts)
