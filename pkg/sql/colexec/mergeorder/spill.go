...
func writeSpillBatch(proc *process.Process, bat *batch.Batch, keyCols []*vector.Vector, writer io.Writer, buf *bytes.Buffer, analyzer process.Analyzer) (int64, error) {
...
func (ctr *container) spillBatchAsRun(proc *process.Process, bat *batch.Batch, keyCols []*vector.Vector, analyzer process.Analyzer) error {
	...
	if _, err = writeSpillBatch(proc, bat, keyCols, run.file, &ctr.spillWriteBuf, analyzer); err != nil {
		run.file.Close()
		return err
	}
	run.batchCount = 1
...
func (ctr *container) spillCachedRuns(proc *process.Process, analyzer process.Analyzer) error {
	for i := range ctr.batchList {
		...
		keyCols, err := ctr.buildSpillKeyColumns(proc, ctr.batchList[i])
		if err != nil {
			return err
		}
		if err := ctr.spillBatchAsRun(proc, ctr.batchList[i], keyCols, analyzer); err != nil {
			freeOrderColumns(proc.Mp(), ctr.batchList[i], keyCols)
			return err
		}
		freeOrderColumns(proc.Mp(), ctr.batchList[i], keyCols)
	...
	for len(ctr.spillReaders) > 0 {
		...
		for len(ctr.spillReaders) > 0 {
			...
			for col := range out.Vecs {
				if err := out.Vecs[col].UnionOne(src.batch.Vecs[col], src.rowIdx, proc.Mp()); err != nil {
					if out != nil {
						out.Clean(proc.Mp())
					}
					run.file.Close()
			...
			if outOrder != nil {
				for col, keyIdx := range ctr.spillKeyIndexes {
					if err := outOrder.Vecs[col].UnionOne(src.orderCols[keyIdx], src.rowIdx, proc.Mp()); err != nil {
						if out != nil {
							out.Clean(proc.Mp())
						}
						if outOrder != nil {
		...
		if _, err := writeSpillBatch(proc, out, keyCols, writer, &ctr.spillWriteBuf, analyzer); err != nil {
			out.Clean(proc.Mp())
			if outOrder != nil {
				outOrder.Clean(proc.Mp())
			}
...
func (ctr *container) sendSpillResult(proc *process.Process, result *vm.CallResult) (bool, error) {
	...
	for len(ctr.spillReaders) > 0 {
		...
		for col := range ctr.buf.Vecs {
			if err := ctr.buf.Vecs[col].UnionOne(src.batch.Vecs[col], src.rowIdx, proc.Mp()); err != nil {
				return false, err
			}
		}
		rows++
