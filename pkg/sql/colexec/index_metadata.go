...
func buildInsertIndexMetaBatch(tableId uint64, databaseId uint64, ct *engine.ConstraintDef, eg engine.Engine, proc *process.Process) (*batch.Batch, error) {
	...
	vec_id := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_ID].ToType())
	bat.Vecs[0] = vec_id

	vec_table_id := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_TABLE_ID].ToType())
	bat.Vecs[1] = vec_table_id

	vec_database_id := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_DATABASE_ID].ToType())
	bat.Vecs[2] = vec_database_id

	vec_name := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_NAME].ToType())
	bat.Vecs[3] = vec_name

	vec_type := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_TYPE].ToType())
	bat.Vecs[4] = vec_type

	vec_algo := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_ALGORITHM].ToType())
	bat.Vecs[5] = vec_algo

	vec_algo_table_type := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_ALGORITHM_TABLE_TYPE].ToType())
	bat.Vecs[6] = vec_algo_table_type

	vec_algo_params := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_ALGORITHM_PARAMS].ToType())
	bat.Vecs[7] = vec_algo_params

	vec_visible := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_IS_VISIBLE].ToType())
	bat.Vecs[8] = vec_visible

	vec_hidden := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_HIDDEN].ToType())
	bat.Vecs[9] = vec_hidden

	vec_comment := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_COMMENT].ToType())
	bat.Vecs[10] = vec_comment

	vec_column_name := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_COLUMN_NAME].ToType())
	bat.Vecs[11] = vec_column_name

	vec_ordinal_position := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_ORDINAL_POSITION].ToType())
	bat.Vecs[12] = vec_ordinal_position

	vec_options := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_OPTIONS].ToType())
	bat.Vecs[13] = vec_options

	vec_index_table := vector.NewOffHeapVecWithType(MO_INDEX_COLTYPE[MO_INDEX_TABLE_NAME].ToType())
