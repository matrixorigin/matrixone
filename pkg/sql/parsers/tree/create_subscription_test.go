...
func TestNewCreateSubscription(t *testing.T) {
	cs := NewCreateSubscription(true, Identifier("testdb"), "", "127.0.0.1:6001", Identifier("pub1"), 60)
	require.NotNil(t, cs)
...
func TestCreateSubscription_Format(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			syncInterval: 0,
			expected:     "create account from '127.0.0.1:6001' publication pub1",
		},
		{
			...
			pubName:      Identifier("pub1"),
			syncInterval: 0,
			expected:     "create database testdb from '127.0.0.1:6001' publication pub1",
		},
		{
			...
			pubName:      Identifier("pub1"),
			syncInterval: 0,
			expected:     "create table testtable from '127.0.0.1:6001' publication pub1",
		},
		{
			...
			syncInterval: 120,
			expected:     "create database testdb from '192.168.1.100:6001' publication mypub sync_interval = 120",
		},
		{
			...
			syncInterval: 30,
			expected:     "create table orders from '10.0.0.1:6001' publication orderspub sync_interval = 30",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cs := NewCreateSubscription(tt.isDatabase, tt.dbName, tt.tableName, tt.fromUri, tt.pubName, tt.syncInterval)
			ctx.Reset()
			cs.Format(ctx)
