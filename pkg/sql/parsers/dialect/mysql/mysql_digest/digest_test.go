...
func TestDigestMatchesMySQL84TokenSemantics(t *testing.T) {
	tests := []struct {
		...
		{
			name: "tagged dollar quoted literal",
			sql:  "SELECT $q$/*!80000$q$",
			text: "SELECT ?",
			hash: "d1b44b0c19af710b5a679907e284acd2ddc285201794bc69a2389d77baedddae",
		},
		{
			name: "dollar quoted terminator inside executable comment",
			sql:  "SELECT /*!80000 $q$*/$q$ + 1 */",
			text: "SELECT ? + ?",
			hash: "18c24a99168954090331d4686d78ade5498aa5cfc6b125c260cd8183fa150bd5",
		},
		{
			name: "dollar signs inside identifier do not start a quote",
			sql:  "SELECT a$$ /*!80401 + 1 */",
			text: "SELECT `a$$`",
			hash: "cf4efaae5b5c5ca7ba3166eb37ba41f1e69c91a4006f2afcd11f44c2607f3aa5",
		},
		{
			name: "adjacent dollar quotes remain separate literals",
			sql:  "SELECT $$a$$$$/*!80000$$",
			text: "SELECT ? ?",
			hash: "330b0454d6423fbb864f3707655da47d815faa8923bdaa068e8e9365e6c97a55",
		},
		{
			name: "adjacent tagged dollar quotes remain separate literals",
			sql:  "SELECT $$a$$$q$/*!80000$q$",
			text: "SELECT ? ?",
			hash: "330b0454d6423fbb864f3707655da47d815faa8923bdaa068e8e9365e6c97a55",
		},
		{
			name: "qualified dollar identifier does not start a quote",
			sql:  "SELECT @@global.$$ /*!80401 + 1 */",
			text: "SELECT @@GLOBAL . `$$`",
			hash: "58c4eba416a474a48989a9393e40f45f9f60b37c65fc991dd9b0eeda5979c042",
		},
		{
		...
		{
			name: "empty executable comment blocks optimizer hint",
			sql:  "SELECT /*!80000 */ /*+ MAX_EXECUTION_TIME(1000) */ 1",
			text: "SELECT ?",
			hash: "d1b44b0c19af710b5a679907e284acd2ddc285201794bc69a2389d77baedddae",
		},
		{
...
func TestDigestLexicalEdgeCases(t *testing.T) {
	tests := []struct {
		...
		{name: "unterminated executable version comment", sql: "/*!", err: true},
		{name: "unterminated executable version code", sql: "/*!80000 SELECT 1", err: true},
...
func TestDigestUnterminatedExecutableVersionComment(t *testing.T) {
	for _, sql := range []string{"/*!", "/*!80000 SELECT 1"} {
		t.Run(sql, func(t *testing.T) {
			_, err := Compute(sql)
			require.Error(t, err)

			zero := 0
			_, err = Compute(sql, Options{MaxDigestLength: &zero})
			require.Error(t, err)
		})
	}
}
...
func TestDigestDefaultNullOnUpdateRemainsLiteral(t *testing.T) {
	withNull, err := Compute("CREATE TABLE t (a TIMESTAMP DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP)")
	require.NoError(t, err)
	withValue, err := Compute("CREATE TABLE t (a TIMESTAMP DEFAULT 1 ON UPDATE CURRENT_TIMESTAMP)")
	require.NoError(t, err)
	require.Equal(t, withValue, withNull)
}
...
func TestDigestRecognizesSupportedCharsetIntroducers(t *testing.T) {
	...
	for _, sql := range []string{
		...
		require.Equal(t, canonical, got, sql)
	}
