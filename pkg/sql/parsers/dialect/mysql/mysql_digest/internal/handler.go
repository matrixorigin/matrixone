...
func (h *tokenHandler) handleIdentifier(tok Token) error {
	...
	if tok.Type != IDENT_QUOTED && isCharsetIntroducer(text) && h.lexer.followedByCharsetLiteral(tok.End) {
...
func isStringLiteral(t int) bool {
	switch t {
	case LEX_HOSTNAME, TEXT_STRING, NCHAR_STRING, DOLLAR_QUOTED_STRING_SYM, PARAM_MARKER:
		return true
	}
	return false
}
