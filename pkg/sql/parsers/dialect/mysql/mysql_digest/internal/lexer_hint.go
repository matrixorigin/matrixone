...
func (l *Lexer) lexHintQuoted(quote byte, ident bool) Token {
	// Hint string arguments use the same backslash rules as ordinary MySQL
	// strings. Quoted identifiers never consume backslash escapes, while
	// single/double-quoted strings do unless NO_BACKSLASH_ESCAPES is active.
	allowBackslashEscape := !ident && (l.sqlMode&MODE_NO_BACKSLASH_ESCAPES) == 0
	for {
		ch := l.peek()
		if l.eof() {
			return l.returnToken(Token{
