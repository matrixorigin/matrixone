...
// pushValue appends a generic value, reducing a preceding value list before
// checking the byte budget. MySQL performs this reduction before attempting to
// store the new token; doing it in the opposite order can mark a digest full
// while there is enough space for the shorter list representation.
func (r *reducer) pushValue() {
	if r.store.full {
		return
	}
	before, last := r.store.peek2()
	if last == ',' && isValueOrValueList(before) {
		r.store.pop(2)
		r.store.push(TOK_GENERIC_VALUE_LIST)
		return
	}
	r.store.push(TOK_GENERIC_VALUE)
}

// pushCloseParen appends a close parenthesis, applying the MySQL digest
// reductions before charging the token against max_digest_length. This keeps
// single-value groups, IN lists, and row lists reducible at an exact budget
// boundary.
func (r *reducer) pushCloseParen() {
	if r.store.full {
		return
	}
	before, last := r.store.peek2()
	if before == '(' && last == TOK_GENERIC_VALUE {
		r.store.pop(2)
		r.pushReducedGroup(TOK_ROW_SINGLE_VALUE)
		return
	}
	if before == '(' && last == TOK_GENERIC_VALUE_LIST {
		r.store.pop(2)
		r.pushReducedGroup(TOK_ROW_MULTIPLE_VALUE)
		return
	}
	r.store.push(')')
	r.reduceAll()
...
func (r *reducer) pushReducedGroup(group int) {
	before, last := r.store.peek2()
	switch {
	case last == IN_SYM && group == TOK_ROW_SINGLE_VALUE:
		r.store.pop(1)
		r.store.push(TOK_IN_GENERIC_VALUE_EXPRESSION)
	case last == IN_SYM && group == TOK_ROW_MULTIPLE_VALUE:
		r.store.pop(1)
		r.store.push(TOK_IN_GENERIC_VALUE_EXPRESSION)
	case last == ',' && isSingleValueRow(before) && group == TOK_ROW_SINGLE_VALUE:
		r.store.pop(2)
		r.store.push(TOK_ROW_SINGLE_VALUE_LIST)
	case last == ',' && isMultiValueRow(before) && group == TOK_ROW_MULTIPLE_VALUE:
		r.store.pop(2)
