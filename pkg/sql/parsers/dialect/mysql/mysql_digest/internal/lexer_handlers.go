...
func (l *Lexer) handleEOL() lexResult {
	if l.inVersionComment {
		...
		return done(Token{
			...
			Start: start,
			End:   l.pos,
			Err:   NewLexError(start, ErrUnterminatedComment, l.input),
		})
	}
...
func (l *Lexer) handleAsterisks() lexResult {
	...
	if l.inVersionComment {
		if c == '*' && l.peek() == '/' {
			// The opening '*' was consumed before entering this state. Consume
			// only the remaining slash; skipping twice also discards the first
			// byte after the executable comment.
			l.skip()
			l.inVersionComment = false
			l.versionCommentStart = -1
			l.hintAllowed = false
			return cont(MY_LEX_START)
		}
	}
...
func (l *Lexer) adjustKeywordForSQLMode(tok int) int {
	...
	if tok != 0 && isFunctionKeyword(tok) && !l.functionKeywordCallFollows() &&
		isFunctionOnlySpelling(l.input[l.tokStart:l.pos]) {
		return IDENT
	}
	return tok
}

// MySQL aliases several no-parentheses temporal spellings and USER to keyword
// tokens that share values with function-only names.  The token value alone
// cannot distinguish CURRENT_DATE from CURDATE (or USER from USER()), so use
// the original spelling before applying the function-name lookahead.  The
// aliases remain keywords without parentheses; the canonical function-only
// spellings still become identifiers unless they form a call.
func isFunctionOnlySpelling(text string) bool {
	switch strings.ToUpper(text) {
	case "CURRENT_DATE", "CURRENT_TIME", "CURRENT_TIMESTAMP",
		"LOCALTIME", "LOCALTIMESTAMP", "USER":
		return false
	default:
		return true
	}
...
// functionKeywordCallFollows applies MySQL's special function-name lookup:
// these spellings are function tokens only when followed by '(', allowing
// whitespace only under IGNORE_SPACE. Otherwise they are ordinary identifiers.
func (l *Lexer) functionKeywordCallFollows() bool {
	pos := l.pos
	if pos < len(l.input) && l.input[pos] == '(' {
		return true
	}
	if l.sqlMode&MODE_IGNORE_SPACE == 0 || pos >= len(l.input) || !isSpace(l.input[pos]) {
		return false
	}
...
func isFunctionKeyword(tok int) bool {
	switch tok {
	case ADDDATE_SYM, BIT_AND_SYM, BIT_OR_SYM, BIT_XOR_SYM,
		CAST_SYM, COUNT_SYM, CURDATE, CURTIME, DATE_ADD_INTERVAL,
		DATE_SUB_INTERVAL, EXTRACT_SYM, GROUP_CONCAT_SYM, MAX_SYM, SUBSTRING,
		MIN_SYM, NOW_SYM, POSITION_SYM, USER, STD_SYM, STDDEV_SAMP_SYM,
		SUBDATE_SYM, SUM_SYM, SYSDATE, TRIM, VARIANCE_SYM, VAR_SAMP_SYM:
		return true
	default:
		return false
	}
...
func (l *Lexer) handleVersionComment() lexResult {
	l.hintAllowed = false
	l.versionCommentStart = l.tokStart
	l.skip() // Skip '!'
