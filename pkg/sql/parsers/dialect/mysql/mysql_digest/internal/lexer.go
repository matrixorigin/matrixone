...
func (l *Lexer) followedByCharsetLiteral(pos int) bool {
	if pos < 0 || pos > len(l.input) {
		return false
	}
	// Use a copy so introducer lookahead recognizes exactly the same whitespace,
	// comments, version-comment contents, and SQL mode as the normal token
	// stream. MySQL accepts quoted, hexadecimal, and binary literals after a
	// character-set introducer.
	lookahead := *l
	lookahead.pos = pos
	lookahead.tokStart = pos
	lookahead.nextState = MY_LEX_START
	switch lookahead.Lex().Type {
	case TEXT_STRING, HEX_NUM, BIN_NUM:
		return true
	default:
		return false
	}
