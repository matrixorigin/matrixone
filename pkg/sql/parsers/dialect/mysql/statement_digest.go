...
const statementDigestMySQLVersionID = 80400
...
func NormalizeStatementDigest(ctx context.Context, sql, sqlMode string, maxDigestLength int) (string, error) {
	...
	for {
		...
		if charsetIntroducer {
			...
			qualifiedIdentifier := priorType == '.' || nextPos < len(digestSQL) && digestSQL[nextPos] == '.'
			charsetIntroducer = !qualifiedIdentifier
		}
...
func digestNullIsColumnAttribute(tokens []digestToken) bool {
	...
	depth := 0
	caseDepth := 0
	expressionEnd := len(tokens) - 1
	if expressionEnd >= 0 && tokens[expressionEnd].text == "NOT" {
		expressionEnd--
	}
	for i := len(tokens) - 1; i >= 0; i-- {
		...
		if token.kind == digestTokenPlain && token.text == ")" {
			depth++
			continue
		}
		if depth > 0 {
		...
		case "CHECK", "AS", "GENERATED", "REFERENCES":
			return false
		}
...
func digestIsColumnTypeToken(text string) bool {
	...
	case "ARRAY", "INT8", "BINARY", "BIT", "BLOB", "BOOL", "BOOLEAN", "CHARACTER", "DATALINK",
		...
		"MULTIPOLYGON", "MULTIPOLYGON32", "NCHAR", "NUMERIC", "POINT", "POINT32", "POLYGON", "POLYGON32",
...
func digestVersionCommentGuard(sql string, start, end int) (contentStart int, active bool) {
	...
	return contentStart, version <= statementDigestMySQLVersionID
}
...
func isDigestHintIdentifierStart(ch byte) bool {
	// NormalizeStatementDigest validates UTF-8 before scanning. Keep all bytes
	// of a multibyte hint identifier in one token, including @query blocks.
	return ch >= utf8.RuneSelf || ch == '_' || ch == '$' || ch >= 'a' && ch <= 'z' || ch >= 'A' && ch <= 'Z'
}
