...
func TestMongoDBSQLSurfaceAndRedaction(t *testing.T) {
	...
	sqls := []string{
		...
		"create external table raw(ts datetime mongodb_path 'ts', value double mongodb_path 'payload.value' mongodb_convert 'try_null') engine=mongodb with ('connection'='sensor', 'database'='nesr', 'collection'='raw', 'max_parallelism'='1')",
	}
