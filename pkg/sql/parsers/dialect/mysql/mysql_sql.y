...
|   NAMES charset_name
    {
        $$ = &tree.VarAssignmentExpr{
            ...
            Value: tree.NewNumValWithType2(constant.MakeString($2), $2, $2, false, tree.P_char),
        }
    }
...
|   NAMES charset_name COLLATE name_string
    {
        $$ = &tree.VarAssignmentExpr{
            Name: strings.ToLower($1),
            Value: tree.NewNumValWithType2(constant.MakeString($2), $2, $2, false, tree.P_char),
            Reserved: tree.NewNumValWithType2(constant.MakeString($4), $4, $4, false, tree.P_char),
        }
    }
|   NAMES DEFAULT
    {
...
set_expr:
    ON
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_char)
    }
|   BINARY
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_char)
...
account_name_or_param:
    ...
    {
        var str = $1.Compare()
        $$ = tree.NewNumValWithType2(constant.MakeString(str), str, str, false, tree.P_char)
    }
|   VALUE_ARG
    {
...
account_identified:
    IDENTIFIED BY STRING
    {
        $$ = *tree.NewAccountIdentified(
            ...
            tree.NewNumValWithType2(constant.MakeString($3), $3, $3, false, tree.P_char),
        )
    }
...
user_identified:
    ...
    $$ = &tree.AccountIdentified{
        ...
        Str: tree.NewNumValWithType2(constant.MakeString($3), $3, $3, false, tree.P_char),
    }
    }
|   IDENTIFIED BY RANDOM PASSWORD
    {
...
|   '{' SNAPSHOT '=' STRING '}'
    {
        $$ = &tree.AtTimeStamp{
           Type: tree.ATTIMESTAMPSNAPSHOT,
           SnapshotName: $4,
           Expr: tree.NewNumValWithType2(constant.MakeString($4), $4, $4, false, tree.P_char),
        }
    }
|   '{' MO_TS '=' expression '}'
    {
...
|   COMMENT_KEYWORD STRING
    {
        ...
        $$ = tree.NewAttributeComment(tree.NewNumValWithType2(constant.MakeString(str), str, str, false, tree.P_char))
    }
...
|   CONVERT '(' expression USING charset_name ')'
    {
        ...
        es := tree.NewNumValWithType2(constant.MakeString($5), $5, $5, false, tree.P_char)
        $$ = &tree.FuncExpr{
            Func: tree.FuncName2ResolvableFunctionReference(name),
...
function_call_aggregate:
    GROUP_CONCAT '(' func_type_opt expression_list order_by_opt separator_opt ')' window_spec_opt
    {
	    name := tree.NewUnresolvedColName($1)
	        ...
	        Func: tree.FuncName2ResolvableFunctionReference(name),
            FuncName: tree.NewCStr($1, 1),
	        Exprs: append($4,tree.NewNumValWithType2(constant.MakeString($6), $6, $6, false, tree.P_char)),
	        Type: $3,
	        WindowSpec: $8,
            OrderBy:$5,
	    }
...
|   CHAR '(' expression_list USING charset_name ')'
    {
        cn := tree.NewNumValWithType2(constant.MakeString($5), $5, $5, false, tree.P_char)
        es := $3
        es = append(es, cn)
        name := tree.NewUnresolvedColName($1)
        $$ = &tree.FuncExpr{
...
|   DATE STRING
    {
        val := tree.NewNumValWithType2(constant.MakeString($2), $2, $2, false, tree.P_char)
        name := tree.NewUnresolvedColName($1)
        $$ = &tree.FuncExpr{
            Func: tree.FuncName2ResolvableFunctionReference(name),
            FuncName: tree.NewCStr($1, 1),
...
|   '(' INTEGRAL ')'
    {
        ...
        str := fmt.Sprintf("%v", $2)
        $$ = tree.NewNumValWithType2(constant.MakeInt64(ival), ival, str, false, tree.P_int64)
    }
...
num_literal:
    INTEGRAL
    {
        ...
        switch v := $1.(type) {
        case uint64:
            $$ = tree.NewNumValWithType2(constant.MakeUint64(v), v, str, false, tree.P_uint64)
        case int64:
            $$ = tree.NewNumValWithType2(constant.MakeInt64(v), v, str, false, tree.P_int64)
        default:
            yylex.Error("parse integral fail")
            goto ret1
        }
...
|   FLOAT
    {
        fval := $1.(float64)
        $$ = tree.NewNumValWithType2(constant.MakeFloat64(fval), fval, yylex.(*Lexer).scanner.LastToken, false, tree.P_float64)
    }
|   DECIMAL_VALUE
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_decimal)
    }
...
|   TRUE
    {
        $$ = tree.NewNumValWithType2(constant.MakeBool(true), true, "true", false, tree.P_bool)
    }
|   FALSE
    {
        $$ = tree.NewNumValWithType2(constant.MakeBool(false), false, "false", false, tree.P_bool)
    }
|   NULL
    {
        $$ = tree.NewNumValWithType2(constant.MakeUnknown(), "null", "null", false, tree.P_null)
    }
|   HEXNUM
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_hexnum)
    }
|   UNDERSCORE_BINARY HEXNUM
    {
        ...
        $$ = tree.NewNumValWithType2(constant.MakeString($2), $2, $2, false, tree.P_bit)
    }
|   DECIMAL_VALUE
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_decimal)
...
|   BIT_LITERAL
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($1), $1, $1, false, tree.P_bit)
    }
|   VALUE_ARG
    {
        $$ = tree.NewParamExpr(yylex.(*Lexer).GetParamIndex())
...
|   UNDERSCORE_BINARY STRING
    {
        $$ = tree.NewNumValWithType2(constant.MakeString($2), $2, $2, false, tree.P_ScoreBinary)
    }
