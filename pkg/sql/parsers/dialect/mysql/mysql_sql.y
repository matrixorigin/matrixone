...
normal_ident:
    ident
    {
        ...
        unResolve.SetUnresolvedNameCStrParts($1.Origin())
        $$ = unResolve
    }
|   ident '.' ident
    {
        unResolve := tree.SetUnresolvedName($1.Compare(), $3.Compare())
        unResolve.SetUnresolvedNameCStrParts($1.Origin(), $3.Origin())
        $$ = unResolve
    }
|   ident '.' ident '.' ident
    {
        unResolve := tree.SetUnresolvedName($1.Compare(), $3.Compare(), $5.Compare())
        unResolve.SetUnresolvedNameCStrParts($1.Origin(), $3.Origin(), $5.Origin())
        $$ = unResolve
    }
    ...
    ident '(' expression_list_opt ')'
    {
        ...
        $$ = &tree.TableFunction{
       	        Func: &tree.FuncExpr{
                    ...
                    FuncName: tree.NewCStr($1.Compare(), yylex.(*Lexer).lower),
        	        Exprs: $3,
...
|   ON UPDATE name_datetime_scale datetime_scale_opt
    {
        ...
        expr := &tree.FuncExpr{
            Func: tree.FuncName2ResolvableFunctionReference(name),
            FuncName: tree.NewCStr($3,yylex.(*Lexer).lower),
            Exprs: es,
        }
...
|   CONVERT '(' expression USING charset_name ')'
    {
        ...
        $$ = &tree.FuncExpr{
            Func: tree.FuncName2ResolvableFunctionReference(name),
            FuncName: tree.NewCStr($1,yylex.(*Lexer).lower),
...
|   expression PIPE_CONCAT expression %prec PIPE_CONCAT
    {
        ...
        $$ = &tree.FuncExpr{
            Func: tree.FuncName2ResolvableFunctionReference(name),
            FuncName: tree.NewCStr($2,yylex.(*Lexer).lower),
