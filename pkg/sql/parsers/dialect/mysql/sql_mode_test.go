...
func TestIgnoreSpaceReservedFunctionIdentifiers(t *testing.T) {
	tests := []struct {
		...
		{name: "qualified sensitive identifier remains valid", query: "select src.count", mode: "STRICT_TRANS_TABLES,IGNORE_SPACE"},
		{name: "qualified sensitive table name remains valid", query: "create table src.count(i int)", mode: "STRICT_TRANS_TABLES,IGNORE_SPACE"},
