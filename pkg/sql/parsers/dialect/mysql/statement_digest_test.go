...
func TestNormalizeStatementDigestCharsetLookaheadSQLMode(t *testing.T) {
	sql := `SELECT _utf8'abc\'`
	for _, maxLength := range []int{0, 1024} {
		got, err := NormalizeStatementDigest(context.Background(), sql, "NO_BACKSLASH_ESCAPES", maxLength)
		require.NoError(t, err)
		if maxLength == 0 {
			require.Empty(t, got)
		} else {
			require.Equal(t, "SELECT (_charset) ?", got)
		}

		_, err = NormalizeStatementDigest(context.Background(), sql, "", maxLength)
		require.Error(t, err)
	}
}

func TestNormalizeStatementDigestRejectsExecutedCharsetVersionComment(t *testing.T) {
	for _, sql := range []string{
		"SELECT _utf8 /*!80000 1 */ 'x'",
		"SELECT _utf8 /*!80000 1; SELECT 2 */ 'x'",
		"SELECT _utf8 /*!80000 ? */ 'x'",
		"SELECT _utf8 /*! 1 */ 'x'",
		"SELECT _utf8 /*!80000 -- c */ 'x'",
		"SELECT _utf8 /*!80000 # c */ 'x'",
	} {
	...
	for _, sql := range []string{
		...
		"SELECT _utf8 /*!80401 1; SELECT 2 */ 'x'",
		"SELECT _utf8 /*!80401 ? */ 'x'",
	} {
		got, err := NormalizeStatementDigest(context.Background(), sql, "", 1024)
		require.NoError(t, err, sql)
		require.Equal(t, "SELECT (_charset) ?", got, sql)
	}
}

func TestNormalizeStatementDigestVersionCommentView(t *testing.T) {
	tests := []struct {
		name string
		sql  string
		want string
		...
		{
			name: "skipped version comment masks semicolon and second statement",
			sql:  "SELECT 1 /*!80401 ; SELECT 2 */",
			want: "SELECT ?",
		},
		{
			name: "target version comment executes",
			sql:  "SELECT 1 /*!80400 + 2 */",
			want: "SELECT ? + ?",
		},
		{
			name: "short version guard is skipped",
			sql:  "SELECT 1 /*!8040 + 2 */",
			want: "SELECT ?",
		},
		{
			name: "six digit newer guard is skipped",
			sql:  "SELECT 1 /*!804010 + 2 */",
			want: "SELECT ?",
		},
		{
			name: "skipped guard blocks following optimizer hint",
			sql:  "SELECT /*!80401 */ /*+ BKA(t) */ 1",
			want: "SELECT ?",
		},
		{
			name: "hash marker in hash line comment is not a version comment",
			sql:  "SELECT 1 # /*!99999\n + 2 /* ordinary */",
			want: "SELECT ? + ?",
		},
		{
			name: "hash marker in dash line comment is not a version comment",
			sql:  "SELECT 1 -- /*!99999\n + 2 /* ordinary */",
			want: "SELECT ? + ?",
		},
		{
			name: "marker in string is not a version comment",
			sql:  "SELECT '/*!99999 */' + 1",
			want: "SELECT ? + ?",
		},
		{
			name: "active comment keeps terminator in quoted body",
			sql:  "SELECT /*!80000 'x */ y' */",
			want: "SELECT ?",
		},
		{
			name: "active comment keeps nested ordinary comment",
			sql:  "SELECT /*!80000 /* c */ 1 */",
			want: "SELECT ?",
		},
		{
			name: "active comment line comment ends before terminator",
			sql:  "SELECT /*!80000 -- c\n 1 */",
			want: "SELECT ?",
		},
		{
			name: "marker in anonymous dollar quoted string",
			sql:  "SELECT $$/*!80000$$",
			want: "SELECT ?",
		},
		{
			name: "marker in tagged dollar quoted string",
		...
		{
			name: "dollar quoted terminator inside active comment",
			sql:  "SELECT /*!80000 $q$*/$q$ + 1 */",
			want: "SELECT ? + ?",
		},
		{
			name: "empty active comment blocks following optimizer hint",
			sql:  "SELECT /*!80000 */ /*+ MAX_EXECUTION_TIME(1000) */ 1",
			want: "SELECT ?",
		},
		{
			name: "empty unversioned comment blocks following optimizer hint",
	...
	// Executable comments expose their body to admission. This is deliberately
	// rejected: the numeric token is not a literal for the preceding charset
	// introducer, even though a later quoted literal follows the comment.
	_, err := NormalizeStatementDigest(context.Background(), "SELECT _utf8 /*!80000 1 */ 'x'", "", 1024)
	require.Error(t, err)
	_, err = NormalizeStatementDigest(context.Background(), "SELECT /*!80000 -- c */ 1", "", 1024)
	require.Error(t, err)
	_, err = NormalizeStatementDigest(context.Background(), "SELECT $q$/*!80000", "", 1024)
	require.Error(t, err)
	require.NotContains(t, err.Error(), "unterminated executable version comment")
	for _, sql := range []string{
		"SELECT /*!80000 \u00a0 */ 1",
		"SELECT /*!80000 \u3000 */ 1",
	} {
...
func TestNormalizeStatementDigestMySQLCounterexamples(t *testing.T) {
	tests := []struct {
		...
		{name: "charset introducer binary literal forms", sql: "SELECT _utf8 X'41', _binary B'1', _latin1 0x41, _ascii 0b1", want: "SELECT (_charset) ? , (_charset) ? , (_charset) ? , (_charset) ?"},
...
func TestNormalizeStatementDigestRejectsInvalidInput(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "charset introducer malformed hex literal", sql: "SELECT _utf8 X'0G'"},
		{name: "charset introducer malformed binary literal", sql: "SELECT _binary B'102'"},
		{name: "charset introducer missing numeric literal", sql: "SELECT _utf8 0x"},
		{name: "charset introducer numeric identifier", sql: "SELECT _utf8 0x41foo"},
...
func TestNormalizeStatementDigestVariablesOperatorsAndLimit(t *testing.T) {
	...
	// ANSI-quoted user variables are emitted as one identifier token, but the
	// leading @ is scanned separately.  When the identifier would exceed the
	// budget, preserve the already admitted @ without appending the full name.
	got, err = NormalizeStatementDigest(context.Background(), `SELECT @"very_long_name", 1`, "ANSI_QUOTES", 4)
	require.NoError(t, err)
	require.Equal(t, "SELECT @", got)
	// The quoted identifier is admitted atomically at its exact budget. Its
	// cost must still prevent a following token from being stored.
	got, err = NormalizeStatementDigest(context.Background(), `SELECT @"very_long_name", 1`, "ANSI_QUOTES", 22)
	require.NoError(t, err)
	require.Equal(t, "SELECT @`very_long_name`", got)
...
func TestNormalizeStatementDigestValidatesCharsetIntroducersBeforeTruncation(t *testing.T) {
	for _, sql := range []string{
		"SELECT _utf8",
		"SELECT _latin1 + 1",
		"SELECT 1, _utf8",
		...
		t.Run(sql, func(t *testing.T) {
			_, err := NormalizeStatementDigest(context.Background(), sql, "", 2)
			require.ErrorContains(t, err, "character set introducer")
		})
	}
}

func TestNormalizeStatementDigestKeepsValidCharsetIntroducersAfterTruncation(t *testing.T) {
	got, err := NormalizeStatementDigest(context.Background(), "SELECT _utf8'a'", "", 2)
	require.NoError(t, err)
	require.Equal(t, "SELECT", got)
}

func TestNormalizeStatementDigestVariableBudget(t *testing.T) {
	tests := []struct {
		name      string
		sql       string
		sqlMode   string
		...
		{name: "user variable prefix", sql: "SELECT @x", maxLength: 4, want: "SELECT @"},
		{name: "user variable value", sql: "SELECT @x", maxLength: 6, want: "SELECT @?"},
		{name: "quoted user variable prefix", sql: "SELECT @'x'", maxLength: 4, want: "SELECT @"},
		{name: "quoted user variable value", sql: "SELECT @'x'", maxLength: 6, want: "SELECT @?"},
		{name: "quoted identifier prefix", sql: "SELECT @`x`", maxLength: 8, want: "SELECT @"},
		{name: "quoted identifier", sql: "SELECT @`x`", maxLength: 9, want: "SELECT @`x`"},
		{name: "system variable prefix", sql: "SELECT @@sql_mode", maxLength: 4, want: "SELECT @"},
		{name: "system variable marker", sql: "SELECT @@sql_mode", maxLength: 6, want: "SELECT @@"},
		{name: "system variable identifier prefix", sql: "SELECT @@sql_mode", maxLength: 17, want: "SELECT @@"},
		{name: "system variable identifier", sql: "SELECT @@sql_mode", maxLength: 18, want: "SELECT @@`sql_mode`"},
		{name: "scoped system variable scope", sql: "SELECT @@session.sql_mode", maxLength: 8, want: "SELECT @@SESSION"},
		{name: "scoped system variable dot", sql: "SELECT @@session.sql_mode", maxLength: 10, want: "SELECT @@SESSION ."},
		{name: "scoped system variable identifier", sql: "SELECT @@session.sql_mode", maxLength: 22, want: "SELECT @@SESSION . `sql_mode`"},
		{name: "quoted system variable prefix", sql: "SELECT @@`a.b`", maxLength: 12, want: "SELECT @@"},
		{name: "quoted system variable with dot", sql: "SELECT @@`a.b`", maxLength: 13, want: "SELECT @@`a.b`"},
		{name: "ordinary system variable scope prefix", sql: "SELECT @@foo.bar", maxLength: 12, want: "SELECT @@"},
		{name: "ordinary system variable scope", sql: "SELECT @@foo.bar", maxLength: 13, want: "SELECT @@`foo`"},
		{name: "ordinary system variable scope dot", sql: "SELECT @@foo.bar", maxLength: 15, want: "SELECT @@`foo` ."},
		{name: "ordinary system variable", sql: "SELECT @@foo.bar", maxLength: 22, want: "SELECT @@`foo` . `bar`"},
		{name: "spaced quoted identifier prefix", sql: "SELECT @ `x`", maxLength: 8, want: "SELECT @"},
		{name: "spaced quoted identifier", sql: "SELECT @ `x`", maxLength: 9, want: "SELECT @`x`"},
		{name: "user variable list reduction", sql: "SELECT @x, 1", maxLength: 1024, want: "SELECT @?, ..."},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := NormalizeStatementDigest(context.Background(), test.sql, test.sqlMode, test.maxLength)
			require.NoError(t, err)
			require.Equal(t, test.want, got)
		})
	}
...
func TestNormalizeStatementDigestANSIQuotedUserVariableBackslash(t *testing.T) {
	got, err := NormalizeStatementDigest(context.Background(), `SELECT @"a\"`, "ANSI_QUOTES", 1024)
	require.NoError(t, err)
	require.Equal(t, "SELECT @`a\\`", got)
}
