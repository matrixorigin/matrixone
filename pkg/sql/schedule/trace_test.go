...
func TestTraceRecorderCapturesSchedulingDecisionChain(t *testing.T) {
	...
	require.Equal(t, "unclassified", trace.Attempts[0].Query.WorkloadClass)
	require.Equal(t, "legacy", trace.Attempts[0].Query.WorkloadPolicySource)
	require.Equal(t, "multi-worker", trace.Attempts[0].Query.WorkloadRouting)
