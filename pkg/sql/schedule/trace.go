...
func (r *TraceRecorder) materializePendingLocalLocked() {
	...
	if pending.hasQuery {
		...
		attempt.Query = &QueryTrace{
			...
			WorkloadPolicySource: string(WorkloadPolicySourceLegacy),
			WorkloadPolicyReason: "workload-policy-not-configured",
			WorkloadRouting:      string(legacyWorkloadRouting(query.execKind)),
...
func traceWorker(worker Worker) WorkerTrace {
	return WorkerTrace{
		...
		Routable: hasWorkerRoute(worker),
		Route:    worker.Route.String(),
		Mcpu:     worker.Mcpu,
		State:    worker.State.String(),
	}
}

func selectionKeyTraceHash(key string) string {
	if key == "" {
		return ""
	}
	return strconv.FormatUint(stableHRWScore("trace", key), 16)
}
