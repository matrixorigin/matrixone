...
func TestHandleServer(t *testing.T) {
	testRPCServer(t, func(rs *server) {
		...
		rs.RegisterRequestHandler(func(_ context.Context, request RPCMessage, sequence uint64, cs ClientSession) error {
			return cs.Write(ctx, request.Message, SyncWrite)
		})

		req := newTestMessage(1)
		f, err := c.Send(ctx, testAddr, req, SyncWrite)
...
func TestHandleServerWriteWithClosedSession(t *testing.T) {
	...
	testRPCServer(t, func(rs *server) {
		...
		rs.RegisterRequestHandler(func(_ context.Context, request RPCMessage, _ uint64, cs ClientSession) error {
			assert.NoError(t, c.Close())
			err := cs.Write(ctx, request.Message, SyncWrite)
			assert.Error(t, err)
			return err
		})
	...
	testRPCServer(t, func(rs *server) {
		...
		rs.RegisterRequestHandler(func(ctx context.Context, msg RPCMessage, seq uint64, cs ClientSession) error {
			...
			if seq == 1 {
				...
				if err := cs.Write(ctx, m, SyncWrite); err != nil {
					return err
				}
				if err := cs.Write(ctx, request, SyncWrite); err != nil {
					return err
				}
			}
			return nil
		...
		req1.payload = []byte{1}
		assert.NoError(t, st.Send(ctx, req1, SyncWrite))

		req2 := newTestMessage(st.ID())
		req2.payload = []byte{2}
		assert.NoError(t, st.Send(ctx, req2, SyncWrite))
...
func TestStreamReadCannotBlockWrite(t *testing.T) {
	testRPCServer(t, func(rs *server) {
		...
		for i := 0; i < n; i++ {
			require.NoError(t, st.Send(ctx, newTestMessage(st.ID()), SyncWrite))
		}
		<-cc
	})
}
...
func TestPingError(t *testing.T) {
	testRPCServer(t, func(rs *server) {
		...
		rs.RegisterRequestHandler(func(_ context.Context, request RPCMessage, _ uint64, cs ClientSession) error {
			return cs.Write(context.Background(), request.Message, SyncWrite)
		})
