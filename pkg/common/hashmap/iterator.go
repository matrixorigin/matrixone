...
func (itr *intHashMapIterator) Find(start, count int, vecs []*vector.Vector) []uint64 {
	defer func() {
		for i := 0; i < count; i++ {
			itr.mp.keys[i] = 0
		}
	...
	return itr.mp.values[:count]
}

func (itr *intHashMapIterator) Insert(start, count int, vecs []*vector.Vector) []uint64 {
	defer func() {
		for i := 0; i < count; i++ {
			itr.mp.keys[i] = 0
		}
