...
// AlterCopyInitSQL must rebuild ivfpq from source on a COPY ALTER: cloneUnaffectedIndexes SKIPS
// the clone (SkipWholeIndex), so the replacement hidden tables start empty, and IvfpqSync only
...
func TestAlterCopyInitSQL(t *testing.T) {
	...
	startFromNow, initSQL, err := (Hooks{}).AlterCopyInitSQL(ctx, ivfpqIndexDefs())
	...
	require.Equal(t, "ALTER TABLE `db1`.`t` ALTER REINDEX `ix` ivfpq FORCE_SYNC", initSQL)
	...
	_, restoreSQL, err := (Hooks{}).RestoreInitSQL(ctx, ivfpqIndexDefs())
	require.NoError(t, err)
	require.Equal(t, "ALTER TABLE `db1`.`t` ALTER REINDEX `ix` ivfpq FORCE_SYNC", restoreSQL)
	...
	btdefs := map[string]*plan.IndexDef{catalog.Ivfpq_TblType_Metadata: {IndexName: "id`x"}}
	_, btSQL, err := (Hooks{}).AlterCopyInitSQL(btctx, btdefs)
	require.NoError(t, err)
	require.Equal(t, "ALTER TABLE `d``b`.`s``rc` ALTER REINDEX `id``x` ivfpq FORCE_SYNC", btSQL)

	_, btRestoreSQL, err := (Hooks{}).RestoreInitSQL(btctx, btdefs)
	require.NoError(t, err)
	require.Equal(t, "ALTER TABLE `d``b`.`s``rc` ALTER REINDEX `id``x` ivfpq FORCE_SYNC", btRestoreSQL)
