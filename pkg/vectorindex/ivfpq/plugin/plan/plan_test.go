...
func TestCanApply_Redirects(t *testing.T) {
	// CanApplyIvfpq on the shared stub panics; recover confirms the
	// redirect line executed (and routes to the ivfpq variant).
	defer func() {
		require.NotNil(t, recover(), "CanApply must reach pb.CanApplyIvfpq")
	}()
...
func TestApplyForSort_Redirects(t *testing.T) {
	defer func() {
		require.NotNil(t, recover(), "ApplyForSort must reach pb.ApplyIndicesForSortUsingIvfpq")
	}()
