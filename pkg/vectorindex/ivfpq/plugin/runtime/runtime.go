...
// SupportedVectorTypes: IVF-PQ (cuvs) accepts f32 and f16 base columns. f16 is
...
// ValidQuantization gates the (quantization, op_type) pair for IVF-PQ (cuvs):
...
func (CatalogHooks) ValidQuantization(quant, op string) error {
	...
	if !metric.ValidQuantization(quant) {
		return moerr.NewNotSupportedNoCtxf(
			"ivfpq quantization %q (supported: float32, float16, int8, uint8)", quant)
	}
	if quant == metric.Quantization_INT8_Str || quant == metric.Quantization_UINT8_Str {
		...
		case metric.OpType_InnerProduct, metric.OpType_CosineDistance:
			return moerr.NewNotSupportedNoCtxf(
				"ivfpq quantization %q is only supported with L2 (op_type 'vector_l2_ops'); the int8/uint8 affine quantizer does not preserve inner-product / cosine geometry", quant)
		}
	}
	return nil
}
...
func (CatalogHooks) ParamsFromTree(idx *tree.Index) (map[string]string, error) {
	...
	if idx.IndexOption.AlgoParamList <= 0 {
		return nil, moerr.NewInvalidInputNoCtx("invalid lists. lists must be > 0 for IVFPQ")
	}
