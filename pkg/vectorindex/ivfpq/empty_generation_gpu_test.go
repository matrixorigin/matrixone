...
// emptyGenDim is divisible by emptyGenPqM, as IVF-PQ requires.
const (
	emptyGenDim  = 16
	emptyGenPqM  = 8
	emptyGenRows = 1000
...
func emptyGenCfg() vectorindex.IndexConfig {
	...
	cfg.CuvsIvfpq.Dimensions = emptyGenDim
...
// emptyGenLoad assembles a loaded IvfpqSearch the way Load does — buildOverflow then
...
func emptyGenLoad(t *testing.T, models []*IvfpqModel[float32, float32]) *IvfpqSearch[float32, float32] {
	t.Helper()
...
func TestIvfpqEmptyGenerationIsNotRetained(t *testing.T) {
	// No metadata rows.
	...
	bare := emptyGenLoad(t, []*IvfpqModel[float32, float32]{{Id: "m0"}, {Id: "m1"}})
...
func TestIvfpqEmptyGenerationKeepsCdcOnlyGeneration(t *testing.T) {
	s := emptyGenLoad(t, []*IvfpqModel[float32, float32]{
...
func TestIvfpqEmptyGenerationKeepsBuiltIndex(t *testing.T) {
	bp := cuvs.DefaultIvfPqBuildParams()
	bp.NLists = 10
	bp.M = emptyGenPqM
	bp.KmeansTrainsetFraction = 1.0
	idx, err := cuvs.NewGpuIvfPq[float32, float32](emptyGenRowsVecs(emptyGenRows), emptyGenRows,
		emptyGenDim, cuvs.L2Expanded, bp, []int{0}, 1, cuvs.SingleGpu, nil)
	require.NoError(t, err)
	require.NoError(t, idx.Start())
	require.NoError(t, idx.Build())

	// The model owns idx from here — IvfpqSearch.Destroy releases it via the model.
	s := emptyGenLoad(t, []*IvfpqModel[float32, float32]{{Id: "m0", Index: idx, Len: emptyGenRows}})
