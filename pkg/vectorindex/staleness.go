...
//   - MAX(storage.chunk_id) of the (CdcTailId, tag=1) event tail: a CDC append bumps it.
//
...
// to (CdcTailId, tag=1) — the exact CDC delta, which every cuvs index writes under activeIndexId
// == CdcTailId — so an unrelated base sub-index's higher chunk_id can't mask a fresh append. An
...
func CdcGenerationSqls(tblcfg IndexTableConfig) (tsSQL, tailSQL string) {
	...
	tailSQL = fmt.Sprintf("SELECT COALESCE(MAX(chunk_id), -1) FROM %s WHERE index_id = %s AND tag = %d",
		sqlquote.QualifiedIdent(tblcfg.DbName, tblcfg.IndexTable), sqlquote.String(CdcTailId), int(Tag_CdcEvents))
	return
}
...
func LoadCdcGeneration(sqlproc *sqlexec.SqlProcess, tblcfg IndexTableConfig) (ts int64, tail int64, err error) {
	defer func() {
		if r := recover(); r != nil {
...
func QueryCdcGeneration(ctx context.Context, cnUUID string, accountID uint32, tblcfg IndexTableConfig) (ts int64, tail int64, err error) {
	defer func() {
		if r := recover(); r != nil {
