...
type RuntimeConfig struct {
	...
	// once per batch (Search then returns empty keys/distances). Only the WAND
	// fulltext index honors it, and only for the no-LIMIT case (return every
	// matching doc, ranked by an upstream ORDER BY) — so it walks and streams
	// without a top-K heap. Other algorithms ignore this field.
