...
// An index whose cost is entirely CDC tail declares ZERO base docs, so before the tail was
// added to Preload it published (0,0): makeRoom took its "nothing to account for" exit BEFORE
// registering a reservation, and two cold misses for DISTINCT named-snapshot keys of that index
// each tested the same pre-allocation free-memory reading, each passed because one tail fits,
// and each then materialized. The reservation is what serializes them, and it only sees an
// arrival that declares bytes in Preload.
func TestGovernorAdmitsDistinctSnapshotKeysOfATailOnlyIndex(t *testing.T) {
	c := newBoundCache(t)
	sp := govProc(t, c, 1, caps{}, hostCap(150))

	base := "__mo_index_secondary_tailonly"
	keys := []string{
		SnapshotKey(base, snapshotTS(100)),
		SnapshotKey(base, snapshotTS(200)),
	}
	...
	algos := make([]*gatedSearch, 0, len(keys))
	errs := make(chan error, len(keys))
	for _, key := range keys {
		// host=100 is the tail's declared peak: what Preload now publishes for an index with
		// no base docs at all.
		algo := &gatedSearch{
			countingSearch: countingSearch{host: 100},
			arrived:        &arrived,
			start:          start,
			gate:           gate,
		}
	...
	arrived.Wait() // both measured, neither admitted
	close(start)

	select {
	case err := <-errs:
		require.Error(t, err, "two tails do not fit under a budget for one")
		require.Contains(t, err.Error(), "index cache is full")
	case <-time.After(10 * time.Second):
		t.Fatal("neither snapshot key was refused; both are materializing")
	}
	...
	require.EqualValues(t, 1, loaded, "the refused key must not have reached its allocation")

	once.Do(func() { close(gate) })
	select {
	case err := <-errs:
		require.NoError(t, err, "and the admitted one completes")
	case <-time.After(10 * time.Second):
		t.Fatal("the admitted load never finished")
	}
}
