...
func TestBuildOverflowBFReleasesHostCopies(t *testing.T) {
	const dim = 3
	models := []*CagraModel[float32, float32]{
		{
			Id:                   "m0",
...
func TestBuildOverflowBFSkipsEmptyModel(t *testing.T) {
	models := []*CagraModel[float32, float32]{
		{Id: "empty"},
...
func TestBuildOverflowBFSearchesCorrectlyAfterRelease(t *testing.T) {
	const dim = 3
...
func TestBuildOverflowBFRanksCorrectlyAfterRelease(t *testing.T) {
	const dim = 2
	m := &CagraModel[float32, float32]{
		Id:            "m0",
		OverflowPkids: []int64{11, 22, 33},
		OverflowVecs:  []float32{0, 0, 10, 0, 20, 0},
	}

	bf, err := buildOverflowBF[float32, float32]([]*CagraModel[float32, float32]{m}, 3, dim, cuvs.L2Expanded, 0, 1)
	require.NoError(t, err)
	t.Cleanup(func() { _ = bf.Destroy() })
