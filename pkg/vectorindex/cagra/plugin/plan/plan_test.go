...
// cagraScanNode mirrors the original test's fixture: vec_col at pos 0,
// id PK at pos 1.
func cagraScanNode() *pbplan.Node {
	return &pbplan.Node{
...
func TestPrepareCagraIndexContext_ResolveThreadsError(t *testing.T) {
	...
	r, err := cagraplan.PrepareContext(b, cagraVecCtx(cagraScanNode()),
...
// (TestPrepareCagraIndexContext_ResolveProbeLimitError omitted — CAGRA
// does not resolve probe_limit; the equivalent error path is the
// batch_window error test above.)
...
func TestPrepareCagraIndexContext_Success(t *testing.T) {
	...
	r, err := cagraplan.PrepareContext(b, cagraVecCtx(cagraScanNode()), cagraMTI(algo))
