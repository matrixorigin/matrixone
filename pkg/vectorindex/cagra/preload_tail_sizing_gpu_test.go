...
func preloadHarness(t *testing.T, sid string, exec executor.SQLExecutor) (*sqlexec.SqlProcess, *CagraSearch[float32, float32]) {
	t.Helper()
	moruntime.SetupServiceBasedRuntime(sid, moruntime.DefaultRuntime())
...
func TestCagraPreloadChargesThePartiallyCoveredTail(t *testing.T) {
	mp := mpool.MustNewZero()
	const legacyChunks, newRows, newChunks = 400, 1, 1
	sqlproc, s := preloadHarness(t, "ut-cagra-preload-covered", &routedExecutor{
...
func TestCagraPreloadRefusesWhenTheChunkCountIsUnreadable(t *testing.T) {
	mp := mpool.MustNewZero()
	sqlproc, s := preloadHarness(t, "ut-cagra-preload-count-err", &routedExecutor{
		meta:     func() (executor.Result, error) { return executor.Result{Mp: mp}, nil },
...
func TestCagraPreloadRefusesWhenNoSizingReadSucceeds(t *testing.T) {
	mp := mpool.MustNewZero()
	sqlproc, s := preloadHarness(t, "ut-cagra-preload-all-err", &routedExecutor{
		meta:     func() (executor.Result, error) { return executor.Result{Mp: mp}, nil },
		coverage: readErr,
		count:    readErr,
	})
...
func TestCagraPreloadFallsBackToChunksOnANarrowTable(t *testing.T) {
	mp := mpool.MustNewZero()
	const chunks = 10
	sqlproc, s := preloadHarness(t, "ut-cagra-preload-narrow", &routedExecutor{
		meta:     func() (executor.Result, error) { return executor.Result{Mp: mp}, nil },
		coverage: readErr,
...
func TestCagraPreloadRefusesWhenMetadataIsUnreadable(t *testing.T) {
	sqlproc, s := preloadHarness(t, "ut-cagra-preload-meta-err", &routedExecutor{
		meta:     readErr,
		coverage: readErr,
		count:    readErr,
	})
