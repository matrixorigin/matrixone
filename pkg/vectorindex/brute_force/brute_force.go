...
func (idx *UsearchBruteForceIndex[T]) Load(proc *process.Process) error {
	return nil
}

func (idx *UsearchBruteForceIndex[T]) Search(proc *process.Process, _queries any, rt vectorindex.RuntimeConfig) (keys any, distances []float64, err error) {
	...
	if queryDeallocator != nil {
		defer queryDeallocator.Deallocate(malloc.NoHints)
	}
...
func (idx *UsearchBruteForceIndex[T]) Destroy() {
	if idx.deallocator != nil {
		idx.deallocator.Deallocate(malloc.NoHints)
		idx.deallocator = nil
		idx.Dataset = nil
...
func (idx *GoBruteForceIndex[T]) Search(proc *process.Process, _queries any, rt vectorindex.RuntimeConfig) (keys any, distances []float64, err error) {
	queries, ok := _queries.([][]T)
	if !ok {
		return nil, nil, moerr.NewInternalErrorNoCtx("queries type invalid")
	}
