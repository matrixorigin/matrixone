...
func TestGoBruteForceSmallQueryCount(t *testing.T) {
	// Generate random dataset
	dsize := 500
	dimension := uint(8)
	dataset := make([][]float32, dsize)
	...
	qsize := 1
	queries := make([][]float32, qsize)
	queries[0] = make([]float32, dimension)
	for j := range queries[0] {
		queries[0][j] = rand.Float32()
	}
	...
	// Use more threads than queries to trigger dataset splitting
	limit := uint(10)
	rt := vectorindex.RuntimeConfig{Limit: limit, NThreads: 4}
	keysAny, dists, err := idx.Search(sqlproc, queries, rt)
	require.NoError(t, err)

	keys := keysAny.([]int64)
	require.Equal(t, int(limit), len(keys))
	require.Equal(t, int(limit), len(dists))
	...
	allRes := make([]res, dsize)
	for j := 0; j < dsize; j++ {
		d, _ := metric.L2DistanceSq(queries[0], dataset[j])
		allRes[j] = res{id: int64(j), dist: float64(d)}
	}
	...
	for j := 0; j < int(limit); j++ {
		require.InDeltaf(t, allRes[j].dist, dists[j], 1e-5, "Distance mismatch at rank %d", j)
		require.Equal(t, allRes[j].id, keys[j], "ID mismatch at rank %d", j)
	}
}
