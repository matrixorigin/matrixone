...
// Internal reduction helpers, designed to be inlined
func sumF32x16(v archsimd.Float32x16) float32 {
	v8 := v.GetLo().Add(v.GetHi())
	v4 := v8.GetLo().Add(v8.GetHi())
	var a [4]float32
	v4.Store(&a)
...
func L2DistanceSqFloat32(a, b []float32) (float32, error) {
	...
	if archsimd.X86.AVX512() {
		...
		sum += sumF32x16(res)
	} else if (archsimd.X86.AVX2() || archsimd.X86.AVX()) {
