...
var upgradeEntries = []versions.UpgradeEntry{upg_test_view, upg_mo_test}

var test_view_sql = fmt.Sprintf("CREATE VIEW information_schema.test_view AS select "+
...
var upg_test_view = versions.UpgradeEntry{
	...
	CheckFunc: func(txn executor.TxnExecutor) (bool, error) {
		isExisted, _, err := versions.CheckViewDefinition(txn, "information_schema", "test_view")
		if err != nil {
			return false, err
		}
...
var upg_mo_test = versions.UpgradeEntry{
	Schema:    "mo_catalog",
	TableName: "mo_test",
	UpgType:   versions.CREATE_NEW_TABLE,
	TableType: versions.BASE_TABLE,
	UpgSql: "CREATE TABLE IF NOT EXISTS mo_catalog.mo_test (" +
		...
		");",
	CheckFunc: func(txn executor.TxnExecutor) (bool, error) {
		isExist, err := versions.CheckTableDefinition(txn, "mo_catalog", "mo_test")
		if err != nil {
			return false, nil
		}
