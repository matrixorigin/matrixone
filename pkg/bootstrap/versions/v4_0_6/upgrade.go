	...
	for _, upgEntry := range tenantUpgEntries {
		...
		if err := upgEntry.Upgrade(txn, uint32(tenantID)); err != nil {
			getLogger(txn.Txn().TxnOptions().CN).Error("tenant upgrade entry execute error",
				zap.Error(err), zap.Int32("tenantId", tenantID), zap.String("version", v.Metadata().Version), zap.String("upgrade entry", upgEntry.String()))
			return err
		}
		getLogger(txn.Txn().TxnOptions().CN).Info("tenant upgrade entry complete",
			zap.String("upgrade entry", upgEntry.String()), zap.Int64("time cost(ms)", time.Since(start).Milliseconds()), zap.String("toVersion", v.Metadata().Version))
	}
...
// rel_createsql is the durable logical table definition, so it is safe to use as
// the migration source instead of relying on physical catalog row order.
	...
	for _, upgEntry := range clusterUpgEntries {
		...
		if err := upgEntry.Upgrade(txn, uint32(txn.Txn().TxnOptions().AccountID)); err != nil {
			getLogger(txn.Txn().TxnOptions().CN).Error("cluster upgrade entry execute error",
				zap.Error(err), zap.String("version", v.Metadata().Version), zap.String("upgrade entry", upgEntry.String()))
			return err
		}
