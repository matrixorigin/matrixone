...
func TestUpgradeEntries(t *testing.T) {
	...
	require.Equal(t, int64(defines.MORPCVersion35), checkConstraints.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion35), tableConstraints.RequiredProtocolVersion)
	...
	for _, entry := range tenantUpgEntries {
		if strings.Contains(entry.UpgSql+entry.PostSql, "mo_current_roles()") {
			require.Equal(t, int64(defines.MORPCVersion35), entry.RequiredProtocolVersion,
	...
	require.Equal(t, int64(defines.MORPCVersion35), roleGrantIndex.RequiredProtocolVersion)
