...
func TestViewsUpgradeProtocolGenerations(t *testing.T) {
	for _, peer := range []int64{defines.MORPCVersion88, defines.MORPCVersion89} {
		t.Run(fmt.Sprintf("views-gate-%d-peer-%d", defines.MORPCVersion89, peer), func(t *testing.T) {
			var executed []string
			...
			err := entry.Upgrade(txn, 0)
			if peer < defines.MORPCVersion89 {
				require.ErrorContains(t, err, "requires all CNs to support protocol version 89")
				require.Empty(t, executed, "an old peer must block before DROP/DELETE or DDL")
			} else {
				require.NoError(t, err)
				require.Contains(t, executed, entry.UpgSql)
...
func TestUpgradeEntries(t *testing.T) {
	...
	for _, entry := range tenantUpgEntries {
		...
		if entry.TableName == "VIEWS" {
			require.Equal(t, int64(defines.MORPCVersion89), entry.RequiredProtocolVersion,
	...
	for i, view := range metadataViews {
		...
		if view.name == "TABLES" || view.name == "COLUMNS" {
			...
			expectedProtocol = defines.MORPCVersion89
		}
