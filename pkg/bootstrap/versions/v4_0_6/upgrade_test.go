...
func TestViewsUpgradeProtocolGenerations(t *testing.T) {
	for _, peer := range []int64{defines.MORPCVersion81, defines.MORPCVersion82} {
		t.Run(fmt.Sprintf("views-gate-%d-peer-%d", defines.MORPCVersion82, peer), func(t *testing.T) {
			var executed []string
			...
			err := entry.Upgrade(txn, 0)
			if peer < defines.MORPCVersion82 {
				require.ErrorContains(t, err, "requires all CNs to support protocol version 82")
				require.Empty(t, executed, "an old peer must block before DROP/DELETE or DDL")
			} else {
				require.NoError(t, err)
				require.Contains(t, executed, entry.UpgSql)
...
func TestUpgradeEntries(t *testing.T) {
	...
	for _, entry := range tenantUpgEntries {
		...
		if entry.TableName == "VIEWS" {
			require.Equal(t, int64(defines.MORPCVersion82), entry.RequiredProtocolVersion,
	...
	for i, view := range metadataViews {
		...
		if view.name == "TABLES" || view.name == "COLUMNS" {
			...
			expectedProtocol = defines.MORPCVersion82
		}
