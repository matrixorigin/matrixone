...
func TestUpgradeEntries(t *testing.T) {
	...
	require.Equal(t, int64(defines.MORPCVersion37), checkConstraints.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion37), tableConstraints.RequiredProtocolVersion)
	...
	for _, entry := range tenantUpgEntries {
		if strings.Contains(entry.UpgSql+entry.PostSql, "mo_current_roles()") {
			require.Equal(t, int64(defines.MORPCVersion37), entry.RequiredProtocolVersion,
	...
	require.Equal(t, int64(defines.MORPCVersion37), roleGrantIndex.RequiredProtocolVersion)
	...
	txnExecutor := newVersionTxnExecutor(t, func(sql string) (executor.Result, error) {
		if strings.Contains(strings.ToLower(sql), "getprotocolversion") {
			return newProtocolVersionResultValue(t,
				`{"method":"GETPROTOCOLVERSION","result":"cn-a:37,cn-b:37"}`), nil
		}
		executed = append(executed, sql)
