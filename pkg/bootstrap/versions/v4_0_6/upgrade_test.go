...
func TestUpgradeEntries(t *testing.T) {
	...
	require.Equal(t, int64(defines.MORPCVersion55), columns.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion55), hideInternalColumns.RequiredProtocolVersion)
	...
	for i, view := range metadataViews {
		...
		if view.name == "TABLES" {
			...
			expectedProtocol = defines.MORPCVersion55
		}
	...
	require.Equal(t, int64(defines.MORPCVersion55), columnsBinaryStrings.RequiredProtocolVersion)
	...
	require.Equal(t, int64(defines.MORPCVersion55), characterSetsUTF8Maxlen.RequiredProtocolVersion)
}
