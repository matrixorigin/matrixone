...
func TestIcebergP1TenantUpgradeEntries(t *testing.T) {
	if len(tenantUpgEntries) != len(icebergsql.P1WriteSystemTableDDLs) {
		t.Fatalf("expected %d Iceberg P1 tenant upgrades, got %d", len(icebergsql.P1WriteSystemTableDDLs), len(tenantUpgEntries))
	}
