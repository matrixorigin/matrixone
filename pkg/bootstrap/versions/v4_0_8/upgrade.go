...
func (v *versionHandle) HandleClusterUpgrade(ctx context.Context, txn executor.TxnExecutor) error {
	for _, table := range []struct{ name, ddl string }{
		...
		entry := versions.UpgradeEntry{Schema: catalog.MO_CATALOG, TableName: table.name, UpgType: versions.CREATE_NEW_TABLE, UpgSql: table.ddl,
			CheckFunc: func(txn executor.TxnExecutor, account uint32) (bool, error) {
				return versions.CheckTableDefinition(txn, account, catalog.MO_CATALOG, table.name)
			},
		}
	...
	result, err := txn.Exec(catalog.MoViewRecoveryInitSQL, executor.StatementOption{})
	if err != nil {
		return err
	}
	result.Close()
