...
func (a AddLogService) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
...
func (a RemoveLogService) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
...
func (a StartLogService) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
...
func (a StopLogService) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
...
func (a KillLogZombie) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
	if store, ok := state.Stores[a.UUID]; ok {
		for _, replicaInfo := range store.Replicas {
			if replicaInfo.ShardID == a.ShardID {
				return false
...
func (a AddDnReplica) IsFinish(_ pb.LogState, state pb.TNStore, _ pb.CNState) bool {
...
func (a RemoveDnReplica) IsFinish(_ pb.LogState, state pb.TNStore, _ pb.CNState) bool {
	for _, info := range state.Stores[a.StoreID].Shards {
		if a.ShardID == info.GetShardID() && a.ReplicaID == info.GetReplicaID() {
			return false
		}
...
func (a StopDnStore) IsFinish(_ pb.LogState, state pb.TNStore, _ pb.CNState) bool {
...
func (a StopLogStore) IsFinish(state pb.LogState, _ pb.TNStore, _ pb.CNState) bool {
	if _, ok := state.Stores[a.StoreID]; ok {
		return false
	}
	return true
...
func (a DeleteCNStore) IsFinish(_ pb.LogState, _ pb.TNStore, state pb.CNState) bool {
	if _, ok := state.Stores[a.StoreID]; ok {
		return false
	}
	return true
