...
func TestIssue27088BinaryPreparedINPreservesWireDomains(t *testing.T) {
	embed.RunBaseClusterTests(t, func(c embed.Cluster) {
		...
		for _, test := range []struct {
			...
			{"select (? + cast(nextval('volatile_seq') as decimal(20,0))) between lo and hi from volatile_bounds", []any{"0"}, true},
			{"select (? + cast(nextval('volatile_seq') as decimal(20,0))) not between hi + 1 and hi + 1 from volatile_bounds", []any{"0"}, true},
			{"select (? + cast(nextval('volatile_seq') as decimal(20,0))) in (?, ?) from volatile_bounds", []any{"0", "1.1", "3.0"}, false},
			{"select (? + cast(nextval('volatile_seq') as decimal(20,0))) not in (?, ?) from volatile_bounds", []any{"0", "1.1", "3.0"}, true},
