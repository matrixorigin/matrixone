...
func TestIssue26875ForeignKeyActions(t *testing.T) {
	embed.RunBaseClusterTests(t, func(c embed.Cluster) {
		...
		connectionsStarted := time.Now()
		conn := openIssue26875Conn(t, ctx, c, 0)
		defer conn.Close()
		conn2 := openIssue26875Conn(t, ctx, c, 1)
		defer conn2.Close()
		t.Logf("MO_UT_SETUP fixture=issue26875 phase=connections duration=%s", time.Since(connectionsStarted))
...
func runIssue26875ReplaceMaintainsIndexedForeignKeyChildren(t *testing.T, ctx context.Context, conn, conn2 *sql.Conn) {
...
func runIssue26875MixedIndexedSetNullAndCascadeActions(t *testing.T, ctx context.Context, conn *sql.Conn) {
...
func runIssue26875CombinedSetNullAndCascadeActions(t *testing.T, ctx context.Context, conn *sql.Conn) {
	for _, prepared := range []bool{false, true} {
		suffix := "literal"
		if prepared {
			suffix = "prepared"
...
func runIssue26875MultiRowSelfSetNullExcludesReplaceOwnedRows(t *testing.T, ctx context.Context, conn *sql.Conn) {
	cases := []struct {
		name string
...
func runIssue26875MultipleUniqueSetNullActionsTerminate(t *testing.T, ctx context.Context, conn *sql.Conn) {
...
func runIssue26875MultilevelCascadeRemapClosure(t *testing.T, ctx context.Context, conn *sql.Conn) {
...
func dropIssue26875Database(t *testing.T, conn *sql.Conn, dbName string) {
	...
	cleanupStatus := "error"
	defer func() {
		t.Logf("MO_UT_SETUP fixture=issue26875 phase=database-drop duration=%s status=%s",
