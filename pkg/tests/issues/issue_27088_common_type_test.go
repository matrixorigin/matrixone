...
func TestIssue27088PreparedDecimalCommonType(t *testing.T) {
	embed.RunBaseClusterTests(t, func(c embed.Cluster) {
		...
		mustExec(t, ctx, conn, `create table common_type (
			id int primary key,
			d decimal(20,4),
			low_bound decimal(20,4),
			high_bound decimal(20,4)
		)`)
		mustExec(t, ctx, conn, `insert into common_type values
			(1, 9007199254740992.0000, 9007199254740992.0000, 9007199254740992.0000),
			(2, 9007199254740992.0001, 9007199254740992.0000, 9007199254740992.0002),
			(3, 9007199254740992.0002, 9007199254740992.0002, 9007199254740992.0003)`)
		...
		t.Run("COM_STMT exact comparison and list", func(t *testing.T) {
			...
			rows, queryErr := equality.QueryContext(ctx, "9007199254740992.0001tail")
			...
			rows, queryErr = inList.QueryContext(ctx,
				"9007199254740992.0000first", "9007199254740992.0002second")
		...
		t.Run("COM_STMT row dependent between", func(t *testing.T) {
			...
			require.NoError(t, prepareErr)
			defer stmt.Close()
			rows, queryErr := stmt.QueryContext(ctx, "9007199254740992.0001tail")
			assertIDs(t, rows, queryErr, 2)
		})
		...
		t.Run("common value functions preserve context", func(t *testing.T) {
			...
			require.Equal(t, "12.5000", decimalValue)
		...
		t.Run("SQL PREPARE uses the same prefix domain", func(t *testing.T) {
			...
			mustExec(t, ctx, conn, "set @issue27088_value = '9007199254740992.0001tail'")
