...
// current shape succeeds. The same lifecycle also verifies that both ALTER TABLE rebuilds keep
// the metadata relkind that hides the index table from restore and CLONE table listings.
func TestV407UpgradeAddsIndexMetadataProvenanceColumns(t *testing.T) {
	embed.RunSingleCNBaseClusterTests(t, func(cluster embed.Cluster) {
		...
		relKind := func(t *testing.T) string {
			t.Helper()
			var k string
			require.NoError(t, conn.QueryRowContext(ctx, fmt.Sprintf(
		...
		hiddenByRelKind := func(t *testing.T) bool {
			t.Helper()
			var n int
			require.NoError(t, conn.QueryRowContext(ctx, fmt.Sprintf(
		...
		before := relKind(t)
		t.Run("RelKindAtCreate", func(t *testing.T) {
			require.Equal(t, catalog.Hnsw_TblType_Metadata, before,
				"a fresh hnsw metadata table carries its algorithm's kind")
			require.True(t, hiddenByRelKind(t), "and is excluded from the restore/CLONE listing")
		})
		...
		t.Run("RelKindAfterDropColumn", func(t *testing.T) {
			require.Equal(t, before, relKind(t), "DROP COLUMN's rebuild must keep the kind too")
		})
		...
		legacy := catalog.IndexMetadataInsertSql(dbName, metaTable, false,
			[]string{catalog.IndexMetadataRow(false, "probe-legacy", "chk", 1, 2, 3, 4)})
		...
		wide := catalog.IndexMetadataInsertSql(dbName, metaTable, true,
			[]string{catalog.IndexMetadataRow(true, "probe-wide", "chk", 1, 2, 3, 4)})
		...
		t.Run("RelKindAfterUpgrade", func(t *testing.T) {
			require.Equal(t, before, relKind(t),
			...
			require.True(t, hiddenByRelKind(t),
		...
		_, err = conn.ExecContext(ctx, catalog.IndexMetadataInsertSql(dbName, metaTable, true,
			[]string{catalog.IndexMetadataRow(true, "probe-wide2", "chk", 1, 2, 3, 4)}))
		require.NoError(t, err)
		_, err = conn.ExecContext(ctx, catalog.IndexMetadataInsertSql(dbName, metaTable, false,
			[]string{catalog.IndexMetadataRow(false, "probe-legacy2", "chk", 1, 2, 3, 4)}))
