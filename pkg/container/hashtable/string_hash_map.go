...
func (ht *StringHashMap) MarshalBinary() ([]byte, error) {
	...
	var activeCells []StringHashMapCell
	...
	for _, cell := range activeCells {
		buf.Write(types.EncodeUint64(&cell.HashState[0]))
		buf.Write(types.EncodeUint64(&cell.HashState[1]))
		buf.Write(types.EncodeUint64(&cell.HashState[2]))
...
func (ht *StringHashMap) MarshalTo(w io.Writer) (n int64, err error) {
	var wn int
	...
	for _, cell := range activeCells {
		wn, err = w.Write(types.EncodeUint64(&cell.HashState[0]))
		n += int64(wn)
		if err != nil {
			return
		}
		wn, err = w.Write(types.EncodeUint64(&cell.HashState[1]))
		n += int64(wn)
		if err != nil {
			return
		}
		wn, err = w.Write(types.EncodeUint64(&cell.HashState[2]))
		n += int64(wn)
		if err != nil {
			return
		}
...
func (ht *StringHashMap) UnmarshalFrom(r io.Reader, allocator malloc.Allocator) (err error) {
	// Read basic metadata
	buf := make([]byte, 8)
	...
	for i := uint64(0); i < numActiveCells; i++ {
		...
		cell.HashState[0] = types.DecodeUint64(buf)
		if _, err = io.ReadFull(r, buf); err != nil {
			return err
		}
		cell.HashState[1] = types.DecodeUint64(buf)
		if _, err = io.ReadFull(r, buf); err != nil {
			return err
		}
		cell.HashState[2] = types.DecodeUint64(buf)
		if _, err = io.ReadFull(r, buf); err != nil {
			return err
		}
