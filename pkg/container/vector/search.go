...
// enforced by the caller, not checked here: the function is wired exclusively as
// a SortedSearchFunc and is reached only via BlockReadFilter.DecideSearchFunc when
// the block's metadata reports IsSorted(); unsorted (or fake-PK) blocks are routed
// to the brute-force LinearCollectOffsetsByPrefixInFactory instead, which needs no
// ordering. Do not call this directly on data that is not known to be sorted.
