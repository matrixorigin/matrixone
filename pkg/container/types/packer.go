...
type PackerWithSchema struct {
	packer *Packer
	tuple  Tuple
	schema []T
}

func NewPackerWithSchema(sz uint64) *PackerWithSchema {
	return &PackerWithSchema{
		packer: NewPackerWithSize(sz),
		tuple:  make(Tuple, 0),
		schema: make([]T, 0),
	}
}

func (p *PackerWithSchema) Reset() {
	p.packer.Reset()
	if len(p.tuple) > 0 {
		p.tuple = p.tuple[:0]
	}
...
func (p *PackerWithSchema) Pakcer() *Packer {
	return p.packer
}

func (p *PackerWithSchema) Tuple() Tuple {
	return p.tuple
}

func (p *PackerWithSchema) Unpack(b []byte) (Tuple, error) {
	p.Reset()
	t, _, err := UnpackWithSchema(b)
	if err != nil {
		return nil, err
	}
	p.tuple = t
...
func (p *PackerWithSchema) UnpackTuple(b []byte) (Tuple, error) {
	var err error
	p.Reset()
	p.tuple, p.schema, err = UnpackTo(b, p.tuple, p.schema)
	if err != nil {
		return nil, err
	}
	return p.tuple, nil
