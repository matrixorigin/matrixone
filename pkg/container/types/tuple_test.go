...
func TestUnpackNthElement(t *testing.T) {
	...
	// Decimal256 followed by string
	packer6 := NewPacker()
	packer6.EncodeDecimal256(Decimal256{42, 0, 0, 0})
	packer6.EncodeStringType([]byte("after_dec256"))
	el5, schema5, err := UnpackNthElement(packer6.GetBuf(), 1)
	require.NoError(t, err)
	require.Equal(t, T_varchar, schema5)
	require.Equal(t, []byte("after_dec256"), el5)
}
