...
func (r *UInt16Ring) Fill(i int64, sel, _ int64, vec *vector.Vector) {
	if v := vec.Col.([]uint16)[sel]; r.Ns[i] == 0 || v < r.Vs[i] {
		r.Vs[i] = v
	}
...
func (r *UInt16Ring) BulkFill(i int64, _ []int64, vec *vector.Vector) {
	vs := vec.Col.([]uint16)
	for _, v := range vs {
...
func (r *UInt16Ring) Mul(_, _ int64) {
}
