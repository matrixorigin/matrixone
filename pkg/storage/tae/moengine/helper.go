...
func SchemaToDefs(schema *catalog.Schema) (defs []storage.TableDef, err error) {
	if schema.Comment != "" {
		commentDef := new(storage.CommentDef)
	...
	for _, col := range schema.ColDefs {
		...
		def := &storage.AttributeDef{
			Attr: storage.Attribute{
				Name:    col.Name,
				Type:    col.Type,
				Primary: col.IsPrimary(),
				Default: storage.MakeDefaultExpr(col.Default.Set, col.Default.Value, col.Default.Null),
			},
		}
		defs = append(defs, def)
	}
	if schema.SortKey != nil && schema.SortKey.IsPrimary() {
		pk := new(storage.PrimaryIndexDef)
...
func DefsToSchema(name string, defs []storage.TableDef) (schema *catalog.Schema, err error) {
	schema = catalog.NewEmptySchema(name)
	pkMap := make(map[string]int)
	for _, def := range defs {
		if pkDef, ok := def.(*storage.PrimaryIndexDef); ok {
