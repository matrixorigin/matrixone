...
import (
	...
	"github.com/matrixorigin/matrixone/pkg/storage/tae/dataio/mockio"
...
func TestEngine(t *testing.T) {
	...
	assert.Nil(t, err)
	err = e.Create(ctx, "db", storage.Snapshot(txn.GetCtx()))
	assert.Nil(t, err)
	names, _ := e.Databases(ctx, storage.Snapshot(txn.GetCtx()))
	assert.Equal(t, 2, len(names))
	dbase, err := e.Database(ctx, "db", storage.Snapshot(txn.GetCtx()))
	assert.Nil(t, err)

	schema := catalog.MockSchema(13, 12)
	defs, err := SchemaToDefs(schema)
	defs[5].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, int32(3), false)
	defs[6].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, nil, true)
	...
	rAttr := rDefs[5].(*storage.AttributeDef).Attr
	assert.Equal(t, int32(3), rAttr.Default.Value.(int32))
	rAttr = rDefs[6].(*storage.AttributeDef).Attr
	assert.Equal(t, true, rAttr.Default.IsNull)
	rAttr = rDefs[7].(*storage.AttributeDef).Attr
...
func TestEngineAllType(t *testing.T) {
	...
	defs, err := SchemaToDefs(schema)
	defs[5].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, uint16(3), false)
	defs[6].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, nil, true)
	assert.NoError(t, err)
	err = dbase.Create(ctx, schema.Name, defs)
	assert.Nil(t, err)
	...
	newbat := batch.New(true, basebat.Attrs)
	...
	bat := catalog.MockBatch(schema, 20)
	defer bat.Close()
	newbat1 := batch.New(true, bat.Attrs)
...
func TestTxnRelation_GetHideKey(t *testing.T) {
	...
	assert.Nil(t, txn.Commit())
	readers, _ := rel.NewReader(ctx, 10, nil, nil)
	delete := batch.New(true, bat.Attrs)
	m := mheap.New(guest.New(1<<20, host.New(1<<20)))
	for _, reader := range readers {
...
func checkSysTable(t *testing.T, name string, dbase storage.Database, txn Txn, relcnt int, schema *catalog.Schema) {
	ctx := context.TODO()
	defs, err := SchemaToDefs(schema)
	defs[5].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, int32(3), false)
	defs[6].(*storage.AttributeDef).Attr.Default = storage.MakeDefaultExpr(true, nil, true)
...
func TestSysRelation(t *testing.T) {
	...
	txn, err := e.StartTxn(nil)
	assert.Nil(t, err)
	err = e.Create(ctx, catalog.SystemTable_DB_Name, storage.Snapshot(txn.GetCtx()))
	assert.Nil(t, err)
	names, _ := e.Databases(ctx, storage.Snapshot(txn.GetCtx()))
	assert.Equal(t, 2, len(names))
	dbase, err := e.Database(ctx, catalog.SystemTable_DB_Name, storage.Snapshot(txn.GetCtx()))
	assert.Nil(t, err)
	schema := catalog.MockSchema(13, 12)
