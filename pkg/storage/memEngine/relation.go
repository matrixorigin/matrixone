...
func (r *relation) GetPrimaryKeys(_ context.Context) ([]*storage.Attribute, error) {
	return nil, nil
}

func (r *relation) GetHideKeys(_ context.Context) ([]*storage.Attribute, error) {
	return nil, nil
}

func (r *relation) TableDefs(_ context.Context) ([]storage.TableDef, error) {
	defs := make([]storage.TableDef, len(r.md.Attrs)+len(r.md.Index))
	for i, attr := range r.md.Attrs {
		defs[i] = &storage.AttributeDef{Attr: attr}
	}
	j := len(r.md.Attrs)
...
func (r *relation) NewReader(_ context.Context, n int, _ *plan.Expr, _ [][]byte) ([]storage.Reader, error) {
	segs := make([]string, r.md.Segs)
	for i := range segs {
		segs[i] = sKey(i, r.id)
	}
	attrs := make(map[string]storage.Attribute)
	{
		for i, attr := range r.md.Attrs {
			attrs[attr.Name] = r.md.Attrs[i]
		}
	}
	rs := make([]storage.Reader, n)
...
func (r *relation) AddTableDef(_ context.Context, _ storage.TableDef) error {
	return nil
}

func (r *relation) DelTableDef(_ context.Context, _ storage.TableDef) error {
	return nil
}
