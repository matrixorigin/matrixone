...
func CreateT1(db engine.Database) {
	...
	}
	{
		...
		{
			vec := vector.NewVec(types.Type{Oid: types.T(types.T_int32), Size: 4, Width: 4, Precision: 0})
			vs := make([]int32, 5)
			vs[0] = 1
			vs[1] = 2
			vs[2] = 2
		...
		{
			vec := vector.NewVec(types.Type{Oid: types.T(types.T_int8), Size: 1, Width: 1, Precision: 0})
			vs := make([]int8, 5)
			vs[0] = 1
			vs[1] = 2
			vs[2] = 4
