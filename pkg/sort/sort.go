...
type sortType interface {
	*vector.Vector | ~[]bool | ~[]int | ~[]int8 | ~[]int16 | ~[]int32 | ~[]int64 |
...
func Sort(desc, nullsLast, hasNull bool, os []int64, vec *vector.Vector) {
	...
	case types.T_char, types.T_varchar, types.T_blob, types.T_text, types.T_binary, types.T_varbinary:
		if !desc {
			genericSort(vec, os, unsafeStringLess)
		} else {
			genericSort(vec, os, unsafeStringGreater)
		}
...
func unsafeStringGreater(data *vector.Vector, i, j int64) bool {
	return data.UnsafeGetStringAt(int(i)) > data.UnsafeGetStringAt(int(j))
}

func unsafeStringLess(data *vector.Vector, i, j int64) bool {
	return data.UnsafeGetStringAt(int(i)) < data.UnsafeGetStringAt(int(j))
}
