...
func TestValidateAuthenticationFreshnessBudget(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantError:      "must be greater than authentication freshness budget 4.000000001s",
		},
		{
			name:           "pairwise skew and headroom boundary",
			maxClockOffset: time.Second,
			connectTimeout: 4*time.Second + time.Nanosecond,
			wantError:      "must be greater than authentication freshness budget 4.000000001s",
		},
		{
			name:           "strictly above pairwise skew and headroom",
			maxClockOffset: time.Second,
			connectTimeout: 4*time.Second + 2*time.Nanosecond,
		},
		{
