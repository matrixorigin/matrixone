...
func genCreateDatabaseTuple(
	...
	bat.Attrs = append(bat.Attrs, catalog.MoDatabaseSchema...)
	{
		...
		if err := vector.AppendFixed(bat.Vecs[idx], []byte(catalog.MO_CATALOG), false, m); err != nil {
			return nil, err
		}
		idx = catalog.MO_DATABASE_CREATESQL_IDX
		bat.Vecs[idx] = vector.NewVector(catalog.MoDatabaseTypes[idx])                   // dat_createsql
		if err := vector.AppendFixed(bat.Vecs[idx], []byte(sql), false, m); err != nil { // TODO
			return nil, err
		}
...
func genCreateColumnTuple(
	...
	bat.SetZs(1, m)
	{
		...
		bat.Vecs[idx] = vector.NewVector(catalog.MoColumnsTypes[idx]) // att_is_clusterby
...
func genCreateTableTuple(
	...
	bat.SetZs(1, m)
	{
		...
		if err := vector.AppendFixed(bat.Vecs[idx], []byte(comment), false, m); err != nil {
			return nil, err
		}
		idx = catalog.MO_TABLES_REL_CREATESQL_IDX
		bat.Vecs[idx] = vector.NewVector(catalog.MoTablesTypes[idx]) // rel_createsql
		if err := vector.AppendFixed(bat.Vecs[idx], []byte(sql), false, m); err != nil {
			return nil, err
		}
