...
func NewStrVector2[T any](opts ...*Options) *StrVector2[T] {
	var capacity int
...
func (vec *StrVector2[T]) GetAllocator() stl.MemAllocator {
	return vec.vdata.GetAllocator()
}

func (vec *StrVector2[T]) Length() int   { return vec.vdata.Length() }
func (vec *StrVector2[T]) Capacity() int { return vec.vdata.Capacity() }
func (vec *StrVector2[T]) Allocated() int {
...
func (vec *StrVector2[T]) Desc() string {
	s := fmt.Sprintf("StrVector2:Len=%d[Rows];Cap=%d[Rows];Allocted:%d[Bytes]",
		vec.Length(),
		vec.Capacity(),
		vec.Allocated())
	return s
}
func (vec *StrVector2[T]) String() string {
	s := vec.Desc()
	end := 100
	if vec.Length() < end {
		end = vec.Length()
...
func (vec *StrVector2[T]) Append(v T) {
	val := any(v).([]byte)
	length := len(val)
...
func (vec *StrVector2[T]) Get(i int) T {
	v := vec.vdata.Get(i)
	if v.IsSmall() {
...
func (vec *StrVector2[T]) Update(i int, v T) {
	val := any(v).([]byte)
	nlen := len(val)
...
func (vec *StrVector2[T]) Delete(i int) (deleted T) {
	deleted = vec.Get(i)
	vec.RangeDelete(i, 1)
	return
}

func (vec *StrVector2[T]) getAreaRange(offset, length int) (min, max int) {
	var pos int
...
func (vec *StrVector2[T]) rangeDeleteNoArea(offset, length int) {
...
func (vec *StrVector2[T]) Clone(offset, length int, allocator ...stl.MemAllocator) stl.Vector[T] {
	opts := &Options{
		Capacity: length,
	}
	if len(allocator) == 0 {
...
func (vec *StrVector2[T]) ReadFrom(r io.Reader) (n int64, err error) {
	var nr int64
...
func (vec *StrVector2[T]) WriteTo(w io.Writer) (n int64, err error) {
	var nr int64
