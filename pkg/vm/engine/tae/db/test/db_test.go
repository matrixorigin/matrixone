...
func TestIncrementalDedupSkipsTNRewrittenObject(t *testing.T) {
	...
	// This test exercises only the TN fallback. The rows predate insertTxn, but
	// a later TN flush gives their physical object a new CreatedAt timestamp.
	// That physical rewrite must not turn old rows into incremental candidates.
