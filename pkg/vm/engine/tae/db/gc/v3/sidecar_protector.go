...
func (p SidecarReadProtector) Begin(_ context.Context) (func(context.Context, []byte, []string, time.Time) error, func(), error) {
	if p.Manager == nil {
		return nil, nil, moerr.NewInternalErrorNoCtxf("sidecar read protector has no manager")
	}
	...
	return register, guard.Close, nil
}
...
func (p SidecarReadProtector) Register(ctx context.Context, readRef []byte, objects []string, expires time.Time) error {
	register, closeProtection, err := p.Begin(ctx)
	if err != nil {
		return err
	}
...
func registerSidecarReadProtection(guard *SyncProtectionGuard, readRef []byte, objects []string, expires time.Time) error {
	...
	return guard.EnsureSyncProtection(jobID, base64.StdEncoding.EncodeToString(data), expires.UnixNano(), jobID)
}
