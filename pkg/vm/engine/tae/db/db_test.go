...
func TestCNBlockRead(t *testing.T) {
	...
	assert.NoError(t, err)
	b1, err := blockio.BlockRead(context.Background(), columns, colIdxs, colTyps, colNulls, fs, pool, metaloc, deltaloc, beforeDel)
	assert.NoError(t, err)
	defer b1.Close()
	...
	b2, err := blockio.BlockRead(context.Background(), columns, colIdxs, colTyps, colNulls, fs, pool, metaloc, deltaloc, afterFirstDel)
	assert.NoError(t, err)
	defer b2.Close()
	assert.Equal(t, columns, b2.Attrs)
	assert.Equal(t, len(columns), len(b2.Vecs))
	assert.Equal(t, 19, b2.Vecs[0].Length())
	b3, err := blockio.BlockRead(context.Background(), columns, colIdxs, colTyps, colNulls, fs, pool, metaloc, deltaloc, afterSecondDel)
	assert.NoError(t, err)
	defer b3.Close()
	assert.Equal(t, columns, b2.Attrs)
	assert.Equal(t, len(columns), len(b2.Vecs))
	...
	b4, err := blockio.BlockRead(context.Background(), []string{catalog.AttrRowID}, []uint16{2}, []types.Type{types.T_Rowid.ToType()}, []bool{false}, fs, pool, metaloc, deltaloc, afterSecondDel)
	assert.NoError(t, err)
	defer b4.Close()
