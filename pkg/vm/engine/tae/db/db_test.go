...
func TestDiffCheckPoints(t *testing.T) {
	...
	assert.Equal(t, 0, tae.BGCheckpointRunner.GetPenddingIncrementalCount())
	checkpoints := tae.BGCheckpointRunner.MaxGlobalCheckpoint()
	t.Log(checkpoints)

	ctx := context.Background()
	data1, err := checkpoints.Read(ctx, tae.Fs)
	ins, del, _, _, err := checkpoints.GetByTableID(tae.Fs, 1000)
	assert.NoError(t, err)
	t.Logf("table %d", 1000)
	if ins != nil {
		t.Log(common.PrintApiBatch(ins, 3))
	...
	t.Log(data1)
	ins1, _, _, _ := data1.GetBlkBatchs()
	vec := ins1.GetVectorByName(pkgcatalog.BlockMeta_MetaLoc)
	t.Log(vec)
	m := make([]objectio.ObjectName, 0)
	for i := 0; i < vec.Length(); i++ {
		metaLoc := objectio.Location(vec.Get(i).([]byte))
		m = append(m, metaLoc.Name())
	}
	assert.Nil(t, err)
	...
	blk := it.GetBlock().GetMeta().(*catalog.BlockEntry)
	delMetaloc := blk.GetMetaLoc()
	t.Log(delMetaloc.Name())
	...
	for it1.Valid() {
		blk := it1.GetBlock().GetMeta().(*catalog.BlockEntry)
		metaloc := blk.GetMetaLoc()
		t.Log(metaloc.Name())
		it1.Next()
	}
	...
	t.Log(time.Since(now))
	data2 := logtail.NewCheckpointData()
	t.Log(data2)
	ins2, _, _, _ := data2.GetBlkBatchs()
	vectorByName1 := ins2.GetVectorByName(pkgcatalog.BlockMeta_MetaLoc)
	t.Log(vectorByName1)

	// diffs := logtail.DiffCheckPoints(data1, data2)
	// t.Log(diffs)
	// assert.Equal(t, vec.Length(), len(diffs))
	// for i := 0; i < len(diffs); i++ {
	// 	assert.Equal(t, m[i], diffs[i])
	// }

	checkpoints1 := tae.BGCheckpointRunner.MaxGlobalCheckpoint()
	t.Log(checkpoints1)
	ins, del, _, _, err = checkpoints1.GetByTableID(tae.Fs, 1000)
	assert.NoError(t, err)
	t.Logf("table %d", 1000)
	if ins != nil {
		t.Log(common.PrintApiBatch(ins, 3))
	...
	// diffs1 := logtail.DiffCheckPoints(data1, data3)
	// t.Log(diffs1)
}
