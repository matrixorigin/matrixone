...
func (m *objOverlapPolicy) onObject(obj *catalog.ObjectEntry, config *BasicPolicyConfig) bool {
	...
	if obj.GenerateHint == objectio.ZMMerge {
		logutil.Info("[MERGE-POLICY] object skipped",
			zap.String("policy", "overlap"),
			zap.String("reason", "this object is generated by zm merge"),
			zap.String("table", obj.GetTable().GetFullName()),
			zap.String("data object", obj.ID().ShortStringEx()),
		)
		return false
	}
