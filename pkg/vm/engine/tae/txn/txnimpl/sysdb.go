...
func (db *txnSysDB) DropRelationByName(name string) (rel handle.Relation, err error) {
	if isSysTable(name) {
		err = moerr.NewInternalErrorNoCtxf("drop relation %s is not permitted", name)
		return
	}
...
func (db *txnSysDB) DropRelationByID(id uint64) (rel handle.Relation, err error) {
	if isSysTableId(id) {
		err = moerr.NewInternalErrorNoCtxf("drop relation %d is not permitted", id)
		return
	}
...
func (db *txnSysDB) TruncateByName(name string) (rel handle.Relation, err error) {
	if isSysTable(name) {
		err = moerr.NewInternalErrorNoCtxf("truncate relation %s is not permitted", name)
		return
	}
...
func (db *txnSysDB) TruncateByID(id uint64, newTableId uint64) (rel handle.Relation, err error) {
	if isSysTableId(id) {
		err = moerr.NewInternalErrorNoCtxf("truncate relation %d is not permitted", id)
		return
	}
