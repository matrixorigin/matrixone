...
func TestAObjMerge(t *testing.T) {
	...
	ret, releaseF, mapping, err := MergeAObj(context.Background(), testPool, batches, 0, uint32(rowCnt), batCnt)
	require.NoError(t, err)
	for i := 0; i < batCnt; i++ {
		for j := 0; j < vecCnt; j++ {
...
func TestAObjMergeAllTypes(t *testing.T) {
	...
	for _, vecType := range vecTypes {
		...
		_, releaseF, _, err := MergeAObj(context.Background(), testPool, batches, 0, 50000, 2)
		require.NoError(t, err)
