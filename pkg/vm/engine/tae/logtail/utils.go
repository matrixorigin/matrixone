...
func (data *CNCheckpointData) ReadFromData(
	...
	for i, table := range meta.tables {
		...
		for it.HasNext() {
			...
			logutil.Infof("tid is %d, load block %v: %d-%d to %d", tableID, block.GetLocation().String(), block.GetStartOffset(), block.GetEndOffset(), bat.Vecs[0].Length())
	...
	for tid, mata := range data.meta {
		for i, table := range mata.tables {
			...
			for _, block := range blockIndexs[idx] {
				...
				logutil.Infof("tid is %d, write block %v to %d-%d, table is %d-%d", tid, blockLoc1.String(), block.GetStartOffset(), block.GetEndOffset(), table.Start, table.End)
