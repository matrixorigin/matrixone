...
func (b *TxnLogtailRespBuilder) visitMetadata(iblk any) {
	...
	node := blk.GetLatestNodeLocked().(*catalog.MVCCNode[*catalog.MetadataMVCCNode])
...
func (b *TxnLogtailRespBuilder) visitTable(itbl any) {
	...
	node := tbl.GetLatestNodeLocked().(*catalog.MVCCNode[*catalog.TableMVCCNode])
...
func (b *TxnLogtailRespBuilder) visitDatabase(idb any) {
	...
	node := db.GetLatestNodeLocked().(*catalog.MVCCNode[*catalog.DBMVCCNode])
	if node.DeletedAt.Equal(txnif.UncommitTS) {
