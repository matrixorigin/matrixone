...
func LoadCheckpointEntriesFromKey(
	...
	reader, err := GetCheckpointReader(ctx, sid, fs, location, version)
	if err != nil {
		return nil, nil, err
	}
	...
	for _, location = range reader.GetLocations() {
		locations = append(locations, &objectio.BackupObject{
			Location: location,
			NeedCopy: true,
		})
...
func ReWriteCheckpointAndBlockFromKey(
	...
	phaseNumber = 1
	// Load checkpoint
	reader, err := GetCheckpointReader(ctx, sid, fs, loc, version)
	if err != nil {
