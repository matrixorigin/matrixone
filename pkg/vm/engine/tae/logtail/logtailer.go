...
func (l *LogtailerImpl) RangeLogtail(
	...
	if reader.HasCatalogChanges() {
		for _, scope := range []Scope{ScopeDatabases, ScopeTables, ScopeColumns} {
			resp, closeCB, err := l.getCatalogRespBuilder(scope, reader, ckpLoc).build(ctx)
			if err != nil {
	...
	for _, table := range dirties.Tables {
		...
		resp, closeCB, err := l.getTableRespBuilder(did, tid, reader, ckpLoc).build(ctx)		if err != nil {
			for _, cb := range closeCBs {
				if cb != nil {
					cb()
				}
...
func (b *tableRespBuilder) build(ctx context.Context) (logtail.TableLogtail, func(), error) {
	resp, closeCB, err := b.collect(ctx)
	if err != nil {
...
func (b *tableRespBuilder) collect(ctx context.Context) (api.SyncLogTailResp, func(), error) {
