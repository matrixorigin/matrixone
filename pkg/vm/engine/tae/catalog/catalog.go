...
func (catalog *Catalog) onReplayDatabase(cmd *EntryCommand) {
	...
	cmd.DB.MVCC.Loop(func(n *common.GenericDLNode[txnbase.VisibleNode]) bool {
		un := n.GetPayload()
		dbun := db.GetExactUpdateNode(un.GetStart())
		if dbun == nil {
			db.InsertNode(un) //TODO isvalid
...
func (catalog *Catalog) onReplayTable(cmd *EntryCommand, dataFactory DataFactory) {
	...
	if err != nil {
		...
		cmd.Table.MVCC.Loop(func(n *common.GenericDLNode[txnbase.VisibleNode]) bool {
			un := n.GetPayload()
			node := rel.GetExactUpdateNode(un.GetStart())
			if node == nil {
				rel.InsertNode(un)
			} else {
				node.UpdateNode(un)
...
func (catalog *Catalog) onReplaySegment(cmd *EntryCommand, dataFactory DataFactory, cache *bytes.Buffer) {
	...
	if err != nil {
		...
		cmd.Segment.MVCC.Loop(func(n *common.GenericDLNode[txnbase.VisibleNode]) bool {
			un := n.GetPayload()
			segun := seg.GetExactUpdateNode(un.GetStart())
			if segun != nil {
				segun.UpdateNode(un)
			} else {
				seg.InsertNode(un)
...
func (catalog *Catalog) onReplayBlock(cmd *EntryCommand, dataFactory DataFactory) {
	...
	if blk == nil {
		...
		cmd.Block.MVCC.Loop(func(n *common.GenericDLNode[txnbase.VisibleNode]) bool {
			un := n.GetPayload()
			blkun := blk.GetExactUpdateNode(un.GetStart())
			if blkun != nil {
				blkun.UpdateNode(un)
