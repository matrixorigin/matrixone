...
func (node *DeleteNode) GeneralDesc() string {
	return fmt.Sprintf("[TS=%d]%v,Cnt=%d", node.commitTs, node.logIndex, node.mask.GetCardinality())
}

func (node *DeleteNode) GeneralVerboseString() string {
	return fmt.Sprintf("[TS=%d]%v,Cnt=%d\nDeletes are %v", node.commitTs, node.logIndex, node.mask.GetCardinality(), node.mask)
}
