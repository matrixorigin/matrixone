...
func (task *mergeBlocksTask) Execute() (err error) {
	...
	writer, err := blockio.NewBlockWriter(task.mergedBlks[0].GetBlockData().GetFs().Service, name.String())
	if err != nil {
		return err
	}
	if schema.HasPK() {
	...
	for i, block := range blocks {
		metaLoc = blockio.EncodeLocationNew(block.GetName(), block.GetExtent(), uint32(batchs[i].Length()), block.GetID())
