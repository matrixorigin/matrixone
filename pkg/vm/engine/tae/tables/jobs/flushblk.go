...
func (task *flushBlkTask) Execute() (err error) {
	if task.meta.GetSchema().IsSingleSortKey() {
		def := task.meta.GetSchema().GetSingleSortKey()
		i := catalog.GetAttrIdx(task.data.Attrs, def.Name)
		sortColumnData := task.data.Vecs[i]
		if err = BuildAndFlushSingleIndex(task.file, task.meta, sortColumnData); err != nil {
			return
		}
	} else if task.meta.GetSchema().IsCompoundSortKey() {
		panic("implement me")
	}
