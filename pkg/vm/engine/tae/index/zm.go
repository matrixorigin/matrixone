...
//  1. If not truncated, maxBuf is be smaller than minBuf.
...
func StrictlyCompareZmMaxAndMin(maxBuf, minBuf []byte, t types.T, scale1, scale2 int32) int {
	if t.FixedLength() < 0 && len(maxBuf) == 30 {
		if len(minBuf) < 30 {
			cmp := bytes.Compare(maxBuf[:len(minBuf)], minBuf)
			if cmp != 0 {
				return cmp
			} else {
				// maxBuf: abcd0000 <- abcczzzza
				// minBuf: abcd
			...
			cmp := bytes.Compare(maxBuf[:29], minBuf[:29])
			if cmp != 0 {
				return cmp
			} else {
