...
func (ls *LocalDisttaeDataSource) batchApplyTombstoneObjects(
	...
	for iter.Next() && deletedMask.Count() < len(rowIds) {
		...
		for idx := 0; idx < int(obj.BlkCnt()) && len(rowIds) > deletedMask.Count(); idx++ {
			...
			if !obj.GetCNCreated() {
				var commitErr error
				commit, commitErr = ioutil.ValidateTombstoneCommitTSColumn(
					len(deletedRowIds), &cacheVectors[1],
				)
				if commitErr != nil {
					release()
					return commitErr
				}
				if cacheVectors[2].GetType().Oid != types.T_any {
					var abortErr error
					aborts, abortErr = ioutil.ValidateTombstoneAbortColumn(
						len(deletedRowIds), &cacheVectors[2],
					)
					if abortErr != nil {
						release()
						return abortErr
