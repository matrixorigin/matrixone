...
type ObjectEntry struct {
	ObjectInfo
	//for cdc,process objects generated by cn only
	objectsCreatedByCn bool
}
...
func (p *PartitionState) HandleObjectInsert(
	...
	//for cdc, check all delete ts in the same batch are empty or not
	allDelTsAreEmptry := true
	for idx := 0; idx < len(stateCol); idx++ {
		if !deleteTSCol[idx].IsEmpty() {
			allDelTsAreEmptry = false
			break
		}
	}
	...
	for idx := 0; idx < len(stateCol); idx++ {
		...
		objEntry.CommitTS = commitTSCol[idx]
		objEntry.Sorted = sortedCol[idx]
		objEntry.objectsCreatedByCn = allDelTsAreEmptry
