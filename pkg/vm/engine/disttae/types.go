...
type Transaction struct {
	...
	//tombstone objects flushed by CN.
...
func (txn *Transaction) StashFlushedTombstones(tid uint64, stats objectio.ObjectStats) {
	txn.tombstoneObjs.Lock()
	defer txn.tombstoneObjs.Unlock()
	txn.tombstoneObjs.data[tid] = append(txn.tombstoneObjs.data[tid], stats)
}
...
func (txn *Transaction) handleRCSnapshot(ctx context.Context, commit bool) error {
	...
	return txn.transferTombstonesLocked(ctx, commit)
}
