...
func (txn *Transaction) RollbackLastStatement(ctx context.Context) error {
	...
	if txn.statementID > 0 {
		...
		for i := end; i < len(txn.writes); i++ {
			...
			//2. Remove the segments generated by this statement till txn commits or rollback.
			//3. GC the s3 objects(data objects and tombstone objects) asynchronously.
	...
	//TODO::rollback S3 block's deletes,clean the txn.deletedBlocks.
