		...
		if txn.writes[i].fileName != "" {
			...
			for idx := 0; idx < len(col); idx++ {
				...
				if _, ok := objsToGC[objName]; !ok {
					objsToGC[objName] = struct{}{}
					objsName = append(objsName, objName)
				}
			}
		}
	}
...
func (e *Entry) String() string {
	return fmt.Sprintf("type=%s, accId=%d, db=(%s-%d) tbl=(%s-%d), filename=%s, batLen=%d, truncate=%v",
		typesNames[e.typ], e.accountId, e.databaseName, e.databaseId, e.tableName, e.tableId, e.fileName, e.bat.Size(), e.truncate)
}
