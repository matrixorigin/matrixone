...
func (db *txnDatabase) Relations(ctx context.Context) ([]string, error) {
	...
	db.op.GetWorkspace().(*Transaction).deletedTableMap.Range(func(k, _ any) bool {
		key := k.(tableKey)
	...
	db.op.GetWorkspace().(*Transaction).createMap.Range(func(k, _ any) bool {
		key := k.(tableKey)
		if key.databaseId == db.databaseId {
	...
	tbls, _ := db.op.GetWorkspace().(*Transaction).engine.catalog.Tables(
...
func (db *txnDatabase) getTableNameById(ctx context.Context, id uint64) (string, error) {
	...
	deleted := false
	db.op.GetWorkspace().(*Transaction).deletedTableMap.Range(func(k, v any) bool {
		key := k.(tableKey)
		val := v.(uint64)
	...
	db.op.GetWorkspace().(*Transaction).createMap.Range(func(k, v any) bool {
		key := k.(tableKey)
		val := v.(*txnTable)
	...
	if tblName == "" {
		...
		tbls, tblIds := db.op.GetWorkspace().(*Transaction).engine.catalog.Tables(
			accountId, db.databaseId, db.op.SnapshotTS())
...
func (db *txnDatabase) RelationByAccountID(
	...
	txn := db.op.GetWorkspace().(*Transaction)
	...
	if _, exist := db.op.GetWorkspace().(*Transaction).deletedTableMap.Load(key); exist {
		return nil, moerr.NewParseError(context.Background(), "table %q does not exist", name)
	}

	p := db.op.GetWorkspace().(*Transaction).proc
	if proc != nil {
		p = proc.(*process.Process)
	}

	rel := db.op.GetWorkspace().(*Transaction).getCachedTable(key, db.op.SnapshotTS())
	if rel != nil {
		rel.proc.Store(p)
		return rel, nil
	}
	...
	if v, ok := db.op.GetWorkspace().(*Transaction).createMap.Load(key); ok {
	...
	db.op.GetWorkspace().(*Transaction).tableCache.tableMap.Store(key, tbl)
	return tbl, nil
}
...
func (db *txnDatabase) Delete(ctx context.Context, name string) error {
	...
	if v, ok := db.op.GetWorkspace().(*Transaction).createMap.Load(k); ok {
		db.op.GetWorkspace().(*Transaction).createMap.Delete(k)
		table := v.(*txnTable)
		id = table.tableId
		rowid = table.rowid
		rowids = table.rowids
		...
		*/
	} else if v, ok := db.op.GetWorkspace().(*Transaction).tableCache.tableMap.Load(k); ok {
		table := v.(*txnTable)
		id = table.tableId
		db.op.GetWorkspace().(*Transaction).tableCache.tableMap.Delete(k)
		rowid = table.rowid
		rowids = table.rowids
	} else {
		item := &cache.TableItem{
	...
	bat, err := genDropTableTuple(rowid, id, db.databaseId,
		name, db.databaseName, db.op.GetWorkspace().(*Transaction).proc.Mp())
	if err != nil {
		return err
	}

	for _, store := range db.op.GetWorkspace().(*Transaction).tnStores {
		if err := db.op.GetWorkspace().(*Transaction).WriteBatch(
	...
	db.op.GetWorkspace().(*Transaction).deletedTableMap.Store(k, id)
	return nil
}
...
func (db *txnDatabase) Truncate(ctx context.Context, name string) (uint64, error) {
	...
	var v any
	var ok bool
	newId, err := db.op.GetWorkspace().(*Transaction).allocateID(ctx)
	if err != nil {
		return 0, err
	}
	accountId, err := defines.GetAccountId(ctx)
	...
	k := genTableKey(accountId, name, db.databaseId)
	v, ok = db.op.GetWorkspace().(*Transaction).createMap.Load(k)
	if !ok {
		v, ok = db.op.GetWorkspace().(*Transaction).tableCache.tableMap.Load(k)
	}
	...
	bat, err := genTruncateTableTuple(rowid, newId, db.databaseId,
		genMetaTableName(oldId)+name, db.databaseName, db.op.GetWorkspace().(*Transaction).proc.Mp())
	if err != nil {
		return 0, err
	}
	for _, store := range db.op.GetWorkspace().(*Transaction).tnStores {
		if err := db.op.GetWorkspace().(*Transaction).WriteBatch(DELETE, 0, catalog.MO_CATALOG_ID, catalog.MO_TABLES_ID,
...
func (db *txnDatabase) Create(ctx context.Context, name string, defs []engine.TableDef) error {
	...
	tableId, err := db.op.GetWorkspace().(*Transaction).allocateID(ctx)
	if err != nil {
		return err
	}
	tbl := new(txnTable)
	...
	}
	{
		...
		bat, err := genCreateTableTuple(
			...
			tbl.rowid, true, db.op.GetWorkspace().(*Transaction).proc.Mp())
		if err != nil {
			return err
		}
		for _, store := range db.op.GetWorkspace().(*Transaction).tnStores {
			if err := db.op.GetWorkspace().(*Transaction).WriteBatch(INSERT, 0, catalog.MO_CATALOG_ID, catalog.MO_TABLES_ID,
	...
	for i, col := range cols {
		tbl.rowids[i] = db.op.GetWorkspace().(*Transaction).genRowId()
		bat, err := genCreateColumnTuple(col, tbl.rowids[i], true,
			db.op.GetWorkspace().(*Transaction).proc.Mp())
		if err != nil {
			return err
		}
		for _, store := range db.op.GetWorkspace().(*Transaction).tnStores {
	...
	tbl.GetTableDef(ctx)
	key := genTableKey(accountId, name, db.databaseId)
	db.op.GetWorkspace().(*Transaction).addCreateTable(key, tbl)
	//CORNER CASE
	//begin;
	...
	db.op.GetWorkspace().(*Transaction).deletedTableMap.Delete(key)
	return nil
}
