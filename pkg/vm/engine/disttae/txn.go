...
func (txn *Transaction) transferTombstonesLocked(ctx context.Context, commit bool) error {
	...
	if txn.statementID > 0 && txn.op.Txn().IsRCIsolation() {
		...
		return txn.forEachTableHasDeletesLocked(
			func(tbl *txnTable) error {
				...
				//transfer in-memory tombstones.
				if err := TransferInMemTombstones(
					ctx,
					tbl,
				...
				//transfer tombstone objects.
				if err := TransferTombstoneObjects(
					ctx,
					tbl,
					types.TimestampToTS(ts),
					types.TimestampToTS(endTs),
