...
func (txn *Transaction) WriteBatch(
	...
	entryIdx := len(txn.writes)
	txn.writes = append(txn.writes, e)

	// Update writesBlockIndex for INSERT entries to optimize deleteTableWrites lookup
	if typ == INSERT && bat != nil && bat.RowCount() > 0 && bat.Vecs[0].GetType().Oid == types.T_Rowid {
		rowids := vector.MustFixedColWithTypeCheck[types.Rowid](bat.GetVector(0))
		if len(rowids) > 0 {
			blockId := rowids[0].CloneBlockID()
			// Initialize writesBlockIndex if nil (for backward compatibility with tests)
			if txn.writesBlockIndex == nil {
				txn.writesBlockIndex = make(map[uint64]map[types.Blockid][]int)
			}
			if txn.writesBlockIndex[tableId] == nil {
				txn.writesBlockIndex[tableId] = make(map[types.Blockid][]int)
			}
			txn.writesBlockIndex[tableId][blockId] = append(txn.writesBlockIndex[tableId][blockId], entryIdx)
		}
	}
...
func (txn *Transaction) deleteTableWrites(
	...
	// Use writesBlockIndex for O(1) lookup instead of O(n) iteration
	tableIndex := txn.writesBlockIndex[tableId]
	if tableIndex == nil {
		return
	}

	// Only iterate over entries that match the blocks we're deleting
	for blockId := range deleteBlkId {
		entryIndices := tableIndex[blockId]
		for _, idx := range entryIndices {
			...
			// nil batch will be generated by compaction or dumpBatch
