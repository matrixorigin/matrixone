...
func TestSnapshotCheckpointsCanServe(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "compacted predecessor retained before global", entries: []*checkpoint.CheckpointEntry{newEntry(t0, t5, checkpoint.ET_Compacted), newEntry(t0, t6, checkpoint.ET_Global), newEntry(t6, t20, checkpoint.ET_Incremental)}, ts: t10, want: true},
