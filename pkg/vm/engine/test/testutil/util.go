...
func CreateEngines2(ctx context.Context, opts TestOptions,
	t *testing.T) (disttaeEngine *TestDisttaeEngine, taeEngine *TestTxnStorage,
	rpcAgent *MockRPCAgent, mp *mpool.MPool) {
	...
	disttaeEngine, err = NewTestDisttaeEngine2(ctx, mp, taeEngine.GetDB().Runtime.Fs.Service, rpcAgent, taeEngine)
	require.Nil(t, err)
...
func CreateCdcEngines(ctx context.Context, opts TestOptions,
	t *testing.T) (cdcEngine *TestCdcEngine, taeEngine *TestTxnStorage,
	rpcAgent *MockRPCAgent, mp *mpool.MPool) {
	...
	cdcEngine, err = NewTestCdcEngine(ctx, mp, taeEngine.GetDB().Runtime.Fs.Service, rpcAgent, taeEngine)
	require.Nil(t, err)

	return
}

type CdcEnginePack struct {
	D       *TestDisttaeEngine
	T       *TestTxnStorage
	R       *MockRPCAgent
	Mp      *mpool.MPool
	...
	t       *testing.T
	cancelF func()
	CdcEng  *TestCdcEngine
}

func InitCdcEnginePack(opts TestOptions, t *testing.T) *CdcEnginePack {
	ctx := context.WithValue(context.Background(), defines.TenantIDKey{}, uint32(0))
	timeout := opts.Timeout
	if timeout == 0 {
		timeout = 5 * time.Minute
	...
	var err error
	pack.D, pack.T, pack.R, pack.Mp = CreateEngines2(pack.Ctx, opts, t)
	pack.CdcEng, err = NewTestCdcEngine(ctx, pack.Mp, pack.T.GetDB().Runtime.Fs.Service, pack.R, pack.T)
	require.Nil(t, err)
	return pack
}

func (p *CdcEnginePack) Close() {
	p.cancelF()
	p.D.Close(p.Ctx)
	p.T.Close(true)
	p.R.Close()
...
func (p *CdcEnginePack) StartCNTxn(opts ...client.TxnOption) client.TxnOperator {
	op, err := p.D.NewTxnOperator(p.Ctx, p.D.Now(), opts...)
	require.NoError(p.t, err)
	require.NoError(p.t, p.D.Engine.New(p.Ctx, op))
	return op
...
func (p *CdcEnginePack) CreateDB(txnop client.TxnOperator, dbname string) engine.Database {
	err := p.D.Engine.Create(p.Ctx, dbname, txnop)
	require.NoError(p.t, err)
	db, err := p.D.Engine.Database(p.Ctx, dbname, txnop)
	require.NoError(p.t, err)
...
func (p *CdcEnginePack) CreateDBAndTables(txnop client.TxnOperator, dbname string, schema ...*catalog2.Schema) (engine.Database, []engine.Relation) {
	db := p.CreateDB(txnop, dbname)
	rels := make([]engine.Relation, 0, len(schema))
	for _, s := range schema {
...
func (p *CdcEnginePack) CreateTableInDB(txnop client.TxnOperator, dbname string, schema *catalog2.Schema) engine.Relation {
	db, err := p.D.Engine.Database(p.Ctx, dbname, txnop)
	require.NoError(p.t, err)
...
func (p *CdcEnginePack) DeleteTableInDB(txnop client.TxnOperator, dbname, tblname string) {
	db, err := p.D.Engine.Database(p.Ctx, dbname, txnop)
	require.NoError(p.t, err)
	require.NoError(p.t, db.Delete(p.Ctx, tblname))
}
