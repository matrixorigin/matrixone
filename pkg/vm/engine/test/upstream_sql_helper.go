...
// queries the snapshot timestamp, and updates the transaction snapshot
func (h *UpstreamSQLHelper) handleSnapshotInQuery(ctx context.Context, query string) error {
	// Check if query contains snapshot clause
	snapshotName, found := extractSnapshotFromSQL(query)
	if !found || snapshotName == "" {
		return nil // No snapshot clause, nothing to do
	}
	...
	if ts == nil {
		return moerr.NewInternalErrorf(ctx, "snapshot %s not found", snapshotName)
	}

	// Update transaction snapshot
	err = txnOp.UpdateSnapshot(ctx, *ts)
	if err != nil {
		return moerr.NewInternalErrorf(ctx, "failed to update transaction snapshot: %v", err)
	}

	logutil.Info("UpstreamSQLHelper: updated transaction snapshot from query",
		zap.String("snapshot_name", snapshotName),
		zap.Int64("snapshot_ts", ts.PhysicalTime),
	)
