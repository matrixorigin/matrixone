...
func TestResourceMgr(t *testing.T) {
	...
	res1 := NewBaseResource("res1", ResTIO, res1H)
	res2H := NewSingleWorkerHandler("res1H")
	res2 := NewBaseResource("res2", ResTIO, res2H)
	res3H := NewPoolHandler(2, nil)
	res3 := NewBaseResource("res3", ResTCPU, res3H)
	err := mgr.Add(res1)
	assert.Nil(t, err)
	err = mgr.Add(res2)
	assert.Nil(t, err)
	...
	assert.Equal(t, 2, mgr.ResourceCountByType(ResTIO))
