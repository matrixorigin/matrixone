...
func TestOperationsMarshalAndUnmarshal(t *testing.T) {
	...
	// prepare test cases
	cases := []struct {
		in           types.ProtoMarshaler
		out          types.ProtoUnmarshaler
...
func marshalThenUnmarshal[
	I types.ProtoMarshaler,
	O types.ProtoUnmarshaler,
](t *testing.T, in I, out O) {
	data := make([]byte, in.ProtoSize())
	n, err := in.MarshalToSizedBuffer(data)
	require.NoError(t, err)
	require.Equal(t, in.ProtoSize(), n)

	err = out.Unmarshal(data[:n])
	require.NoError(t, err)
}
