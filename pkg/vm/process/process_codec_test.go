...
func TestBuildProcessInfoGenericAndStatementHashSQLModeResolution(t *testing.T) {
	...
	proc.Base.SessionInfo.ExplicitZeroTemporalCastReturnsNull = false
	proc.SetResolveVariableFunc(func(string, bool, bool) (interface{}, error) {
	...
	require.NoError(t, err)
	require.Equal(t, "ANSI_QUOTES", hashProcessInfo.SessionInfo.SqlMode)
	...
	require.Equal(t, "ANSI_QUOTES", ordinaryForward.SessionInfo.SqlMode)
	require.False(t, ordinaryForward.SessionInfo.ExplicitZeroTemporalCastReturnsNull)
	ordinaryDecoded, err := svc.Decode(defines.AttachAccountId(context.Background(), 42), ordinaryForward)
	require.NoError(t, err)
	defer ordinaryDecoded.Free()
	ordinaryDecoded.Base.IsFrontend = false
	require.Equal(t, "ANSI_QUOTES", ordinaryDecoded.Base.SessionInfo.SqlMode)
	ordinaryDecoded.SetResolveVariableFunc(func(string, bool, bool) (interface{}, error) {
		return "STRICT_TRANS_TABLES,NO_ZERO_DATE", nil
	})
	resolvedMode, err := ResolveSQLMode(ordinaryDecoded)
	require.NoError(t, err)
	require.Equal(t, "ANSI_QUOTES", resolvedMode,
		"the hash consumer must use the forwarded origin snapshot, not the intermediate resolver")
	version.BuildCommitID = originBuild

	// Decode the ordinary forward before the second hash hop. Even if this
	// intermediate CN has a different resolver, the second hash forward must
	// retain the original snapshot.
	second, err := ordinaryDecoded.BuildProcessInfoWithStatementHash("select 1")
	require.NoError(t, err)
	require.Equal(t, "ANSI_QUOTES", second.SessionInfo.SqlMode)
	require.False(t, second.SessionInfo.ExplicitZeroTemporalCastReturnsNull)
