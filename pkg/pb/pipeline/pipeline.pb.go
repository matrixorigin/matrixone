...
type ProcessInfo struct {
	...
	IvfBloomFilter       []byte            `protobuf:"bytes,10,opt,name=ivf_bloom_filter,json=ivfBloomFilter,proto3" json:"ivf_bloom_filter,omitempty"`
	XXX_NoUnkeyedLiteral struct{}          `json:"-"`
	XXX_unrecognized     []byte            `json:"-"`
	XXX_sizecache        int32             `json:"-"`
}
...
func (m *ProcessInfo) GetIvfBloomFilter() []byte {
	if m != nil {
		return m.IvfBloomFilter
	}
	return nil
}
...
=======
	...
	0x72, 0xb0, 0xfa, 0x5d, 0x1d, 0xfd, 0x60, 0x33, 0xf5, 0xea, 0x91, 0x34, 0x12, 0xa7, 0x46, 0xd2,
...
func (m *ProcessInfo) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.IvfBloomFilter) > 0 {
		i -= len(m.IvfBloomFilter)
		copy(dAtA[i:], m.IvfBloomFilter)
		i = encodeVarintPipeline(dAtA, i, uint64(len(m.IvfBloomFilter)))
		i--
		dAtA[i] = 0x52
	}
	{
...
func (m *ProcessInfo) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 10:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field IvfBloomFilter", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.IvfBloomFilter = append(m.IvfBloomFilter[:0], dAtA[iNdEx:postIndex]...)
			if m.IvfBloomFilter == nil {
				m.IvfBloomFilter = []byte{}
			}
			iNdEx = postIndex
		default:
			iNdEx = preIndex
