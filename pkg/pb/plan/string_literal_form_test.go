...
func TestRequiresMORPCVersion64ExactDecimalInt64Cast(t *testing.T) {
	cast := func(sourceType, targetType int32, overload int32) *Expr {
		return &Expr{
			Typ: Type{Id: targetType},
			Expr: &Expr_F{F: &Function{
				Func: &ObjectRef{
					Obj:     int64(normalCastFunctionID)<<32 | int64(overload),
					ObjName: "cast",
				},
				Args: []*Expr{
					{
						Typ:  Type{Id: sourceType},
						Expr: &Expr_Col{Col: &ColRef{ColPos: 0}},
					},
					{
						Typ:  Type{Id: targetType},
						Expr: &Expr_T{T: &TargetType{}},
					},
				},
			}},
		}
	...
	for _, decimalType := range []int32{
		planDecimal64TypeID,
		planDecimal128TypeID,
		planDecimal256TypeID,
	} {
		required, err := RequiresMORPCVersion64ExactDecimalInt64Cast(
			cast(decimalType, planInt64TypeID, normalCastOverloadID))
		require.NoError(t, err)
		require.True(t, required, "decimal type %d", decimalType)
	}

	for name, expr := range map[string]*Expr{
		"explicit decimal cast already exact": cast(planDecimal128TypeID, planInt64TypeID, 1),
		"normal decimal to uint64":            cast(planDecimal128TypeID, 28, normalCastOverloadID),
		"normal int64 to int64":               cast(planInt64TypeID, planInt64TypeID, normalCastOverloadID),
		"different function":                  cast(planDecimal128TypeID, planInt64TypeID, normalCastOverloadID),
	} {
		t.Run(name, func(t *testing.T) {
			if name == "different function" {
				expr.GetF().Func.Obj = int64(123) << 32
			}
			required, err := RequiresMORPCVersion64ExactDecimalInt64Cast(expr)
			require.NoError(t, err)
			require.False(t, required)
		})
	}

	nested := &Expr{Expr: &Expr_F{F: &Function{
		Func: &ObjectRef{ObjName: "coalesce"},
		Args: []*Expr{cast(planDecimal256TypeID, planInt64TypeID, normalCastOverloadID)},
	}}}
	features, err := RequiredRemoteExpressionFeatures(nested)
	require.NoError(t, err)
	require.True(t, features.ExactDecimalInt64Cast)
	require.True(t, features.Any())
}
