...
func TestTunnelReplaceConn(t *testing.T) {
	...
	tu.replaceServerConn(newMySQLConn("server", newServerProxy, 0, nil, nil, nil, 0), false)
...
func TestPipeCancelError(t *testing.T) {
	...
	cc := newMySQLConn("client", clientProxy, 0, nil, nil, nil, 0)
	sc := newMySQLConn("server", serverProxy, 0, nil, nil, nil, 0)
...
func TestPipeMultipleStartAndPause(t *testing.T) {
	...
	go func() {
		receiver := newMySQLConn("receiver", server, 0, nil, nil, nil, 0)
		for {
...
func TestCanStartTransfer(t *testing.T) {
	...
	t.Run("inTxn", func(t *testing.T) {
		tu := &tunnel{}
		tu.mu.scp = &pipe{}
		tu.mu.scp.src = newMySQLConn("", nil, 0, nil, nil, nil, 0)
...
func TestReplaceServerConn(t *testing.T) {
	...
	require.NotNil(t, sc)
	newServerC := newMySQLConn("new-server", newSC.RawConn(), 0, nil, nil, nil, 0)
