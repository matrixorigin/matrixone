...
func newConnCache(
	ctx context.Context, logger *log.MOLogger, opts ...connCacheOption,
) ConnCache {
	var mc clusterservice.MOCluster
	mcAny, ok := runtime.ProcessLevelRuntime().GetGlobalVariables(runtime.ClusterService)
	if ok {
		mc = mcAny.(clusterservice.MOCluster)
	}
	cc := &connCache{
		ctx:             ctx,
		logger:          logger,
