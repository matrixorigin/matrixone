...
func shardCacheKey(key fscache.CacheKey) uint8 {
	sum64 := uint64(key.Offset) ^ uint64(key.Sz) ^ maphash.String(seed, key.Path)
	sum32 := uint32(sum64) ^ uint32(sum64>>32)
	sum16 := uint16(sum32) ^ uint16(sum32>>16)
	sum8 := uint8(sum16) ^ uint8(sum16>>8)
	return sum8
}
