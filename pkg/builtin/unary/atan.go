...
func init() {
	extend.FunctionRegistry["atan"] = builtin.Atan
	extend.UnaryReturnTypes[builtin.Atan] = func(extend extend.Extend) types.T {
		return types.T_float64
	}

	extend.UnaryStrings[builtin.Atan] = func(e extend.Extend) string {
		return fmt.Sprintf("atan(%s)", e)
	}

	overload.OpTypes[builtin.Atan] = overload.Unary
	overload.UnaryOps[builtin.Atan] = []*overload.UnaryOp{
		{ // T_uint8
			Typ:        types.T_uint8,
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				vector.SetCol(resultVector, atan.AtanUint8(origVecCol, results))
		...
		{ // T_uint16
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanUint16(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_uint32
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanUint32(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_uint64
		...
		{ // T_int8
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanInt8(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_int16
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanInt16(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_int32
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanInt32(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_int64
		...
		{ // T_float32
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				resultVector.Col = results
				nulls.Set(resultVector.Nsp, origVec.Nsp)
				vector.SetCol(resultVector, atan.AtanFloat32(origVecCol, results))
				return resultVector, nil
			},
		},
		{ // T_float64
			...
			Fn: func(origVec *vector.Vector, proc *process.Process, _ bool) (*vector.Vector, error) {
				...
				if origVec.Ref == 1 || origVec.Ref == 0 {
					origVec.Ref = 0
					atan.AtanFloat64(origVecCol, origVecCol)
					return origVec, nil
				}
				resultVector, err := process.Get(proc, 8*int64(len(origVecCol)), types.Type{Oid: types.T_float64, Size: 8})
				if err != nil {
