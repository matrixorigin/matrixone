...
public:
    ...
    void build() override {
        std::unique_lock<std::shared_mutex> lock(this->mutex_);
        if (this->is_loaded_ && this->filename_.empty()) return;
    ...
    void build_internal(raft_handle_wrapper_t& handle) {
        ...
        if (!this->filename_.empty()) {
            if (is_mg) {
                mg_index_.reset(new mg_index(cuvs::neighbors::cagra::deserialize<T, uint32_t>(*res, this->filename_)));
                this->count = 0;
            ...
            if (is_mg) {
                ...
                mg_index_.reset(new mg_index(cuvs::neighbors::cagra::build(*res, mg_params, dataset_view)));
                handle.sync_all_devices();
            } else {
                ...
                index_.reset(new cagra_index(cuvs::neighbors::cagra::build(*res, index_params, raft::make_const_mdspan(dataset_device->view()))));
            }
            handle.sync_all_devices();
        }
    }
...
cagra_search_result_t gpu_cagra_t<T>::search_batch_internal(const T* queries_data, uint64_t num_queries, uint32_t limit, const cagra_search_params_t& sp) {
    ...
    return this->worker->template submit_batched<search_result_t>(batch_key, search_req_t{queries_data, num_queries}, exec_fn).get();
}

template <typename T>
cagra_search_result_t gpu_cagra_t<T>::search_float_batch_internal(const float* queries_data, uint64_t num_queries, uint32_t limit, const cagra_search_params_t& sp) {
