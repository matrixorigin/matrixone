...
GpuShardedCagraIndexC GpuShardedCagraIndex_New(const float* dataset_data, uint64_t count_vectors, uint32_t dimension, 
    ...
    try {
        ...
        auto* index = new matrixone::GpuShardedCagraIndex<float>(dataset_data, count_vectors, dimension, metric, intermediate_graph_degree, graph_degree, device_vec, nthread);
        return static_cast<GpuShardedCagraIndexC>(index);
...
GpuShardedCagraIndexC GpuShardedCagraIndex_NewFromFile(const char* filename, uint32_t dimension, 
    ...
    try {
        cuvs::distance::DistanceType metric = convert_distance_type_sharded_cagra(metric_c);
        std::vector<int> device_vec(devices, devices + num_devices);
        auto* index = new matrixone::GpuShardedCagraIndex<float>(std::string(filename), dimension, metric, device_vec, nthread);
        return static_cast<GpuShardedCagraIndexC>(index);
    } catch (const std::exception& e) {
...
void GpuShardedCagraIndex_Load(GpuShardedCagraIndexC index_c, void* errmsg) {
    if (errmsg) *(static_cast<char**>(errmsg)) = nullptr;
    try {
        auto* index = static_cast<matrixone::GpuShardedCagraIndex<float>*>(index_c);
        if (index) index->Load();
    } catch (const std::exception& e) {
...
GpuShardedCagraSearchResultC GpuShardedCagraIndex_Search(GpuShardedCagraIndexC index_c, const float* queries_data, 
    ...
    try {
        auto* index = static_cast<matrixone::GpuShardedCagraIndex<float>*>(index_c);
        if (index) {
            auto* search_result = new matrixone::GpuShardedCagraIndex<float>::SearchResult;
            *search_result = index->Search(queries_data, num_queries, query_dimension, limit, itopk_size);
            return static_cast<GpuShardedCagraSearchResultC>(search_result);
        }
    } catch (const std::exception& e) {
