...
template <typename T>
static gpu_cagra_c merge_cagra_impl(gpu_cagra_c* indices, uint32_t num_indices, uint32_t nthread, const std::vector<int>& device_vec, quantization_t qtype) {
    std::vector<matrixone::gpu_cagra_t<T>*> cpp_indices;
    for (uint32_t i = 0; i < num_indices; ++i) {
        cpp_indices.push_back(static_cast<matrixone::gpu_cagra_t<T>*>(static_cast<gpu_cagra_any_t*>(indices[i])->ptr));
    }
    auto merged = matrixone::gpu_cagra_t<T>::merge(cpp_indices, nthread, device_vec);
    return static_cast<gpu_cagra_c>(new gpu_cagra_any_t(qtype, merged.release()));
}
    ...
    try {
        ...
        switch (qtype) {
            case Quantization_F32:
                index_ptr = new matrixone::gpu_cagra_t<float>(static_cast<const float*>(dataset_data), count_vectors, dimension, metric, intermediate_graph_degree, graph_degree, device_vec, nthread, force_mg);
                break;
            case Quantization_F16:
                index_ptr = new matrixone::gpu_cagra_t<half>(static_cast<const half*>(dataset_data), count_vectors, dimension, metric, intermediate_graph_degree, graph_degree, device_vec, nthread, force_mg);
                break;
            case Quantization_INT8:
                index_ptr = new matrixone::gpu_cagra_t<int8_t>(static_cast<const int8_t*>(dataset_data), count_vectors, dimension, metric, intermediate_graph_degree, graph_degree, device_vec, nthread, force_mg);
                break;
            case Quantization_UINT8:
                index_ptr = new matrixone::gpu_cagra_t<uint8_t>(static_cast<const uint8_t*>(dataset_data), count_vectors, dimension, metric, intermediate_graph_degree, graph_degree, device_vec, nthread, force_mg);
                break;
        }
        return static_cast<gpu_cagra_c>(new gpu_cagra_any_t(qtype, index_ptr));
...
gpu_cagra_c gpu_cagra_new_from_file(const char* filename, uint32_t dimension, distance_type_t metric_c, 
    ...
    try {
        ...
        switch (qtype) {
            case Quantization_F32:
                index_ptr = new matrixone::gpu_cagra_t<float>(std::string(filename), dimension, metric, device_vec, nthread, force_mg);
                break;
            case Quantization_F16:
                index_ptr = new matrixone::gpu_cagra_t<half>(std::string(filename), dimension, metric, device_vec, nthread, force_mg);
                break;
            case Quantization_INT8:
                index_ptr = new matrixone::gpu_cagra_t<int8_t>(std::string(filename), dimension, metric, device_vec, nthread, force_mg);
                break;
            case Quantization_UINT8:
                index_ptr = new matrixone::gpu_cagra_t<uint8_t>(std::string(filename), dimension, metric, device_vec, nthread, force_mg);
                break;
        }
        return static_cast<gpu_cagra_c>(new gpu_cagra_any_t(qtype, index_ptr));
    } catch (const std::exception& e) {
        set_errmsg_cagra(errmsg, "Error in gpu_cagra_new_from_file", e);
        return nullptr;
    }
}
...
void gpu_cagra_save(gpu_cagra_c index_c, const char* filename, void* errmsg) {
    ...
    try {
        auto* any = static_cast<gpu_cagra_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32: static_cast<matrixone::gpu_cagra_t<float>*>(any->ptr)->save(std::string(filename)); break;
            case Quantization_F16: static_cast<matrixone::gpu_cagra_t<half>*>(any->ptr)->save(std::string(filename)); break;
            case Quantization_INT8: static_cast<matrixone::gpu_cagra_t<int8_t>*>(any->ptr)->save(std::string(filename)); break;
            case Quantization_UINT8: static_cast<matrixone::gpu_cagra_t<uint8_t>*>(any->ptr)->save(std::string(filename)); break;
        }
    } catch (const std::exception& e) {
...
gpu_cagra_search_result_c gpu_cagra_search(gpu_cagra_c index_c, const void* queries_data, 
    ...
    try {
        ...
        switch (any->qtype) {
            case Quantization_F32: {
                auto res = std::make_unique<matrixone::gpu_cagra_t<float>::search_result_t>();
                *res = static_cast<matrixone::gpu_cagra_t<float>*>(any->ptr)->search(static_cast<const float*>(queries_data), num_queries, query_dimension, limit, itopk_size);
                result_ptr = res.release();
                break;
            }
            case Quantization_F16: {
                auto res = std::make_unique<matrixone::gpu_cagra_t<half>::search_result_t>();
                *res = static_cast<matrixone::gpu_cagra_t<half>*>(any->ptr)->search(static_cast<const half*>(queries_data), num_queries, query_dimension, limit, itopk_size);
                result_ptr = res.release();
                break;
            }
            case Quantization_INT8: {
                auto res = std::make_unique<matrixone::gpu_cagra_t<int8_t>::search_result_t>();
                *res = static_cast<matrixone::gpu_cagra_t<int8_t>*>(any->ptr)->search(static_cast<const int8_t*>(queries_data), num_queries, query_dimension, limit, itopk_size);
                result_ptr = res.release();
                break;
            }
            case Quantization_UINT8: {
                auto res = std::make_unique<matrixone::gpu_cagra_t<uint8_t>::search_result_t>();
                *res = static_cast<matrixone::gpu_cagra_t<uint8_t>*>(any->ptr)->search(static_cast<const uint8_t*>(queries_data), num_queries, query_dimension, limit, itopk_size);
                result_ptr = res.release();
                break;
            }
        }
...
void gpu_cagra_get_results(gpu_cagra_search_result_c result_c, uint64_t num_queries, uint32_t limit, int64_t* neighbors, float* distances) {
    if (!result_c) return;
    auto* search_result = static_cast<matrixone::gpu_cagra_t<float>::search_result_t*>(result_c);
...
gpu_cagra_c gpu_cagra_merge(gpu_cagra_c* indices, uint32_t num_indices, uint32_t nthread, const int* devices, uint32_t num_devices, void* errmsg) {
    ...
    try {
        auto* first = static_cast<gpu_cagra_any_t*>(indices[0]);
        quantization_t qtype = first->qtype;
        std::vector<int> device_vec(devices, devices + num_devices);
        switch (qtype) {
            case Quantization_F32: return merge_cagra_impl<float>(indices, num_indices, nthread, device_vec, qtype);
            case Quantization_F16: return merge_cagra_impl<half>(indices, num_indices, nthread, device_vec, qtype);
            case Quantization_INT8: return merge_cagra_impl<int8_t>(indices, num_indices, nthread, device_vec, qtype);
            case Quantization_UINT8: return merge_cagra_impl<uint8_t>(indices, num_indices, nthread, device_vec, qtype);
            default: throw std::runtime_error("Unsupported quantization type for gpu_cagra_merge");
        }
    } catch (const std::exception& e) {
