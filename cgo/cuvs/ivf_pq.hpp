...
public:
        ...
        auto init_fn = [&](raft_handle_wrapper_t& handle) -> std::any {
            ...
            if (index_) {
                ...
                if (rank < (int)mg_index_->ann_interfaces_.size() && mg_index_->ann_interfaces_[rank].index_.has_value()) {
                    handle.set_index_ptr(static_cast<const ivf_pq_index*>(&mg_index_->ann_interfaces_[rank].index_.value()));
                }
            }
            return std::any();
        };
        ...
        this->worker->broadcast([&](raft_handle_wrapper_t& handle) -> std::any {
            ...
            if (!local_index && !this->replicated_indices_.empty()) {
                ...
                if (it != this->replicated_indices_.end()) {
                    local_index = static_cast<const ivf_pq_index*>(it->second.get());
                }
            }
    ...
    void build_internal(raft_handle_wrapper_t& handle) {
        // Optimization: Do NOT hold the global mutex for the duration of the build
        // as it is a collective call where all participating threads need to run concurrently.
        ...
        if (this->dist_mode == DistributionMode_REPLICATED) {
            ...
            auto built_mg_index = cuvs::neighbors::ivf_pq::build(*mg_res, mg_params, raft::make_const_mdspan(dataset_pinned.view()));
    ...
    search_result_t search_internal(raft_handle_wrapper_t& handle, const T* queries_data, uint64_t num_queries, uint32_t limit, const ivf_pq_search_params_t& sp) {
        ...
        // std::cout << "[DEBUG " << get_timestamp() << "] IVF-PQ search_internal: num_queries=" << num_queries << " limit=" << limit << " device=" << handle.get_device_id() << std::endl;
    ...
    search_result_t search_float_internal(raft_handle_wrapper_t& handle, const float* queries_data, uint64_t num_queries, uint32_t query_dimension, 
        ...
        // std::cout << "[DEBUG " << get_timestamp() << "] IVF-PQ search_float_internal: num_queries=" << num_queries << " limit=" << limit << " device=" << handle.get_device_id() << std::endl;
        ...
        if (this->dist_mode == DistributionMode_SHARDED && is_snmg_handle(*res) && mg_index_) {
            ...
            if (local_index) {
                ...
                std::string msg = "IVF-PQ search_float error: No valid index found for device " + std::to_string(handle.get_device_id()) + 
                                 " (Mode: " + mode_name(this->dist_mode) + ")";
                throw std::runtime_error(msg);
            }
            handle.sync();
