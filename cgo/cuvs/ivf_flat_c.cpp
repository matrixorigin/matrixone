...
// gpu_ivf_flat_rows_fitting: capacity for this index shape, computed entirely in C++.
//
...
int gpu_ivf_flat_rows_fitting(uint64_t dim, uint64_t elem_size,
    ...
    try {
        ...
        if (out_trainset_rows) {
            ...
            const auto tcost = matrixone::ivf_flat_trainset_cost(dim, elem_size);
        ...
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_rows_fitting", e.what());
        return -1;
    } catch (...) {
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_rows_fitting", "unknown C++ exception");
        return -1;
    }
}
