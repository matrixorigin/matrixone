...
gpu_ivf_flat_c gpu_ivf_flat_new(const void* dataset_data, uint64_t count_vectors, uint32_t dimension, 
 ...
 "Error in gpu_ivf_flat_new", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_new", "unknown C++ exception");
    }
    return nullptr;
}
...
gpu_ivf_flat_c gpu_ivf_flat_new_empty(uint64_t total_count, uint32_t dimension, distance_type_t metric_c, 
 ...
 "Error in gpu_ivf_flat_new_empty", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_new_empty", "unknown C++ exception");
    }
    return nullptr;
}
...
gpu_ivf_flat_c gpu_ivf_flat_load_file(const char* filename, uint32_t dimension, distance_type_t metric_c,
 ...
 "Error in gpu_ivf_flat_load_file", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_load_file", "unknown C++ exception");
    }
    return nullptr;
}
...
void gpu_ivf_flat_destroy(gpu_ivf_flat_c index_c, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_destroy", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_destroy", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_start(gpu_ivf_flat_c index_c, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_start", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_start", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_build(gpu_ivf_flat_c index_c, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_build", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_build", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_add_chunk(gpu_ivf_flat_c index_c, const void* chunk_data, uint64_t chunk_count, const int64_t* ids, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_add_chunk", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_add_chunk", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_add_chunk_float(gpu_ivf_flat_c index_c, const float* chunk_data, uint64_t chunk_count, const int64_t* ids, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_add_chunk_float", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_add_chunk_float", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_train_quantizer(gpu_ivf_flat_c index_c, const float* train_data, uint64_t n_samples, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_train_quantizer", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_train_quantizer", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_set_batch_window(gpu_ivf_flat_c index_c, int64_t window_us, void* errmsg) {
    ...
    try {
        auto* any = static_cast<gpu_ivf_flat_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32: static_cast<gpu_ivf_flat_t<float>*>(any->ptr)->set_batch_window(window_us); break;
            case Quantization_F16: static_cast<gpu_ivf_flat_t<half>*>(any->ptr)->set_batch_window(window_us); break;
            case Quantization_INT8: static_cast<gpu_ivf_flat_t<int8_t>*>(any->ptr)->set_batch_window(window_us); break;
            case Quantization_UINT8: static_cast<gpu_ivf_flat_t<uint8_t>*>(any->ptr)->set_batch_window(window_us); break;
            default: break;
        }
    } catch (const std::exception& e) {
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_set_batch_window", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_set_batch_window", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_set_dynb_conservative_dispatch(gpu_ivf_flat_c index_c, bool enable, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_set_dynb_conservative_dispatch", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_set_dynb_conservative_dispatch", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_set_quantizer(gpu_ivf_flat_c index_c, float min, float max, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_set_quantizer", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_set_quantizer", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_get_quantizer(gpu_ivf_flat_c index_c, float* min, float* max, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_get_quantizer", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_get_quantizer", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_save(gpu_ivf_flat_c index_c, const char* filename, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_save", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_save", "unknown C++ exception");
    }
}
...
void gpu_ivf_flat_load_dir(gpu_ivf_flat_c index_c, const char* dir,
    ...
    try {
        auto* any = static_cast<gpu_ivf_flat_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32:   static_cast<gpu_ivf_flat_t<float>*>(any->ptr)->load_dir(dir, target_mode); break;
            case Quantization_F16:   static_cast<gpu_ivf_flat_t<half>*>(any->ptr)->load_dir(dir, target_mode); break;
            case Quantization_INT8:  static_cast<gpu_ivf_flat_t<int8_t>*>(any->ptr)->load_dir(dir, target_mode); break;
            case Quantization_UINT8: static_cast<gpu_ivf_flat_t<uint8_t>*>(any->ptr)->load_dir(dir, target_mode); break;
            default: break;
        }
    } catch (const std::exception& e) {
...
gpu_ivf_flat_search_res_t gpu_ivf_flat_search(gpu_ivf_flat_c index_c, const void* queries_data, uint64_t num_queries, 
 ...
 "Error in gpu_ivf_flat_search", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_search", "unknown C++ exception");
    }
    return result;
}
...
gpu_ivf_flat_search_res_t gpu_ivf_flat_search_float(gpu_ivf_flat_c index_c, const float* queries_data, uint64_t num_queries, 
    ...
    try {
        ...
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_search_float", "unknown C++ exception");
    }
    return result;
}
...
uint64_t gpu_ivf_flat_search_float_async(gpu_ivf_flat_c index_c, const float* queries_data, uint64_t num_queries, 
    ...
    try {
        ...
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_search_float_async", "unknown C++ exception");
        return 0;
    }
}
...
char* gpu_ivf_flat_info(gpu_ivf_flat_c index_c, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_info", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_info", "unknown C++ exception");
        return nullptr;
    }
}
...
void gpu_ivf_flat_get_centers(gpu_ivf_flat_c index_c, void* centers, void* errmsg) {
 ...
 "Error in gpu_ivf_flat_get_centers", e.what());
        ...
        matrixone::set_errmsg(errmsg,
 "Error in gpu_ivf_flat_get_centers", "unknown C++ exception");
    }
}
...
gpu_ivf_flat_search_res_t gpu_ivf_flat_search_float_with_filter(gpu_ivf_flat_c index_c, const float* queries_data,
    ...
    try {
        ...
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_search_float_with_filter", "unknown C++ exception");
    }
    return result;
}

uint64_t gpu_ivf_flat_search_float_with_filter_async(gpu_ivf_flat_c index_c, const float* queries_data,
                                                      uint64_t num_queries, uint32_t query_dimension,
                                                      uint32_t limit, ivf_flat_search_params_t sp,
                                                      const char* preds_json, void* errmsg) {
    if (errmsg) *(static_cast<char**>(errmsg)) = nullptr;
    try {
        ...
        std::string preds = preds_json ? preds_json : "";
        switch (any->qtype) {
            case Quantization_F32:   return static_cast<gpu_ivf_flat_t<float>*>(any->ptr)->search_float_with_filter_async(queries_data, num_queries, query_dimension, limit, sp, preds);
            case Quantization_F16:   return static_cast<gpu_ivf_flat_t<half>*>(any->ptr)->search_float_with_filter_async(queries_data, num_queries, query_dimension, limit, sp, preds);
            case Quantization_INT8:  return static_cast<gpu_ivf_flat_t<int8_t>*>(any->ptr)->search_float_with_filter_async(queries_data, num_queries, query_dimension, limit, sp, preds);
            case Quantization_UINT8: return static_cast<gpu_ivf_flat_t<uint8_t>*>(any->ptr)->search_float_with_filter_async(queries_data, num_queries, query_dimension, limit, sp, preds);
            default: return 0;
        }
    } catch (const std::exception& e) {
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_search_float_with_filter_async", e.what());
        return 0;
    } catch (...) {
        matrixone::set_errmsg(errmsg, "Error in gpu_ivf_flat_search_float_with_filter_async", "unknown C++ exception");
        return 0;
    }
}
