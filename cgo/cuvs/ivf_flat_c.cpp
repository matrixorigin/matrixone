...
struct gpu_ivf_flat_any_t {
    ...
    ~gpu_ivf_flat_any_t() {
        switch (qtype) {
            case Quantization_F32: delete static_cast<gpu_ivf_flat_t<float>*>(ptr); break;
            case Quantization_F16: delete static_cast<gpu_ivf_flat_t<half>*>(ptr); break;
            case Quantization_INT8: delete static_cast<gpu_ivf_flat_t<int8_t>*>(ptr); break;
            case Quantization_UINT8: delete static_cast<gpu_ivf_flat_t<uint8_t>*>(ptr); break;
            default: break;
        }
    }
};
    ...
    try {
        ...
        switch (qtype) {
            case Quantization_F32:
                ptr = new gpu_ivf_flat_t<float>(static_cast<const float*>(dataset_data), count_vectors, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_F16:
                ptr = new gpu_ivf_flat_t<half>(static_cast<const half*>(dataset_data), count_vectors, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_INT8:
                ptr = new gpu_ivf_flat_t<int8_t>(static_cast<const int8_t*>(dataset_data), count_vectors, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_UINT8:
                ptr = new gpu_ivf_flat_t<uint8_t>(static_cast<const uint8_t*>(dataset_data), count_vectors, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
...
gpu_ivf_flat_c gpu_ivf_flat_new_empty(uint64_t total_count, uint32_t dimension, distance_type_t metric_c, 
    ...
    try {
        ...
        switch (qtype) {
            case Quantization_F32:
                ptr = new gpu_ivf_flat_t<float>(total_count, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_F16:
                ptr = new gpu_ivf_flat_t<half>(total_count, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_INT8:
                ptr = new gpu_ivf_flat_t<int8_t>(total_count, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            case Quantization_UINT8:
                ptr = new gpu_ivf_flat_t<uint8_t>(total_count, dimension, metric_c, build_params, devs, nthread, dist_mode);
                break;
            default: return nullptr;
        }
        static_cast<gpu_index_base_t<float, ivf_flat_build_params_t>*>(ptr)->start();
...
void gpu_ivf_flat_add_chunk(gpu_ivf_flat_c index_c, const void* chunk_data, uint64_t chunk_count, void* errmsg) {
    ...
    try {
        auto* any = static_cast<gpu_ivf_flat_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32: static_cast<gpu_ivf_flat_t<float>*>(any->ptr)->add_chunk(static_cast<const float*>(chunk_data), chunk_count); break;
            case Quantization_F16: static_cast<gpu_ivf_flat_t<half>*>(any->ptr)->add_chunk(static_cast<const half*>(chunk_data), chunk_count); break;
            case Quantization_INT8: static_cast<gpu_ivf_flat_t<int8_t>*>(any->ptr)->add_chunk(static_cast<const int8_t*>(chunk_data), chunk_count); break;
            case Quantization_UINT8: static_cast<gpu_ivf_flat_t<uint8_t>*>(any->ptr)->add_chunk(static_cast<const uint8_t*>(chunk_data), chunk_count); break;
            default: break;
        }
    } catch (const std::exception& e) {
        matrixone::set_errmsg(errmsg, 
...
void gpu_ivf_flat_add_chunk_float(gpu_ivf_flat_c index_c, const float* chunk_data, uint64_t chunk_count, void* errmsg) {
    ...
    try {
        auto* any = static_cast<gpu_ivf_flat_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32: static_cast<gpu_ivf_flat_t<float>*>(any->ptr)->add_chunk_float(chunk_data, chunk_count); break;
            case Quantization_F16: static_cast<gpu_ivf_flat_t<half>*>(any->ptr)->add_chunk_float(chunk_data, chunk_count); break;
            case Quantization_INT8: static_cast<gpu_ivf_flat_t<int8_t>*>(any->ptr)->add_chunk_float(chunk_data, chunk_count); break;
            case Quantization_UINT8: static_cast<gpu_ivf_flat_t<uint8_t>*>(any->ptr)->add_chunk_float(chunk_data, chunk_count); break;
            default: break;
        }
    } catch (const std::exception& e) {
        matrixone::set_errmsg(errmsg, 
