...
type GpuIvfFlat[T VectorType] struct {
    cIndex C.gpu_ivf_flat_c
    n_list uint32
    dimension uint32
}

// NewGpuIvfFlat creates a new GpuIvfFlat instance for building from dataset.
...
func NewGpuIvfFlat[T VectorType](dataset []T, count_vectors uint64, dimension uint32, metric DistanceType, n_list uint32, devices []int, nthread uint32, force_mg bool) (*GpuIvfFlat[T], error) {
    ...
    return &GpuIvfFlat[T]{cIndex: cIndex, n_list: n_list, dimension: dimension}, nil
}

// NewGpuIvfFlatFromFile creates a new GpuIvfFlat instance for loading from file.
func NewGpuIvfFlatFromFile[T VectorType](filename string, dimension uint32, metric DistanceType, devices []int, nthread uint32, force_mg bool) (*GpuIvfFlat[T], error) {
    ...
    var errmsg *C.char
    cIndex := C.gpu_ivf_flat_new_from_file(
        c_filename,
        C.uint32_t(dimension),
        C.distance_type_t(metric),
        &cDevices[0],
    ...
    if cIndex == nil {
        return nil, fmt.Errorf("failed to create GpuIvfFlat from file")
    }
    return &GpuIvfFlat[T]{cIndex: cIndex, n_list: 0, dimension: dimension}, nil
}

// Load loads the index to the GPU
func (gbi *GpuIvfFlat[T]) Load() error {
    if gbi.cIndex == nil {
        return fmt.Errorf("GpuIvfFlat is not initialized")
    }
    var errmsg *C.char
    C.gpu_ivf_flat_load(gbi.cIndex, unsafe.Pointer(&errmsg))
    if errmsg != nil {
        errStr := C.GoString(errmsg)
        C.free(unsafe.Pointer(errmsg))
        return fmt.Errorf("%s", errStr)
    }
    gbi.n_list = uint32(C.gpu_ivf_flat_get_n_list(gbi.cIndex))
    return nil
}

// Save saves the index to file
func (gbi *GpuIvfFlat[T]) Save(filename string) error {
    if gbi.cIndex == nil {
        return fmt.Errorf("GpuIvfFlat is not initialized")
    }
    c_filename := C.CString(filename)
    ...
    var errmsg *C.char
    C.gpu_ivf_flat_save(gbi.cIndex, c_filename, unsafe.Pointer(&errmsg))
    if errmsg != nil {
        errStr := C.GoString(errmsg)
        C.free(unsafe.Pointer(errmsg))
        return fmt.Errorf("%s", errStr)
...
func (gbi *GpuIvfFlat[T]) Search(queries []T, num_queries uint64, query_dimension uint32, limit uint32, n_probes uint32) ([]int64, []float32, error) {
	if gbi.cIndex == nil {
		return nil, nil, fmt.Errorf("GpuIvfFlat is not initialized")
	}
	...
	C.gpu_ivf_flat_get_results(cResult, C.uint64_t(num_queries), C.uint32_t(limit), (*C.int64_t)(unsafe.Pointer(&neighbors[0])), (*C.float)(unsafe.Pointer(&distances[0])))
    runtime.KeepAlive(neighbors)
    runtime.KeepAlive(distances)

	C.gpu_ivf_flat_free_search_result(cResult);

	return neighbors, distances, nil
}

// Destroy frees the C++ gpu_ivf_flat_t instance
func (gbi *GpuIvfFlat[T]) Destroy() error {
    if gbi.cIndex == nil {
        return nil
    }
    var errmsg *C.char
    C.gpu_ivf_flat_destroy(gbi.cIndex, unsafe.Pointer(&errmsg))
    gbi.cIndex = nil
    if errmsg != nil {
        errStr := C.GoString(errmsg)
        C.free(unsafe.Pointer(errmsg))
...
func (gbi *GpuIvfFlat[T]) GetCenters() ([]float32, error) {
	if gbi.cIndex == nil {
	...
	var errmsg *C.char
	C.gpu_ivf_flat_get_centers(gbi.cIndex, (*C.float)(&centers[0]), unsafe.Pointer(&errmsg))
