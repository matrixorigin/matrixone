...
void gpu_ivf_pq_add_filter_chunk(gpu_ivf_pq_c index_c, uint32_t col_idx,
    ...
    try {
        auto* any = static_cast<gpu_ivf_pq_any_t*>(index_c);
        switch (any->qtype) {
            case Quantization_F32:   static_cast<gpu_ivf_pq_t<float>*>(any->ptr)->add_filter_chunk(col_idx, data, nrows); break;
            case Quantization_F16:   static_cast<gpu_ivf_pq_t<half>*>(any->ptr)->add_filter_chunk(col_idx, data, nrows); break;
            case Quantization_INT8:  static_cast<gpu_ivf_pq_t<int8_t>*>(any->ptr)->add_filter_chunk(col_idx, data, nrows); break;
            case Quantization_UINT8: static_cast<gpu_ivf_pq_t<uint8_t>*>(any->ptr)->add_filter_chunk(col_idx, data, nrows); break;
            default: break;
        }
    } catch (const std::exception& e) {
