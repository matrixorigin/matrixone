...
#define ASSERT_LE(val1, val2) do { if (!((val1) <= (val2))) { REPORT_FAILURE("ASSERT_LE failed: " #val1 " vs " #val2); } } while (0)
#define ASSERT_LT(val1, val2) do { if (!((val1) < (val2))) { REPORT_FAILURE("ASSERT_LT failed: " #val1 " vs " #val2); } } while (0)
