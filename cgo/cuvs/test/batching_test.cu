...
TEST(DynamicBatchingTest, CagraConcurrentSearch) {
    ...
    gpu_cagra_t<float, float> index(dataset.data(), count, dimension, DistanceType_L2Expanded, bp, devices, 8, DistributionMode_SINGLE_GPU);
...
TEST(DynamicBatchingTest, IvfPqConcurrentSearch) {
    ...
    bp.n_lists = 10;
    bp.m = 8;
    gpu_ivf_pq_t<float, float> index(dataset.data(), count, dimension, DistanceType_L2Expanded, bp, devices, 8, DistributionMode_SINGLE_GPU);
