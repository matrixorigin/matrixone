...
class IVFRankMode(Enum):
    ...
    Attributes:
        PRE: Pre-ranking mode. Uses IVF index to filter candidates first,
             then applies ranking within those candidates. Fastest but may
             miss relevant results if they're in non-selected centroids.
             Best for: High-throughput, latency-sensitive scenarios.
        
        POST: Post-ranking mode. Retrieves expanded candidates from IVF index,
              then re-ranks using exact distance computation. Slower but higher
              accuracy. Best for: High-recall requirements.
        
        FORCE: Force mode. Explicitly enforces IVF index usage regardless of
               query optimizer's decision. Balanced speed/accuracy based on
               probe limit settings. Best for: Predictable index usage.
    """
    ...
    def __init__(
        ...
        if mode is None:
            ...
            try:
                ...
                raise ValueError(
                    f"Invalid IVF rank mode: {mode}. "
                    f"Must be one of: {', '.join(m.value for m in IVFRankMode)}"
                )
        else:
            raise ValueError(
                f"mode must be IVFRankMode or string, got {type(mode).__name__}"
            )
