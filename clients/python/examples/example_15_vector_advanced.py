        ...
        try:
            ...
            for doc_data in async_docs_data:
                ...
                await self.async_client.execute(
                    ...
                    (doc_data['title'], doc_data['content'], doc_data['category'], 
                     doc_data['metadata_json'], vector_str, doc_data['created_at'])
                )
    ...
    def test_batch_operations(self, AdvancedDocument):
        ...
        try:
            ...
            for i, query_vector in enumerate(query_vectors, 1):
                ...
                ).order_by(AdvancedDocument.embedding.l2_distance(query_vector)).limit(5).all()
            ...
            query_patterns = [
                {
                    ...
                    'query': lambda: self.client.query(AdvancedDocument).select(
                        ...
                        AdvancedDocument.title,
                        AdvancedDocument.embedding.l2_distance(query_vector).label('distance')
                    ).order_by(AdvancedDocument.embedding.l2_distance(query_vector)).limit(10).all()
                },
                {
                    ...
                    'query': lambda: self.client.query(AdvancedDocument).select(
                        ...
                        AdvancedDocument.title,
                        AdvancedDocument.embedding.l2_distance(query_vector).label('distance')
                    ).filter(AdvancedDocument.category == 'Batch').order_by(
                        AdvancedDocument.embedding.l2_distance(query_vector)
                    ).limit(10).all()
                },
                {
                    'name': 'Range Search',
                    'query': lambda: self.client.query(AdvancedDocument).select(
                        ...
                        AdvancedDocument.embedding.l2_distance(query_vector) < 2.0
                    ).order_by(AdvancedDocument.embedding.l2_distance(query_vector)).limit(10).all()
                }
            ]
