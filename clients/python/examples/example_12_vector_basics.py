...
class VectorBasicsDemo:
    ...
    def test_vector_parameter_formats(self, TestDocument):
        ...
        try:
            ...
            list_distance = self.session.query(
                TestDocument.embedding.l2_distance(vector_list).label('distance')
            ).scalar()
            
            str_distance = self.session.query(
                TestDocument.embedding.l2_distance(vector_str).label('distance')
            ).scalar()
    ...
    def test_distance_functions(self, TestDocument):
        ...
        # Insert test data
        test_docs = [
            ('AI Introduction', 'Introduction to artificial intelligence', np.random.rand(16).tolist()),
            ('Machine Learning', 'Machine learning fundamentals', np.random.rand(16).tolist()),
            ('Deep Learning', 'Deep learning concepts and applications', np.random.rand(16).tolist()),
            ('Natural Language Processing', 'NLP techniques and models', np.random.rand(16).tolist()),
        ...
        for func_name, func in distance_functions:
            ...
            try:
                ...
                result = self.session.query(
                    TestDocument.id,
                    TestDocument.title,
                    func(query_vector).label('distance')
                ).order_by(func(query_vector)).limit(3).all()
