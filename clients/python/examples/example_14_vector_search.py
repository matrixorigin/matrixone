...
class VectorSearchDemo:
    ...
    def insert_test_data(self, SearchDocument, num_docs=1000):
        ...
        categories = ['AI', 'Machine Learning', 'Deep Learning', 'NLP', 'Computer Vision', 'Robotics']
        tags_list = ['research', 'tutorial', 'application', 'theory', 'practice', 'advanced', 'beginner']
    ...
    def test_similarity_search(self, SearchDocument):
        ...
        for metric_name, distance_func in distance_metrics:
            ...
            try:
                ...
                ).order_by(distance_func(query_vector)).limit(10).all()
        ...
        try:
            ...
            for threshold in thresholds:
                ...
                results = self.session.query(
                    ...
                    SearchDocument.embedding.l2_distance(query_vector) < threshold
                ).order_by(SearchDocument.embedding.l2_distance(query_vector)).all()
    ...
    def test_hybrid_search(self, SearchDocument):
        ...
        try:
            ...
            for scenario in scenarios:
                ...
                ).order_by(SearchDocument.embedding.l2_distance(query_vector)).limit(5).all()
        ...
        try:
            ...
            for i, query_vector in enumerate(query_vectors, 1):
                ...
                results = self.session.query(
                    ...
                    SearchDocument.title,
                    SearchDocument.embedding.l2_distance(query_vector).label('distance')
                ).order_by(SearchDocument.embedding.l2_distance(query_vector)).limit(3).all()
    ...
    def test_search_optimization(self, SearchDocument):
        ...
        try:
            ...
            start_time = time.time()
            results_all = self.session.query(SearchDocument).order_by(
                SearchDocument.embedding.l2_distance(query_vector)
            ).limit(10).all()
            time_all = time.time() - start_time
            
            # With projection (select only needed columns)
            start_time = time.time()
            results_proj = self.session.query(
                SearchDocument.id,
                SearchDocument.title,
                SearchDocument.embedding.l2_distance(query_vector).label('distance')
            ).order_by(SearchDocument.embedding.l2_distance(query_vector)).limit(10).all()
            ...
            for limit in limits:
                ...
                results = self.session.query(
                    ...
                    SearchDocument.title,
                    SearchDocument.embedding.l2_distance(query_vector).label('distance')
                ).order_by(SearchDocument.embedding.l2_distance(query_vector)).limit(limit).all()
                search_time = time.time() - start_time
    ...
    def test_search_result_ranking(self, SearchDocument):
        ...
        try:
            ...
            for i, result in enumerate(results, 1):
                ...
                print(f"       L2: {result.l2_distance:.4f}, Cosine: {result.cosine_distance:.4f}, Inner: {result.inner_product:.4f}")
