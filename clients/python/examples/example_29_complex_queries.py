...
class Product(Base):
    ...
    __table_args__ = (
        FulltextIndex('ftidx_product', ['product_name', 'description'], algorithm=FulltextAlgorithmType.BM25),
    )
...
class ComplexQueryDemo:
    ...
    def test_fulltext_standalone(self):
        ...
        stmt = (
            select(Product.product_name, Product.price, Product.category_id)
            .where(boolean_match('product_name', 'description').must('laptop'))
        )
    ...
    def test_fulltext_groupby(self):
        ...
        stmt = (
            ...
            .where(
                boolean_match('product_name', 'description').must(
                    group().medium('python', 'machine')
                )
            )
