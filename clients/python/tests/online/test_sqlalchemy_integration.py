...
class TestSQLAlchemyIntegration:
    ...
    async def test_async_sqlalchemy_operations(self, test_async_client):
        ...
        try:
            ...
            await test_async_client.execute("CREATE TABLE IF NOT EXISTS async_users (id INT PRIMARY KEY AUTO_INCREMENT, username VARCHAR(50), email VARCHAR(100))")
            
            # Clear any existing data
            await test_async_client.execute("DELETE FROM async_users")
            
            # Insert user
            await test_async_client.execute("INSERT INTO async_users (username, email) VALUES ('async_user', 'async@example.com')")
            ...
            # Update user
            await test_async_client.execute("UPDATE async_users SET email = 'updated_async@example.com' WHERE username = 'async_user'")
