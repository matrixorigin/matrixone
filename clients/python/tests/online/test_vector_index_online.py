...
class TestVectorIndexOnline:
    ...
    def test_vector_index_with_data_operations(self, client, Base, Session):
        ...
        sample_data = [
            {"id": 1, "embedding": [0.1] * 64, "title": "Document 1", "category": "tech"},
            {"id": 2, "embedding": [0.2] * 64, "title": "Document 2", "category": "science"},
            {"id": 3, "embedding": [0.3] * 64, "title": "Document 3", "category": "tech"},
        ]
    ...
    def test_vector_index_ddl_definition_with_session(self, engine, Base, Session):
        ...
        try:
            sample_docs = [
                DocumentWithDDLIndex(id=1, embedding=[0.1] * 128, title="DDL Document 1", category="tech"),
                DocumentWithDDLIndex(id=2, embedding=[0.2] * 128, title="DDL Document 2", category="science"),
                DocumentWithDDLIndex(id=3, embedding=[0.3] * 128, title="DDL Document 3", category="tech"),
                DocumentWithDDLIndex(id=4, embedding=[0.4] * 128, title="DDL Document 4", category="science"),
            ]
            ...
            results = (
                session.query(DocumentWithDDLIndex)
                .order_by(DocumentWithDDLIndex.embedding.l2_distance(query_vector))
                .limit(3)
                .all()
            )
            ...
            cosine_results = (
                session.query(DocumentWithDDLIndex)
                .order_by(DocumentWithDDLIndex.embedding.cosine_distance(query_vector))
                .limit(2)
                .all()
            )
