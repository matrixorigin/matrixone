...
class TestTransactionInsertMethods:
    ...
    async def test_async_transaction_rollback_insert(self, async_client_setup):
        ...
        try:
            async with client.transaction() as tx:
                # Insert data
                data = {'name': 'Async Rollback Test', 'age': 42, 'email': 'async_rollback@example.com'}
                result = await tx.insert(f"{test_db}.test_table", data)
                assert result.affected_rows == 1
    ...
    def test_sync_transaction_mixed_operations(self, sync_client_setup):
        ...
        with client.transaction() as tx:
            ...
            # Direct execute
            result = tx.execute(f"INSERT INTO {test_db}.test_table (name, age, email) VALUES ('Mixed Test 4', 23, 'mixed4@example.com')")
            assert result.affected_rows == 1
    ...
    async def test_async_transaction_mixed_operations(self, async_client_setup):
        ...
        async with client.transaction() as tx:
            ...
            # Direct execute
            result = await tx.execute(f"INSERT INTO {test_db}.test_table (name, age, email) VALUES ('Async Mixed Test 4', 27, 'async_mixed4@example.com')")
            assert result.affected_rows == 1
