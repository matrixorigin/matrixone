...
class TestAsyncClientOnline:
    ...
    @pytest.mark.asyncio
    async def test_async_snapshot_query_functionality(self, test_async_client, test_database):
        """Test async snapshot query functionality"""
        test_db, test_table = test_database
        ...
        try:
            ...
            result = (
                await test_async_client.query(test_table, database=test_db, snapshot=snapshot_name)
                .select("*")
                .where("value > ?", 150)
                .execute()
            )
    ...
    @pytest.mark.asyncio
    async def test_async_snapshot_query_syntax_validation(self, test_async_client, test_database):
        """Test that async snapshot queries use correct syntax"""
        test_db, test_table = test_database
