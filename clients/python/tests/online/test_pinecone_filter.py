...
class TestPineconeFilter:
    ...
    def test_filter_with_or_operator(self, client, test_data_setup):
        ...
        for match in results.matches:
            assert (match.metadata["director"] == "Christopher Nolan" or 
                   match.metadata["rating"] >= 8.9)
    ...
    def test_filter_with_nin_operator(self, client, test_data_setup):
        ...
        results = index.query(
            vector=query_vector,
            top_k=10,
            filter={"genre": {"$nin": ["action", "sci-fi"]}}
        )
    ...
    def test_filter_with_ne_operator(self, client, test_data_setup):
        ...
        results = index.query(
            vector=query_vector,
            top_k=10,
            filter={"director": {"$ne": "Christopher Nolan"}}
        )
    ...
    def test_filter_with_special_characters(self, client, test_data_setup):
        ...
        results = index.query(
            vector=query_vector,
            top_k=10,
            filter={"director": "Director with apostrophe's name"}
        )
