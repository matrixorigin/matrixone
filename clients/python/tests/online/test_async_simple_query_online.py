...
class TestAsyncSimpleQueryOnline:
    ...
    async def async_client_setup(self):
        ...
        # Insert test data
        test_articles = [
            (1, "Python Async Programming", "Learn Python async programming with asyncio and async/await", "Programming", "python,async,asyncio,programming", "Alice", "2024-01-15"),
            (2, "Machine Learning with Python", "Complete guide to machine learning algorithms and implementation", "AI", "python,ml,ai,algorithms", "Bob", "2024-01-20"),
            (3, "JavaScript Async Development", "Modern JavaScript development with async/await and promises", "Programming", "javascript,async,promises,web", "Charlie", "2024-01-25"),
            (4, "Data Science Fundamentals", "Introduction to data science concepts and tools", "AI", "data,science,analytics,python", "David", "2024-02-01"),
            (5, "Deprecated Python Libraries", "Old Python libraries that should be avoided", "Programming", "python,deprecated,legacy", "Eve", "2024-02-05"),
            (6, "Advanced Machine Learning", "Deep learning and neural networks with TensorFlow", "AI", "ai,deeplearning,tensorflow,python", "Frank", "2024-02-10"),
            (7, "Web Development Best Practices", "Modern practices for building web applications", "Programming", "web,best practices,modern,async", "Grace", "2024-02-15"),
            (8, "Artificial Intelligence Overview", "Introduction to AI concepts and applications", "AI", "ai,overview,introduction", "Henry", "2024-02-20"),
            (9, "Async Database Operations", "How to perform async database operations efficiently", "Database", "async,database,operations,performance", "Ivy", "2024-02-25"),
            (10, "Python Web Frameworks", "Comparison of Python web frameworks including async support", "Programming", "python,web,frameworks,async,fastapi", "Jack", "2024-03-01")
        ]
        ...
        try:
            await client.execute("CREATE FULLTEXT INDEX ft_async_articles ON test_async_simple_query_articles(title, content)")
        except Exception as e:
    ...
    async def test_async_basic_natural_language_search(self, async_client_setup):
        ...
        results = await (client.fulltext_index.simple_query("test_async_simple_query_articles")
    ...
    async def test_async_search_by_category(self, async_client_setup):
        ...
        client, test_db = async_client_setup
        # Search AI category
        ai_results = await (client.fulltext_index.simple_query("test_async_simple_query_articles")
                           .columns("title", "content")
                           .search("learning")
                           .where("category = 'AI'")
                           .execute())
        ...
        prog_results = await (client.fulltext_index.simple_query("test_async_simple_query_articles")
                             .columns("title", "content")
                             .search("programming")
                             .where("category = 'Programming'")
                             .execute())
    ...
    async def test_async_error_handling_invalid_table(self, async_client_setup):
        ...
        with pytest.raises(QueryError):
            await (client.fulltext_index.simple_query("nonexistent_table")
                   .columns("title", "content")
                   .search("test")
                   .execute())
    ...
    async def test_async_limit_with_offset(self, async_client_setup):
        ...
        client, test_db = async_client_setup
        # Get first 3 results
        first_results = await (client.fulltext_index.simple_query("test_async_simple_query_articles")
                              .columns("title", "content")
                              .search("python")
                              .limit(3, 0)
                              .execute())
        ...
        next_results = await (client.fulltext_index.simple_query("test_async_simple_query_articles")
                             .columns("title", "content")
                             .search("python")
                             .limit(3, 3)
                             .execute())
    ...
    async def test_async_concurrent_searches(self, async_client_setup):
        ...
        async def search_task(search_term):
            ...
            return await (client.fulltext_index.simple_query(table_name)
                         .columns("title", "content")
                         .search(search_term)
                         .execute())
...
class TestAsyncTransactionSimpleQueryOnline:
    ...
    async def async_tx_client_setup(self):
        ...
        # Insert test data
        test_articles = [
            (1, "Transaction Python Guide", "Learn Python database transactions", "Programming", "python,transaction,database"),
            (2, "Async Transaction Patterns", "Modern async transaction patterns", "Programming", "async,transaction,patterns"),
            (3, "Database Transaction Best Practices", "Best practices for database transactions", "Database", "database,transaction,best practices"),
            (4, "Python Async Database", "Async database operations in Python", "Programming", "python,async,database"),
            (5, "Transaction Isolation Levels", "Understanding transaction isolation levels", "Database", "transaction,isolation,levels")
        ]
    ...
    async def test_async_transaction_simple_query_basic(self, async_tx_client_setup):
        ...
        async with client.transaction() as tx:
            results = await (tx.fulltext_index.simple_query("test_async_tx_articles")
                           .columns("title", "content")
                           .search("python")
                           .execute())
    ...
    async def test_async_transaction_simple_query_error_handling(self, async_tx_client_setup):
        ...
        async with client.transaction() as tx:
            with pytest.raises(QueryError):
                await (tx.fulltext_index.simple_query("nonexistent_table")
                       .columns("title", "content")
                       .search("test")
                       .execute())
