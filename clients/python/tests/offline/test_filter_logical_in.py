...
class TestFilterLogicalIn:
    ...
    def test_filter_logical_in_with_fulltext_filter(self):
        ...
        query2 = self.client.query(Article).filter(logical_in(
            Article.id,
        ...
        assert "WHERE id IN (SELECT id FROM table WHERE MATCH(title, content) AGAINST('+python programming -java' IN BOOLEAN MODE))" in sql2
    ...
    def test_filter_logical_in_combined_with_other_conditions(self):
        ...
        query = (self.client.query(Article)
