...
class TestUnifiedFilter(unittest.TestCase):
    ...
    def test_complex_nested_condition(self):
        ...
        query = self.product_query.filter(or_(
            and_(Product.category == "电子产品", Product.price > 2000),
            and_(Product.category == "图书", Product.stock > 400)
        ))
        sql, params = query._build_sql()
        
        expected_sql = "SELECT * FROM products WHERE category = '电子产品' AND price > 2000 OR category = '图书' AND stock > 400"
        self.assertEqual(sql, expected_sql)
        self.assertEqual(params, [])
    ...
    def test_complex_and_or_combination(self):
        ...
        query = self.user_query.filter(and_(
    ...
    def test_multiple_or_conditions(self):
        ...
        query = self.user_query.filter(or_(
            User.city == "北京",
            User.city == "上海",
            User.age > 30
        ))
    ...
    def test_filter_with_having(self):
        ...
        expected_sql = "SELECT category, COUNT(*) as count FROM products WHERE price > 100 GROUP BY category HAVING COUNT(*) > 1"
        self.assertEqual(sql, expected_sql)
        self.assertEqual(params, [])
