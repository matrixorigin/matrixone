...
class VersionManager:
    ...
    def is_feature_available(self, feature_name: str) -> bool:
        ...
        if requirement.min_version and not self.is_version_compatible(
            requirement.min_version, current_version, ">="
        ):
            return False
        
        # Check maximum version requirement
        if requirement.max_version and not self.is_version_compatible(
            requirement.max_version, current_version, "<="
        ):
            return False
    ...
    def get_version_hint(self, feature_name: str, error_context: str = "") -> str:
        ...
        if requirement.min_version and not self.is_version_compatible(
            ...
            hints.append(f"Feature '{feature_name}' requires backend version {requirement.min_version} or higher, "
                        f"but current version is {current_version}")
        
        if requirement.max_version and not self.is_version_compatible(
            requirement.max_version, current_version, "<="
        ):
            hints.append(f"Feature '{feature_name}' is not supported in backend version {requirement.max_version} or higher, "
                        f"but current version is {current_version}")
...
def requires_version(min_version: str = None, 
