...
class TestAccountManager(unittest.TestCase):
    ...
    def test_alter_account_suspend(self):
        ...
        # Mock successful execution and get_account
        mock_result = Mock()
        mock_result.rows = [('test_account', 'admin_user', datetime.now(), 'SUSPENDED', 'Test account', datetime.now(), 'Security violation')]
        self.client.execute = Mock(return_value=mock_result)
        ...
        account = self.account_manager.alter_account(
            account_name="test_account",
            suspend=True,
            suspend_reason="Security violation"
        )
    ...
    def test_list_accounts_success(self):
        ...
        mock_result.rows = [
            ...
            ('account2', 'admin2', datetime.now(), 'SUSPENDED', 'Account 2', datetime.now(), 'Security issue')
        ]
        self.client.execute = Mock(return_value=mock_result)
    ...
    def test_alter_user_lock(self):
        ...
        # Mock successful execution - first call for ALTER, second call for get_user
        mock_result = Mock()
        mock_result.rows = [('test_user', 'localhost', 'test_account', datetime.now(), 'LOCKED', 'Test user', datetime.now(), 'Security violation')]
        self.client.execute = Mock(return_value=mock_result)
