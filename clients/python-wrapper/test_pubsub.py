...
class TestPubSubManager(unittest.TestCase):
    ...
    def test_create_database_publication_success(self):
        ...
        self.client.execute.assert_any_call(
            "CREATE PUBLICATION `db_pub1` DATABASE `central_db` ACCOUNT `acc1`"
        )
    ...
    def test_alter_publication_success(self):
        ...
        self.assertEqual(self.client.execute.call_count, 2)
        self.client.execute.assert_any_call(
            "ALTER PUBLICATION `db_pub1` ACCOUNT `acc2`"
        )
    ...
    def test_create_subscription_success(self):
        ...
        # Mock successful execution
        mock_result = Mock()
        mock_result.rows = [('db_pub1', 'sys', 'central_db', '*', None, datetime.now(), 'sub_db1', datetime.now(), 0)]
        self.client.execute = Mock(return_value=mock_result)
        ...
        self.assertEqual(self.client.execute.call_count, 2)
        self.client.execute.assert_any_call(
            "CREATE DATABASE `sub_db1` FROM `sys` PUBLICATION `db_pub1`"
        )
    ...
    def test_list_subscriptions_success(self):
        ...
        mock_result = Mock()
        mock_result.rows = [
            ('db_pub1', 'sys', 'central_db', '*', None, datetime.now(), 'sub_db1', datetime.now(), 0),
            ('table_pub1', 'sys', 'central_db', 'products', None, datetime.now(), 'sub_table1', datetime.now(), 0)
        ]
        self.client.execute = Mock(return_value=mock_result)
...
class TestTransactionPubSubManager(unittest.TestCase):
    ...
    def test_transaction_get_publication(self):
        ...
        self.transaction_wrapper.execute.assert_called_with(
            "SHOW PUBLICATIONS WHERE pub_name = 'db_pub1'"
        )
