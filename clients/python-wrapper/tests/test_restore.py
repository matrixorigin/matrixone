...
class TestRestoreManager(unittest.TestCase):
    ...
    def test_restore_with_executor_cluster(self):
        ...
        result = self.restore_manager.restore_with_executor(
            'cluster', 'cluster_snap1', executor=mock_executor
        )
        
        # Verify
        self.assertTrue(result)
        mock_executor.execute.assert_called_once_with(
            "RESTORE CLUSTER FROM SNAPSHOT `cluster_snap1`"
        )
    ...
    def test_restore_with_executor_database(self):
        ...
        result = self.restore_manager.restore_with_executor(
            'database', 'acc1_db_snap1', account_name='acc1', 
            database_name='db1', executor=mock_executor
        )
    ...
    def test_restore_with_executor_table(self):
        ...
        result = self.restore_manager.restore_with_executor(
            'table', 'acc1_tab_snap1', account_name='acc1', 
            database_name='db1', table_name='t1', executor=mock_executor
        )
    ...
    def test_restore_with_executor_invalid_type(self):
        ...
        with self.assertRaises(RestoreErrorClass):
            self.restore_manager.restore_with_executor(
                'invalid', 'snap1'
            )
    ...
    def test_restore_with_executor_missing_params(self):
        ...
        with self.assertRaises(RestoreErrorClass):
            self.restore_manager.restore_with_executor(
                'database', 'snap1', account_name='acc1'
            )
        
        # Test missing table name for table restore
        with self.assertRaises(RestoreErrorClass):
            self.restore_manager.restore_with_executor(
                'table', 'snap1', account_name='acc1', database_name='db1'
            )
