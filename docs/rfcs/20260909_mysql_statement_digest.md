...
| `max_digest_length` | `SessionInfo.MaxDigestLength` plus `MaxDigestLengthSet`, serialized in `pipeline.SessionInfo` fields 18/19 | A remote CN honors a validated explicit snapshot, including zero; a STATEMENT_DIGEST remote scope snapshots the resolver once before dispatch, while unrelated scopes do not resolve this optional variable. |
...
substituting a receiving node's compiled default. Resolver errors, invalid
variable types, invalid snapshots, or out-of-range lengths fail before a
STATEMENT_DIGEST remote scope is dispatched; they are never converted into an
unset value or a receiver-local default.

### Mixed-version safety and rollback

`STATEMENT_DIGEST` has plan function ID 579 and may occur inside a remote
scope. Remote planning therefore applies two fail-closed gates: the
coordinator must advertise `MORPCVersion73`, and the send path probes the
actual selected worker from the serialized pipeline's destination node. A
missing, unknown, failed, or below-`MORPCVersion73` worker probe rejects the
send before a sender is created. Versions 70, 71, and 72 are already allocated
by the current base branch to row-dependent CONV bases, checked integer
arithmetic, and corrected IP semantics. Version 73 is reserved specifically
for this feature; both gates name the required version in their errors.
...
| Mixed-version behavior | Remote-planning tests proving coordinator and selected-worker v70/v71/v72 rejection and v73 acceptance for plans containing `STATEMENT_DIGEST`, including missing/unknown/probe-failure destinations and post-placement worker downgrade, while preserving each base capability's own v70/v71/v72 gates. |
...
the function once all nodes that can receive its plans satisfy MORPC v73.
