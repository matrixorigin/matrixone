...
labels. Every CN exposes a controllable pipeline address, work state, authority
liveness lease/revision, capability version, membership generation, and
execution incarnation. The fixture exposes barriers for placement reached,
RemoteRun started, workspace dirtied, drain published, and cleanup completed.
...
| CG-AUTH-10 | B | Target consumes and publishes an attach-blocked session, but consume response or attach acknowledgement is lost; a different or restarted Proxy incarnation also tries status/attach | Normal consume/attach by the claiming Proxy incarnation | Idempotent status returns the exact target/session tuple only to the authenticated claimant; that Proxy either attaches once or revokes and observes cleanup before a new nonce. The other incarnation cannot observe or take over the tuple. Claimant loss reaches bounded lease cleanup unless an explicit issuer-authorized transfer CAS exists. The old session runs zero SQL and cannot coexist with a retry session. |
...
| CG-AUTH-12 | B | Restart/re-elect the ticket service in each of Issued, Claimed, and Consumed-before-attach | Same transitions without restart | Restart durably advances issuer epoch before new issue; old Issued/Claimed tickets fail late consume, while the exact Consumed target/incarnation/session tuple survives for attach or bounded revoke/lease cleanup. No replay, second session, or orphan survives the terminal bound. |
...
| CG-BIND-02 | B | On a `ga1` ingress connection, change session execution selection to `ga2` outside a transaction and run a stateless read | Fresh connection whose ingress is `ga2` | Relational/runtime-user operators execute only on `ga2`; original ingress remains `ga1` and performs only authenticated planning/topology coordination, fan-in, serialization, and protocol forwarding. Its separately accounted control cost contains no fallback fragment; no volatile, side-effecting, or user-defined function is evaluated there, and no implicit migration occurs. |
...
| CG-PREP-03 | B | PREPARE while a role/account binding authorizes `ga2`, revoke that authorization, then EXECUTE | Authorization generations unchanged | EXECUTE reauthorizes and fails before workspace, lock, operator, or RemoteRun start; cached compile state cannot retain the old grant. |
| CG-PREP-04 | B | PREPARE a clean stateless read, dirty the ingress transaction workspace, then EXECUTE with ingress included/excluded from the pinned group | Direct execution after the write | EXECUTE re-derives hard constraints: included ingress rebuilds topology and reads its write, while excluded ingress fails before any remote/local operator. It never reuses the clean topology. |
...
| CG-GEN-06 | B | Restart/re-elect the control-plane owner after publishing generation N, then reconcile a change | Restart with no topology change | Replacement durably advances authority epoch before accepting lease refreshes and cannot publish an old-epoch callback. Unchanged topology may remain at membership N after fresh proofs; changed topology publishes greater than N, never zero/reused, and ambiguous state remains unavailable rather than label-reconstructed. |
| CG-GEN-07 | B | Restart a `ga2` member while retaining its service ID/address, including overlap or a delayed heartbeat from the old process | No restart | Execution incarnation changes before RPC admission; two live claimants make the group unavailable until one is terminal, then the authority seals a generation greater than N. Old callbacks cannot overwrite it, old-incarnation tickets/routes fail closed, and fresh work uses only the new tuple. |
| CG-GEN-08 | B | Two distinct live service/incarnation tuples advertise one pipeline address, or one tuple has conflicting current addresses | Unique one-to-one member routes | Ambiguous generation is unavailable before placement; no consumer resolves by address, drops one tuple as a duplicate, or silently forms a partial group. Recovery seals a greater unambiguous generation. |
| CG-GEN-09 | B | A member's authority heartbeat lease expires while its last advertised WorkState remains Working | Lease remains valid | New attempts reject the stale snapshot or use an explicitly sealed greater generation that removes the member; no consumer treats cached Working as health or silently shrinks generation N. |
| CG-GEN-10 | B | Renew healthy member leases repeatedly without changing service/incarnation, route, work state, or capability | Expire one lease and later restore eligibility | Healthy renewals advance the authority-epoch/liveness revision proof and deadline but retain membership generation and do not invalidate prepared topology, tickets, or grants; expiry/recovery cannot masquerade as same-generation renewal and follows CG-GEN-09. |
...
| CG-DRAIN-04 | B | Delay registry/seal delivery to a receiver holding Ready and a valid lease; deliver an old route grant | Receiver claim before local seal | Authority remains DrainRequested without that receiver's acknowledgement; claims serialize with its local seal. After all acks and effective Draining, even a stale-cache receiver rejects unclaimed routes before construction; siblings unwind once. |
| CG-DRAIN-05 | B | Partition a participant, expire the control request, restart authority, replay old-epoch ack | Complete current-epoch acknowledgements | Request terminates pending/failed, durable DrainRequested remains, and neither lease expiry nor stale ack permits effective Draining. Restarted receivers cannot reopen old grants; membership cannot drop an unacknowledged participant. |
...
| CG-WIRE-02 | B | Materialize a `ga1` generation-N route, then remove its target or publish N+1 before remote claim/start | Route claim linearizes before the barrier | Unclaimed stale route is rejected before operator start and coordinator rebuilds/retries only with fresh authorization; already-claimed control follows pinned lifecycle rules. |
...
| CG-WIRE-06 | B | Receiver restarts after claim or start but before acknowledgement/completion, reusing its service ID/address | Same route without restart | New incarnation rejects the old grant before operator start; the coordinator cannot rejoin the replacement process and creates no retry until old-incarnation loss plus durable transaction/idempotency outcome is terminal. At most one DML side effect occurs. |
| CG-WIRE-07 | B | User/context data or an authenticated but unauthorized CN fabricates/changes account, role, group, generation, coordinator, or authorization-proof fields | Grant derived from the sealed Security snapshot by the authorized coordinator incarnation | Every fabricated/altered grant fails before route claim or operator construction; internally consistent public metadata and transport authentication alone never confer attempt authorization. |
| CG-WIRE-08 | B | Execute nested A -> B -> C RemoteRun, then omit or alter C's grant, parent route, expected sender, target, or authorization proof | Root coordinator pre-derives the complete target-specific route DAG | Every hop carries the same attempt authorization and a unique edge grant; B can forward but cannot mint or widen C's grant. C verifies B as the expected transport peer and rejects every altered/missing edge before claim/operator start; all predecessor/child route owners terminate once. |
...
| Member liveness lease | Authority heartbeat/sealed registry view | Group registry | refresh/expiry/explicit removal; expired snapshot rejects new attempts/route claims | latest proof per configured member plus actively pinned bounded views |
...
| Handoff ticket/claim | Post-auth authorization resolution / Proxy atomic claim | Security ticket service until target consume linearizes | consume/abort/cancel/route failure/claim lease/TTL/service restart epoch | issue rate × TTL plus bounded durable tombstones |
...
| Remote route claim | Receiver validates `(attempt_id, route_id, target incarnation)` | Remote attempt | completion/error/cancel/expiry/incarnation loss; replay tombstone then expires | active routes + bounded incarnation-scoped tombstones |
