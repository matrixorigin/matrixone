...
| CG-DRAIN-04 | B | Route is materialized but remains unclaimed when Draining publishes | Route claim linearizes before Draining | Unclaimed route is rejected before operator construction, sibling routes unwind once, and no retry starts while Draining; the pre-barrier claim may reach Started and finish only before the drain deadline. |
