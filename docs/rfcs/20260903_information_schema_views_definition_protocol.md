...
MORPC v68 is allocated as `MORPCLatestVersion + 1` from official main v67,
which is assigned to the named process timezone identity contract. It is specific to
these functions and the persisted VIEWS definition. The v4.0.6 VIEWS upgrade
waits for common v68. New tenant initialization at v67 or below installs the
predecessor VIEWS DDL, which has no function reference; v68 installs the new
DDL. Pipeline preparation, remote
marshal, and remote unmarshal reject a pipeline containing either function ID
below v68. The receiver check protects stale prepared work as well as normal
sender dispatch. Before admitting any v67-or-earlier CN during rollback,
...
the DDL before v68 was rejected because an old CN cannot bind the metadata functions.
...
Protocol tests cover the v67 predecessor rejection and v68 acceptance at
prepare, sender, and receiver boundaries. System-view tests prove v67 tenant
initialization uses the predecessor DDL and v68 uses the parser-derived DDL;
upgrade tests prove the VIEWS entry requires v68. The predecessor-init test is
