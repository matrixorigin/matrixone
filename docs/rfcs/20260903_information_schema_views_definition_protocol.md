...
MORPC v63 is allocated as `MORPCLatestVersion + 1` from official main v62,
which is already assigned to VARCHAR OCT overload identities. It is specific to
this function and the persisted VIEWS definition. The v4.0.6 VIEWS upgrade
waits for common v63. New tenant initialization at v62 or below installs the
predecessor VIEWS DDL, which has no function reference; v63 installs the new
DDL. Pipeline preparation, remote
marshal, and remote unmarshal reject a pipeline containing either function ID
below v63. The receiver check protects stale prepared work as well as normal
sender dispatch. Before admitting any v62-or-earlier CN during rollback,
operators must restore `InformationSchemaViewsLegacyDDL` and wait for that
catalog change to converge; merely draining v63-dependent requests is not
...
the DDL before v63 was rejected because an old CN cannot bind the metadata functions.
...
Protocol tests cover the v62 predecessor rejection and v63 acceptance at
prepare, sender, and receiver boundaries. System-view tests prove v62 tenant
initialization uses the predecessor DDL and v63 uses the parser-derived DDL;
upgrade tests prove the VIEWS entry requires v63. The predecessor-init test is
