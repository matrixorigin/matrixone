...
planner writes `required_protocol_version: 88` into `ViewData`, including when
...
HAKeeper publishes the durable read floor during the v88 decoder barrier, while
the authoring floor remains closed until the all-CN admission and catalog fence
complete. Therefore an authoring or read floor of 0 or v87 fails closed, an
all-v88 fenced CN can create the VIEWS definition, and an ordinary view without
...
MORPC v88 is allocated as `MORPCLatestVersion + 1` from official main v87 at
`3fe7840e34983141e579dee720741fb962847e4c`, which already owns v70 through v87;
v87 is the grouping-provenance capability. The two function IDs
...
v88. New tenant initialization installs the new VIEWS DDL only after the local
coordinator and every CN in the current inventory have positively confirmed v88;
...
row. A bounded post-upgrade reconciliation pass later rechecks common v88 and
reuses the guarded transactional entry to replace only that predecessor
definition. Any cluster with a CN below v88, including the immediate predecessor
v87, preserves all existing metadata definitions, including the v58 COLUMNS
contract. Pipeline preparation, remote marshal, and
remote unmarshal reject a
pipeline containing either function ID below v88. The receiver check protects
stale prepared work as well as normal sender dispatch. Before the v88 HAKeeper
protocol-floor activation is durably committed, a cancelled rollout may stop
the v88 upgrade, keep or restore `InformationSchemaViewsLegacyDDL`
transactionally, wait for catalog and in-flight work to converge, and admit
v87 only after verifying that no v88 maintenance worker can re-install the
new definition. After the v88 floor is committed, the floor is monotonic and
v87 binaries must not be admitted: restoring the view text alone cannot undo
the decoder barrier. The supported recovery is forward recovery with v88-
...
Merely draining below-v88 requests is not sufficient because the new persisted
...
the DDL before v88 was rejected because an old CN cannot bind the metadata functions.
...
Protocol tests cover the v87 predecessor
rejection and v88 acceptance at prepare, sender, and receiver boundaries,
...
authoring/read floor values 0, v87, and v88, and binding the exact generated
...
fall back to the predecessor VIEWS DDL, while an all-v88 inventory uses the
...
durable definition marker, retries only after a positive all-CN v88 check, and
...
its VIEWS entry require v88. The predecessor-init test proves that the
...
generation can retry and publish only after a committed v88 gate.
