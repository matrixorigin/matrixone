...
planner writes `required_protocol_version: 87` into `ViewData`, including when
...
HAKeeper publishes the durable read floor during the v87 decoder barrier, while
the authoring floor remains closed until the all-CN admission and catalog fence
complete. Therefore an authoring or read floor of 0 or v86 fails closed, an
all-v87 fenced CN can create the VIEWS definition, and an ordinary view without
...
MORPC v87 is allocated as `MORPCLatestVersion + 1` from official main v86 at
`aeee7db5099b4347a5c2e5f8d3e4785d76884100`, which already owns v70 through v86;
v86 is the extended IP overload/result and expression metadata capability. The two function IDs
...
v87. New tenant initialization installs the new VIEWS DDL only after the local
coordinator and every CN in the current inventory have positively confirmed v87;
...
row. A bounded post-upgrade reconciliation pass later rechecks common v87 and
reuses the guarded transactional entry to replace only that predecessor
definition. Any cluster with a CN below v87, including the immediate predecessor
v86, preserves all existing metadata definitions, including the v58 COLUMNS
contract. Pipeline preparation, remote marshal, and
remote unmarshal reject a
pipeline containing either function ID below v87. The receiver check protects
stale prepared work as well as normal sender dispatch. Before the v87 HAKeeper
protocol-floor activation is durably committed, a cancelled rollout may stop
the v87 upgrade, keep or restore `InformationSchemaViewsLegacyDDL`
transactionally, wait for catalog and in-flight work to converge, and admit
v86 only after verifying that no v87 maintenance worker can re-install the
new definition. After the v87 floor is committed, the floor is monotonic and
v86 binaries must not be admitted: restoring the view text alone cannot undo
the decoder barrier. The supported recovery is forward recovery with v87
...
Merely draining below-v87 requests is not sufficient because the new persisted
...
the DDL before v87 was rejected because an old CN cannot bind the metadata functions.
...
Protocol tests cover the v86 predecessor
rejection and v87 acceptance at prepare, sender, and receiver boundaries,
...
authoring/read floor values 0, v86, and v87, and binding the exact generated
...
fall back to the predecessor VIEWS DDL, while an all-v87 inventory uses the
...
durable definition marker, retries only after a positive all-CN v87 check, and
...
its VIEWS entry require v87. The
