...
MORPC v69 is allocated as `MORPCLatestVersion + 1` from official main v68,
which is assigned to the complete GROUP_CONCAT cut reporting contract. It is specific to
these functions and the persisted VIEWS definition. The v4.0.6 VIEWS upgrade
waits for common v69. New tenant initialization at v68 or below installs the
predecessor VIEWS DDL, which has no function reference; v69 installs the new
DDL. Pipeline preparation, remote
marshal, and remote unmarshal reject a pipeline containing either function ID
below v69. The receiver check protects stale prepared work as well as normal
sender dispatch. Before admitting any v68-or-earlier CN during rollback,
operators must restore `InformationSchemaViewsLegacyDDL` and wait for that
catalog change to converge; merely draining v68-dependent requests is not
...
the DDL before v69 was rejected because an old CN cannot bind the metadata functions.
...
Protocol tests cover the v68 predecessor rejection and v69 acceptance at
prepare, sender, and receiver boundaries. System-view tests prove v68 tenant
initialization uses the predecessor DDL and v69 uses the parser-derived DDL;
upgrade tests prove the VIEWS entry requires v69. The predecessor-init test is
