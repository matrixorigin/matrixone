...
- CTE 消费者的窗口需要按隐藏行标识分区；HAVING 需要区分聚合空输入和过滤掉聚合结果；集合运算需同步身份列的每个分支与输出 schema。v1 尚未实现上述改写，必须在规划期拒绝这三种相关消费者（包括 UNION ALL），而不能返回错误结果或内部 schema 错误。这些是待后续补齐的兼容能力，拒绝不等于支持。
