...
the refresh rule. Non-prepared statements use the current session limit. At the
top-level statement boundary this limit is also stored in the request context,
so every assignment in one SET and any nested RHS execution use the same
snapshot; a newly assigned `max_error_count` takes effect at the next boundary.
...
The remote receiver uses the smaller of the configured MORPC body limit and the
existing 64 MiB result-fragment limit as the terminal budget (and subtracts
protobuf message overhead). Receivers created outside an RPC server use the
default 100 MiB MORPC body limit as the compatibility fallback.
