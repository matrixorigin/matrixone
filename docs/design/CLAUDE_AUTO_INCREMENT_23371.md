...
- CACHE 使用新 V69；V56～V68 保留上游原含义（最新合并 be30a70fe5）。追加 `PreInsertAutoIDCache` wire-only opcode，不改变已有 opcode 数值；接收端先验证，再还原为普通 PRE_INSERT。
- `SetupServiceBasedRuntime` 的本机 latest 不是 CN/TN 最低版本证明。V69/新 opcode 只补充局部及 PRE_INSERT 传输拒绝；旧 TN/旧直连 CN 必须由运维先停止，混合版本启用/运行和旧节点重入均不支持。该约束已经由用户批准，不增加平台 admission。
