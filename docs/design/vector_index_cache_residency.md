...
| **CREATE-time capability gate**: below `MOProtocolVersion 59` a metadata table is born in the legacy shape, so an index created mid-rollout carries no provenance until something widens it | §12 | Eric — 2026-09-08 |
...
|---|---|---|
| mixed CNs, a table is CREATED | a NEW CN would create a six-column table that an OLD CN's positional writer then fails on | `CREATE INDEX` builds the LEGACY shape until `MOProtocolVersion >= 59`; the migration widens it later (§12) |
...
|---|---|---|---|
| Is any un-upgraded CN still out there? | deployment | `MOProtocolVersion >= MORPCVersion59` | CREATE-time schema width, CDC tail row emission, the v4_0_7 migration |
