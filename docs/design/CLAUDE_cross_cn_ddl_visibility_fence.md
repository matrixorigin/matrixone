...
- Markerless startup preserves the current-main v43 baseline before the v44 cut.
...
HAKeeper is the first owner of cluster epoch and membership linearization. CN local metadata is not authoritative for cluster membership. CN generation ownership is monotonic independently of ViewMetadataAdmission activation: once generation N+1 owns a UUID, an N heartbeat can never overwrite incarnation state, even before the admission protocol is enabled. It may only contribute an already-committed monotonic DDL frontier.

## 4. Protocol states

A CN is in one of these logical states:

1. **Baseline v43**: v44 not deployed; existing v38-v43 contracts remain active. Public ingress may be open.
...
5. **Cluster committed**: HAKeeper atomically validated exact `(serviceID, generation, queryAddress)` membership and advanced epoch to 44.
6. **Locally committed**: CN persisted `44`; only then may it republish ingress and unblock DDL.
...
2. Every CN runs v44-capable code but markerless CNs keep protocol baseline v43.
...
Current main already deploys v43 semantics. A fresh v44-capable process without a DDL marker therefore remains at v43, not v37. The v44 DDL state is separate from unrelated v38-v43 capabilities.
...
|---|---|
| v43 baseline preserved | markerless default-v44 startup UT |
...
- Chosen baseline: preserve current-main v43 before v44 activation.
