...
The extended discrete-percentile admission boundary is MORPC v84. Main owns
v76-v83 for NaN-ordered percentiles, HLL/DISTINCT/group-key state, string
numeric result contracts, table-owned auto-ID cache, fulltext2 JSON probe-tail,
and bounded conditional string domains; those gates remain unchanged. Prepared
approximate percentiles retain main's v76 explicit
direction configuration, including its SQL NaN ordering, rather than
translating a descending percentile to `1-p`.
...
v84 decoder rejects a missing envelope, unknown metadata version, or invalid
...
This representation is not a promise that pre-v84 workers can execute extended
...
provided by fail-closed remote admission at v84, not by asking an old worker to
...
diagnostics, `MORPCVersion67` to named process timezone identity,
`MORPCVersion68` to complete GROUP_CONCAT cut reporting for strict writes,
`MORPCVersion69` to exact bounded per-partition RANK with boundary ties,
`MORPCVersion70` to row-dependent and unsigned CONV bases,
`MORPCVersion71` to checked integer arithmetic overloads,
`MORPCVersion72` to corrected IP function semantics and overload identities,
`MORPCVersion73` to the widened DECIMAL SUM partial state in #28624,
`MORPCVersion74` to corrected numeric HEX overload identities,
`MORPCVersion75` to persistent data-branch identity, `MORPCVersion76` to
NaN-ordered percentiles and compatible HLL states, `MORPCVersion77` to typed
HLL keys, `MORPCVersion78` to variable-length GROUP keys, `MORPCVersion79` to
canonical opaque DISTINCT keys, `MORPCVersion80` to string numeric result
contracts, `MORPCVersion81` to opt-in table-owned AUTO_ID_CACHE,
`MORPCVersion82` to the self-completing fulltext2 JSON probe-tail contract, and
`MORPCVersion83` to bounded COALESCE character and binary result domains. This
feature therefore uses `MORPCVersion84`; the #28624 implementation is already
part of this head, so a v84 binary implements the complete monotonic protocol
prefix.
...
configured protocol version is below 84,
...
1. deploy binaries containing the v84 executor and decoder to every
   participating CN while the configured protocol remains at most 83;
...
3. after every participating worker is capable, raise the configured protocol
   to 84 and enable distributed use; and
4. monitor protocol rejection, allocation/spill errors, and result regressions.

To roll back, first lower the configured protocol to 83 so no new extended
...
spill cleanup. Downgrading a live query or injecting a v84 partial state into an
...
- Silently running extended types only on the coordinator would avoid a wire
  ...
  later optimizer changes. An explicit fail-closed v84 contract is selected.
...
| v83 rejection, v84 admission, and unchanged v17 numeric boundary | `pkg/sql/compile/remote_expr_test.go` |
| build and changed-package regression | `make build`; targeted `go test` commands recorded on PR #28540 |

Before enabling protocol 84 in a release, deployment qualification must also
...
saved payload, and protocol 84 as the no-downgrade remote boundary. Window
