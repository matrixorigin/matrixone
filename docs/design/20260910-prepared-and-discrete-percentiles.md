...
v66 decoder rejects a missing envelope, unknown metadata version, or invalid
...
This representation is not a promise that pre-v66 workers can execute extended
...
provided by fail-closed remote admission at v66, not by asking an old worker to
...
`MORPCVersion64` to typed BIN/CONV execution contracts, and `MORPCVersion65`
to the widened DECIMAL SUM partial state in #28624. This feature therefore
uses `MORPCVersion66` and must merge after #28624, so a v66 binary implements
...
configured protocol version is below 66,
...
1. merge and deploy #28624, then deploy binaries containing the v66 executor
...
3. after every participating worker is capable, raise the configured protocol
   to 66 and enable distributed use; and
4. monitor protocol rejection, allocation/spill errors, and result regressions.

To roll back, first lower the configured protocol to 65 so no new extended
...
spill cleanup. Downgrading a live query or injecting a v66 partial state into an
...
- Silently running extended types only on the coordinator would avoid a wire
  ...
  later optimizer changes. An explicit fail-closed v66 contract is selected.
...
| v65 rejection, v66 admission, and unchanged v17 numeric boundary | `pkg/sql/compile/remote_expr_test.go` |
| build and changed-package regression | `make build`; targeted `go test` commands recorded on PR #28540 |

Before enabling protocol 66 in a release, deployment qualification must also
...
saved payload, and protocol 66 as the no-downgrade remote boundary. Window
