...
> **3.0-dev backport note**: On 3.0-dev `mo_catalog.mo_snapshots` has no
> `kind` column. To avoid a schema migration on a stable branch, this
> backport identifies branch-managed rows entirely by their sname prefix
> `__mo_branch_<child_tid>` (see `databranchutils.BranchSnapshotSnamePrefix`).
> Every user-facing filter that on `main` reads `kind != 'branch'` is
> implemented here as `sname NOT LIKE '__mo_branch_%'`. Functional
> behaviour and invariants are otherwise identical to the `main` design.
...
- **Correctness under GC**: guarantee that any `diff <tar> against <base>`
  ...
  and GC.
- **Zero new storage format**: reuse the existing `mo_snapshots` table.
  3.0-dev has no `kind` column, so branch-managed rows are identified by
  their `__mo_branch_<child_tid>` sname prefix — no schema migration.
