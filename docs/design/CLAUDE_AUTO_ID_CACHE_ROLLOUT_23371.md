...
- 最新 main 的 V57 属于 Arrow LOAD，V58 已由 binary-string/runtime-domain 协议使用。合并 `d3e8aced87` 时保留上游 V58，将本 PR 尚未合入的 CACHE 门槛从初版 V58 顺延为 **V59**；再合并 `401b967dc1` 时，上游 V59 已用于 FORMAT，CACHE 顺延 **V60**；合并 `cd8e8d9134` 后上游占用 V60～V62，CACHE 顺延 V63；再合并 `065a675286` 后上游占用 V63，CACHE 顺延 **V64**；再合并 `77dbf54644` 后上游占用 V64，CACHE 顺延 **V65**；再合并 `887576e6b8` 后上游占用 V65/V66，CACHE 顺延 **V67**；再合并 `be30a70fe5` 后上游占用 V67/V68，CACHE 顺延 **V69**；再合并 `a72a224ce8` 后上游占用 V69，CACHE 顺延 **V70**；再合并 `2ea4ec2e6e` 后上游占用 V70～V72，CACHE 顺延 **V73**；再合并 `4cd720e0ce` 后上游占用 V73，CACHE 顺延 **V74**；既有 opcode/字段编号不变。
...
- V74 本机检查与追加的 `PreInsertAutoIDCache` wire opcode 保护携带策略的 PRE_INSERT 传输：旧 decoder 走 unknown operator 拒绝；新 receiver 在构造 scope/operator 之前检查开关和版本。该标记不覆盖所有 CN/TN metadata 通道，因此不能替代全角色升级，更不是集群 admission 证明。
...
- 实现：默认关闭配置；DDL/构建 PRE_INSERT/直接 service.Create/冷 cache 分层拒绝；V74 与 wire-only opcode；既有 schema 属性仍是唯一持久所有者。
- 运维配置：
  ```toml
  [cn.auto-increment]
