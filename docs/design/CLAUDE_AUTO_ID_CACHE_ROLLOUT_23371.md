...
- 最新 main 的 **V57 已属于 Arrow LOAD**；若增加 CACHE 的局部传输版本，当前应分配 V58，仍须在实现时重新核对。
...
- V58 本机检查与追加的 `PreInsertAutoIDCache` wire opcode 保护携带策略的 PRE_INSERT 传输：旧 decoder 走 unknown operator 拒绝；新 receiver 在构造 scope/operator 之前检查开关和版本。该标记不覆盖所有 CN/TN metadata 通道，因此不能替代全角色升级，更不是集群 admission 证明。
...
- 实现：默认关闭配置；DDL/构建 PRE_INSERT/直接 service.Create/冷 cache 分层拒绝；V58 与 wire-only opcode；既有 schema 属性仍是唯一持久所有者。
- 运维配置：
  ```toml
  [cn.auto-increment]
