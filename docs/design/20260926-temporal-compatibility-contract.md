...
| C07 Warning ownership | Each executed failing operation publishes its specified diagnostic once; two distinct evaluated operations may legitimately produce two. A statement-invariant scalar operand of a comparison, including an implicit numeric coercion inserted by the binder or constant temporal arithmetic, emits its diagnostic once when logically activated, whether the comparison is in WHERE or JOIN ON. An inactive arm or an unvisited input emits none; empty output alone does not prove inactivity. User-written string-to-numeric CAST retains its established per-evaluated-row warning contract, as do write adjustments and row-dependent conversions. Optimization, predicate copying, storage pruning, pipeline count and CN placement cannot change these counts. Respect the session's retention limit; do not retain unbounded messages or emit twice at normalization and arithmetic. | Constant-folding is semantically transparent; warning state cannot leak into the next prepared execution. Hard failures must retain their original error and leave cleanup usable. |
...
C07 governs the ownership of a diagnostic already specified by the selected
operation. It does not add a warning to a conversion path that currently has
none. In particular, a prepared DECIMAL comparison may select a prefix parser
that does not report the warning emitted by a literal's DOUBLE comparison;
that domain-selection difference needs its own compatibility decision.
For a constant JOIN key, both logical inputs must contain a row before its
diagnostic is activated. HashBuild's speculative key preparation alone does
not publish a warning. This remains true when the constant makes the key NULL,
when the inputs produce no match, and when the planner exchanges build and
probe sides.
...
| C32 Explicit TIME conversion | CAST-to-TIME is an explicit conversion boundary: valid out-of-domain duration is clamped to the target TIME endpoint with warning 1292. TIME(expr) and TIME literals retain their existing function grammar; their public results also obey the TIME range, clamping valid out-of-domain function values with warning 1292. New TIME(expr) bindings retain TIME(n), DATETIME(n), or TIMESTAMP(n) source FSP; DECIMAL(p,s) uses min(max(s,0),6); valid ordinary string literals use their known fractional width up to 6; dynamic strings and untyped markers use FSP6; FLOAT/DOUBLE retain their existing BIGINT coercion and FSP0. The evaluator rounds once to its bound FSP and never hides microseconds behind FSP0 metadata. Old persisted plans obey their already-declared FSP. Permissive CAST text-prefix conversion is allowed only by the declared text/mode rules and emits 1292; do not trim digits from an invalid minute/second to invent a valid prefix. Entirely malformed permissive CAST input returns NULL with conversion warning. Strict malformed CAST retains its error. This does not authorize clipping inside arithmetic. |
