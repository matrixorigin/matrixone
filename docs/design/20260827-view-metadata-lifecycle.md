...
- **待审批语义 checkpoint**：`96c5a71ac2874e6a5243681da25b4b918c8252bd`（包含最终 authority-disabled 兼容修复、compile 边界证据、订阅表 Information Schema 可见性合并，以及隐式 View heading 长度兼容修复的 executable/evidence head）。审批必须覆盖包含本修订与该 checkpoint 的 exact PR head；获批前状态保持 `Pending re-approval`。
