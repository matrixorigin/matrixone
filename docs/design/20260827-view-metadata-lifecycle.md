...
Review `5218599374` 暴露 owned DATA BRANCH DELETE 在等待目标 database lock 后收到 `ErrTxnNeedRetry` 时，会在 post-lock validation 前把 retry 泄漏给客户端。`a7e85733cb` 对 owned background transaction 只允许一次完整重启：rollback/begin 后重新取得 lineage lifecycle admission，再重复 target lock 与 validation；shared/explicit transaction 和非 retry 错误不进入该路径。issue26068 race 十次、包含其前序 shared-cluster tests 的 race 五次、frontend coverage 三次、restart helper focused 十次及完整 frontend 通过。合并 main `01d60e1c4d` 后无冲突，前序 shared-cluster sequence race 五次及完整 frontend/compile 再次通过。

CI run `35063135359` 的 UT producer 是 `TestDataBranchDiffAsFile` 在同一数据库内并行运行互不相关 case：一个 case 的 persistent staging-table DDL 与另一个 case 的 lineage lifecycle transaction 形成合法 deadlock victim。该 fixture 不验证跨 case 并发；`9006a20e93` 将共享数据库的 case 并发度固定为一，不增加 SQL retry、不放宽数据断言。当前 PR 修复前 race 十次可复现，exact main `66b1672403` 对照 race 十次通过；修复并合并该 main 后 race 五次、完整 dml/frontend/compile 通过。该 run 的 Proxy 是 Docker Hub connection reset；PESS fulltext2 copy-alter 是 main 新增用例的 CDC tail/MATCH 时序失败，Coverage 仅为 producer fallout，均未以修改本 PR lifecycle 语义掩盖。
