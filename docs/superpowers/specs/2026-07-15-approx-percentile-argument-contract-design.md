...
- Prepared-statement parameters and variables are rejected because they are not compile-time constants for this aggregate configuration.
...
Add one shared validation step in `BindFuncExprImplByPlanExpr`, before implicit casts are inserted. Use the existing constant-expression classifier in strict compile-time mode, and reject literal `NULL` separately. Because aggregate and window binding both use this entry point, the rule applies consistently to `constructGroup` and `constructWindow` without duplicated binder logic.
...
- Planner accepts a numeric literal and a foldable numeric expression, and rejects prepared parameters.
